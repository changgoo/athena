...
def prepare(**kwargs):
    ...
    for flux in _fluxes:
        athena.configure(
            "b", prob="linear_wave", coord="cartesian", flux=flux, **kwargs
        )
...
def analyze():
    ...
    for i, flux in enumerate(_fluxes):
        ...
        if data[3][15] / data[3][6] > 8.0:
            msg = (
                flux_str + "maximum relative error in L-going Alfven wave too large %g"
            )
            logger.warning(msg, data[3][15] / data[3][6])
        if data[5][13] > 8.0:
            msg = (
                flux_str + "maximum relative error in L-going slow wave too large %g",
            )
        ...
        if data[1][4] > 4.5e-8:
            logger.warning(
                flux_str + "RMS error in L-going fast wave too large %g", data[1][4]
            )
            analyze_status = False
        if data[1][4] / data[0][4] > 0.4:
            logger.warning(
                flux_str + "not converging for L-going fast wave %g %g",
                data[0][4],
                data[1][4],
            )
            analyze_status = False
        ...
        if data[3][4] > 4.0e-8:
            logger.warning(
                flux_str + "RMS error in L-going Alfven wave too large %g", data[3][4]
            )
            analyze_status = False
        if data[3][4] / data[2][4] > 0.4:
            logger.warning(
                flux_str + "not converging for L-going Alfven wave %g %g",
                data[2][4],
                data[3][4],
            )
            analyze_status = False
        ...
        if data[5][4] > 5.0e-8:
            logger.warning(
                flux_str + "RMS error in L-going slow wave too large %g", data[5][4]
            )
            analyze_status = False
        if data[5][4] / data[4][4] > 0.4:
            logger.warning(
                flux_str + "not converging for L-going slow wave %g %g",
                data[4][4],
                data[5][4],
            )
            analyze_status = False
        ...
        if data[7][4] > 2.75e-8:
            logger.warning(
                flux_str + "RMS error in entropy wave too large %g", data[7][4]
            )
            analyze_status = False
        if data[7][4] / data[6][4] > 0.4:
            logger.warning(
                flux_str + "not converging for entropy wave %g %g",
                data[6][4],
                data[7][4],
            )
            analyze_status = False
        ...
        if data[8][4] != data[9][4]:
            logger.warning(
                flux_str + "error in L/R-going fast waves not equal %g %g",
                data[8][4],
                data[9][4],
            )
            analyze_status = False
