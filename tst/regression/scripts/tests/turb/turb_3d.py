...
def prepare(**kwargs):
    ...
    athena.configure("mpi", "fft", prob="turb", **kwargs)
    athena.make()
    os.system("mv bin/athena bin/athena_mpi_fft")
    os.system("mv obj obj_mpi_fft")

    athena.configure("fft", prob="turb", **kwargs)
    athena.make()
...
def analyze():
    ...
    if diff[1] > 1.0e-7:
        logger.warning(
            "Turb runs with serial and 2 MPI ranks are different %g", diff[1]
        )
        analyze_status = False
    if diff[2] > 1.0e-7:
        logger.warning(
            "Turb runs with serial and 4 MPI ranks are different %g", diff[2]
        )
        analyze_status = False
