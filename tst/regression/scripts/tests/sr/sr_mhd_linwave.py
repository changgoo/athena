...
def prepare(**kwargs):
    ...
    athena.configure(
        "sb", prob="gr_linear_wave", coord="cartesian", flux="hlld", **kwargs
    )
    athena.make()
...
def run(**kwargs):
    ...
    for wave_flag in range(7):
        wavespeed = calculate_wavespeed(
            rho, pgas, vx, vy, vz, bx, by, bz, gamma_adi, wave_flag
        )
...
def analyze():
    ...
    for wave_flag in range(7):
        if data[2 * wave_flag + 1][4] > high_res_errors[wave_flag]:
            logger.warning(
                "{0} wave error too large ({1} vs. {2})".format(
                    names[wave_flag],
                    data[2 * wave_flag + 1][4],
                    high_res_errors[wave_flag],
                )
            )
            status = False
        if data[2 * wave_flag + 1][4] / data[2 * wave_flag][4] > error_ratio:
            logger.warning(
                "{0} wave error not converging ({1} to {2})".format(
...
def calculate_wavespeed(rho, pgas, vx, vy, vz, bx, by, bz, gamma_adi, wave_flag):
    ...
    a3 = (
        -factor_a * 4.0 * u[0] ** 4 * vx
        + factor_b * 2.0 * u[0] ** 2 * vx
        + 2.0 * b[0] * b[1]
    )
    a2 = (
        factor_a * 6.0 * u[0] ** 4 * vx**2
        + factor_b * u[0] ** 2 * (1.0 - vx**2)
        + b[0] ** 2
        - b[1] ** 2
    )
    a1 = (
        -factor_a * 4.0 * u[0] ** 4 * vx**3
        - factor_b * 2.0 * u[0] ** 2 * vx
        - 2.0 * b[0] * b[1]
    )
