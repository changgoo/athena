...
def prepare(**kwargs):
    ...
    athena.configure(
        "b", "omp", prob="linear_wave", coord="cartesian", flux="hlld", **kwargs
    )
    athena.make()
...
def analyze():
    ...
    if data[0][4] != data[1][4]:
        msg = (
            "Linear wave error from serial calculation vs. single thread not identical"
        )
