...
def prepare(**kwargs):
    ...
    for i in athena.global_config_args:
        ...
        if tmp[0] == "--flux" and len(tmp) == 2:
            _fluxes = [tmp[1]]
    for flux in _fluxes:
        athena.configure(prob="linear_wave", coord="cartesian", flux=flux, **kwargs)
...
def analyze():
    ...
    for i, flux in enumerate(_fluxes):
        ...
        if data[1][4] > {"hlle": 4.3e-8}.get(flux, 3.7e-8):
            logger.warning(
                flux_str + "error in L-going sound wave too large %g", data[1][4]
            )
            analyze_status = False
        if data[1][4] / data[0][4] > {"hlle": 0.33}.get(flux, 0.325):
            logger.warning(
                flux_str + "not converging for L-going sound wave %g %g",
                data[0][4],
                data[1][4],
            )
            analyze_status = False
        ...
        if data[4][4] != data[5][4]:
            logger.warning(
                flux_str + "error in L/R-going sound waves not equal %g %g",
                data[4][4],
                data[5][4],
            )
            analyze_status = False
