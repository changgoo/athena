...
def prepare(**kwargs):
    ...
    athena.configure("mpi", prob="jeans", grav="mg", **kwargs)
    athena.make()
    ...
    athena.configure(prob="jeans", grav="mg", **kwargs)
    athena.make()
...
def analyze():
    ...
    if data[8][4] > 1.0e-7:
        logger.warning(
            "Linear wave error is too large for BlockFFT gravity %g", data[8][4]
        )
        analyze_status = False
    if abs(data[1][4] - data[5][4]) > 1.0e-6:
        logger.warning(
    ...
    if abs(data[7][4] - data[5][4]) > 5.0e-4:
        ...
        analyze_status = False
    if abs(data[8][4] - data[7][4]) > 1.0e-6:
        logger.warning(
