...
def prepare(*args, **kwargs):
    ...
    athena.configure("mpi", "fft", prob="msa", grav="blockfft", *args, **kwargs)
    athena.make()
...
def run(**kwargs):
    for n in resolution_range:
        arguments = [
            "job/problem_id=SwingAmplification_" + repr(n),
            "mesh/nx1=" + repr(n),
            "mesh/x1min={}".format(-Lx / 2.0),
            "mesh/x1max={}".format(Lx / 2.0),
            "mesh/nx2=" + repr(n),
            "mesh/x2min={}".format(-Lx / 2.0),
            "mesh/x2max={}".format(Lx / 2.0),
...
def analyze():
    ...
    if data[1][4] > 2.3e-7:
        logger.warning("error in the shearing wave too large %g", data[1][4])
        analyze_status = False
    if data[1][4] / data[0][4] > 0.25:
        logger.warning("not converging for shearing wave %g %g", data[0][4], data[1][4])
        analyze_status = False
