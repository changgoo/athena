...
def prepare(*args, **kwargs):
    ...
    athena.configure(
        "b", *args, prob="linear_wave", flux="hlld", eos="adiabatic", **kwargs
    )
    athena.make()
...
def analyze():
    ...
    for nx, err_tol in zip(resolution_range, error_rel_tols):
        logging.info(
            "[Decaying 3D Linear Wave {}]: " "Mesh size {} x {} x {}".format(
                method, nx, nx / 2, nx / 2
            )
        )
        ...
        if error_rel > err_tol:
            logger.warning(
                "[Decaying 3D Linear Wave {}]: decay rate disagrees"
    ...
    logger.info(
        "[Decaying 3D Linear Wave]: convergence rate of decay rate error = {}".format(
            rate
        )
    )
    if rate < rate_tols[-1]:
