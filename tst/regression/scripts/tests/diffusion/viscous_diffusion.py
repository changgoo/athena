...
def prepare(*args, **kwargs):
    ...
    athena.configure(prob="visc", *args, eos="isothermal", flux="roe", **kwargs)
    athena.make()
...
def run(**kwargs):
    for integrator in sts_integrators:
        for n in resolution_range:
            arguments = [
                "job/problem_id=ViscousDiffusion_" + repr(n) + "_" + integrator,
...
def analyze():
    ...
    for i in range(len(sts_integrators)):
        for n in resolution_range:
            x1v, v2 = athena_read.tab(
                "bin/ViscousDiffusion_"
                + str(n)
                + "_"
                + sts_integrators[i]
    ...
    for i in range(len(sts_integrators)):
        ...
        logger.info(
            "[Viscous Diffusion {}]: Convergence order = {}".format(method, conv[i])
        )
