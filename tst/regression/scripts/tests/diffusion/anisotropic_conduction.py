...
def analyze():
    ...
    for nx in resolution_range:
        logger.info('[Anisotropic Ring Diffusion {}]: '
                    'Mesh size {} x {} x {}'.format(method, nx, nx, 1))
        ...
        if np.all(error_tol):
            ...
            logger.warning('[Anisotropic Ring Diffusion {}]: '
                           'L1 error is too large -- '.format(method),
                           error_tol)
