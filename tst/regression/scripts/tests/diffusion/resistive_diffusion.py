...
def prepare(*args, **kwargs):
    ...
    athena.configure("b", *args, prob="resist", eos="isothermal", **kwargs)
    athena.make()
...
def run(**kwargs):
    for integrator in sts_integrators:
        for n in resolution_range:
            arguments = [
                "job/problem_id=ResistiveDiffusion_" + repr(n) + "_" + integrator,
                "output2/file_type=tab",
                "output2/variable=bcc2",
...
def analyze():
    ...
    for i in range(len(sts_integrators)):
        for n in resolution_range:
            x1v, bcc2 = athena_read.tab(
                "bin/ResistiveDiffusion_"
                + str(n)
                + "_"
                + sts_integrators[i]
            ...
            analytic = (
                _amp
                / np.sqrt(4.0 * np.pi * _eta * (_t0 + _tf))
                * np.exp(-(x1v**2.0) / (4.0 * _eta * (_t0 + _tf)))
            )
    ...
    for i in range(len(sts_integrators)):
        ...
        logger.info(
            "[Resistive Diffusion {}]: Convergence order = {}".format(method, conv[i])
        )
