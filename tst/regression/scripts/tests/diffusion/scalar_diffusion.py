...
def prepare(*args, **kwargs):
    ...
    athena.configure(
        prob="scalar_diff", *args, eos="isothermal", flux="roe", nscalars=1, **kwargs
    )
    athena.make()
...
def run(**kwargs):
    for integrator in sts_integrators:
        for n in resolution_range:
            arguments = [
                "job/problem_id=ScalarDiffusion_" + repr(n) + "_" + integrator,
                "output2/file_type=tab",
                "output2/variable=r0",
                "output2/data_format=%24.16e",
                "output2/dt={}".format(_tf),
                "time/cfl_number=0.8",
                "time/tlim={}".format(_tf),
                "time/nlim=10000",
                "time/sts_integrator={}".format(integrator),
                "time/ncycle_out=0",
                "mesh/nx1=" + repr(n),
                "mesh/x1min={}".format(-_Lx1 / 2.0),
                "mesh/x1max={}".format(_Lx1 / 2.0),
                "mesh/ix1_bc=outflow",
                "mesh/ox1_bc=outflow",
                "mesh/nx2=1",
                "mesh/x2min=-1.0",
...
def analyze():
    ...
    for i in range(len(sts_integrators)):
        for n in resolution_range:
            x1v, r0 = athena_read.tab(
                "bin/ScalarDiffusion_"
                + str(n)
                + "_"
                + sts_integrators[i]
            ...
            analytic = (
                _amp
                / np.sqrt(4.0 * np.pi * _nu * (_t0 + _tf))
                * np.exp(-(x1v**2.0) / (4.0 * _nu * (_t0 + _tf)))
            )
    ...
    for i in range(len(sts_integrators)):
        ...
        conv.append(
            np.diff(np.log(np.array(l1ERROR[i])))
            / np.diff(np.log(np.array(resolution_range)))
        )
        logger.info(
            "[Scalar Diffusion {}]: Convergence order = {}".format(method, conv[i])
        )
        ...
        if conv[i] > rate_tols[i]:
            ...
            logger.info(
                "[Scalar Diffusion {}]: " "Scheme converging at expected order.".format(
                    method
                )
            )
