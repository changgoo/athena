...
def prepare(**kwargs):
    ...
    athena.configure("g", prob="gr_shock_tube", coord="minkowski", flux="llf", **kwargs)
    athena.make()
...
def analyze():
    ...
    for i in range(1, 5):
        ...
        x_new, _, _, data_new = athena_read.vtk(
            "bin/gr_hydro_shock{0}.block0.out1.00001.vtk".format(i)
        )
        ...
        for header, tol in zip(headers, tols_particular):
            ...
            array_ref = (
                array_ref[0, 0, :]
                if len(header) == 1
                else array_ref[0, 0, :, header[1]]
            )
            array_new = data_new[header[0]]
            array_new = (
                array_new[0, 0, :]
                if len(header) == 1
                else array_new[0, 0, :, header[1]]
            )
            if header[0] == "Etot":
