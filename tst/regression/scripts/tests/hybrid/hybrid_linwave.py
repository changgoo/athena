...
def prepare(**kwargs):
    ...
    athena.configure(
        "b", "mpi", "omp", prob="linear_wave", coord="cartesian", flux="hlld", **kwargs
    )
    athena.make()
...
def analyze():
    ...
    if data[0][4] != data[1][4]:
        logger.warning(
            ...
            "single thread not identical %g %g",
            data[0][4],
            data[1][4],
        )
    ...
    if abs(data[2][4] - data[0][4]) > 5.0e-4:
        logger.warning(
            "Linear wave error differences between 4 threads vs."
            "serial is too large %g %g",
            data[2][4],
            data[0][4],
        )
        analyze_status = False
