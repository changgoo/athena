...
def prepare(**kwargs):
    ...
    athena.configure("b", "hdf5", prob="orszag_tang", flux="hlld", **kwargs)
    athena.make()
...
def analyze():
    ...
    slice_data = athena_read.tab(
        filename="bin/TestOutputs.block0.out2.00010.tab", raw=True, dimensions=1
    )
    ...
    sum_data = athena_read.tab(
        filename="bin/TestOutputs.block0.out3.00010.tab", raw=True, dimensions=1
    )
