...
def prepare(**kwargs):
    ...
    athena.configure("b", "mpi", "hdf5", "h5double", prob="from_array", **kwargs)
    athena.make()
    ...
    energy = (
        np.ones((1, nb1, nx3, nx2, nx1)) / (gamma - 1.0) + 0.5 * b1v[None, ...] ** 2
    )
...
def analyze():
    ...
    output_vars_cons = output_vars[
        num_vars_pre_cons : num_vars_pre_cons + num_vars_cons
    ]
    ...
    output_vars_field = output_vars[
        num_vars_pre_field : num_vars_pre_field + num_vars_field
    ]
