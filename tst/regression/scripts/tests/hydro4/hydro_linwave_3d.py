...
error_tols = [
    ((5.6e-9, 3.6e-10), (4.05e-9, 2.65e-10)),
...
def analyze():
    ...
    for (torder, xorder), err_tol, rate_tol in zip(solvers, error_tols, rate_tols):
        ...
        for i in range(1, num_nx1):
            rate = log(rms_errs[i - 1] / rms_errs[i]) / log(
                nx1_range[i] / nx1_range[i - 1]
            )
        ...
        # 64x32x32 resolution
        if not np.allclose(
            solver_results[-2, 4], solver_results[-1, 4], atol=5e-16, rtol=1e-5
        ):
