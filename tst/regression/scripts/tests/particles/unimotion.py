...
def prepare(**kwargs):
    ...
    athena.configure('p', 'mpi',
                     prob='particle_uniform_motion',
                     **kwargs)
    athena.make()
...
def run(**kwargs):
    ...
    arguments = ['job/problem_id=pUniMotion_N2',
                 'output2/dt=-1',
                 'time/ncycle_out=10',
                 'problem/npartot=100',
                 ]
    ...
    athena.mpirun(kwargs['mpirun_cmd'], kwargs['mpirun_opts'], 4,
                  'particles/athinput.particle_uniform_motion', arguments)

    # Construct a list of arguments to the executable.
    arguments = ['job/problem_id=pUniMotion_N3',
                 'output2/dt=-1',
                 'time/ncycle_out=10',
    ...
    # Construct a list of arguments to the executable.
    arguments = ['job/problem_id=pUniMotion_N4',
                 'output2/dt=-1',
                 'time/ncycle_out=10',
...
def analyze():
    ...
    for i, err in enumerate(errg):
        if err > 1.E-16:
            logger.warning("np=%d: gas density error is too large %g", 10**(i+2), err)
            analyze_status = False

    for i, erri, errf in zip(range(3), errp0, errp1):
        if np.abs(erri-errf) > 1.E-16:
            logger.warning("np=%d: initial - final is too large %g", 10**(i+2), erri-errf)
            analyze_status = False
        if errf > 2.E-2*np.sqrt(10**(2-i)):
            logger.warning("np=%d: final particle error is too large %g", 10**(i+2), errf)
            analyze_status = False
