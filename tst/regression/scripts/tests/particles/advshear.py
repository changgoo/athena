...
def read_tab(i, parid=0, base='bin/', pid='pAdvShear'):
    ...
    import glob
    parall = []
    tabfiles = glob.glob('{}{}.block*.out3.{:05d}.par{}.tab'.format(base, pid, i, parid))
...
def run(**kwargs):
    ...
    arguments = ['job/problem_id=pAdvShear',
                 'output2/dt=-1',
                 'time/ncycle_out=10',
                 'time/tlim=2',
                 'output3/dt=0.5',
    ...
    athena.mpirun(kwargs['mpirun_cmd'], kwargs['mpirun_opts'], 4,
                  'particles/athinput.particle_advection_shear', arguments)
...
def analyze():
    ...
    for parid in [0, 1]:
        ...
        dythresh = 0.22
        dyexp = np.mod(((par['x2']-np.interp(par['x1'], x, y))+dythresh), Ly)-dythresh

        outlier = ((dyexp > dythresh) | (dyexp < -dythresh))
        nout = outlier.sum()
        logger.info("p%d -- found %d outliers with offset tolerence %g",
                    parid, nout, dythresh)
        if nout > 100:
