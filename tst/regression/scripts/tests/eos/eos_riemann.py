...
_tests = [
    [1e-07, 0.00, 0.150, 1.25e-8, 0.0, 0.062, 0.25],
    [4e-06, 0.00, 0.120, 4e-08, 0.00, 0.019, 0.3],
    [8e-07, 1.10, 0.006, 4e-07, -1.7, 0.006, 1.5],
    [5e-07, 1.50, 0.006, 4e-07, -1.8, 0.006, 1.5],
    [8e-05, -0.8, 0.095, 8e-05, 0.80, 0.095, 0.25],
    [6e-05, -0.5, 0.095, 8e-05, 0.90, 0.095, 0.25],
]
_thresh = [
    [5.1e-10, 6.5e-11, 0.0039],
...
def run(**kwargs):
    for flux in _fluxes:
        ...
        for n, test in enumerate(_tests):
            ...
            args += [
                "job/problem_id=eos_riemann_{0:}_{1:02d}".format(flux, n),
                "time/ncycle_out=0",
            ]
...
def analyze():
    ...
    for flux in _fluxes:
        for n, state in enumerate(_states):
            ...
            xi = (0.5 * x_ref[:-1] + 0.5 * x_ref[1:]) / _tests[n]["time/tlim"]
