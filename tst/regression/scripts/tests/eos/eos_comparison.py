...
def prepare(**kwargs):
    ...
    athena.configure(
        prob="shock_tube", coord="cartesian", flux="hllc", eos="general/ideal", **kwargs
    )
    athena.make()
    src = os.path.join("bin", "athena")
    ...
    athena.configure(
        prob="shock_tube", coord="cartesian", flux="hllc", eos="adiabatic", **kwargs
    )
    athena.make()
...
def run(**kwargs):
    arguments = {
        0: [
            "hydro/gamma={0:}",
            "job/problem_id=Sod_adiabatic_{1:}",
            "time/ncycle_out=0",
    ...
    tmp = zip(tmp, [1e-07, 0.00, 3e-8, 1.25e-8, 0.00, 1e-9, 0.25])
    ic = [i[0].format(i[1]) for i in tmp]
    athena.run(
        "hydro/athinput.sod", ic + arguments["binary"], lcov_test_suffix="eos_hllc"
    )
    athena.run(
        "hydro/athinput.sod", ic + arguments["ascii"], lcov_test_suffix="eos_hllc"
    )
...
def analyze():
    ...
    for t in [10, 25]:
        ...
        for var in ["rho", "press"]:
            norm = comparison.l1_norm(x_ref, data_ref[var][loc])
            diff = (
                comparison.l1_diff(x_ref, data_ref[var][loc], x_new, data_new[var][loc])
                / norm
            )
            ...
            diff = (
                comparison.l1_diff(
                    x_ref, data_ref[var][loc], x_ascii, data_ascii[var][loc]
                )
                / norm
            )
