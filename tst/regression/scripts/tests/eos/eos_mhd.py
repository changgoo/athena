...
# Solution data
eos_rj2a = [
    [1.08, 1.2, 1.0e-2, 0.5, 1.0155412503859613, 0.5641895835477563, 0.95],
    [
        1.527257978792011,
        0.6073696428720876,
        0.13086751616368864,
...
def prepare(**kwargs):
    ...
    athena.configure(
        "b", prob="shock_tube", coord="cartesian", eos="general/hydrogen", **kwargs
    )
    athena.make()
...
def run(**kwargs):
    ...
    args = [
        "hydro/eos_rho_unit=1e-7",
        "hydro/eos_egas_unit=2e-8",
        "time/ncycle_out=100",
...
def analyze():
    ...
    for n, state in enumerate(_mhd_states):
        ...
        xi = (0.5 * x_ref[:-1] + 0.5 * x_ref[1:]) / _mhd_tests[n]["time/tlim"]
    ...
    # compute errors
    errors = [
        comparison.l1_norm(x_ref, data["rho"][0, 0] - data_ref[0]),
