...
def sod_test(eos=None, gamma=None, plot=True):
    ...
    wl.complete()
    wr = StateVector(p=0.1, rho=0.125, u=0, eos=eos)
    wr.complete()
...
def eos_test(plot=True):
    ...
    r0 = 1e-7
    p0 = 3e-8
    wl = dict(rho=r0, p=p0, u=0.0)
    wr = dict(rho=0.125 * r0, p=1e-9, u=0.0)
    eoss = [SimpleHydrogen()]  # first used EOS
    # initialize Riemann problem
    sols = [
        RiemannSol(
            StateVector(eos=eoss[-1], **wl), StateVector(eos=eoss[-1], **wr), eoss[-1]
        )
    ]
