...
class StateVector(object):
    ...
    def complete(self, ignore_u=False):
        ...
        if indep in need:
            ...
            if sol is None:
                ...
                def f(y):
                    eos_f(self.rho, np.exp(y)) / val - 1.0
    ...
    def flux(self):
        ...
        ram = self.ram()
        return self.rho * self.u, ram + self.p, (0.5 * ram + self.p + self.ei) * self.u
...
class RiemannSol(object):
    ...
    def __init__(self, left, right, eos):
        ...
        b = 0.01 * np.sqrt(
            max(self.left.p / self.left.rho, self.right.p / self.right.rho)
        )
        self._norm["speed"] = 1.0 / max(a, b)
        # set lower bound for middle pressure
        if left.u <= 0 and right.u >= 0:
            self._pl = min(left.p_min(), right.p_min())
        else:
            self._pl = 0.5 * min(left.p, right.p)
        self._pu = (
            max(self.left.ram(), self.right.ram()) * 2
        )  # upper bound for middle pressure
    ...
    def _shock_jump(self, edge, mid):
        ...
        if self.eos.ideal:
            ...
            def f(fm):
                ...
                ei = (edge.ei * fsqr + 0.5 * dp * psum * edge.rho) / (
                    fsqr - dp * edge.rho
                )
                out = self.eos.ei_of_rho_p(rho, mid.p) / ei - 1.0
                return out

            # determine bounding fluxes (fa, fb) for root find
            eu = 2.0 * self._pu
            if dp > 0:
                fa = (1.0 + 1e-4) * (dp * edge.rho) ** 0.5
            else:
                fa = 1e-99
            fb = 10.0 * np.sqrt(0.5 * dp * (2.0 * eu + psum) * edge.rho / edge.ei)
            # find root
        ...
        try:
            ...
            if self.waves[0]["speed"][0] == xi:
                if self.waves[0]["kind"] == "simple":
                    return self.left
                tmp = {
                    i: (self.left[i] if self.left[i] == self.lmid[i] else np.nan)
                    for i in args
                }
            ...
            if self.waves[1]["speed"][0] == xi:
                tmp = {
                    i: (self.lmid[i] if self.lmid[i] == self.rmid[i] else np.nan)
                    for i in args
                }
                return StateVector(eos=self.eos, **tmp)
            if self.waves[2]["speed"][0] == xi:
                if self.waves[2]["kind"] == "simple":
                    return self.rmid
                tmp = {
                    i: (self.right[i] if self.rmid[i] == self.right[i] else np.nan)
                    for i in args
                }
                return StateVector(eos=self.eos, **tmp)
            if self.waves[2]["speed"][1] == xi:
    ...
    def plot_sol(
        ...
        if var.size > 1:
            ...
            if xi_min is None:
                xi_min = min(-0.1, 1.1 * ws[0]["speed"][0])
            if xi_max is None:
                xi_max = max(0.1, 1.1 * ws[2]["speed"][1])
    ...
    def fan_plot(self, xlim=None, ylim=None, fig=True):
        ...
        r = ylim.min() + 0.8 * dy
        th = 0.5 * np.pi + 0.5 * np.arctan2(1, ws[0]["speed"][0])
        plt.text(r * np.cos(th), r * np.sin(th), r"$\mathbf{U}_{L}$", **opt)
        th = 0.5 * np.arctan2(1, ws[0]["speed"][1]) + 0.5 * np.arctan(
            1.0 / ws[1]["speed"][0]
        )
        plt.text(r * np.cos(th), r * np.sin(th), r"$\mathbf{U}_{L*}$", **opt)
        th = 0.5 * np.arctan2(1, ws[1]["speed"][1]) + 0.5 * np.arctan(
            1.0 / ws[2]["speed"][0]
        )
        plt.text(r * np.cos(th), r * np.sin(th), r"$\mathbf{U}_{R*}$", **opt)
        th = 0.5 * np.arctan2(1, ws[2]["speed"][1])
        plt.text(r * np.cos(th), r * np.sin(th), r"$\mathbf{U}_{R}$", **opt)
        opt.pop("ha")
        opt.pop("va")
        aopt = dict(
        ...
        plt.text(0, ylim.max() + 0.05 * dy, "$t$", ha="center", va="bottom", **opt)
        plt.text(xlim.max() + 0.025 * dx, 0, "$x$", ha="left", va="center", **opt)
...
class RareInt(object):
    ...
    def __init__(self, owner, edge, sign=None, slow=False):
        ...
        if self.ideal:
            ...
            def tmp(p):
                g = self.eos._g
                x = 0.5 * self.eos._gm1 / g
                c = np.sqrt(self.edge.p ** (1.0 / g) / (g * self.edge.rho))
                return edge.rho * (p / edge.p) ** (1 / g), c * (
                    p**x - self.edge.p**x
                ) / x
    ...
    def _rare_ode(self, y, p):
        ...
        am2 = 1.0 / self.eos.asq_of_rho_p(y[0], p)
