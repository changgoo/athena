...
def mk_ideal(gamma=5.0 / 3.0, n=2, fn=None, out_type=None):
    dlim = np.linspace(-24.0, 4.0, n)
    elim = np.linspace(-10.0, 20.0, n)

    e, d = np.meshgrid(1e1**elim, 1e1**dlim)
    g = gamma
    gm1 = g - 1.0

    varlist = [gm1, 1.0 / gm1, g]
    ...
    if fn is None:
        ...
        if out_type == "ascii":
            ...
            fn += "data"
    opt = dict(fn=fn, eOp=1.0 / gm1, out_type=out_type)
    if out_type == "hdf5":
        opt["ftype"] = "float32"
