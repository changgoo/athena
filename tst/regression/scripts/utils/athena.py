    ...
    try:
        ...
        try:
            subprocess.check_call(
                configure_command + global_config_args, stdout=out_log, stderr=err_log
            )
...
def make(clean_first=True, obj_only=False):
    ...
    out_log = (
        open(os.devnull, "w") if global_silent else LogPipe("athena.make", logging.INFO)
    )
    try:
...
def restart(input_filename, arguments):
    ...
    try:
        ...
        try:
            ...
            logging.getLogger("athena.run").debug(
                "Executing (restart): " + " ".join(cmd)
            )
            subprocess.check_call(cmd, stdout=out_log)
        except subprocess.CalledProcessError as err:
            raise AthenaError(
...
def mpirun(
    mpirun_cmd, mpirun_opts, nproc, input_filename, arguments, lcov_test_suffix=None
):
    current_dir = os.getcwd()
    os.chdir("bin")
    out_log = LogPipe("athena.run", logging.INFO)
    try:
        ...
        run_command = (
            [mpirun_cmd]
            + mpirun_opts
            + ["-n", str(nproc), "./athena", "-i", input_filename_full]
        )
        run_command = list(filter(None, run_command))  # remove any empty strings
        try:
            cmd = run_command + arguments + global_run_args
            logging.getLogger("athena.run").debug(
                "Executing (mpirun): " + " ".join(cmd)
            )
            subprocess.check_call(cmd, stdout=out_log)
        except subprocess.CalledProcessError as err:
            raise AthenaError(
