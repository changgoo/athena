...
def main(**kwargs):
            ...
            if ispkg:
                dir_test_names = [
                    ...
                    for _, name, _ in iter_modules(
                        path=["scripts/tests/" + directory], prefix=directory + "."
                    )
                ]
        ...
        for test in tests:
            ...
            if "/" in test:  # specific test specified
                ...
                dir_test_names = [
                    name
                    for _, name, _ in iter_modules(
                        path=["scripts/tests/" + test], prefix=test + "."
                    )
                ]
                test_names.extend(dir_test_names)
    ...
    logger.info(
        "Summary: {0} out of {1} {2} passed\n".format(
            num_passed, num_tests, test_string
        )
    )
...
def log_init(args):
    ...
    if kwargs.pop("diagnostics_file"):
        ...
        record = logger.makeRecord(
            "athena.tests", logging.INFO, None, None, "Starting new tests", None, None
        )
...
if __name__ == "__main__":
    help_msg = "names of tests to run, relative to scripts/tests/," "excluding .py"
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "code coverage command to run after a successful test;"
    ...
    parser.add_argument(
        "--logfile", type=str, default=None, help="set filename of logfile"
    )
    parser.add_argument(
        "--diagnostics_file",
        default=False,
