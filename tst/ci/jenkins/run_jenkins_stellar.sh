...
# cooling regression
time python -u ./run_tests.py cooling --config=--cxx=icpx --silent

# particle regression
#(changgoo) temporarily suppressed as it occasionally causes segfalut
#time python -u ./run_tests.py particles --config=--cxx=icpx --mpirun=srun --mpirun_opts=--job-name='ICC particles' --silent
...
#(changgoo) temporarily suppressed as it didn't work on stellar (dunno why)
