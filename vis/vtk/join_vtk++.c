...
/* Added capability to read non-uniform grid stored in RECTILINEAR_GRID dataset
   containing X,Y,Z coordinates. This code can automatically recognize uniform
   and non-uniform linear grids. -- Wendy Ju, Jan. 04, 2015 */
...
int main(int argc, char *argv[]) {
  ...
  domain_3d = (VTK_Domain ***)calloc_3d_array(NGrid_z, NGrid_y, NGrid_x,
                                              sizeof(VTK_Domain));
  if (domain_3d == NULL)
...
static void init_domain_1d(void) {
  ...
  for (i = 0; i < file_count; i++) {
    ...
    if (strcmp(line, "DATASET STRUCTURED_POINTS") == 0)
      grid_version = 1;
    else if (strcmp(line, "DATASET RECTILINEAR_GRID") == 0)
      grid_version = 2;
    else
      join_error("Unsupported file type: %s", line);
    ...
    if (grid_version == 1) { // fixed linear grids in athena 4.2
      ...
      fscanf(fp, "SPACING %le %le %le\n", &(domain_1d[i].dx),
             &(domain_1d[i].dy), &(domain_1d[i].dz));
      printf("SPACING %e %e %e\n", domain_1d[i].dx, domain_1d[i].dy,
             domain_1d[i].dz);
      ...
      if (nx > 1)
        domain_1d[i].dx = 1.0; // just to make it nonzero
      else
        domain_1d[i].dx = 0.0;
      if (ny > 1)
        domain_1d[i].dy = 1.0;
...
static int compare_ox(const void *p1, const void *p2) {
  ...
  if (((VTK_Domain *)p1)->ox < ((VTK_Domain *)p2)->ox)
    return -1;
  else if (((VTK_Domain *)p1)->ox > ((VTK_Domain *)p2)->ox)
    return 1;
  else
    return 0;
}
...
static int compare_oy(const void *p1, const void *p2) {
  ...
  if (((VTK_Domain *)p1)->oy < ((VTK_Domain *)p2)->oy)
    return -1;
  else if (((VTK_Domain *)p1)->oy > ((VTK_Domain *)p2)->oy)
    return 1;
  else
    return 0;
}
...
static int compare_oz(const void *p1, const void *p2) {
  ...
  if (((VTK_Domain *)p1)->oz < ((VTK_Domain *)p2)->oz)
    return -1;
  else if (((VTK_Domain *)p1)->oz > ((VTK_Domain *)p2)->oz)
    return 1;
  else
    return 0;
}
