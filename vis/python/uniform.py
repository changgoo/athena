...
def main(**kwargs):
    ...
    for n in file_nums_local:
        ...
        nx1 = (
            attrs["RootGridSize"][0] * 2**level if attrs["MeshBlockSize"][0] > 1 else 1
        )
        nx2 = (
            attrs["RootGridSize"][1] * 2**level if attrs["MeshBlockSize"][1] > 1 else 1
        )
        nx3 = (
            attrs["RootGridSize"][2] * 2**level if attrs["MeshBlockSize"][2] > 1 else 1
        )
        ...
        with h5py.File(output_filename, "w") as f:
            ...
            f.create_dataset(
                "LogicalLocations", data=[0, 0, 0], dtype=">i8", shape=(1, 3)
            )
            ...
            for dataset_name, num_vars in zip(
                ...
                f.create_dataset(
                    dataset_name, dtype=">f4", shape=(num_vars, 1, nx3, nx2, nx1)
                )
        ...
        if kwargs["x"]:
            with open(output_filename + ".xdmf", "w") as f:
                ...
                for nx, xf_string in zip((nx1, nx2, nx3), ("x1f", "x2f", "x3f")):
                    ...
                    f.write(
                        (
                            '        <DataItem Dimensions="3 2" NumberType="Int">'
...
if __name__ == "__main__":
    ...
    parser.add_argument("start", type=int, help="first file number to be converted")
    parser.add_argument("end", type=int, help="last file number to be converted")
    parser.add_argument(
        "stride", type=int, default=0, help="stride in file numbers to be converted"
    )
    parser.add_argument(
        "-m", action="store_true", help="flag indicating this is an MPI job"
    )
    parser.add_argument(
        "-x",
        action="store_false",
    ...
    parser.add_argument(
        "-q", "--quantities", type=str, nargs="+", help="names of quantities to extract"
    )
