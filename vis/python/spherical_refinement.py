...
def main(**kwargs):
    ...
    if (
        num_r % num_r_block != 0
        or num_theta % num_theta_block != 0
    ...
    if max_levels > 0 and (
        ...
        raise RuntimeError(
            "blocks must have even number of cells in all dimensions to \
        support refinement"
        )
    ...
    if minimum_width is None:
        ...
        theta2 = theta_adjust(
            pos_face(theta_min, theta_max, 1.0, num_theta, 1), theta_compress
        )
        w_r_min, w_theta_min, w_phi_min = widths(
            r1, r2, theta1, theta2, delta_phi, metric, parameters
        )
    ...
    for L in range(max_levels + 1):
        ...
        for i in range(num_blocks_r + 1):
            r_bounds[L][i] = pos_face(
                r_min, r_max, r_ratio ** (1.0 / 2**L), num_r * 2**L, i * num_r_block
            )
        for j in range(num_blocks_theta + 1):
            theta_unadjusted = pos_face(
                theta_min, theta_max, 1.0, num_theta * 2**L, j * num_theta_block
            )
        ...
        for i in range(num_blocks_r):
            ...
            for j in range(num_blocks_theta):
                ...
                w_r, w_theta, w_phi = widths(
                    r1, r2, theta1, theta2, delta_phi / 2**L, metric, parameters
                )
    ...
    num_blocks[0] = (
        (num_r / num_r_block)
        * (num_theta / num_theta_block)
        * (num_phi / num_phi_block)
    )
        ...
        for i in range(num_blocks_r + 1):
            ...
            if j_lim != previous_j_lim:
                if previous_j_lim >= 0:
                    ...
                    num_blocks_refined = (
                        num_refined_r * num_refined_theta * num_refined_phi
                    )
    ...
    if num_refinement == 0:
        ...
        print(
            "\nThe following {} input blocks can be used to specify maximal refinement:".format(
                num_refinement
            )
        )
    for refinement_num, (L, i_start, i_end, j_lim) in enumerate(refinement_regions):
        ...
        if j_lim == 0:
            ...
            theta1_unadjusted = pos_face(
                theta_bounds[L][j_lim],
                theta_bounds[L][j_lim + 1],
                1.0,
                num_theta_block,
                num_theta_block / 2,
            )
    ...
    for L in range(max_levels + 1):
        ...
        string = (
            "    Level {0}:"
            + " " * (max_digits_level - digits_level)
            + " {1:"
            + repr(max_digits_count)
            + "d}"
        )
    ...
    if r_ratio != r_ratio_optimal:
        print(
            "Suggested ratio (so that r / Delta r is constant): "
    ...
    if output is not None:
        plot_grid(
            refinement, r_bounds, theta_bounds, output, colormap, grid_refined, log
        )
...
def log_ratio(f, n):
    ...
    def res(ratio):
        return 1.0 + 1.0 / (f - 1.0) * (1.0 - f / ratio ** (n - 1)) * (
            ratio**n - 1.0
        ) / (ratio - 1.0)
...
def theta_adjust(theta_unadjusted, theta_compress):
    theta_adjusted = theta_unadjusted + (1.0 - theta_compress) / 2.0 * np.sin(
        2.0 * theta_unadjusted
    )
...
def plot_grid(refinement, r_bounds, theta_bounds, output, colormap, grid_refined, log):
    ...
    if theta_bounds[0][0] == 0.0:
        ...
        theta_vals = np.concatenate(
            (
                -theta_vals[0:1],
                theta_vals,
                2.0 * np.pi - theta_vals[::-1],
                2.0 * np.pi + theta_vals[0:1],
            )
        )
    else:
        theta_north = theta[1:] - eps * delta_theta
    ...
    for level, refinement_current in enumerate(refinement[:-1]):
        ...
        for (i, j), refined in np.ndenumerate(refinement_current):
            if refined:
                levels[
                    i * block_size + 1 : (i + 1) * block_size + 1,
    ...
    for r_val in r_vals_grid:
        ...
        theta4 = 180.0 - theta2
        arc = patches.Arc(
            (0, 0), r_val_plot * 2, r_val_plot * 2, theta1=theta2, theta2=theta1
        )
        ax.add_artist(arc)
        arc = patches.Arc(
            (0, 0), r_val_plot * 2, r_val_plot * 2, theta1=theta3, theta2=theta4
        )
        ax.add_artist(arc)
    ...
    for level in range(min(max_level, grid_refined)):
        for (i, j), refined in np.ndenumerate(refinement[level]):
            if refined:
                ...
                theta4 = 180.0 - theta2
                arc = patches.Arc(
                    (0, 0), r_val * 2, r_val * 2, theta1=theta2, theta2=theta1
                )
                ax.add_artist(arc)
                arc = patches.Arc(
                    (0, 0), r_val * 2, r_val * 2, theta1=theta3, theta2=theta4
                )
                ax.add_artist(arc)
...
if __name__ == "__main__":
    ...
    parser.add_argument("r_min", type=float, help="minimum radius")
    parser.add_argument("r_max", type=float, help="maximum radius")
    parser.add_argument(
        "theta_min",
        type=float,
        help=(
            "minimum polar angle (maximum is assumed to be pi-complement); polar "
            "boundaries assumed if this is 0"
        ),
    )
    parser.add_argument("num_r", type=int, help="number of cells in radial direction")
    parser.add_argument(
        "num_theta", type=int, help="number of cells in polar direction"
    )
    parser.add_argument(
        "num_phi", type=int, help="number of cells in azimuthal direction"
    )
    parser.add_argument(
        "num_r_block", type=int, help="number of cells in radial direction in one block"
    )
    parser.add_argument(
        "num_theta_block",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "parameters (mass M, possibly spin a, 0 <= a < M) to be used if metric is "
            "specified"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--minimum_width", type=float, help="override for smallest allowed cell width"
    )
    parser.add_argument(
        "-o",
        "--output",
        help=(
            'name of image file to write showing grid; use "show" to show interactive '
            "plot instead"
        ),
    )
    parser.add_argument("-c", "--colormap", default="cool", help="name of colormap")
    parser.add_argument(
        "-g",
        "--grid_refined",
        type=int,
