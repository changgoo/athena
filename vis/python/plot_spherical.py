...
def main(**kwargs):
    ...
    if kwargs["theta_compression"] is not None:
        ...
        if quantities[0] == "Levels":
            ...
            data = athena_read.athdf(
                kwargs["data_file"], quantities=quantities, level=level
            )
    ...
    if kwargs["midplane"]:
        ...
        theta_face_extended = np.concatenate(
            (theta_face, 2.0 * np.pi - theta_face[-2::-1])
        )
    ...
    if kwargs["stream"] is not None:
        ...
        if kwargs["midplane"]:
            ...
            phi_grid_stream_coord = np.pi + np.arctan2(
                -y_grid_stream.T, -x_grid_stream.T
            )
            phi_grid_stream_pix = (
                (phi_grid_stream_coord + phi[0]) / (2.0 * np.pi + 2.0 * phi[0])
            ) * (nx3 + 1)
        else:
            ...
            if kwargs["theta_compression"] is None:
                theta_grid_stream_pix = (
                    (theta_grid_stream_coord + theta[0])
                    / (2.0 * np.pi + 2.0 * theta[0])
                ) * (2 * nx2 + 1)
            else:
                ...
                for (i, j), theta_val in np.ndenumerate(theta_grid_stream_coord):
                    ...
                    if index < 0:
                        ...
                        theta_grid_stream_pix[i, j] = index + (
                            (theta_val - theta_extended[index])
        ...
        for (i, j), r_val in np.ndenumerate(r_grid_stream_coord):
            ...
            if index < 0:
                ...
                r_grid_stream_pix[i, j] = index + (r_val - r[index]) / (
                    r[index + 1] - r[index]
                )
            else:
    ...
    if kwargs["midplane"]:
        if nx2 % 2 == 0:
            vals = np.mean(
                data[kwargs["quantity"]][:, nx2 / 2 - 1 : nx2 / 2 + 1, :], axis=1
            )
        else:
        ...
        if kwargs["average"]:
            ...
            vals_right = 0.5 * (
                data[kwargs["quantity"]][-1, :, :] + data[kwargs["quantity"]][0, :, :]
            )
            vals_left = 0.5 * (
                data[kwargs["quantity"]][(nx3 / 2) - 1, :, :]
    ...
    if kwargs["stream"] is not None:
        if kwargs["midplane"]:
            if nx2 % 2 == 0:
                vals_r = np.mean(
                    data[kwargs["stream"] + "1"][:, nx2 / 2 - 1 : nx2 / 2 + 1, :],
                    axis=1,
                ).T
                vals_phi = np.mean(
                    data[kwargs["stream"] + "3"][:, nx2 / 2 - 1 : nx2 / 2 + 1, :],
                    axis=1,
                ).T
            else:
    ...
    if kwargs["stream"] is not None:
        if kwargs["midplane"]:
            ...
            vals_r = map_coordinates(
                vals_r, (r_grid_stream_pix, phi_grid_stream_pix), order=1, cval=np.nan
            )
            vals_phi = np.hstack((vals_phi[:, -1:], vals_phi, vals_phi[:, :1]))
            vals_phi = map_coordinates(
                vals_phi, (r_grid_stream_pix, phi_grid_stream_pix), order=1, cval=np.nan
            )
        else:
            vals_r = np.hstack(
                (
            ...
            vals_r = map_coordinates(
                vals_r, (r_grid_stream_pix, theta_grid_stream_pix), order=1, cval=np.nan
            )
            vals_theta = np.hstack(
                (
                    vals_theta_left[:, :1],
                    vals_theta_right,
                    vals_theta_left[:, ::-1],
    ...
    plt.figure()
    im = plt.pcolormesh(
        x_grid, y_grid, vals, cmap=cmap, vmin=vmin, vmax=vmax, norm=norm
    )
    if kwargs["stream"] is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("quantity", help="name of quantity to be plotted")
    parser.add_argument(
        "output_file",
        help=(
            "name of output to be (over)written, possibly including "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "flag indicating plot should be midplane (r,phi) rather " "than (r,theta)"
        ),
    )
    parser.add_argument(
        "-a",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "refinement level to be used in plotting (default: max " "level in file)"
        ),
    )
    parser.add_argument(
        "-r", "--r_max", type=float, default=None, help="maximum radial extent of plot"
    )
    parser.add_argument(
        "--logr",
        action="store_true",
        help="flag indicating data should be plotted logarithmically in " "radius",
    )
    parser.add_argument(
        "-c",
        "--colormap",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "data value to correspond to colormap minimum; use "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "data value to correspond to colormap maximum; use "
    ...
    parser.add_argument(
        "--stream_density", type=float, default=1.0, help="density of stream lines"
    )
    parser.add_argument(
        "--stream_samples",
        type=int,
