...
Real CosmicRay::Get_SigmaParallel(Real rho, Real Press, Real ecr,
                                  Real grad_pc_par) {
  Real sigma_par;
  ...
  if (self_consistent_flag == 0) {
    ...
    if (ecr < TINY_NUMBER)
      ecr_in_cgs = TINY_NUMBER * punit->EnergyDensity;
    ...
    Real sigma_in = 0.1 * 3. * PI / 16. * grad_pc_par * punit->e_in_code /
                    punit->c_in_code / kinenergy_in_code / std::sqrt(rho_ion) *
                    2. / (nu_in_code * nn);
    Real sigma_nll =
        std::sqrt(0.1 * 3. * PI / 16. * grad_pc_par * punit->e_in_code /
                  punit->c_in_code / kinenergy_in_code / std::sqrt(rho_ion) /
                  (0.3 * vi * punit->c_in_code));
...
Real Get_IonFraction(Real Temp, Real rho, Real ecr, Real ion_rate_norm) {
  ...
  if (Temp < 2e4)
    xi = 0.5 * (std::sqrt(std::pow(beta + gammaCR + xM, 2) + 4. * beta) -
                (beta + gammaCR + xM)) +
         xM;
  else
    xi = 1.099;
