      ...
      for (int j = jl; j <= ju; ++j) {
        ...
        for (int i = il; i <= iu; ++i) {
          ...
          pcr->sigma_diff(0, k, j, i) = pcr->Get_SigmaParallel(
              prim(IDN, k, j, i), prim(IPR, k, j, i), u_cr(CRE, k, j, i),
          ...
          if (pcr->stream_flag) {
            ...
            if (va < TINY_NUMBER) {
              ...
              pcr->sigma_adv(0, k, j, i) =
                  std::fabs(b_grad_pc) /
                  (btot * va * (1.0 + 1.0 / 3.0) * invlim * u_cr(CRE, k, j, i));
            }
            pcr->sigma_adv(1, k, j, i) = pcr->max_opacity;
            pcr->sigma_adv(2, k, j, i) = pcr->max_opacity;
          }
          // Here we calculate the angles of B needed to compute the rotation
          // matrix The information stored in the array b_angle is
...
CosmicRay::CosmicRay(MeshBlock *pmb, ParameterInput *pin)
    : u_cr(NCR, pmb->ncells3, pmb->ncells2, pmb->ncells1),
      ...
      // constructor overload resolution of non-aggregate class type
      // AthenaArray<Real>
      ...
      flux{{NCR, pmb->ncells3, pmb->ncells2, pmb->ncells1 + 1},
           ...
           {NCR, pmb->ncells3 + 1, pmb->ncells2, pmb->ncells1,
            (pmb->pmy_mesh->f3 ? AthenaArray<Real>::DataStatus::allocated
                               : AthenaArray<Real>::DataStatus::empty)}},
      pmy_block(pmb), cr_bvar(pmb, &u_cr, &coarse_cr_, flux),
      UserSourceTerm_{} {
  Mesh *pm = pmy_block->pmy_mesh;
  // "Enroll" in S/AMR by adding to vector of tuples of pointers in
  ...
  // Input parameters
  vmax = pin->GetOrAddReal("cr", "vmax",
                           1.0); // this should be in code units already
  ...
  ion_rate_norm = pin->GetOrAddReal(
      "cr", "ion_rate_norm",
...
void CosmicRay::EnrollTemperatureFunction(
    CRTemperatureFunc MyTemperatureFunction) {
