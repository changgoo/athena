...
void CRIntegrator::CalculateFluxes(AthenaArray<Real> &w, AthenaArray<Real> &bcc,
  ...
  for (int k = 0; k < ncells3; ++k) {
    for (int j = 0; j < ncells2; ++j) {
      ...
      for (int i = 0; i < ncells1; ++i) {
        Real cr_sound_x =
            vel_flx_flag_ *
            std::sqrt((4.0 / 9.0) * cr(CRE, k, j, i) / w(IDN, k, j, i)) *
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      if (order == 1) {
        pmb->precon->DonorCellX1(k, j, is - 1, ie + 1, ucr_vel_, ucr_l_,
                                 ucr_r_);
      } else if (order == 2) {
        pmb->precon->PiecewiseLinearX1(k, j, is - 1, ie + 1, ucr_vel_, ucr_l_,
                                       ucr_r_);
      } else {
        pmb->precon->PiecewiseParabolicX1(k, j, is - 1, ie + 1, ucr_vel_,
                                          ucr_l_, ucr_r_);
      }
  ...
  if (pmb->pmy_mesh->f2) {
    ...
    for (int k = kl; k <= ku; ++k) {
      ...
      if (order == 1) {
        ...
        pmb->precon->PiecewiseLinearX2(k, js - 1, il, iu, ucr_vel_, ucr_l_,
                                       ucr_r_);
      } else {
        pmb->precon->PiecewiseParabolicX2(k, js - 1, il, iu, ucr_vel_, ucr_l_,
                                          ucr_r_);
      }
      ...
      for (int j = js; j <= je + 1; ++j) {
        if (order == 1) {
          ...
          pmb->precon->PiecewiseLinearX2(k, j, il, iu, ucr_vel_, ucr_lb_,
                                         ucr_r_);
        } else {
          pmb->precon->PiecewiseParabolicX2(k, j, il, iu, ucr_vel_, ucr_lb_,
                                            ucr_r_);
        }
  ...
  if (pmb->pmy_mesh->f3) {
    ...
    for (int j = jl; j <= ju; ++j) {
      if (order == 1) {
        ...
        pmb->precon->PiecewiseLinearX3(ks - 1, j, il, iu, ucr_vel_, ucr_l_,
                                       ucr_r_);
      } else {
        pmb->precon->PiecewiseParabolicX3(ks - 1, j, il, iu, ucr_vel_, ucr_l_,
                                          ucr_r_);
      }
      ...
      for (int k = ks; k <= ke + 1; ++k) {
        ...
        if (order == 1) {
          ...
          pmb->precon->PiecewiseLinearX3(k, j, il, iu, ucr_vel_, ucr_lb_,
                                         ucr_r_);
        } else {
          pmb->precon->PiecewiseParabolicX3(k, j, il, iu, ucr_vel_, ucr_lb_,
                                            ucr_r_);
        }
...
#pragma omp simd
        ...
        // need to subtract the coordinate source term to get the actual grad Pc
        // for c
        ...
        for (int i = is; i <= ie; ++i) {
          ...
          if (pcr->self_consistent_flag == 0) {
            ...
            Real rhoi = pcr->Get_IonDensity(w(IDN, k, j, i), w(IPR, k, j, i),
                                            cr(CRE, k, j, i));
          ...
          if (pcr->stream_flag) {
            ...
            if (va > TINY_NUMBER) {
              pcr->sigma_adv(0, k, j, i) =
                  std::fabs(b_grad_pc) / (std::sqrt(pb) * va * (4.0 / 3.0) *
                                          invlim * cr(CRE, k, j, i));
          ...
          pcr->sigma_diff(0, k, j, i) = pcr->Get_SigmaParallel(
              w(IDN, k, j, i), w(IPR, k, j, i), cr(CRE, k, j, i),
              std::fabs(b_grad_pc) / std::sqrt(pb));
          if (pcr->perp_diff_flag == 0) {
            pcr->sigma_diff(1, k, j, i) = pcr->max_opacity;
            pcr->sigma_diff(2, k, j, i) = pcr->max_opacity;
          } else {
            pcr->sigma_diff(1, k, j, i) =
                pcr->sigma_diff(0, k, j, i) * pcr->perp_to_par_diff;
          ...
          RotateVec(pcr->b_angle(0, k, j, i), pcr->b_angle(1, k, j, i),
                    pcr->b_angle(2, k, j, i), pcr->b_angle(3, k, j, i), dpcdx,
                    dpcdy, dpcdz);

          RotateVec(pcr->b_angle(0, k, j, i), pcr->b_angle(1, k, j, i),
                    pcr->b_angle(2, k, j, i), pcr->b_angle(3, k, j, i), v1, v2,
                    v3);
