...
//----------------------------------------------------------------------------------------
//! \fn Multigrid::Multigrid(MultigridDriver *pmd, MeshBlock *pmb, int invar,
//! int nghost)
//  \brief Multigrid constructor

Multigrid::Multigrid(MultigridDriver *pmd, MeshBlock *pmb, int invar,
                     int nghost)
    : pmy_driver_(pmd), pmy_block_(pmb), ngh_(nghost), nvar_(invar),
      defscale_(1.0) {
  if (pmy_block_ != nullptr) {
    ...
    if (size_.nx1 != size_.nx2 || size_.nx1 != size_.nx3) {
      ...
      msg << "### FATAL ERROR in Multigrid::Multigrid" << std::endl
          << "The Multigrid solver requires logically cubic MeshBlock."
          << std::endl;
      ATHENA_ERROR(msg);
      return;
    }
  ...
  if (pmy_block_ == nullptr) { // root
    ...
    for (int l = 0; l < 20; l++) {
      if (size_.nx1 % (1 << l) == 0 && size_.nx2 % (1 << l) == 0 &&
          size_.nx3 % (1 << l) == 0) {
        nbx = size_.nx1 / (1 << l), nby = size_.nx2 / (1 << l),
        nbz = size_.nx3 / (1 << l);
        nlevel_ = l + 1;
      }
    }
    ...
    if (nmaxr != 1 && Globals::my_rank == 0) {
      std::cout
          ...
          << " as the coarsest level is not solved exactly but iteratively."
          << std::endl;
    }
    if (nbx * nby * nbz > 100 && Globals::my_rank == 0) {
      std::cout
          ...
          << nbx << " x " << nby << " x " << nbz << " = " << nbx * nby * nbz
          << std::endl
          << "Multigrid should still work, but this is not efficient "
             "configuration "
...
//----------------------------------------------------------------------------------------
//! \fn void Multigrid::LoadFinestData(const AthenaArray<Real> &src, int ns, int
//! ngh)
...
//----------------------------------------------------------------------------------------
//! \fn void Multigrid::LoadSource(const AthenaArray<Real> &src, int ns, int
//! ngh,
//                                 Real fac)
...
void Multigrid::RestrictFMGSource() {
  ...
  for (current_level_ = nlevel_ - 1; current_level_ > 0; current_level_--) {
    ...
    ie = is + (size_.nx1 >> ll) - 1, je = js + (size_.nx2 >> ll) - 1,
    ke = ks + (size_.nx3 >> ll) - 1;
    Restrict(src_[current_level_ - 1], src_[current_level_], is, ie, js, je, ks,
             ke);
  }
  return;
}
...
void Multigrid::RetrieveResult(AthenaArray<Real> &dst, int ns, int ngh) {
  ...
  int ie = size_.nx1 + ngh_ + sngh - 1, je = size_.nx2 + ngh_ + sngh - 1,
      ke = size_.nx3 + ngh_ + sngh - 1;
  for (int v = 0; v < nvar_; ++v) {
    int ndst = ns + v;
...
void Multigrid::RestrictBlock() {
  ...
  Restrict(src_[current_level_ - 1], def_[current_level_], is, ie, js, je, ks,
           ke);

  // Full Approximation Scheme - restrict the variable itself
  if (pmy_driver_->ffas_)
    Restrict(u_[current_level_ - 1], u_[current_level_], is, ie, js, je, ks,
             ke);
...
void Multigrid::ProlongateAndCorrectBlock() {
  ...
  ProlongateAndCorrect(u_[current_level_ + 1], u_[current_level_], is, ie, js,
                       je, ks, ke, ngh_, ngh_, ngh_);
...
void Multigrid::FMGProlongateBlock() {
  ...
  FMGProlongate(u_[current_level_ + 1], u_[current_level_], is, ie, js, je, ks,
                ke, ngh_, ngh_, ngh_);
...
void Multigrid::SmoothBlock(int color) {
  ...
  Smooth(u_[current_level_], src_[current_level_], -ll, is, ie, js, je, ks, ke,
         color);
...
void Multigrid::CalculateDefectBlock() {
  ...
  CalculateDefect(def_[current_level_], u_[current_level_],
                  src_[current_level_], -ll, is, ie, js, je, ks, ke);
...
void Multigrid::CalculateFASRHSBlock() {
  ...
  CalculateFASRHS(src_[current_level_], u_[current_level_], -ll, is, ie, js, je,
                  ks, ke);
...
void Multigrid::SubtractAverage(MGVariable type, int n, Real ave) {
  AthenaArray<Real> &dst =
      (type == MGVariable::src) ? src_[nlevel_ - 1] : u_[nlevel_ - 1];
  int is, ie, js, je, ks, ke;
  is = js = ks = 0;
  ie = is + size_.nx1 + 1, je = js + size_.nx2 + 1, ke = ks + size_.nx3 + 1;
  for (int k = ks; k <= ke; ++k) {
...
//  \brief get the value on the coarsest level in the MG block (type: 0=src,
//  1=u)
...
//----------------------------------------------------------------------------------------
//! \fn void Multigrid::SetData(MGVariable type, int n, int k, int j, int i,
//! Real v)
...
//----------------------------------------------------------------------------------------
//! \fn void Multigrid::Restrict(AthenaArray<Real> &dst, const AthenaArray<Real>
//! &src,
    ...
    for (int k = kl, fk = kl; k <= ku; ++k, fk += 2) {
      for (int j = jl, fj = jl; j <= ju; ++j, fj += 2) {
        for (int i = il, fi = il; i <= iu; ++i, fi += 2)
          dst(v, k, j, i) =
              0.125 *
              (src(v, fk, fj, fi) + src(v, fk, fj, fi + 1) +
...
//      const AthenaArray<Real> &src, int il, int iu, int jl, int ju, int kl,
//      int ku, int fil, int fjl, int fkl)
//  \brief Actual implementation of prolongation and correction

void Multigrid::ProlongateAndCorrect(AthenaArray<Real> &dst,
                                     const AthenaArray<Real> &src, int il,
    ...
    for (int k = kl, fk = fkl; k <= ku; ++k, fk += 2) {
      for (int j = jl, fj = fjl; j <= ju; ++j, fj += 2) {
        for (int i = il, fi = fil; i <= iu; ++i, fi += 2) {
          dst(v, fk, fj, fi) +=
              0.015625 *
              (27.0 * src(v, k, j, i) + src(v, k - 1, j - 1, i - 1) +
               9.0 * (src(v, k, j, i - 1) + src(v, k, j - 1, i) +
                      src(v, k - 1, j, i)) +
               3.0 * (src(v, k - 1, j - 1, i) + src(v, k - 1, j, i - 1) +
                      src(v, k, j - 1, i - 1)));
          dst(v, fk, fj, fi + 1) +=
              0.015625 *
              (27.0 * src(v, k, j, i) + src(v, k - 1, j - 1, i + 1) +
...
//           const AthenaArray<Real> &src, int il, int iu, int jl, int ju, int
//           kl, int ku int fil, int fjl, int fkl)
//  \brief Actual implementation of FMG prolongation

void Multigrid::FMGProlongate(AthenaArray<Real> &dst,
                              const AthenaArray<Real> &src, int il, int iu,
                              int jl, int ju, int kl, int ku, int fil, int fjl,
                              int fkl) {
  for (int v = 0; v < nvar_; ++v) {
    for (int k = kl, fk = fkl; k <= ku; ++k, fk += 2) {
      for (int j = jl, fj = fjl; j <= ju; ++j, fj += 2) {
        for (int i = il, fi = fil; i <= iu; ++i, fi += 2) {
          dst(v, fk, fj, fi) =
              (+125. * src(v, k - 1, j - 1, i - 1) +
               750. * src(v, k - 1, j - 1, i) -
               75. * src(v, k - 1, j - 1, i + 1) +
               750. * src(v, k - 1, j, i - 1) + 4500. * src(v, k - 1, j, i) -
               450. * src(v, k - 1, j, i + 1) -
               75. * src(v, k - 1, j + 1, i - 1) -
               450. * src(v, k - 1, j + 1, i) +
               45. * src(v, k - 1, j + 1, i + 1) +
               750. * src(v, k, j - 1, i - 1) + 4500. * src(v, k, j - 1, i) -
               450. * src(v, k, j - 1, i + 1) + 4500. * src(v, k, j, i - 1) +
               27000. * src(v, k, j, i) - 2700. * src(v, k, j, i + 1) -
               450. * src(v, k, j + 1, i - 1) - 2700. * src(v, k, j + 1, i) +
               270. * src(v, k, j + 1, i + 1) -
               75. * src(v, k + 1, j - 1, i - 1) -
               450. * src(v, k + 1, j - 1, i) +
               45. * src(v, k + 1, j - 1, i + 1) -
               450. * src(v, k + 1, j, i - 1) - 2700. * src(v, k + 1, j, i) +
               270. * src(v, k + 1, j, i + 1) +
               45. * src(v, k + 1, j + 1, i - 1) +
               270. * src(v, k + 1, j + 1, i) -
               27. * src(v, k + 1, j + 1, i + 1)) /
              32768.0;
          dst(v, fk, fj, fi + 1) =
              (-75. * src(v, k - 1, j - 1, i - 1) +
               750. * src(v, k - 1, j - 1, i) +
               125. * src(v, k - 1, j - 1, i + 1) -
               450. * src(v, k - 1, j, i - 1) + 4500. * src(v, k - 1, j, i) +
               750. * src(v, k - 1, j, i + 1) +
               45. * src(v, k - 1, j + 1, i - 1) -
               450. * src(v, k - 1, j + 1, i) -
               75. * src(v, k - 1, j + 1, i + 1) -
               450. * src(v, k, j - 1, i - 1) + 4500. * src(v, k, j - 1, i) +
               750. * src(v, k, j - 1, i + 1) - 2700. * src(v, k, j, i - 1) +
               27000. * src(v, k, j, i) + 4500. * src(v, k, j, i + 1) +
               270. * src(v, k, j + 1, i - 1) - 2700. * src(v, k, j + 1, i) -
               450. * src(v, k, j + 1, i + 1) +
               45. * src(v, k + 1, j - 1, i - 1) -
               450. * src(v, k + 1, j - 1, i) -
               75. * src(v, k + 1, j - 1, i + 1) +
               270. * src(v, k + 1, j, i - 1) - 2700. * src(v, k + 1, j, i) -
               450. * src(v, k + 1, j, i + 1) -
               27. * src(v, k + 1, j + 1, i - 1) +
               270. * src(v, k + 1, j + 1, i) +
               45. * src(v, k + 1, j + 1, i + 1)) /
          ...
          dst(v, fk, fj + 1, fi) =
              (-75. * src(v, k - 1, j - 1, i - 1) -
               ...
               75. * src(v, k - 1, j + 1, i + 1) -
               450. * src(v, k, j - 1, i - 1) - 2700. * src(v, k, j - 1, i) +
               270. * src(v, k, j - 1, i + 1) + 4500. * src(v, k, j, i - 1) +
               27000. * src(v, k, j, i) - 2700. * src(v, k, j, i + 1) +
               750. * src(v, k, j + 1, i - 1) + 4500. * src(v, k, j + 1, i) -
               450. * src(v, k, j + 1, i + 1) +
               45. * src(v, k + 1, j - 1, i - 1) +
          ...
          dst(v, fk, fj + 1, fi + 1) =
              (+45. * src(v, k - 1, j - 1, i - 1) -
               ...
               125. * src(v, k - 1, j + 1, i + 1) +
               270. * src(v, k, j - 1, i - 1) - 2700. * src(v, k, j - 1, i) -
               450. * src(v, k, j - 1, i + 1) - 2700. * src(v, k, j, i - 1) +
               27000. * src(v, k, j, i) + 4500. * src(v, k, j, i + 1) -
               450. * src(v, k, j + 1, i - 1) + 4500. * src(v, k, j + 1, i) +
               750. * src(v, k, j + 1, i + 1) -
               27. * src(v, k + 1, j - 1, i - 1) +
