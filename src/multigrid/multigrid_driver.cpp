...
MultigridDriver::MultigridDriver(Mesh *pm, MGBoundaryFunc *MGBoundary,
                                 int invar)
    : nvar_(invar), mode_(0), // 0: V(1,1) FMG one sweep, 1: FMG + iterative, 2:
                              // V(1,1) iterative
...
//  \brief initialize the source assuming that the source terms are already
//  loaded
...
void MultigridDriver::SetupMultigrid() {
  ...
  if (nreflevel_ > 0 && pmy_mesh_->amr_updated) {
    ...
    for (int l = 0; l < nreflevel_;
         ++l) { // increase the octet array size if needed
...
#ifdef MPI_PARALLEL
  MPI_Allgatherv(MPI_IN_PLACE, nblist_[Globals::my_rank] * nvar_,
                 MPI_ATHENA_REAL, rootbuf_, nvlisti_, nvslisti_,
                 MPI_ATHENA_REAL, MPI_COMM_MULTIGRID);
#endif
...
void MultigridDriver::TransferFromBlocksToRoot(bool initflag) {
  ...
  for (Multigrid *pmg : vmg_) {
    for (int v = 0; v < nvar_; ++v)
      rootbuf_[pmg->pmy_block_->gid * nv + v] =
          pmg->GetCoarsestData(MGVariable::src, v);
    if (ffas_ && !initflag) {
      for (int v = 0; v < nvar_; ++v)
        rootbuf_[pmg->pmy_block_->gid * nv + nvar_ + v] =
            pmg->GetCoarsestData(MGVariable::u, v);
    }
  }

#ifdef MPI_PARALLEL
  if (!initflag)
    MPI_Allgatherv(MPI_IN_PLACE, nblist_[Globals::my_rank] * nv,
                   MPI_ATHENA_REAL, rootbuf_, nvlist_, nvslist_,
                   MPI_ATHENA_REAL, MPI_COMM_MULTIGRID);
  else
    MPI_Allgatherv(MPI_IN_PLACE, nblist_[Globals::my_rank] * nvar_,
                   MPI_ATHENA_REAL, rootbuf_, nvlisti_, nvslisti_,
  ...
  for (int n = 0; n < pmy_mesh_->nbtotal; ++n) {
    ...
    if (loc.level == locrootlevel_) {
      ...
      if (ffas_ && !initflag) {
        for (int v = 0; v < nvar_; ++v)
          mgroot_->SetData(MGVariable::u, v, k, j, i,
                           rootbuf_[n * nv + nvar_ + v]);
      }
    } else {
...
//  \brief Transfer the data from the root grid to the coarsest level of each
//  MeshBlock
...
void MultigridDriver::OneStepToFiner(int nsmooth) {
  ...
  if (current_level_ >= nrootlevel_ + nreflevel_ - 1) { // MeshBlocks
    if (current_level_ == ntotallevel_ - 2)
      flag = 2;
  ...
  while (def > eps_) {
    ...
    if (niter > 0 && def / olddef > 0.9) {
      ...
      if (Globals::my_rank == 0)
        std::cout << "### Warning in MultigridDriver::SolveIterative"
                  << std::endl
    ...
    if (niter > 100) {
      if (Globals::my_rank == 0) {
        std::cout
            ...
            << "Aborting because the # iterations is too large, niter > 100."
            << std::endl
            << "Check the solution as it may not be accurate enough."
            << std::endl;
      }
      break;
    }
...
void MultigridDriver::SolveCoarsestGrid() {
  int ni = (std::max(nrbx1_, std::max(nrbx2_, nrbx3_)) >> (nrootlevel_ - 1));
...
#ifdef MPI_PARALLEL
  if (nrm == MGNormType::max)
    MPI_Allreduce(MPI_IN_PLACE, &norm, 1, MPI_ATHENA_REAL, MPI_MAX,
                  MPI_COMM_MULTIGRID);
  else
  ...
  if (nreflevel_ > 0) {
    ...
    for (int l = nreflevel_ - 1; l >= 1; --l) { // fine octets to coarse octets
      for (int o = 0; o < noctets_[l]; ++o) {
        ...
        mgroot_->Restrict(cbuf_, octets_[l][o].src, os_, oe_, os_, oe_, os_,
                          oe_);
        for (int v = 0; v < nvar_; ++v)
  ...
  if (lev >= 1) { // fine octets to coarse octets
    for (int o = 0; o < noctets_[lev]; ++o) {
      ...
      mgroot_->CalculateDefect(octets_[lev][o].def, octets_[lev][o].u,
                               octets_[lev][o].src, lev + 1, os_, oe_, os_, oe_,
                               os_, oe_);
      mgroot_->Restrict(cbuf_, octets_[lev][o].def, os_, oe_, os_, oe_, os_,
                        oe_);
      for (int v = 0; v < nvar_; ++v)
        octets_[lev - 1][oid].src(v, ok, oj, oi) = cbuf_(v, ngh, ngh, ngh);
      if (ffas_) {
        mgroot_->Restrict(cbuf_, octets_[lev][o].u, os_, oe_, os_, oe_, os_,
                          oe_);
        for (int v = 0; v < nvar_; ++v)
    ...
    for (int o = 0; o < noctets_[0]; ++o) {
      ...
      mgroot_->CalculateDefect(octets_[0][o].def, octets_[0][o].u,
                               octets_[0][o].src, 1, os_, oe_, os_, oe_, os_,
                               oe_);
...
void MultigridDriver::CalculateFASRHSOctets() {
  ...
  for (int o = 0; o < noctets_[lev]; ++o)
    mgroot_->CalculateFASRHS(octets_[lev][o].src, octets_[lev][o].u, lev + 1,
...
void MultigridDriver::SmoothOctets(int color) {
  ...
  for (int o = 0; o < noctets_[lev]; ++o)
    mgroot_->Smooth(octets_[lev][o].u, octets_[lev][o].src, lev + 1, os_, oe_,
                    os_, oe_, os_, oe_, color);
  return;
}
      ...
      if (ffas_) {
        for (int v = 0; v < nvar_; ++v) {
          for (int k = 0; k <= 2; ++k) {
            for (int j = 0; j <= 2; ++j) {
              for (int i = 0; i <= 2; ++i) {
                cbuf_(v, k, j, i) = u(v, rk + k, rj + j, ri + i) -
                                    uold(v, rk + k, rj + j, ri + i);
              }
            }
          }
        }
        mgroot_->ProlongateAndCorrect(octets_[0][o].u, cbuf_, ngh, ngh, ngh,
                                      ngh, ngh, ngh, ngh, ngh, ngh);
      } else {
        mgroot_->ProlongateAndCorrect(octets_[0][o].u, u, ri + 1, ri + 1,
                                      rj + 1, rj + 1, rk + 1, rk + 1, ngh, ngh,
                                      ngh);
      }
    }
      ...
      if (ffas_) {
        for (int v = 0; v < nvar_; ++v) {
          for (int k = 0; k <= 2; ++k) {
            for (int j = 0; j <= 2; ++j) {
              for (int i = 0; i <= 2; ++i)
                cbuf_(v, k, j, i) = uc(v, ck + k, cj + j, ci + i) -
                                    ucold(v, ck + k, cj + j, ci + i);
            }
          }
        }
        mgroot_->ProlongateAndCorrect(octets_[flev][o].u, cbuf_, ngh, ngh, ngh,
                                      ngh, ngh, ngh, ngh, ngh, ngh);
      } else {
        mgroot_->ProlongateAndCorrect(octets_[flev][o].u, uc, ci + 1, ci + 1,
                                      cj + 1, cj + 1, ck + 1, ck + 1, ngh, ngh,
                                      ngh);
      }
    }
  }
...
void MultigridDriver::FMGProlongateOctets() {
  ...
  if (flev == 0) { // from root to octets
    for (int o = 0; o < noctets_[0]; ++o) {
      ...
      mgroot_->FMGProlongate(octets_[0][o].u, mgroot_->GetCurrentData(), ri, ri,
                             rj, rj, rk, rk, ngh, ngh, ngh);
    }
  } else { // from coarse octets to fine octets
    for (int o = 0; o < noctets_[flev]; ++o) {
      const LogicalLocation &loc = octets_[flev][o].loc;
      ...
      mgroot_->FMGProlongate(octets_[flev][o].u, octets_[clev][cid].u, ci, ci,
                             cj, cj, ck, ck, ngh, ngh, ngh);
    }
  }
...
void MultigridDriver::SetBoundariesOctets(bool fprolong, bool folddata) {
  ...
  for (int o = 0; o < noctets_[lev]; ++o) {
    ...
    if (fprolong && octets_[lev][o].fleaf == true)
      continue;
    const LogicalLocation &loc = octets_[lev][o].loc;
    ...
    for (int ox3 = -1; ox3 <= 1; ++ox3) {
      ...
      for (int ox2 = -1; ox2 <= 1; ++ox2) {
        ...
        for (int ox1 = -1; ox1 <= 1; ++ox1) {
          ...
          if (nloc.lx1 < 0) {
            if (MGBoundaryFunction_[BoundaryFace::inner_x1] ==
                MGPeriodicInnerX1)
          ...
          if (nloc.lx1 >= (nrbx1_ << lev)) {
            if (MGBoundaryFunction_[BoundaryFace::outer_x1] ==
                MGPeriodicOuterX1)
              nloc.lx1 = 0;
            else
              continue;
          ...
          if (octetmap_[lev].count(nloc) == 1) { // on the same level
            ...
            SetOctetBoundarySameLevel(u, un, uold, unold, ox1, ox2, ox3,
                                      folddata);
            ...
            if (lev > 0) { // from octet
              ...
              SetOctetBoundaryFromCoarser(un, unold, nloc, ox1, ox2, ox3,
                                          false);
            }
          }
...
//   const AthenaArray<Real> &un, AthenaArray<Real> &uold, const
//   AthenaArray<Real> &unold, int ox1, int ox2, int ox3, bool folddata)
...
void MultigridDriver::SetOctetBoundarySameLevel(AthenaArray<Real> &dst,
                                                const AthenaArray<Real> &un,
                                                AthenaArray<Real> &uold,
                                                const AthenaArray<Real> &unold,
  ...
  if (ox1 == 0)
    is = ngh, ie = ngh + 1, nis = ngh;
  else if (ox1 < 0)
    is = 0, ie = ngh - 1, nis = ngh + 1;
  else
    is = ngh + 2, ie = ngh + 2, nis = ngh;
  if (ox2 == 0)
    js = ngh, je = ngh + 1, njs = ngh;
  else if (ox2 < 0)
    js = 0, je = ngh - 1, njs = ngh + 1;
  else
    js = ngh + 2, je = ngh + 2, njs = ngh;
  if (ox3 == 0)
    ks = ngh, ke = ngh + 1, nks = ngh;
  else if (ox3 < 0)
    ks = 0, ke = ngh - 1, nks = ngh + 1;
  else
    ks = ngh + 2, ke = ngh + 2, nks = ngh;
  ...
  for (int v = 0; v < nvar_; ++v)
    cbuf_(v, ck, cj, ci) =
        fac *
        (un(v, l, l, l) + un(v, l, l, r) + un(v, l, r, l) + un(v, r, l, l) +
  ...
  if (folddata) {
    ...
    for (int v = 0; v < nvar_; ++v)
      cbufold_(v, ck, cj, ci) =
          fac * (unold(v, l, l, l) + unold(v, l, l, r) + unold(v, l, r, l) +
                 unold(v, r, l, l) + unold(v, r, r, l) + unold(v, r, l, r) +
                 unold(v, l, r, r) + unold(v, r, r, r));
  }
...
//----------------------------------------------------------------------------------------
//! \fn void MultigridDriver::SetOctetBoundaryFromCoarser(const
//! AthenaArray<Real> &un,
//                            const AthenaArray<Real> &unold, const
//                            LogicalLocation &loc, int ox1, int ox2, int ox3,
//                            bool folddata) {
//  \brief set a boundary in the coarse buffer from a neighbor Octet on the
//  coarser level

void MultigridDriver::SetOctetBoundaryFromCoarser(
    const AthenaArray<Real> &un, const AthenaArray<Real> &unold,
    const LogicalLocation &loc, int ox1, int ox2, int ox3, bool folddata) {
  int ngh = mgroot_->ngh_;
  int ci, cj, ck;
...
//                                         const LogicalLocation &loc, bool
//                                         fcbuf)
...
void MultigridDriver::ApplyPhysicalBoundariesOctet(AthenaArray<Real> &u,
  ...
  if (loc.lx1 == 0 &&
    ...
    MGBoundaryFunction_[BoundaryFace::inner_x1](
        u, time, nvar_, l, r, bjs, bje, bks, bke, ngh, x0, y0, z0, dx, dy, dz);
  ...
  if (loc.lx2 == 0 &&
    ...
    MGBoundaryFunction_[BoundaryFace::inner_x2](
        u, time, nvar_, bis, bie, l, r, bks, bke, ngh, x0, y0, z0, dx, dy, dz);
  if (loc.lx2 == (nrbx2_ << lev) - 1 &&
      MGBoundaryFunction_[BoundaryFace::outer_x2] != MGPeriodicOuterX2 &&
  ...
  if (loc.lx3 == 0 &&
    ...
    MGBoundaryFunction_[BoundaryFace::inner_x3](
        u, time, nvar_, bis, bie, bjs, bje, l, r, ngh, x0, y0, z0, dx, dy, dz);
  if (loc.lx3 == (nrbx3_ << lev) - 1 &&
      MGBoundaryFunction_[BoundaryFace::outer_x3] != MGPeriodicOuterX3 &&
        ...
        for (int ox1 = -1; ox1 <= 1; ++ox1) {
          ...
          if (octetmap_[lev].count(nloc) == 1) { // same or finer
            ...
            if (lev > 0) { // from octet
              ...
              const AthenaArray<Real> &un = octets_[lev - 1][cid].u;
              const AthenaArray<Real> &unold = octets_[lev - 1][cid].uold;
              SetOctetBoundaryFromCoarser(un, unold, loc, ox1, ox2, ox3,
                                          folddata);
            } else { // from root
              const AthenaArray<Real> &un = mgroot_->GetCurrentData();
              const AthenaArray<Real> &unold = mgroot_->GetCurrentOldData();
...
void MultigridDriver::ProlongateOctetBoundaries(AthenaArray<Real> &u,
  ...
  for (int v = 0; v < nvar_; ++v)
    cbuf_(v, ngh, ngh, ngh) =
        fac * (u(v, l, l, l) + u(v, l, l, r) + u(v, l, r, l) + u(v, r, l, l) +
               u(v, r, r, l) + u(v, r, l, r) + u(v, l, r, r) + u(v, r, r, r));
  if (folddata) {
    for (int v = 0; v < nvar_; ++v)
      cbufold_(v, ngh, ngh, ngh) =
          fac * (uold(v, l, l, l) + uold(v, l, l, r) + uold(v, l, r, l) +
                 uold(v, r, l, l) + uold(v, r, r, l) + uold(v, r, l, r) +
                 uold(v, l, r, r) + uold(v, r, r, r));
  }
        ...
        if (ncoarse_[ox3 + 1][ox2 + 1][ox1 + 1]) { // coarser
          ...
          for (int v = 0; v < nvar_; ++v) {
            if (fk >= 0 && fj >= 0 && fi >= 0)
              u(v, fk, fj, fi) =
                  0.015625 *
                  (27.0 * cb(v, k, j, i) + cb(v, k - 1, j - 1, i - 1) +
                   9.0 * (cb(v, k, j, i - 1) + cb(v, k, j - 1, i) +
                          cb(v, k - 1, j, i)) +
                   3.0 * (cb(v, k - 1, j - 1, i) + cb(v, k - 1, j, i - 1) +
                          cb(v, k, j - 1, i - 1)));
          ...
          if (folddata) {
            for (int v = 0; v < nvar_; ++v) {
              if (fk >= 0 && fj >= 0 && fi >= 0)
                uold(v, fk, fj, fi) =
                    0.015625 *
                    (27.0 * co(v, k, j, i) + co(v, k - 1, j - 1, i - 1) +
                     9.0 * (co(v, k, j, i - 1) + co(v, k, j - 1, i) +
                            co(v, k - 1, j, i)) +
                     3.0 * (co(v, k - 1, j - 1, i) + co(v, k - 1, j, i - 1) +
                            co(v, k, j - 1, i - 1)));
              if (fk >= 0 && fj >= 0 && fi < flim)
                uold(v, fk, fj, fi + 1) =
                    0.015625 *
