...
class PassiveScalars {
public:
  // TODO(felker): pin is currently only used for checking ssprk5_4, otherwise
  // unused. Leaving as ctor parameter in case of run-time "nscalars" option
  ...
  // "primitive vars" = (density-normalized) mass fraction/concentration of each
  // species
  ...
  AthenaArray<Real>
      mass_flux_fc[3]; // deep copy of Hydro intermediate flux quantities
  AthenaArray<Real> s_cc, r_cc; // cell-centered approximations
  // (only needed for 4th order EOS evaluations that have explicit dependence on
  ...
  void AddFluxDivergence_STS(const Real wght, int stage,
                             ...
                             AthenaArray<Real> &s_flux_div_out);
  void CalculateFluxes(AthenaArray<Real> &s, const int order);
  void CalculateFluxes_STS();

  //! \note
  //! for now, not creating subfolder "scalars_diffusion/", nor class
  //! ScalarDiffusion that is would have an instance contained within
  //! PassiveScalars like HydroDiffusion approach. Consider creating an
  //! encapsulated class as these features are generalized.
  ...
  //! \note
  //! No need for nu_scalar array, nor counterpart to
  //! HydroDiffusion::CalcDiffusionFlux wrapper function since, currently:
  //! - nu_scalar_iso must be constant across the mesh
  //!   (does not depend on local fluid or field variables),
  //! - there is only one type of
  //!   passive scalar diffusion process (nu_scalar_aniso disabled, no "eta"l,
  //!   etc.)
  //! - nu_scalar_iso is identical for all NSCALARS
  void DiffusiveFluxIso(const AthenaArray<Real> &prim_r,
...
private:
  ...
  // TODO(felker): dedpulicate these arrays and the same named ones in
  // HydroDiffusion
