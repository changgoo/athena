...
//!   approximations (flux_fc in calculate_fluxes.cpp is currently not saved
//!   persistently in Hydro class but each flux dir is temp. stored in 4D
//!   scratch array scr1_nkji_)
...
#pragma omp simd
  ...
  if (order == 4) {
    ...
    pmb->pcoord->LaplacianX1All(x1flux, laplacian_all_fc, 0, NSCALARS - 1, kl,
                                ku, jl, ju, is, ie + 1);
...
#pragma omp simd
    ...
    if (order == 4) {
      ...
      pmb->pcoord->LaplacianX2All(x2flux, laplacian_all_fc, 0, NSCALARS - 1, kl,
                                  ku, js, je + 1, il, iu);
...
#pragma omp simd
    ...
    if (order == 4) {
      ...
      pmb->pcoord->LaplacianX3All(x3flux, laplacian_all_fc, 0, NSCALARS - 1, ks,
                                  ke + 1, jl, ju, il, iu);
