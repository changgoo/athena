...
void PassiveScalars::AddDiffusionFluxes() {
  if (scalar_diffusion_defined) {
    ...
    // TODO(felker): copied wholesale from HydroDiffusion::AddDiffusionFlux, see
    // notes
...
#pragma omp simd
        for (int i = is; i <= ie + 1; ++i) {
          ...
          // = 0.5*(kappa(DiffProcess::iso,k,j,i) +
          // kappa(DiffProcess::iso,k,j,i-1));
          rho_face = 0.5 * (w(IDN, k, j, i) + w(IDN, k, j, i - 1));
          dprim_r_dx =
              (prim_r(n, k, j, i) - prim_r(n, k, j, i - 1)) / pco->dx1v(i - 1);
...
#pragma omp simd
          for (int i = il; i <= iu; ++i) {
            ...
            dprim_r_dy = (prim_r(n, k, j, i) - prim_r(n, k, j - 1, i)) /
                         pco->h2v(i) / pco->dx2v(j - 1);
...
#pragma omp simd
          for (int i = il; i <= iu; ++i) {
            ...
            dprim_r_dz = (prim_r(n, k, j, i) - prim_r(n, k - 1, j, i)) /
                         pco->dx3v(k - 1) / pco->h31v(i) / pco->h32v(j);
...
Real PassiveScalars::NewDiffusionDt() {
  ...
  // Commented-out future extensions: local diffusion coefficients, anisotropic
  // diffusion for passive scalars: AthenaArray<Real> &nu_scalar_t =
  // nu_scalar_tot_;
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      // #pragma omp simd
      //         for (int i=il; i<=iu; ++i) nu_scalar_t(i) +=
      //         nu(DiffProcess::iso,k,j,i);
      //       }
      //       if (nu_scalar_aniso > 0.0) {
      // #pragma omp simd
      //         for (int i=il; i<=iu; ++i) nu_scalar_t(i) +=
      //         nu(DiffProcess::aniso,k,j,i);
      //       }
...
#pragma omp simd
      ...
      if (nu_scalar_iso > 0.0) { // || (nu_scalar_aniso > 0.0)) {
        for (int i = il; i <= iu; ++i) {
          dt_diff = std::min(dt_diff,
                             static_cast<Real>(SQR(len(i)) * fac /
                                               (nu_scalar_iso + TINY_NUMBER)));
