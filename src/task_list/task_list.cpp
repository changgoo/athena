...
// #include <vector> // formerly needed for vector of MeshBlock ptrs in
// DoTaskListOneStage
...
TaskListStatus TaskList::DoAllAvailableTasks(MeshBlock *pmb, int stage,
  ...
  for (int i = ts.indx_first_task; i < ntasks; i++) {
    ...
    } else if (skip == 0) { // this task is already done AND it is at the top of
                            // the list
      ts.indx_first_task++;
    }
  }
...
//! \brief completes all tasks in this list, will not return until all are tasks
//! done
...
#pragma omp parallel for num_threads(nthreads) schedule(dynamic, 1)
  ...
  while (nmb_left > 0) {
    //! \note
    //! KNOWN ISSUE: Workaround for unknown OpenMP race condition. See #183 on
    //! GitHub.
#pragma omp parallel for reduction(- : nmb_left) num_threads(nthreads)         \
    schedule(dynamic, 1)
    for (int i = 0; i < nmb; ++i) {
      if (DoAllAvailableTasks(pmesh->my_blocks(i), stage,
                              pmesh->my_blocks(i)->tasks) ==
...
void TaskList::OutputAllTaskTime(const int ncycle, std::string basename) {
  ...
  MPI_Allreduce(MPI_IN_PLACE, all_task_time, ntask_time, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &time_per_step, 1, MPI_DOUBLE, MPI_SUM,
                MPI_COMM_WORLD);
#endif
  if (Globals::my_rank == 0) {
    ...
    int j = 0;
    os << "# ncycle=" << ncycle << ", TaskList=" << task_list_name
       << ", time=" << time_per_step << std::endl;
    for (int i = 0; i < ntasks; i++) {
      Task &taski = task_list_[i];
      if (taski.lb_time) {
        os << "  " << taski.task_name << ", time=" << all_task_time[j]
