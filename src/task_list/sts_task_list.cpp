...
//!
//! REFERENCE:
//! Meyer, C. D., Balsara, D. S., & Aslam, T. D. 2014, J. Comput. Phys., 257A,
//! 594-626
...
SuperTimeStepTaskList::SuperTimeStepTaskList(ParameterInput *pin, Mesh *pm,
  ...
  if (!(pmb->phydro->hdif.hydro_diffusion_defined)
      // short-circuit evaluation makes these safe (won't dereference
      // pscalars=nullptr):
      &&
  ...
  // assemble a vector containing the subset of NHYDRO indices updated inside
  // STS
  do_sts_hydro = false;
  do_sts_field = false;
  do_sts_scalar = false;
  if (pmb->phydro->hdif.hydro_diffusion_defined) {
    do_sts_hydro = true;
    if (pmb->phydro->hdif.nu_iso > 0.0 || pmb->phydro->hdif.nu_aniso > 0.0) {
    ...
    if (pmb->phydro->hdif.kappa_iso > 0.0 ||
      ...
      if (!std::binary_search(sts_idx_subset.begin(), sts_idx_subset.end(),
                              IEN)) {
        sts_idx_subset.push_back(IEN);
      }
    }
      ...
      if (pm->multilevel) { // SMR or AMR
        if (do_sts_scalar) {
          if (do_sts_hydro) {
            ...
            if (SHEAR_PERIODIC) {
              AddTask(PROLONG,
                      (SEND_FLD | RECV_FLDSH | SEND_SCLR | RECV_SCLRSH));
            } else {
...
//---------------------------------------------------------------------------------------
//! Sets id and dependency for "ntask" member of task_list_ array, then iterates
//! value of ntask.
...
#pragma omp single
  {
    ...
    Real stage_ratio =
        nstages_time_int * dt_ratio / (tot_nstages_sts + nstages_time_int);
        ...
        if (pm->dt_diagnostics == 0) {
          if (stage == nstages) {
            std::cout << "stage=" << stage << "/" << nstages
                      ...
                      << " ratio=" << std::setprecision(ratio_precision)
                      << dt_ratio << " stage_ratio=" << stage_ratio
                      << std::setprecision(dt_precision) << std::endl;
          }
        } else {
...
TaskStatus SuperTimeStepTaskList::ClearAllBoundary_STS(MeshBlock *pmb,
                                                       int stage) {
...
TaskStatus SuperTimeStepTaskList::CalculateHydroFlux_STS(MeshBlock *pmb,
                                                         int stage) {
  Hydro *ph = pmb->phydro;
  if (stage <= nstages) {
...
TaskStatus SuperTimeStepTaskList::CalculateScalarFlux_STS(MeshBlock *pmb,
                                                          int stage) {
  PassiveScalars *ps = pmb->pscalars;
  if (stage <= nstages) {
...
TaskStatus SuperTimeStepTaskList::IntegrateHydro_STS(MeshBlock *pmb,
                                                     int stage) {
  Hydro *ph = pmb->phydro;
  Field *pf = pmb->pfield;
  ...
  if (stage <= nstages) {
    ...
    pmb->pcoord->AddCoordTermsDivergence_STS(wght, stage, ph->flux, ph->u,
                                             ph->fl_div);
...
TaskStatus SuperTimeStepTaskList::IntegrateScalars_STS(MeshBlock *pmb,
                                                       int stage) {
  PassiveScalars *ps = pmb->pscalars;
  // set registers
  if (pmb->pmy_mesh->sts_integrator == "rkl2" && stage == 1) {
...
TaskStatus SuperTimeStepTaskList::IntegrateField_STS(MeshBlock *pmb,
                                                     int stage) {
  Field *pf = pmb->pfield;
...
TaskStatus SuperTimeStepTaskList::Prolongation_STS(MeshBlock *pmb, int stage) {
  BoundaryValues *pbval = pmb->pbval;
  if (stage <= nstages) {
    Real time = pmb->pmy_mesh->time;
    if (pmb->pmy_mesh->sts_loc == TaskType::op_split_after)
      time += pmb->pmy_mesh->dt;
...
TaskStatus SuperTimeStepTaskList::Primitives_STS(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    //! \note
    //! At beginning of this task, ph->w and pf->bcc contain the previous
    //! stage's output. Now, we wish to update ph->w and pf->bcc with the
    //! present ph->u and pf->b. In TimeIntegratorTaskList::Primitives, ph->w1
    //! is used to contain the present stage's output, however, after performing
    //! w.SwapAthenaArray(ph->w1), the resulting ph->w does not contain correct
    //! values for the ghost zones. This is resolved by calling
    //! TimeIntegratorTaskList::ApplyPhysicalBoundaries, however, in STS,
    //! ApplyPhysicalBoundaries only cycles through boundary types integrated in
    //! SuperTimeStepTaskList, **not all** boundary types.  We still need the
    //! correct values in ghost zones for all other non-STS-integrated
    //! boundaries, thus we choose to directly update ph->w, instead of storing
    //! in an intermediate ph->w1. ph->w1 is only used in RELATAVISTIC DYNAMICS,
    ...
    if (do_sts_scalar) {
      pmb->peos->PassiveScalarConservedToPrimitive(
          ps->s, ph->w, ps->r, ps->r, pmb->pcoord, il, iu, jl, ju, kl, ku);
    }
    ...
    if (pmb->precon->xorder == 4) {
      ...
      if (do_sts_hydro || do_sts_field) {
        pmb->peos->ConservedToPrimitiveCellAverage(ph->u, ph->w, pf->b, ph->w,
...
TaskStatus SuperTimeStepTaskList::PhysicalBoundary_STS(MeshBlock *pmb,
                                                       int stage) {
  BoundaryValues *pbval = pmb->pbval;
  if (stage <= nstages) {
    if (do_sts_hydro || do_sts_field) {
...
TaskStatus SuperTimeStepTaskList::CheckRefinement_STS(MeshBlock *pmb,
                                                      int stage) {
  if (stage != nstages)
    return TaskStatus::success; // only do on last stage
