...
//! \brief derived class for time integrator task list. Can create task lists
//! for one of many different time integrators (e.g. van Leer, RK2, RK3, etc.)
...
TimeIntegratorTaskList::TimeIntegratorTaskList(ParameterInput *pin, Mesh *pm) {
  ...
  //! the algorithm and the CFL number stability limit when coupled to the
  //! single-stage spatial operator. Currently, the explicit, multistage
  //! time-integrators must be expressed as 2S-type algorithms as in Ketcheson
  //! (2010) Algorithm 3, which incudes 2N (Williamson) and 2R (van der Houwen)
  //! popular 2-register low-storage RK methods. The 2S-type integrators depend
  //! on a bidiagonally sparse Shu-Osher representation; at each stage l: \f[
  ...
  //! integrator. Previous timestep \f$U^{0} = U^n\f$ is given, and the
  //! integrator solves for \f$U^{l}\f$ for 1 <= l <= nstages.
  //!
  //! \note
  //! The 2x RHS evaluations of Div(F) and source terms per stage is avoided by
  //! adding another weighted average / caching of these terms each stage. The
  //! API and framework is extensible to three register 3S* methods, although
  //! none are currently implemented.
  //!
  //! \note
  //! Notation: exclusively using "stage", equivalent in lit. to "substage" or
  //! "substep" (infrequently "step"), to refer to the intermediate values of
  //! U^{l} between each "timestep" = "cycle" in explicit, multistage methods.
  //! This is to disambiguate the temporal integration from other iterative
  //! sequences;  generic "Step" is often used for sequences in code, e.g.
  //! main.cpp: "Step 1: MPI"
  //!
  //! \note
  //! main.cpp invokes the tasklist in a for () loop from stage=1 to
  //! stage=ptlist->nstages
  //!
  //! \todo (felker):
  //! - validate Field and Hydro diffusion with RK3, RK4, SSPRK(5,4)
  integrator = pin->GetOrAddString("time", "integrator", "vl2");
  task_list_name = "TimeIntegrator";
  ...
  if (integrator == "rk4" || integrator == "ssprk5_4") {
    ...
    if (SHEAR_PERIODIC) {
      ...
      msg << "### FATAL ERROR in TimeIntegratorTaskList constructor"
          << std::endl
          << "integrator=" << integrator
          << " does not work with shear periodic boundary." << std::endl;
      ATHENA_ERROR(msg);
    }
    ...
    if (ORBITAL_ADVECTION && pm->multilevel) {
      ...
      msg << "### FATAL ERROR in TimeIntegratorTaskList constructor"
          << std::endl
          << "integrator=" << integrator
          << " does not work with orbital advection and "
  ...
  if (integrator == "vl2") {
    ...
    //!   coeff. advection of classical RK4 + 4th or 1st order (limiter engaged)
    //!   fluxes
    nstages_main = 4;
    cfl_limit = 1.3925; // Colella (2011) eq 101; 1st order flux is most severe
                        // constraint
    ...
    if (ORBITAL_ADVECTION) {
      ...
      for (int l = 0; l < nstages - 1; l++) {
        ...
        stage_wghts[l].ebeta =
            stage_wghts[l].gamma_1 * temp_prev + stage_wghts[l].gamma_2 * temp +
            stage_wghts[l].gamma_3 * 0.0 + stage_wghts[l].beta;
    ...
    //! - Because it is an SSP method, we can use the SSP coefficient c=1.508 to
    //! to
    //!   trivially relate the CFL constraint to the RK1 CFL=1 (for first-order
    //!   fluxes).
    //! - There is no need to perform stability analysis from scratch
    //!   (unlike e.g. the linear stability analysis for classical/non-SSP RK4
    //!   in Colella (2011)).
    //! - However, PLM and PPM w/o the limiter engaged are unconditionally
    //! unstable
    ...
    for (int n = 0; n < nstages; n++) {
      if (stage_wghts[n].main_stage) {
        if (n_main == 0) {
          ...
          stage_wghts[n].delta =
              0.096059710526147; // u1 <- (u^n + d*u^(2) + d'*u^(3))
    ...
    msg << "### FATAL ERROR in TimeIntegratorTaskList constructor" << std::endl
        << "integrator=" << integrator << " not valid time integrator"
        << std::endl;
    ATHENA_ERROR(msg);
  }
  ...
  if (cfl_number > cfl_limit && pm->fluid_setup == FluidFormulation::evolve) {
    std::cout << "### Warning in TimeIntegratorTaskList constructor"
              << std::endl
              << "User CFL number " << cfl_number << " must be smaller than "
              << cfl_limit << " for integrator=" << integrator << " in "
              << pm->ndim << "D simulation" << std::endl
              << "Setting to limit" << std::endl;
  ...
  // Now assemble list of tasks for each stage of time integrator
  {
    ...
    if (MAGNETIC_FIELDS_ENABLED) { // MHD
      ...
      // TODO(felker): these nested conditionals are horrible now. Add option to
      // AddTask for "wait for all previously added tasks"?
...
//----------------------------------------------------------------------------------------
//!  Sets id and dependency for "ntask" member of task_list_ array, then
//!  iterates value of ntask.
...
void TimeIntegratorTaskList::AddTask(const TaskID &id, const TaskID &dep) {
  ...
  //!   VERB_OBJECT is confusing with ObjectVerb(). E.g. seeing SEND_HYD in the
  //!   task list assembly would lead the user to believe the corresponding
  //!   function is SendHydro(), when it is actually HydroSend() --- Probaby
  //!   change function names to active voice VerbObject() since
  //!   "HydroFluxCalculate()" doesn't sound quite right.
  //! - There are exceptions to the "verb+object" convention in some TASK_NAMES
  //! and
  //!   TaskFunc, e.g. NEW_DT + NewBlockTimeStep() and AMR_FLAG +
  //!   CheckRefinement(), SRCTERM_HYD and HydroSourceTerms(), USERWORK,
  //!   PHY_BVAL, PROLONG, CONS2PRIM,
  //!   ... Although, AMR_FLAG = "flag blocks for AMR" should be FLAG_AMR in
  //!   VERB_OBJECT
  using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
  if (id == CLEAR_ALLBND) {
    ...
    task_list_[ntasks].task_name.append("CalculateHydroFlux");
  } else if (id == CALC_FLDFLX) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::CalculateEMF);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("CalculateEMF");
  } else if (id == SEND_HYDFLX) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendHydroFlux);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendHydroFlux");
  } else if (id == SEND_FLDFLX) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendEMF);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendEMF");
  } else if (id == RECV_HYDFLX) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveAndCorrectHydroFlux);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveAndCorrectHydroFlux");
  } else if (id == RECV_FLDFLX) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveAndCorrectEMF);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveAndCorrectEMF");
  } else if (id == INT_HYD) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::IntegrateHydro);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("IntegrateHydro");
  } else if (id == INT_FLD) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
    ...
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendField");
  } else if (id == RECV_HYD) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
    ...
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveField");
  } else if (id == SETB_HYD) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
    ...
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendHydroShear");
  } else if (id == RECV_HYDSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveHydroShear);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveHydroShear");
  } else if (id == SEND_FLDSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendFieldShear);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendFieldShear");
  } else if (id == RECV_FLDSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveFieldShear);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveFieldShear");
  } else if (id == SEND_EMFSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendEMFShear);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendEMFShear");
    ...
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("Prolongation");
  } else if (id == CONS2PRIM) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::Primitives);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("Primitives");
  } else if (id == PHY_BVAL) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
  ...
  } else if (id == SEND_PMSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendParticleMeshShear);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendParticleMeshShear");
  } else if (id == RECV_PMSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveParticleMeshShear);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveParticleMeshShear");
  } else if (id == SETB_PM) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
    ...
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("CalculateCRFlux");
  } else if (id == INT_CR) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::IntegrateCR);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("IntegrateCR");
  } else if (id == SEND_CRFLX) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendCRFlux);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendCRFlux");
  } else if (id == RECV_CRFLX) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveAndCorrectCRFlux);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveAndCorrectCRFlux");
  } else if (id == SEND_CR) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendCR);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendCR");
  } else if (id == RECV_CR) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveCR);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveCR");
  } else if (id == SRCTERM_CR) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::AddSourceTermsCR);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("AddSourceTermsCR");
  } else if (id == SETB_CR) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SetBoundariesCR);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SetBoundariesCR");
  } else if (id == SEND_CRSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendCRShear);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendCRShear");
  } else if (id == RECV_CRSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveCRShear);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveCRShear");
  } else if (id == SEND_CRFLXSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::SendCRFluxShear);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("SendCRFluxShear");
  } else if (id == RECV_CRFLXSH) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::ReceiveCRFluxShear);
    task_list_[ntasks].lb_time = false;
    task_list_[ntasks].task_name.append("ReceiveCRFluxShear");
  } else if (id == CR_OPACITY) {
    task_list_[ntasks].TaskFunc =
        static_cast<TaskStatus (TaskList::*)(MeshBlock *, int)>(
            &TimeIntegratorTaskList::CROpacity);
    task_list_[ntasks].lb_time = true;
    task_list_[ntasks].task_name.append("CROpacity");
    ...
    if (MAGNETIC_FIELDS_ENABLED) { // MHD
      ...
      if (integrator == "ssprk5_4") {
        ...
        msg << "### FATAL ERROR in TimeIntegratorTaskList::StartupTaskList\n"
            << "integrator=" << integrator
            << " is currently incompatible with MHD." << std::endl;
        ATHENA_ERROR(msg);
      }
    }
    if (NSCALARS > 0) {
  ...
  if (pmb->pmy_mesh->cooling) {
    if (pmb->pcool->bookkeeping) {
      if (integrator == "rk2") {
        ...
        std::stringstream msg;
        msg << "Energy book keeping is designed only for rk2 and vl2 "
               "integrators"
            << std::endl;
        ATHENA_ERROR(msg);
      }
...
TaskStatus TimeIntegratorTaskList::CalculateHydroFlux(MeshBlock *pmb,
                                                      int stage) {
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage) {
      if (pmb->pmy_mesh->fluid_setup == FluidFormulation::diffusion) {
        ...
        if ((integrator == "vl2") &&
            (stage - stage_wghts[0].orbital_stage == 1)) {
        ...
        if (phydro->fofc_enabled) {
          phydro->FirstOrderFluxCorrection(
              stage_wghts[stage - 1].delta, stage_wghts[stage - 1].gamma_1,
              stage_wghts[stage - 1].gamma_2, stage_wghts[stage - 1].beta);
        }
      }
    }
...
//----------------------------------------------------------------------------------------
// Functions to communicate fluxes between MeshBlocks for flux correction with
// AMR
...
TaskStatus TimeIntegratorTaskList::ReceiveAndCorrectHydroFlux(MeshBlock *pmb,
                                                              int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
...
TaskStatus TimeIntegratorTaskList::ReceiveAndCorrectEMF(MeshBlock *pmb,
                                                        int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
...
TaskStatus TimeIntegratorTaskList::IntegrateHydro(MeshBlock *pmb, int stage) {
  ...
  if ((STS_ENABLED &&
       (pmb->pmy_mesh->fluid_setup == FluidFormulation::diffusion)) ||
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage) {
      ...
      if (ave_wghts[0] == 0.0 && ave_wghts[1] == 1.0 && ave_wghts[2] == 0.0) {
        ...
        pmb->WeightedAve(pf->b, pf->b1, pf->b2, pf->b0, pf->ct_update,
                         ave_wghts);
      }
...
TaskStatus TimeIntegratorTaskList::AddSourceTermsHydro(MeshBlock *pmb,
                                                       int stage) {
  Hydro *ph = pmb->phydro;
  Field *pf = pmb->pfield;
  PassiveScalars *ps = pmb->pscalars;
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage) {
      ...
      ph->hsrc.AddHydroSourceTerms(t_start_stage, dt, ph->flux, ph->w, ps->r,
                                   pf->bcc, ph->u, ps->s);
    }
    return TaskStatus::next;
  }
...
//----------------------------------------------------------------------------------------
//! Functions to calculate hydro diffusion fluxes (stored in
//! HydroDiffusion::visflx[], cndflx[], added at the end of
...
TaskStatus TimeIntegratorTaskList::SetBoundariesHydro(MeshBlock *pmb,
                                                      int stage) {
  if (stage <= nstages) {
    pmb->phydro->hbvar.SwapHydroQuantity(pmb->phydro->u,
                                         HydroBoundaryQuantity::cons);
...
TaskStatus TimeIntegratorTaskList::SetBoundariesField(MeshBlock *pmb,
                                                      int stage) {
  if (stage <= nstages) {
...
TaskStatus TimeIntegratorTaskList::ReceiveHydroShear(MeshBlock *pmb,
                                                     int stage) {
  bool ret;
  ret = false;
  if (stage <= nstages) {
...
TaskStatus TimeIntegratorTaskList::SendHydroFluxShear(MeshBlock *pmb,
                                                      int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
...
TaskStatus TimeIntegratorTaskList::ReceiveHydroFluxShear(MeshBlock *pmb,
                                                         int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
...
TaskStatus TimeIntegratorTaskList::ReceiveFieldShear(MeshBlock *pmb,
                                                     int stage) {
  bool ret;
  ret = false;
  if (stage <= nstages) {
...
TaskStatus TimeIntegratorTaskList::IntegrateParticles(MeshBlock *pmb,
                                                      int stage) {
  if (integrator == "vl2") {
    for (Particles *ppar : pmb->ppars)
...
TaskStatus TimeIntegratorTaskList::SendParticlesShear(MeshBlock *pmb,
                                                      int stage) {
  for (Particles *ppar : pmb->ppars)
    ppar->SendParticlesShear();
  return TaskStatus::success;
}

TaskStatus TimeIntegratorTaskList::ReceiveParticlesShear(MeshBlock *pmb,
                                                         int stage) {
  bool ret_all(true), ret(false);
  for (Particles *ppar : pmb->ppars) {
...
TaskStatus TimeIntegratorTaskList::ReceiveParticleMesh(MeshBlock *pmb,
                                                       int stage) {
  bool ret_all(true), ret(false);
  for (Particles *ppar : pmb->ppars) {
    if (ppar->ppm->updated)
...
TaskStatus TimeIntegratorTaskList::SetBoundariesParticleMesh(MeshBlock *pmb,
                                                             int stage) {
  for (Particles *ppar : pmb->ppars) {
    if (ppar->ppm->updated) {
      ppar->ppm->pmbvar->SetBoundaries();
      // TODO(SMOON) currently, updated=false when solving Poisson because of
      // this line, although the PM quantities are still consistent with
      // Particle positions until new particle integration. Proper position of
      // updated=false might be moved.
...
TaskStatus TimeIntegratorTaskList::SendParticleMeshShear(MeshBlock *pmb,
                                                         int stage) {
  for (Particles *ppar : pmb->ppars) {
    if (ppar->ppm->updated)
...
TaskStatus TimeIntegratorTaskList::ReceiveParticleMeshShear(MeshBlock *pmb,
                                                            int stage) {
  bool ret_all(true), ret(false);
  for (Particles *ppar : pmb->ppars) {
    if (ppar->ppm->updated)
...
TaskStatus TimeIntegratorTaskList::Primitives(MeshBlock *pmb, int stage) {
  ...
  int il = pmb->is, iu = pmb->ie, jl = pmb->js, ju = pmb->je, kl = pmb->ks,
      ku = pmb->ke;
  if (pbval->nblevel[1][1][0] != -1)
    il -= NGHOST;
  if (pbval->nblevel[1][1][2] != -1)
    iu += NGHOST;
  if (pbval->nblevel[1][0][1] != -1)
    jl -= NGHOST;
  if (pbval->nblevel[1][2][1] != -1)
    ju += NGHOST;
  if (pbval->nblevel[0][1][1] != -1)
    kl -= NGHOST;
  if (pbval->nblevel[2][1][1] != -1)
    ku += NGHOST;
  ...
  if (stage <= nstages) {
    ...
    // For the second order integrators VL2 and RK2, the prim_old initial guess
    // for the Newton-Raphson solver in GR EOS uses the following abscissae:
    ...
    if (NSCALARS > 0) {
      ...
      pmb->peos->PassiveScalarConservedToPrimitive(
          ps->s, ph->u, ps->r, ps->r, pmb->pcoord, il, iu, jl, ju, kl, ku);
    }
    // fourth-order EOS:
...
TaskStatus TimeIntegratorTaskList::CalculateScalarFlux(MeshBlock *pmb,
                                                       int stage) {
  PassiveScalars *ps = pmb->pscalars;
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage) {
...
TaskStatus TimeIntegratorTaskList::ReceiveScalarFlux(MeshBlock *pmb,
                                                     int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage) {
      ...
      if (ave_wghts[0] == 0.0 && ave_wghts[1] == 1.0 && ave_wghts[2] == 0.0)
        ...
        pmb->WeightedAve(ps->s, ps->s1, ps->s2, ps->s0, ps->s_fl_div,
                         ave_wghts);
      ...
      if (stage == 4 && integrator == "ssprk5_4") {
        ...
        pmb->WeightedAve(ps->s2, ps->s1, ps->s2, ps->s0, ps->s_fl_div,
                         ave_wghts);
...
TaskStatus TimeIntegratorTaskList::SetBoundariesScalars(MeshBlock *pmb,
                                                        int stage) {
  if (stage <= nstages) {
...
TaskStatus TimeIntegratorTaskList::DiffuseScalars(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_after) {
      // TODO(felker): adapted directly from HydroDiffusion::ClearFlux.
      // Deduplicate
      ps->diffusion_flx[X1DIR].ZeroClear();
      ps->diffusion_flx[X2DIR].ZeroClear();
      ps->diffusion_flx[X3DIR].ZeroClear();

      // unlike HydroDiffusion, only 1x passive scalar diffusive process is
      // allowed, so there is no need for counterpart to wrapper fn
      // HydroDiffusion::CalcDiffusionFlux
...
TaskStatus TimeIntegratorTaskList::ReceiveScalarsShear(MeshBlock *pmb,
                                                       int stage) {
  bool ret;
  ret = false;
  if (stage <= nstages) {
...
TaskStatus TimeIntegratorTaskList::SendScalarsFluxShear(MeshBlock *pmb,
                                                        int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
...
TaskStatus TimeIntegratorTaskList::ReceiveScalarsFluxShear(MeshBlock *pmb,
                                                           int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
...
TaskStatus TimeIntegratorTaskList::SendHydroOrbital(MeshBlock *pmb, int stage) {
  if (!stage_wghts[stage - 1].orbital_stage) {
    ...
    if (!porb->orbital_advection_active)
      return TaskStatus::success;
    Hydro *ph = pmb->phydro;
    PassiveScalars *ps = pmb->pscalars;
...
TaskStatus TimeIntegratorTaskList::ReceiveHydroOrbital(MeshBlock *pmb,
                                                       int stage) {
  if (!stage_wghts[stage - 1].orbital_stage) {
    return TaskStatus::success;
  } else {
...
TaskStatus TimeIntegratorTaskList::ReceiveFieldOrbital(MeshBlock *pmb,
                                                       int stage) {
  if (!stage_wghts[stage - 1].orbital_stage) {
    return TaskStatus::success;
  } else {
...
TaskStatus TimeIntegratorTaskList::CalculateHydroOrbital(MeshBlock *pmb,
                                                         int stage) {
  if (!stage_wghts[stage - 1].orbital_stage) {
    return TaskStatus::success;
  } else {
...
TaskStatus TimeIntegratorTaskList::CalculateFieldOrbital(MeshBlock *pmb,
                                                         int stage) {
  if (!stage_wghts[stage - 1].orbital_stage) {
    return TaskStatus::success;
  } else {
...
//----------------------------------------------------------------------------------
// Functions for cosmic ray transport
...
TaskStatus TimeIntegratorTaskList::ReceiveAndCorrectCRFlux(MeshBlock *pmb,
                                                           int stage) {
  bool flag_cr = true;
...
TaskStatus TimeIntegratorTaskList::ReceiveCRFluxShear(MeshBlock *pmb,
                                                      int stage) {
  if (stage <= nstages) {
    if (stage_wghts[stage - 1].main_stage ||
        pmb->pmy_mesh->sts_loc == TaskType::op_split_before ||
...
TaskStatus TimeIntegratorTaskList::AddSourceTermsCR(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    // Both u and ir are partially updated, only w is from the beginning of the
    // step
    pcr->pcrintegrator->AddSourceTerms(pmb, dt, ph->u, ph->w, pf->bcc,
                                       pcr->u_cr);
  } else {
    return TaskStatus::fail;
  }
