...
#pragma omp parallel for num_threads(nthreads) schedule(dynamic, 1)
    for (auto itr = pmd->vmg_.begin(); itr < pmd->vmg_.end(); itr++) {
      Multigrid *pmg = *itr;
      if (DoAllAvailableTasks(pmg, pmg->ts_) == TaskListStatus::complete)
        nmg_left--;
    }
  }
...
//----------------------------------------------------------------------------------------
//! \fn void MultigridTaskList::AddMultigridTask(const TaskID& id, const TaskID&
...
TaskStatus
MultigridTaskList::ProlongateBoundaryForProlongation(Multigrid *pmg) {
  pmg->pmgbval->ProlongateMultigridBoundaries(pmg->pmy_driver_->ffas_);
  return TaskStatus::success;
}

//----------------------------------------------------------------------------------------
//! \fn void MultigridTaskList::SetMGTaskListToFiner(int nsmooth, int ngh, int
//! flag) \brief Set the task list for prolongation and post smoothing
