...
FFTGravitySolverTaskList::FFTGravitySolverTaskList(ParameterInput *pin,
                                                   Mesh *pm) {
  ...
  if (integrator == "vl2") {
    ...
    if (ORBITAL_ADVECTION) {
      ...
      msg << "### FATAL ERROR in FFTGravitySolverTaskList constructor"
          << std::endl
    ...
    msg << "### FATAL ERROR in FFTGravitySolverTaskList constructor"
        << std::endl
        << "integrator=" << integrator << " not tested with FFT gravity"
        << std::endl;
    ATHENA_ERROR(msg);
  }
...
//----------------------------------------------------------------------------------------
//! \fn void FFTGravitySolverTaskList::AddTask(const TaskID& id, const TaskID&
//! dep) \brief Sets id and dependency for "ntask" member of task_list_ array,
...
TaskStatus FFTGravitySolverTaskList::ClearFFTGravityBoundary(MeshBlock *pmb,
                                                             int stage) {
...
TaskStatus FFTGravitySolverTaskList::SendFFTGravityBoundary(MeshBlock *pmb,
                                                            int stage) {
...
TaskStatus FFTGravitySolverTaskList::SendFFTGravityShear(MeshBlock *pmb,
                                                         int stage) {
  if (stage <= nstages) {
...
TaskStatus FFTGravitySolverTaskList::ReceiveFFTGravityShear(MeshBlock *pmb,
                                                            int stage) {
  bool ret;
  ret = false;
  if (stage <= nstages) {
...
TaskStatus FFTGravitySolverTaskList::SetFFTGravityBoundary(MeshBlock *pmb,
                                                           int stage) {
  pmb->pgrav->gbvar.SetBoundaries();
  return TaskStatus::success;
}

TaskStatus FFTGravitySolverTaskList::PhysicalBoundary(MeshBlock *pmb,
                                                      int stage) {
