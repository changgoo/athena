...
public:
  ...
  AthenaArray(int nx2, int nx1, DataStatus init = DataStatus::allocated)
      : pdata_(nullptr), nx1_(nx1), nx2_(nx2), nx3_(1), nx4_(1), nx5_(1),
        nx6_(1), state_(init) {
    AllocateData();
  }
  AthenaArray(int nx3, int nx2, int nx1,
              DataStatus init = DataStatus::allocated)
      : pdata_(nullptr), nx1_(nx1), nx2_(nx2), nx3_(nx3), nx4_(1), nx5_(1),
        nx6_(1), state_(init) {
    AllocateData();
  }
  AthenaArray(int nx4, int nx3, int nx2, int nx1,
              DataStatus init = DataStatus::allocated)
      : pdata_(nullptr), nx1_(nx1), nx2_(nx2), nx3_(nx3), nx4_(nx4), nx5_(1),
        nx6_(1), state_(init) {
    AllocateData();
  }
  AthenaArray(int nx5, int nx4, int nx3, int nx2, int nx1,
              DataStatus init = DataStatus::allocated)
      : pdata_(nullptr), nx1_(nx1), nx2_(nx2), nx3_(nx3), nx4_(nx4), nx5_(nx5),
        nx6_(1), state_(init) {
    AllocateData();
  }
  ...
  T operator()(const int n, const int i) const { return pdata_[i + nx1_ * n]; }
...
private:
  ...
  DataStatus state_; // describe what "pdata_" points to and ownership of
                     // allocated data
...
AthenaArray<T>::AthenaArray(const AthenaArray<T> &src) {
  ...
  if (src.pdata_) {
    std::size_t size =
        (src.nx1_) * (src.nx2_) * (src.nx3_) * (src.nx4_) * (src.nx5_);
...
AthenaArray<T>::operator=(const AthenaArray<T> &src) {
  if (this != &src) {
...
template <typename T>
__attribute__((nothrow)) AthenaArray<T> &
AthenaArray<T>::operator=(AthenaArray<T> &&src) {
  if (this != &src) {
...
AthenaArray<T>::NewAthenaArray(int nx5, int nx4, int nx3, int nx2, int nx1) {
  state_ = DataStatus::allocated;
  nx1_ = nx1;
  nx2_ = nx2;
  nx3_ = nx3;
...
//  \brief resizes the last dimension either by augmenting zeros or cutting the
//  data.

template <typename T> void AthenaArray<T>::ResizeLastDimension(int new_nx1) {
...
//! requested
//!         (could replace all "new[]" calls in NewAthenaArray function
//!         overloads)

template <typename T> void AthenaArray<T>::AllocateData() {
  switch (state_) {
  case DataStatus::empty:
  case DataStatus::shallow_slice: // init=shallow_slice should never be passed
                                  // to ctor
    break;
  case DataStatus::allocated:
    // allocate memory and initialize to zero
...
//                                             const int j, const int il, const
//                                             int n) {
