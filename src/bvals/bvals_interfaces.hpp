...
//! identifiers for types of neighbor blocks (connectivity with current
//! MeshBlock)
...
constexpr const bool flip_across_pole_hydro[] = {false, false, true, true,
                                                 false};
//! flags to mark which variables are reversed across polar boundary
...
//! \note
//! User-provided ctor is unnecessary and prevents the type from being POD and
//! aggregate. This struct's implicitly-defined or defaulted default ctor is
//! trivial, implying that NeighborIndexes is a trivial type. Combined with
//! standard layout --> POD. Advantages:
//!  - No user-provided ctor: value initialization first performs zero
//!  initialization
//!    (then default initialization if ctor is non-trivial)
//!  - Aggregate type: supports aggregate initialization {}
//!  - POD type: safely copy objects via memcpy, no memory padding in the
//!  beginning of
//!    object, C portability, supports static initialization

struct NeighborIndexes { // aggregate and POD
  int ox1, ox2,
      ox3; // 3-vec of offsets in {-1,0,+1} relative to this block's (i,j,k)
  int fi1,
      fi2; // 2-vec for identifying refined neighbors (up to 4x face neighbors
...
struct NeighborBlock { // aggregate and POD type. Inheritance breaks
                       // standard-layout-> POD : SimpleNeighborBlock,
                       // NeighborIndexes {
  // composition:
  ...
  bool shear; //!> flag indicating boundary is attaching shearing periodic
              //! boundaries. > (used only in flux_correction_fc.cpp, for now)

  void SetNeighbor(int irank, int ilevel, int igid, int ilid, int iox1,
                   int iox2, int iox3, NeighborConnect itype, int ibid,
                   int itargetid, bool ipolar, bool ishear, int ifi1 = 0,
                   int ifi2 = 0);
};
...
struct BoundaryData { // aggregate and POD (even when MPI_PARALLEL is defined)
  ...
  // currently, sflag[] is only used by Multgrid (send buffers are reused each
  // stage in red-black comm. pattern; need to check if they are available) and
  // shearing box
...
template <int n = 4> struct ShearNeighborData {
  static constexpr int kMaxNeighbor = n;
...
//              Merely lists functions and their argument lists that must be
//              implemented in derived classes to form a somewhat-strict
//              contract for functionality (can always implement as a
//              do-nothing/no-op function if a derived class instance is the
//              exception to the rule and does not use a particular interface
//              function)
//----------------------------------------------------------------------------------------
...
public:
  ...
  //! create unique tags for each MeshBlock/buffer/quantity and initalize MPI
  //! requests:
...
//! \brief contains methods for managing MPI send/recvs and associated
//! loads/stores from communication buffers.
//!
//! \todo (felker):
...
public:
  ...
  // universal buffer management methods for Cartesian grids (unrefined and
  // SMR/AMR)
...
//!    far-field/freestream (outflow, reflect), coordinate (spherical polar),
//!    etc.
...
public:
  ...
  //---------------
  virtual void ReflectInnerX1(Real time, Real dt, int il, int jl, int ju,
                              int kl, int ku, int ngh) = 0;
  virtual void ReflectOuterX1(Real time, Real dt, int iu, int jl, int ju,
                              int kl, int ku, int ngh) = 0;
  virtual void ReflectInnerX2(Real time, Real dt, int il, int iu, int jl,
                              int kl, int ku, int ngh) = 0;
  virtual void ReflectOuterX2(Real time, Real dt, int il, int iu, int ju,
                              int kl, int ku, int ngh) = 0;
  virtual void ReflectInnerX3(Real time, Real dt, int il, int iu, int jl,
                              int ju, int kl, int ngh) = 0;
  virtual void ReflectOuterX3(Real time, Real dt, int il, int iu, int jl,
                              int ju, int ku, int ngh) = 0;

  virtual void OutflowInnerX1(Real time, Real dt, int il, int jl, int ju,
                              int kl, int ku, int ngh) = 0;
  virtual void OutflowOuterX1(Real time, Real dt, int iu, int jl, int ju,
                              int kl, int ku, int ngh) = 0;
  virtual void OutflowInnerX2(Real time, Real dt, int il, int iu, int jl,
                              int kl, int ku, int ngh) = 0;
  virtual void OutflowOuterX2(Real time, Real dt, int il, int iu, int ju,
                              int kl, int ku, int ngh) = 0;
  virtual void OutflowInnerX3(Real time, Real dt, int il, int iu, int jl,
                              int ju, int kl, int ngh) = 0;
  virtual void OutflowOuterX3(Real time, Real dt, int il, int iu, int jl,
                              int ju, int ku, int ngh) = 0;

  virtual void PolarWedgeInnerX2(Real time, Real dt, int il, int iu, int jl,
                                 int kl, int ku, int ngh) = 0;
  virtual void PolarWedgeOuterX2(Real time, Real dt, int il, int iu, int ju,
                                 int kl, int ku, int ngh) = 0;
};

//----------------------------------------------------------------------------------------
// Abstract classes containing mix of pure virtual, virtual, and concrete
// functoins
//----------------------------------------------------------------------------------------
...
class BoundaryVariable : public BoundaryCommunication,
...
public:
  ...
  virtual int ComputeFluxCorrectionBufferSize(const NeighborIndexes &ni,
                                              int cng) = 0;
...
protected:
  // deferred initialization of BoundaryData objects in derived class
  // constructors
  BoundaryData<> bd_var_, bd_var_flcor_;
  // derived class dtors are also responsible for calling
  ...
  void CopyShearBufferSameProcess(SimpleNeighborBlock &snb, int ssize,
