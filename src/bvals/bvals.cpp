...
BoundaryValues::BoundaryValues(MeshBlock *pmb, BoundaryFlag *input_bcs,
    ...
    : BoundaryBase(pmb->pmy_mesh, pmb->loc, pmb->block_size, input_bcs),
      pmy_block_(pmb), sb_data_{}, sb_flux_data_{} {
  ...
  nface_ = 2;
  nedge_ = 0;
  CheckBoundaryFlag(block_bcs[BoundaryFace::inner_x1],
                    CoordinateDirection::X1DIR);
  CheckBoundaryFlag(block_bcs[BoundaryFace::outer_x1],
                    CoordinateDirection::X1DIR);

  if (pmb->block_size.nx2 > 1) {
    nface_ = 4;
    nedge_ = 4;
    CheckBoundaryFlag(block_bcs[BoundaryFace::inner_x2],
                      CoordinateDirection::X2DIR);
    CheckBoundaryFlag(block_bcs[BoundaryFace::outer_x2],
                      CoordinateDirection::X2DIR);
  }
  ...
  if (pmb->block_size.nx3 > 1) {
    nface_ = 6;
    nedge_ = 12;
    CheckBoundaryFlag(block_bcs[BoundaryFace::inner_x3],
                      CoordinateDirection::X3DIR);
    CheckBoundaryFlag(block_bcs[BoundaryFace::outer_x3],
                      CoordinateDirection::X3DIR);
  }
  // Perform compatibilty checks of user selections of polar vs. polar_wedge
  // boundaries
  ...
  // polar boundary edge-case: single MeshBlock spans the entire azimuthal (x3)
  // range
  ...
  // prevent reallocation of contiguous memory space for each of 4x possible
  // calls to std::vector<BoundaryVariable *>.push_back() in Hydro, Field,
  // Gravity, PassiveScalars
  bvars.reserve(3);
  ...
  // reserve phys=0 for former TAG_AMR=8; now hard-coded in
  // Mesh::CreateAMRMPITag()
  ...
  if (pmy_mesh_->shear_periodic) {
    // It is required to know the reconstruction scheme before pmb->precon is
    // defined. If a higher-order scheme than PPM is implemented, xgh_ must be
    // larger than 2.
    ...
    if (std::strcmp(COORDINATE_SYSTEM, "cartesian") != 0) {
      std::stringstream msg;
      msg << "### FATAL ERROR in BoundaryValues Class" << std::endl
          << "shear_periodic boundaries work only in cartesian coordinates."
          << std::endl;
      ATHENA_ERROR(msg);
    }
    ...
    if (integrator == "rk4" || integrator == "ssprk5_4") {
      std::stringstream msg;
      msg << "### FATAL ERROR in BoundaryValues Class" << std::endl
          << "shear_periodic boundaries do not work with rk4 or ssprk5_4."
          << std::endl
          << "Check <time> integrator parameter in the input file."
          << std::endl;
      ATHENA_ERROR(msg);
    }
    if (pmy_mesh_->OrbitalVelocity_ != nullptr) {
      std::stringstream msg;
      msg << "### FATAL ERROR in BoundaryValues Class" << std::endl
          << "shear_periodic boundaries require the default orbital velocity "
             "profile."
          << std::endl;
      ATHENA_ERROR(msg);
    }
    ...
    // nblx2 is only used for allocating SimpleNeighborBlock arrays; nblx1 for
    // loc_shear
    // TODO(felker): initialize loc_shear{0, pmy_mesh_->nrbx2*(1L <<
    // pmb->loc.level - ..)} in ctor member initializer list and update as
    // refinement occurs. And nblx2
    ...
    if (shearing_box == 1) {
      ...
      for (int upper = 0; upper < 2; upper++) {
        if (pmb->loc.lx1 ==
            loc_shear[upper]) { // if true for shearing inner blocks
          is_shear[upper] = true;
...
void BoundaryValues::SetupPersistentMPI() {
  ...
  // KGF: begin exclusive shearing-box section in
  // BoundaryValues::SetupPersistentMPI() initialize the shearing block lists
  if (shearing_box != 0) {
    ...
    if (pmy_mesh_->adaptive) {
      std::stringstream msg;
      msg << "### FATAL ERROR in BoundaryValues Class" << std::endl
          << "shear_periodic boundaries do NOT work with AMR at present."
          << std::endl;
      ATHENA_ERROR(msg);
    } else {
      for (int upper = 0; upper < 2; upper++) {
        if (is_shear[upper]) {
          ...
          if (mbt == nullptr || mbt->GetGid() == -1) {
            std::stringstream msg;
            msg << "### FATAL ERROR in BoundaryValues Class" << std::endl
                << "shear_periodic boundaries do NOT work if MeshBlocks "
                   "contacting "
    ...
    qomL_ =
        pmb->porb->OrbitalVelocity(pmb->porb, pmy_mesh_->mesh_size.x1min, 0,
                                   0) -
...
void BoundaryValues::StartReceivingSubset(
    BoundaryCommSubset phase, std::vector<BoundaryVariable *> bvars_subset) {
  for (auto bvars_it = bvars_subset.begin(); bvars_it != bvars_subset.end();
       ++bvars_it) {
    (*bvars_it)->StartReceiving(phase);
  }

  // KGF: begin shearing-box exclusive section of original
  // StartReceivingForInit() find send_block_id and recv_block_id;
  if (shearing_box != 0) {
...
void BoundaryValues::StartReceivingShear(BoundaryCommSubset phase) {
  ...
  case BoundaryCommSubset::all:
    ...
    // KGF: cannot simply combine StartReceivingShear() at end of
    // StartReceiving() (which is done for ClearBoundary), because the
    // "shared"/non-virtual fn BoundaryValues::FindShearBlock() must be called
    // in between 2x fns
  ...
  case BoundaryCommSubset::gr_amr:
    ...
    msg << "### FATAL ERROR in BoundaryValues::StartReceiving" << std::endl
        << "BoundaryCommSubset::gr_amr was passed as the 'phase' argument "
           "while\n"
        << "SHEARING_BOX=1 is enabled. Shearing box calculations are "
           "currently\n"
...
//!                                         std::vector<BoundaryVariable *>
//!                                         bvars_subset)
//! \brief clean up the boundary flags after each loop
//!
//! \note
//! BoundaryCommSubset::mesh_init corresponds to initial exchange of conserved
//! fluid variables and magentic fields, while BoundaryCommSubset::gr_amr
//! corresponds to fluid primitive variables sent only in the case of GR with
//! refinement

void BoundaryValues::ClearBoundarySubset(
    BoundaryCommSubset phase, std::vector<BoundaryVariable *> bvars_subset) {
  for (auto bvars_it = bvars_subset.begin(); bvars_it != bvars_subset.end();
       ++bvars_it) {
...
//----------------------------------------------------------------------------------------
//! \fn void BoundaryValues::ApplyPhysicalBoundaries(const Real time, const Real
//! dt,
//!                                        std::vector<BoundaryVariable *>
//!                                        bvars_subset)
...
//!
//! \note
//! - temporarily hardcode Hydro and Field access for coupling in EOS U(W) +
//! calc bcc
//!   and when passed to user-defined boundary function stored in function
//!   pointer array
...
void BoundaryValues::ApplyPhysicalBoundaries(
  ...
  // Extend the transverse limits that correspond to periodic boundaries as they
  // are updated: x1, then x2, then x3
  ...
  // KGF: temporarily hardcode Hydro and Field access for coupling in EOS U(W) +
  // calc bcc and when passed to user-defined boundary function stored in
  // function pointer array

  // KGF: COUPLING OF QUANTITIES (must be manually specified)
  // downcast BoundaryVariable ptrs to known derived class types: RTTI via
  // dynamic_cast HydroBoundaryVariable *phbvar =
  //     dynamic_cast<HydroBoundaryVariable *>(bvars_main_int[0]);
  Hydro *ph = pmb->phydro;

  //! \todo (felker):
  //! - passing nullptrs (pf) if no MHD (coarse_* no longer in MeshRefinement)
  //!   (may be fine to unconditionally directly set to pmb->pfield. See
  //!   bvals_refine.cpp)
  ...
  if (apply_bndry_fn_[BoundaryFace::inner_x1]) {
    DispatchBoundaryFunctions(pmb, pco, time, dt, pmb->is, pmb->ie, bjs, bje,
                              bks, bke, NGHOST, ph->w, pf->b, pf->bcc,
                              pcr->u_cr, BoundaryFace::inner_x1, bvars_subset);
    // KGF: COUPLING OF QUANTITIES (must be manually specified)
    if (MAGNETIC_FIELDS_ENABLED) {
    ...
    pmb->peos->PrimitiveToConserved(ph->w, pf->bcc, ph->u, pco,
                                    pmb->is - NGHOST, pmb->is - 1, bjs, bje,
                                    bks, bke);
    if (NSCALARS > 0) {
      pmb->peos->PassiveScalarPrimitiveToConserved(
          ps->r, ph->u, ps->s, pco, pmb->is - NGHOST, pmb->is - 1, bjs, bje,
          bks, bke);
    }
  }
  ...
  if (apply_bndry_fn_[BoundaryFace::outer_x1]) {
    DispatchBoundaryFunctions(pmb, pco, time, dt, pmb->is, pmb->ie, bjs, bje,
                              bks, bke, NGHOST, ph->w, pf->b, pf->bcc,
                              pcr->u_cr, BoundaryFace::outer_x1, bvars_subset);
    // KGF: COUPLING OF QUANTITIES (must be manually specified)
    if (MAGNETIC_FIELDS_ENABLED) {
      pmb->pfield->CalculateCellCenteredField(pf->b, pf->bcc, pco, pmb->ie + 1,
                                              pmb->ie + NGHOST, bjs, bje, bks,
                                              bke);
    }
    pmb->peos->PrimitiveToConserved(ph->w, pf->bcc, ph->u, pco, pmb->ie + 1,
                                    pmb->ie + NGHOST, bjs, bje, bks, bke);
    if (NSCALARS > 0) {
      pmb->peos->PassiveScalarPrimitiveToConserved(
          ps->r, ph->u, ps->s, pco, pmb->ie + 1, pmb->ie + NGHOST, bjs, bje,
          bks, bke);
    }
  }
  ...
  if (pmb->block_size.nx2 > 1) { // 2D or 3D
    ...
    if (apply_bndry_fn_[BoundaryFace::inner_x2]) {
      DispatchBoundaryFunctions(pmb, pco, time, dt, bis, bie, pmb->js, pmb->je,
                                bks, bke, NGHOST, ph->w, pf->b, pf->bcc,
                                pcr->u_cr, BoundaryFace::inner_x2,
                                bvars_subset);
      // KGF: COUPLING OF QUANTITIES (must be manually specified)
      if (MAGNETIC_FIELDS_ENABLED) {
        pmb->pfield->CalculateCellCenteredField(pf->b, pf->bcc, pco, bis, bie,
                                                pmb->js - NGHOST, pmb->js - 1,
                                                bks, bke);
      }
      pmb->peos->PrimitiveToConserved(ph->w, pf->bcc, ph->u, pco, bis, bie,
                                      pmb->js - NGHOST, pmb->js - 1, bks, bke);
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(ps->r, ph->u, ps->s, pco,
                                                     bis, bie, pmb->js - NGHOST,
                                                     pmb->js - 1, bks, bke);
      }
    }
    ...
    if (apply_bndry_fn_[BoundaryFace::outer_x2]) {
      DispatchBoundaryFunctions(pmb, pco, time, dt, bis, bie, pmb->js, pmb->je,
                                bks, bke, NGHOST, ph->w, pf->b, pf->bcc,
                                pcr->u_cr, BoundaryFace::outer_x2,
                                bvars_subset);
      // KGF: COUPLING OF QUANTITIES (must be manually specified)
      if (MAGNETIC_FIELDS_ENABLED) {
        pmb->pfield->CalculateCellCenteredField(pf->b, pf->bcc, pco, bis, bie,
      ...
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(
            ps->r, ph->u, ps->s, pco, bis, bie, pmb->je + 1, pmb->je + NGHOST,
            bks, bke);
      }
    }
  }
  ...
  if (pmb->block_size.nx3 > 1) { // 3D
    ...
    if (apply_bndry_fn_[BoundaryFace::inner_x3]) {
      DispatchBoundaryFunctions(pmb, pco, time, dt, bis, bie, bjs, bje, pmb->ks,
                                pmb->ke, NGHOST, ph->w, pf->b, pf->bcc,
                                pcr->u_cr, BoundaryFace::inner_x3,
                                bvars_subset);
      // KGF: COUPLING OF QUANTITIES (must be manually specified)
      if (MAGNETIC_FIELDS_ENABLED) {
        pmb->pfield->CalculateCellCenteredField(pf->b, pf->bcc, pco, bis, bie,
                                                bjs, bje, pmb->ks - NGHOST,
                                                pmb->ks - 1);
      }
      pmb->peos->PrimitiveToConserved(ph->w, pf->bcc, ph->u, pco, bis, bie, bjs,
                                      bje, pmb->ks - NGHOST, pmb->ks - 1);
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(
            ps->r, ph->u, ps->s, pco, bis, bie, bjs, bje, pmb->ks - NGHOST,
            pmb->ks - 1);
      }
    }
    ...
    if (apply_bndry_fn_[BoundaryFace::outer_x3]) {
      DispatchBoundaryFunctions(pmb, pco, time, dt, bis, bie, bjs, bje, pmb->ks,
                                pmb->ke, NGHOST, ph->w, pf->b, pf->bcc,
                                pcr->u_cr, BoundaryFace::outer_x3,
                                bvars_subset);
      // KGF: COUPLING OF QUANTITIES (must be manually specified)
      if (MAGNETIC_FIELDS_ENABLED) {
        pmb->pfield->CalculateCellCenteredField(pf->b, pf->bcc, pco, bis, bie,
      ...
      pmb->peos->PrimitiveToConserved(ph->w, pf->bcc, ph->u, pco, bis, bie, bjs,
                                      bje, pmb->ke + 1, pmb->ke + NGHOST);
      if (NSCALARS > 0) {
        pmb->peos->PassiveScalarPrimitiveToConserved(
            ps->r, ph->u, ps->s, pco, bis, bie, bjs, bje, pmb->ke + 1,
            pmb->ke + NGHOST);
      }
    }
  }
...
//!
//! \note
//! - should "bvars_it" be fixed in this class member function? Or passed as
//! argument?
void BoundaryValues::DispatchBoundaryFunctions(
    MeshBlock *pmb, Coordinates *pco, Real time, Real dt, int il, int iu,
    int jl, int ju, int kl, int ku, int ngh, AthenaArray<Real> &prim,
    FaceField &b, AthenaArray<Real> &bcc, AthenaArray<Real> &u_cr,
    BoundaryFace face, std::vector<BoundaryVariable *> bvars_subset) {
  if (block_bcs[face] == BoundaryFlag::user) { // user-enrolled BCs
    pmy_mesh_->BoundaryFunction_[face](pmb, pco, prim, b, time, dt, il, iu, jl,
                                       ju, kl, ku, NGHOST);

    if (CR_ENABLED) {
      pmy_mesh_->CRBoundaryFunc_[face](pmb, pco, pmb->pcr, prim, bcc, u_cr,
                                       time, dt, il, iu, jl, ju, kl, ku,
                                       NGHOST);
    }
  }
  // KGF: this is only to silence the compiler -Wswitch warnings about not
  // handling the "undef" case when considering all possible BoundaryFace
  // enumerator values. If "undef" is actually passed to this function, it will
  // likely die before that ATHENA_ERROR() call, as it tries to access
  // block_bcs[-1]
  std::stringstream msg;
  ...
  for (auto bvars_it = bvars_subset.begin(); bvars_it != bvars_subset.end();
    ...
    default:
      ...
      msg_flag << "### FATAL ERROR in DispatchBoundaryFunctions" << std::endl
               ...
               << "block_bcs[" << face
               << "] = BoundaryFlag::" << GetBoundaryString(block_bcs[face])
               << std::endl;
      ATHENA_ERROR(msg);
      break;
...
//--------------------------------------------------------------------------------------
//! \fn void BoundaryValues::ComputeShear(const Real time_fc, const Real
//! time_int) \brief Calculate the following quantities: send_gid recv_gid
//! send_lid recv_lid send_rank recv_rank, send_size_hydro  recv_size_hydro: for
//! MPI_Irecv eps_,joverlap_: for update the conservative
    ...
    for (int upper = 0; upper < 2; upper++) {
      if (is_shear[upper]) {
        ...
        for (int j = 0; j < nblx2; j++) {
          // find global index of current MeshBlock on the shearing boundary
          // block list
          if (shbb_[upper][j].gid == pmb->gid)
            jblock = j;
        }
        int bshift;
        // send js+jo : je+jo
    ...
    for (int upper = 0; upper < 2; upper++) {
      if (is_shear[upper]) {
        ...
        if (jo <= xgh_ + NGHOST - 2 * nx2) {
          // case 1
          // send from this to 0: jb+(Ng-3), 1: jb+(Ng-2), 2: jb+(Ng-1), 3:
          // jb+(Ng) recv to this from 0: jb-(Ng-3), 1: jb-(Ng-2), 2: jb-(Ng-1),
          // 3: jb-(Ng)
          bshift = -3;
        } else if (jo <= xgh_ + NGHOST - nx2) {
          // case 2
          // send from this to 0: jb+(Ng-2), 1: jb+(Ng-1), 2: jb+(Ng), 3:
          // jb+(Ng+1) recv to this from 0: jb-(Ng-2), 1: jb-(Ng-1), 2: jb-(Ng),
          // 3: jb-(Ng+1)
          bshift = -2;
        } else if (jo <= xgh_ + NGHOST) {
          // case 3
          // send from this to 0: jb+(Ng-1), 1: jb+(Ng), 2: jb+(Ng+1), 3:
          // jb+(Ng+2) recv to this from 0: jb-(Ng-1), 1: jb-(Ng), 2: jb-(Ng+1),
          // 3: jb-(Ng+2)
          bshift = -1;
        } else if (jo <= xgh_ + NGHOST + nx2) {
          // case 4
          // send from this to 0: jb+(Ng), 1: jb+(Ng+1), 2: jb+(Ng+2), 3:
          // jb+(Ng+3) recv to this from 0: jb-(Ng), 1: jb-(Ng+1), 2: jb-(Ng+2),
          // 3: jb-(Ng+3)
          bshift = 0;
        } else {
          std::stringstream msg;
...
//--------------------------------------------------------------------------------------
//! \brief Public function, to be called in MeshBlock ctor for keeping MPI tag
//! bitfields consistent across MeshBlocks, even if certain MeshBlocks only
//! construct a subset of physical variable classes
