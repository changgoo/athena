...
CellCenteredBoundaryVariable::CellCenteredBoundaryVariable(
    ...
    : BoundaryVariable(pmb), var_cc(var), coarse_buf(coarse_var),
      x1flux(var_flux[X1DIR]), x2flux(var_flux[X2DIR]), x3flux(var_flux[X3DIR]),
      nl_(0), nu_(var->GetDim4() - 1), flip_across_pole_(nullptr) {
  //! \note
  //! CellCenteredBoundaryVariable should only be used w/ 4D or 3D (nx4=1)
  //! AthenaArray For now, assume that full span of 4th dim of input AthenaArray
  //! should be used:
  //! ---> get the index limits directly from the input AthenaArray
  //! <=nu_ (inclusive), <nx4 (exclusive)
  if (nu_ < 0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in CellCenteredBoundaryVariable constructor"
        << std::endl
        << "An 'AthenaArray<Real> *var' of nx4_ = " << var->GetDim4()
        << " was passed\n"
  ...
  if (pbval_->shearing_box != 0) {
    ...
    for (int upper = 0; upper < 2; upper++) {
      if (pbval_->is_shear[upper]) {
        shear_cc_[upper].NewAthenaArray(nu_ + 1, nc3, NGHOST,
                                        nc2 + 2 * xgh + 1);
        shear_var_flx_[upper].NewAthenaArray(nu_ + 1, nc3, nc2);
        shear_map_flx_[upper].NewAthenaArray(nu_ + 1, nc3, 1,
                                             nc2 + 2 * xgh + 1);
...
CellCenteredBoundaryVariable::~CellCenteredBoundaryVariable() {
  ...
  if ((pmy_mesh_->multilevel) || (pbval_->shearing_box != 0))
...
int CellCenteredBoundaryVariable::ComputeVariableBufferSize(
    const NeighborIndexes &ni, int cng) {
...
int CellCenteredBoundaryVariable::ComputeFluxCorrectionBufferSize(
  ...
  if (pmy_mesh_->multilevel) {
    if (ni.ox1 != 0)
      size2 = (pmb->block_size.nx2 + 1) / 2 * (pmb->block_size.nx3 + 1) / 2 *
              (nu_ + 1);
    if (ni.ox2 != 0)
      size2 = (pmb->block_size.nx1 + 1) / 2 * (pmb->block_size.nx3 + 1) / 2 *
              (nu_ + 1);
    if (ni.ox3 != 0)
      size2 = (pmb->block_size.nx1 + 1) / 2 * (pmb->block_size.nx2 + 1) / 2 *
              (nu_ + 1);
  }
...
//!                                                             NeighborBlock&
//!                                                             nb)
//! \brief Set cell-centered boundary buffers for sending to a block on the same
//! level
...
//!                                                             NeighborBlock&
//!                                                             nb)
//! \brief Set cell-centered boundary buffers for sending to a block on the
//! coarser level
...
int CellCenteredBoundaryVariable::LoadBoundaryBufferToCoarser(
  ...
  int p = 0;
  pmr->RestrictCellCenteredValues(var, coarse_var, nl_, nu_, si, ei, sj, ej, sk,
                                  ek);
...
//!                                                               NeighborBlock&
//!                                                               nb)
//! \brief Set cell-centered prolongation buffer received from a block on a
//! coarser level
...
#pragma omp simd linear(p)
    ...
    BufferUtility::UnpackData(buf, coarse_var, nl_, nu_, si, ei, sj, ej, sk, ek,
                              p);
  }
  return;
}
  ...
  for (int n = 0; n < pbval_->nneighbor; n++) {
    ...
    if (nb.snb.rank != Globals::my_rank) {
      if (nb.snb.level == mylevel) { // same
        ...
        rsize =
            ((nb.ni.ox1 == 0) ? ((pmb->block_size.nx1 + 1) / 2 + cng1) : cng1) *
      ...
      if (nb.ni.type == NeighborConnect::face) {
        if (nb.snb.level != mylevel) {
          ...
          if (nb.fid == 0 || nb.fid == 1)
            size = ((pmb->block_size.nx2 + 1) / 2) *
                   ((pmb->block_size.nx3 + 1) / 2);
          else if (nb.fid == 2 || nb.fid == 3)
            size = ((pmb->block_size.nx1 + 1) / 2) *
                   ((pmb->block_size.nx3 + 1) / 2);
          else // (nb.fid == 4 || nb.fid == 5)
            size = ((pmb->block_size.nx1 + 1) / 2) *
                   ((pmb->block_size.nx2 + 1) / 2);
          size *= (nu_ + 1);
          if (nb.snb.level < mylevel) { // send to coarser
            ...
            tag =
                pbval_->CreateBvalsMPITag(pmb->lid, nb.bufid, cc_flx_phys_id_);
...
//----------------------------------------------------------------------------------------
//! \fn void CellCenteredBoundaryVariable::StartReceiving(BoundaryCommSubset
//! phase) \brief initiate MPI_Irecv()
...
//----------------------------------------------------------------------------------------
//! \fn void CellCenteredBoundaryVariable::ClearBoundary(BoundaryCommSubset
//! phase) \brief clean up the boundary flags after each loop
  ...
  if (pbval_->shearing_box == 1) {
    ...
    if (phase == BoundaryCommSubset::all) {
      for (int upper = 0; upper < 2; upper++) {
        if (pbval_->is_shear[upper]) {
          for (int n = 0; n < 3; n++) {
            if (pbval_->sb_flux_data_[upper].send_neighbor[n].rank == -1)
              continue;
            shear_bd_flux_[upper].flag[n] = BoundaryStatus::waiting;
#ifdef MPI_PARALLEL
    ...
    for (int upper = 0; upper < 2; upper++) {
      if (pbval_->is_shear[upper]) {
        for (int n = 0; n < 4; n++) {
          if (pbval_->sb_data_[upper].send_neighbor[n].rank == -1)
            continue;
          shear_bd_var_[upper].flag[n] = BoundaryStatus::waiting;
#ifdef MPI_PARALLEL
          if (pbval_->sb_data_[upper].send_neighbor[n].rank !=
              Globals::my_rank) {
