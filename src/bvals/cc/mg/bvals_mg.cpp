...
//----------------------------------------------------------------------------------------
//! \fn MGBoundaryValues::MGBoundaryValues(Multigrid *pmg, BoundaryFlag
//! *input_bcs) \brief Constructor of the MGBoundaryValues class

MGBoundaryValues::MGBoundaryValues(Multigrid *pmg, BoundaryFlag *input_bcs)
    : BoundaryBase(pmg->pmy_driver_->pmy_mesh_, pmg->loc_, pmg->size_,
                   input_bcs),
      pmy_mg_(pmg) {
#ifdef MPI_PARALLEL
  ...
  if (pmy_mg_->pmy_block_ == nullptr) {
    ...
    for (int i = 0; i < 6; ++i) {
      if (block_bcs[i] == BoundaryFlag::periodic ||
          block_bcs[i] == BoundaryFlag::block)
...
void MGBoundaryValues::ApplyPhysicalBoundaries(int flag) {
  ...
  int bjs = js, bje = je;
  int bks = ks, bke = ke;
  // dx,dy,dz: cell spacing, x0,y0,z0: origins, x = x0+i*dx (integer = cell
  // center), etc.
  ...
  if (MGBoundaryFunction_[BoundaryFace::inner_x1] != nullptr)
    MGBoundaryFunction_[BoundaryFace::inner_x1](dst, time, nvar, is, ie, bjs,
                                                bje, bks, bke, ngh, x0, y0, z0,
                                                dx, dy, dz);
  // Apply boundary function on outer-x1
  if (MGBoundaryFunction_[BoundaryFace::outer_x1] != nullptr)
    MGBoundaryFunction_[BoundaryFace::outer_x1](dst, time, nvar, is, ie, bjs,
                                                bje, bks, bke, ngh, x0, y0, z0,
                                                dx, dy, dz);

  // Apply boundary function on inner-x2
  if (MGBoundaryFunction_[BoundaryFace::inner_x2] != nullptr)
    MGBoundaryFunction_[BoundaryFace::inner_x2](dst, time, nvar, bis, bie, js,
                                                je, bks, bke, ngh, x0, y0, z0,
                                                dx, dy, dz);
  // Apply boundary function on outer-x2
  if (MGBoundaryFunction_[BoundaryFace::outer_x2] != nullptr)
    MGBoundaryFunction_[BoundaryFace::outer_x2](dst, time, nvar, bis, bie, js,
                                                je, bks, bke, ngh, x0, y0, z0,
                                                dx, dy, dz);
  ...
  if (MGBoundaryFunction_[BoundaryFace::inner_x3] != nullptr)
    MGBoundaryFunction_[BoundaryFace::inner_x3](dst, time, nvar, bis, bie, bjs,
                                                bje, ks, ke, ngh, x0, y0, z0,
                                                dx, dy, dz);
  // Apply boundary function on outer-x3
  if (MGBoundaryFunction_[BoundaryFace::outer_x3] != nullptr)
    MGBoundaryFunction_[BoundaryFace::outer_x3](dst, time, nvar, bis, bie, bjs,
                                                bje, ks, ke, ngh, x0, y0, z0,
                                                dx, dy, dz);
    ...
    if (nb.snb.rank != Globals::my_rank) {
      ...
      if (nb.snb.level == loc.level) { // same
        if (nb.ni.type == NeighborConnect::face)
          ...
          size = nc * ngh * ngh;
        else if (nb.ni.type == NeighborConnect::corner)
          size = ngh * ngh * ngh;
        ...
        if (nb.ni.type == NeighborConnect::face)
          size = SQR(nc / 2 + cngh) * cngh;
        else if (nb.ni.type == NeighborConnect::edge)
          size = (nc / 2 + cngh) * cngh * cngh;
        else if (nb.ni.type == NeighborConnect::corner)
          size = cngh * cngh * cngh;
      } else { // finer
        if (nb.ni.type == NeighborConnect::face)
        ...
        if (type == BoundaryQuantity::mggrav_f)
          continue;
        if (nb.ni.type == NeighborConnect::edge)
          size = nc / 2 * ngh * ngh;
        else if (nb.ni.type == NeighborConnect::corner)
          size = ngh * ngh * ngh;
      }
  ...
  for (int n = 0; n < nneighbor; n++) {
    ...
    if (nb.snb.rank != Globals::my_rank) {
      if (!(type == BoundaryQuantity::mggrav_f &&
        ...
        MPI_Wait(&(bdata_.req_send[nb.bufid]),
                 MPI_STATUS_IGNORE); // Wait for Isend
    }
#endif
  }
  return;
...
//!                                                const NeighborBlock& nb, bool
//!                                                folddata)
//! \brief Set Multigrid boundary buffers for sending to a block on the same
//! level

int MGBoundaryValues::LoadMultigridBoundaryBufferSameLevel(
    Real *buf, const NeighborBlock &nb, bool folddata) {
  ...
  if (pmy_mg_->pmy_driver_->nreflevel_ > 0) {
    int cn = ngh - 1, fs = ngh, cs = ngh, fe = fs + nc - 1,
        ce = cs + nc / 2 - 1;
...
//!                                                const NeighborBlock& nb, bool
//!                                                folddata)
//! \brief Set Multigrid boundary buffers for sending to a block on the coarser
//! level

int MGBoundaryValues::LoadMultigridBoundaryBufferToCoarser(
    Real *buf, const NeighborBlock &nb, bool folddata) {
  const AthenaArray<Real> &u = pmy_mg_->GetCurrentData();
  const AthenaArray<Real> &old = pmy_mg_->GetCurrentOldData();
  int ngh = pmy_mg_->ngh_, nvar = pmy_mg_->nvar_;
...
//!                                        const NeighborBlock& nb, bool
//!                                        folddata)
//! \brief Set Multigrid boundary buffers for sending to a block on the finer
//! level
int MGBoundaryValues::LoadMultigridBoundaryBufferToFiner(
    Real *buf, const NeighborBlock &nb, bool folddata) {
  const AthenaArray<Real> &u = pmy_mg_->GetCurrentData();
  const AthenaArray<Real> &old = pmy_mg_->GetCurrentOldData();
  int nc = pmy_mg_->GetCurrentNumberOfCells();
...
//----------------------------------------------------------------------------------------
//! \fn bool MGBoundaryValues::SendMultigridBoundaryBuffers(BoundaryQuantity
//! type,
//!                                                         bool folddata)
...
bool MGBoundaryValues::SendMultigridBoundaryBuffers(BoundaryQuantity type,
  ...
  for (int n = 0; n < nneighbor; n++) {
    NeighborBlock &nb = neighbor[n];
    if (bdata_.sflag[nb.bufid] == BoundaryStatus::completed)
      continue;
...
void MGBoundaryValues::SetMultigridBoundarySameLevel(const Real *buf,
  ...
  if (nb.ni.ox1 == 0)
    ...
    si = nc + ngh, ei = nc + 2 * ngh - 1;
  else
    si = 0, ei = ngh - 1;
  if (nb.ni.ox2 == 0)
    sj = ngh, ej = nc + ngh - 1;
  else if (nb.ni.ox2 > 0)
    sj = nc + ngh, ej = nc + 2 * ngh - 1;
  else
    sj = 0, ej = ngh - 1;
  if (nb.ni.ox3 == 0)
    sk = ngh, ek = nc + ngh - 1;
  else if (nb.ni.ox3 > 0)
    sk = nc + ngh, ek = nc + 2 * ngh - 1;
  else
    sk = 0, ek = ngh - 1;
  ...
  if (pmy_mg_->pmy_driver_->nreflevel_ > 0) {
    ...
    BufferUtility::UnpackData(buf, cbuf_, 0, nvar - 1, si, ei, sj, ej, sk, ek,
                              p);
    if (folddata)
      BufferUtility::UnpackData(buf, cbufold_, 0, nvar - 1, si, ei, sj, ej, sk,
                                ek, p);
  }
...
//----------------------------------------------------------------------------------------
//! \fn bool MGBoundaryValues::ReceiveMultigridBoundaryBuffers(BoundaryQuantity
//! type,
//!                                                            bool folddata)
...
bool MGBoundaryValues::ReceiveMultigridBoundaryBuffers(BoundaryQuantity type,
  ...
  for (int n = 0; n < nneighbor; n++) {
    NeighborBlock &nb = neighbor[n];
    if (bdata_.flag[nb.bufid] == BoundaryStatus::completed)
      continue;
    if (type == BoundaryQuantity::mggrav_f && nb.snb.level > loc.level &&
        nb.ni.type != NeighborConnect::face)
      continue;
    if (bdata_.flag[nb.bufid] == BoundaryStatus::waiting) {
      if (nb.snb.rank == Globals::my_rank) { // on the same process
        ...
        int test;
        MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, mgcomm_, &test,
                   MPI_STATUS_IGNORE);
        ...
        for (int j = sj, fj = fsj; j <= ej; ++j, fj += 2) {
          for (int i = si, fi = fsi; i <= ei; ++i, fi += 2) {
            if (fk >= 0 && fj >= 0 && fi >= 0)
              dst(v, fk, fj, fi) =
                  0.015625 *
                  (27.0 * cbuf_(v, k, j, i) + cbuf_(v, k - 1, j - 1, i - 1) +
                   9.0 * (cbuf_(v, k, j, i - 1) + cbuf_(v, k, j - 1, i) +
                          cbuf_(v, k - 1, j, i)) +
                   3.0 *
                       (cbuf_(v, k - 1, j - 1, i) + cbuf_(v, k - 1, j, i - 1) +
                        cbuf_(v, k, j - 1, i - 1)));
        ...
        for (int k = sk, fk = fsk; k <= ek; ++k, fk += 2) {
          for (int j = sj, fj = fsj; j <= ej; ++j, fj += 2) {
            for (int i = si, fi = fsi; i <= ei; ++i, fi += 2) {
              if (fk >= 0 && fj >= 0 && fi >= 0)
                old(v, fk, fj, fi) =
                    0.015625 * (27.0 * cbufold_(v, k, j, i) +
                                cbufold_(v, k - 1, j - 1, i - 1) +
                                9.0 * (cbufold_(v, k, j, i - 1) +
                                       cbufold_(v, k, j - 1, i) +
                                       cbufold_(v, k - 1, j, i)) +
                                3.0 * (cbufold_(v, k - 1, j - 1, i) +
                                       cbufold_(v, k - 1, j, i - 1) +
                                       cbufold_(v, k, j - 1, i - 1)));
              if (fk >= 0 && fj >= 0 && fi < flim)
                old(v, fk, fj, fi + 1) =
                    0.015625 * (27.0 * cbufold_(v, k, j, i) +
                                cbufold_(v, k - 1, j - 1, i + 1) +
...
//----------------------------------------------------------------------------------------
//! \fn int
//! MGGravityBoundaryValues::LoadMultigridBoundaryBufferToCoarserFluxCons(
//!                                               Real *buf, const
//!                                               NeighborBlock& nb)
//! \brief Set Multigrid boundary buffers for sending to a block on the coarser
//! level
...
int MGGravityBoundaryValues::LoadMultigridBoundaryBufferToCoarserFluxCons(
...
//----------------------------------------------------------------------------------------
//! \fn int
//! MGGravityBoundaryValues::LoadMultigridBoundaryBufferToFinerFluxCons(Real
//! *buf,
//!                                                             const
//!                                                             NeighborBlock&
//!                                                             nb)
//! \brief Set Multigrid boundary buffers for sending to a block on the finer
//! level
int MGGravityBoundaryValues::LoadMultigridBoundaryBufferToFinerFluxCons(
    Real *buf, const NeighborBlock &nb) {
  const AthenaArray<Real> &u = pmy_mg_->GetCurrentData();
  int nc = pmy_mg_->GetCurrentNumberOfCells();
  int ngh = pmy_mg_->ngh_, nvar = pmy_mg_->nvar_;
...
//!                                      const Real *buf, const NeighborBlock&
//!                                      nb)
//! \brief Set hydro boundary received from a block on the same level

void MGGravityBoundaryValues::SetMultigridBoundaryFromCoarserFluxCons(
...
void MGGravityBoundaryValues::SetMultigridBoundaryFromFinerFluxCons(
    const Real *buf, const NeighborBlock &nb) {
  AthenaArray<Real> &dst = pmy_mg_->GetCurrentData();
  int nc = pmy_mg_->GetCurrentNumberOfCells();
  int ngh = pmy_mg_->ngh_, nvar = pmy_mg_->nvar_;
  ...
  for (int k = sk; k <= ek; ++k) {
    for (int j = sj; j <= ej; ++j) {
      for (int i = si; i <= ei; ++i) {
        dst(0, k, j, i) =
            ot * (4.0 * buf[p++] - dst(0, k + ok, j + oj, i + oi));
      }
    }
  }
  return;
...
void MGBoundaryValues::ProlongateMultigridBoundariesFluxCons() {
  ...
  for (int ox1 = -1; ox1 <= 1; ox1 += 2) {
    if (nblevel[1][1][ox1 + 1] == loc.level - 1) {
      int i, fi, fig;
      if (ox1 > 0)
        i = ce + 1, fi = fe, fig = fe + 1;
      else
        i = cs - 1, fi = fs, fig = fs - 1;
      if (MGBoundaryFunction_[BoundaryFace::inner_x2] != nullptr)
        MGBoundaryFunction_[BoundaryFace::inner_x2](cbuf_, time, nvar, i, i, cs,
                                                    ce, cs, ce, ngh, x0, y0, z0,
                                                    dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::outer_x2] != nullptr)
        MGBoundaryFunction_[BoundaryFace::outer_x2](cbuf_, time, nvar, i, i, cs,
                                                    ce, cs, ce, ngh, x0, y0, z0,
                                                    dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::inner_x3] != nullptr)
        MGBoundaryFunction_[BoundaryFace::inner_x3](cbuf_, time, nvar, i, i, cs,
                                                    ce, cs, ce, ngh, x0, y0, z0,
                                                    dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::outer_x3] != nullptr)
        MGBoundaryFunction_[BoundaryFace::outer_x3](cbuf_, time, nvar, i, i, cs,
                                                    ce, cs, ce, ngh, x0, y0, z0,
                                                    dx, dy, dz);
      for (int k = cs, fk = fs; k <= ce; ++k, fk += 2) {
        for (int j = cs, fj = fs; j <= ce; ++j, fj += 2) {
          ...
          dst(0, fk, fj, fig) =
              ot * (2.0 * (ccval - gx2c - gx3c) + u(0, fk, fj, fi));
          dst(0, fk, fj + 1, fig) =
              ot * (2.0 * (ccval + gx2c - gx3c) + u(0, fk, fj + 1, fi));
          dst(0, fk + 1, fj, fig) =
              ot * (2.0 * (ccval - gx2c + gx3c) + u(0, fk + 1, fj, fi));
          dst(0, fk + 1, fj + 1, fig) =
              ot * (2.0 * (ccval + gx2c + gx3c) + u(0, fk + 1, fj + 1, fi));
        }
      }
    }
  }
  ...
  for (int ox2 = -1; ox2 <= 1; ox2 += 2) {
    if (nblevel[1][ox2 + 1][1] == loc.level - 1) {
      int j, fj, fjg;
      if (ox2 > 0)
        j = ce + 1, fj = fe, fjg = fe + 1;
      else
        j = cs - 1, fj = fs, fjg = fs - 1;
      if (MGBoundaryFunction_[BoundaryFace::inner_x1] != nullptr)
        MGBoundaryFunction_[BoundaryFace::inner_x1](cbuf_, time, nvar, cs, ce,
                                                    j, j, cs, ce, ngh, x0, y0,
                                                    z0, dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::outer_x1] != nullptr)
        MGBoundaryFunction_[BoundaryFace::outer_x1](cbuf_, time, nvar, cs, ce,
                                                    j, j, cs, ce, ngh, x0, y0,
                                                    z0, dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::inner_x3] != nullptr)
        MGBoundaryFunction_[BoundaryFace::inner_x3](cbuf_, time, nvar, cs, ce,
                                                    j, j, cs, ce, ngh, x0, y0,
                                                    z0, dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::outer_x3] != nullptr)
        MGBoundaryFunction_[BoundaryFace::outer_x3](cbuf_, time, nvar, cs, ce,
                                                    j, j, cs, ce, ngh, x0, y0,
                                                    z0, dx, dy, dz);
      for (int k = cs, fk = fs; k <= ce; ++k, fk += 2) {
        for (int i = cs, fi = fs; i <= ce; ++i, fi += 2) {
          ...
          Real gx3c = 0.125 * (SIGN(gx3m) + SIGN(gx3p)) *
                      std::min(std::abs(gx3m), std::abs(gx3p));
          dst(0, fk, fjg, fi) =
              ot * (2.0 * (ccval - gx1c - gx3c) + u(0, fk, fj, fi));
          dst(0, fk, fjg, fi + 1) =
              ot * (2.0 * (ccval + gx1c - gx3c) + u(0, fk, fj, fi + 1));
          dst(0, fk + 1, fjg, fi) =
              ot * (2.0 * (ccval - gx1c + gx3c) + u(0, fk + 1, fj, fi));
          dst(0, fk + 1, fjg, fi + 1) =
              ot * (2.0 * (ccval + gx1c + gx3c) + u(0, fk + 1, fj, fi + 1));
        }
      }
    }
  }
  ...
  for (int ox3 = -1; ox3 <= 1; ox3 += 2) {
    if (nblevel[ox3 + 1][1][1] == loc.level - 1) {
      int k, fk, fkg;
      if (ox3 > 0)
        k = ce + 1, fk = fe, fkg = fe + 1;
      else
        k = cs - 1, fk = fs, fkg = fs - 1;
      if (MGBoundaryFunction_[BoundaryFace::inner_x1] != nullptr)
        MGBoundaryFunction_[BoundaryFace::inner_x1](cbuf_, time, nvar, cs, ce,
                                                    cs, ce, k, k, ngh, x0, y0,
                                                    z0, dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::outer_x1] != nullptr)
        MGBoundaryFunction_[BoundaryFace::outer_x1](cbuf_, time, nvar, cs, ce,
                                                    ...
                                                    z0, dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::inner_x2] != nullptr)
        MGBoundaryFunction_[BoundaryFace::inner_x2](cbuf_, time, nvar, cs, ce,
                                                    cs, ce, k, k, ngh, x0, y0,
                                                    z0, dx, dy, dz);
      if (MGBoundaryFunction_[BoundaryFace::outer_x2] != nullptr)
        MGBoundaryFunction_[BoundaryFace::outer_x2](cbuf_, time, nvar, cs, ce,
                                                    cs, ce, k, k, ngh, x0, y0,
                                                    z0, dx, dy, dz);
      for (int j = cs, fj = fs; j <= ce; ++j, fj += 2) {
        for (int i = cs, fi = fs; i <= ce; ++i, fi += 2) {
          ...
          Real gx2c = 0.125 * (SIGN(gx2m) + SIGN(gx2p)) *
                      std::min(std::abs(gx2m), std::abs(gx2p));
          dst(0, fkg, fj, fi) =
              ot * (2.0 * (ccval - gx1c - gx2c) + u(0, fk, fj, fi));
          dst(0, fkg, fj, fi + 1) =
              ot * (2.0 * (ccval + gx1c - gx2c) + u(0, fk, fj, fi + 1));
          dst(0, fkg, fj + 1, fi) =
              ot * (2.0 * (ccval - gx1c + gx2c) + u(0, fk, fj + 1, fi));
          dst(0, fkg, fj + 1, fi + 1) =
              ot * (2.0 * (ccval + gx1c + gx2c) + u(0, fk, fj + 1, fi + 1));
        }
      }
    }
  }
