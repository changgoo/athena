...
ParticleMeshBoundaryVariable::ParticleMeshBoundaryVariable(
    ...
    : CellCenteredBoundaryVariable(pmb, var, coarse_var, var_flux), ppm_(ppm) {
  var_buf.NewAthenaArray(var->GetDim4(), pmb->ncells3, pmb->ncells2,
                         pmb->ncells1);
}
...
int ParticleMeshBoundaryVariable::LoadBoundaryBufferSameLevel(
...
void ParticleMeshBoundaryVariable::SetBoundarySameLevel(
    Real *buf, const NeighborBlock &nb) {
  MeshBlock *pmb = pmy_block_;
  int si, sj, sk, ei, ej, ek;
...
//!                                       AthenaArray<Real> &src, Real *buf, int
//!                                       nb)
//! \brief Set PM shearing boundary buffers for sending to a block on the same
//! level
...
#ifdef MPI_PARALLEL
            int tag =
                pbval_->CreateBvalsMPITag(snb.lid, shid, shear_cc_phys_id_);
            MPI_Request *sreq = &shear_bd_var_[upper].req_send[n];
            MPI_Isend(sbuf, size, MPI_ATHENA_REAL, snb.rank, tag,
                      MPI_COMM_WORLD, sreq);
#endif
          }
        }
...
void ParticleMeshBoundaryVariable::SetShearingBoxBoundaryBuffers() {
  ...
  for (int upper = 0; upper < 2; upper++) {
    if (pbval_->is_shear[upper]) { // check inner boundaries
      // step 1 -- (optionally) apply shear to shear_cc_ y-mom (does nothing by
      // default)
      ...
      for (int n = nl_; n <= nu_; n++) {
        ...
        for (int k = kl; k <= ku; k++) {
          for (int i = 0; i < NGHOST; i++) {
            if (xorder <= 2) {
              porb->RemapFluxPlm(pflux, pbuf, eps, 1 - upper, k, i, jl, ju + 1,
                                 xgh);
            } else {
              porb->RemapFluxPpm(pflux, pbuf, eps, 1 - upper, k, i, jl, ju + 1,
                                 xgh);
            }
            const int shift = xgh + 1 - upper;
            // set corresponding active zone indices where the shifted buffer to
            // be added
            int ii = ia[upper] + i;
            for (int j = jl; j <= ju; j++) {
              var(n, k, j, ii) +=
                  pbuf(k, i, j + shift) - (pflux(j + 1) - pflux(j));
            }
          }
        }
      }
...
//----------------------------------------------------------------------------------------
//! \fn void ParticleMeshBoundaryVariable::ShearQuantities(AthenaArray<Real>
//! &shear_cc_,
//!                                                   bool upper)
...
void ParticleMeshBoundaryVariable::ShearQuantities(AthenaArray<Real> &shear_cc_,
  ...
  for (int k = kl; k <= ku; k++) {
    for (int i = 0; i < NGHOST; i++) {
      for (int j = jl; j <= ju; j++) {
        shear_cc_(ppm_->imom2, k, i, j) +=
            sign[upper] * qomL * shear_cc_(ppm_->idens, k, i, j);
      }
    }
  }
  return;
...
//----------------------------------------------------------------------------------------
//! \fn void
//! ParticleMeshBoundaryVariable::ReceiveAndSetShearingBoxBoundariesWithWait()
//! \brief receive and set the boundary data for initialization

void ParticleMeshBoundaryVariable::
    ReceiveAndSetShearingBoxBoundariesWithWait() {
...
#endif
        SetShearingBoxBoundarySameLevel(shear_cc_[upper],
