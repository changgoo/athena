...
//----------------------------------------------------------------------------------------
//! \fn int CellCenteredBoundaryVariable::LoadFluxBoundaryBufferSameLevel(Real
//! *buf,
//!                                                  const NeighborBlock& nb)
//! \brief Set surface flux buffers for sending to a block on the same level

int CellCenteredBoundaryVariable::LoadFluxBoundaryBufferSameLevel(
...
//----------------------------------------------------------------------------------------
//! \fn int CellCenteredBoundaryVariable::LoadFluxBoundaryBufferToCoarser(Real
//! *buf,
//!                                                        const NeighborBlock&
//!                                                        nb)
//! \brief Set surface flux buffers for sending to a block on the coarser level

int CellCenteredBoundaryVariable::LoadFluxBoundaryBufferToCoarser(
    Real *buf, const NeighborBlock &nb) {
  MeshBlock *pmb = pmy_block_;
...
void CellCenteredBoundaryVariable::SetFluxBoundarySameLevel(
    Real *buf, const NeighborBlock &nb) {
  MeshBlock *pmb = pmy_block_;
  int p = 0;
  int i;
...
void CellCenteredBoundaryVariable::SetFluxBoundaryFromFiner(
    Real *buf, const NeighborBlock &nb) {
  MeshBlock *pmb = pmy_block_;
  int p = 0;
  if (nb.fid == BoundaryFace::inner_x1 || nb.fid == BoundaryFace::outer_x1) {
