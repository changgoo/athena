...
//!                                       nb)
//! \brief Set CC shearing boundary buffers for sending to a block on the same
//! level
      ...
      for (int n = 0; n < 4; n++) {
        ...
        if (snb.rank != -1) {
          ...
          if (snb.rank == Globals::my_rank) { // on the same process
            CopyShearBufferSameProcess(
                snb, shear_send_count_cc_[upper][n] * ssize, n, upper);
          } else { // MPI
#ifdef MPI_PARALLEL
            int tag = pbval_->CreateBvalsMPITag(snb.lid, n + offset[upper],
                                                shear_cc_phys_id_);
            MPI_Isend(shear_bd_var_[upper].send[n],
                      shear_send_count_cc_[upper][n] * ssize, MPI_ATHENA_REAL,
...
void CellCenteredBoundaryVariable::SetShearingBoxBoundarySameLevel(
  ...
  int p = 0;
  BufferUtility::UnpackData(buf, src, nl_, nu_, sj, ej, si, ei, sk, ek, p);
  return;
}
...
void CellCenteredBoundaryVariable::SetShearingBoxBoundaryBuffers() {
  ...
  for (int upper = 0; upper < 2; upper++) {
    if (pbval_->is_shear[upper]) { // check inner boundaries
      // step 1 -- (optionally) apply shear to shear_cc_ (does nothing by
      // default)
...
//----------------------------------------------------------------------------------------
//! \fn void
//! CellCenteredBoundaryVariable::StartReceivingShear(BoundaryCommSubset phase)
//! \brief
void CellCenteredBoundaryVariable::StartReceivingShear(
        ...
        for (int n = 0; n < 3; n++) {
          ...
          if ((target_rank != Globals::my_rank) && (target_rank != -1)) {
            ...
            tag = pbval_->CreateBvalsMPITag(
                pmy_block_->lid, n + tag_offset1[upper], shear_flx_phys_id_);
    ...
    if (pbval_->is_shear[upper]) {
      for (int n = 0; n < 4; n++) {
        ...
        if ((target_rank != Globals::my_rank) && (target_rank != -1)) {
          ...
          tag = pbval_->CreateBvalsMPITag(
              pmy_block_->lid, n + tag_offset2[upper], shear_cc_phys_id_);
