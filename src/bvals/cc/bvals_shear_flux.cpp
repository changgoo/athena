...
//! \brief Set shearing boundary flux buffers for sending to a block on the same
//! level
      ...
      for (int n = 0; n < 3; n++) {
        ...
        if (snb.rank != -1) {
          ...
          if (snb.rank == Globals::my_rank) { // on the same process
            CopyShearFluxSameProcess(
                snb, shear_send_count_flx_[upper][n] * ssize, n, upper);
          } else { // MPI
#ifdef MPI_PARALLEL
            int tag = pbval_->CreateBvalsMPITag(snb.lid, n + offset[upper],
                                                shear_flx_phys_id_);
            MPI_Isend(shear_bd_flux_[upper].send[n],
                      shear_send_count_flx_[upper][n] * ssize, MPI_ATHENA_REAL,
...
//! \brief Set shearing box boundary flux data received from a block on the same
//! level
  ...
  for (int upper = 0; upper < 2; upper++) {
    if (pbval_->is_shear[upper]) { // check inner boundaries
      ...
      for (int n = nl_; n <= nu_; n++) {
        ...
        for (int k = ks; k <= ke; k++) {
          if (xorder <= 2) {
            porb->RemapFluxPlm(pflux, pbuf, eps, 1 - upper, k, 0, js, je + 1,
                               xgh);
          } else {
            porb->RemapFluxPpm(pflux, pbuf, eps, 1 - upper, k, 0, js, je + 1,
                               xgh);
          }
          const int shift = xgh + 1 - upper;
          for (int j = js; j <= je; j++) {
            x1flux(n, k, j, ii) =
                0.5 * (x1flux(n, k, j, ii) + pbuf(k, 0, j + shift) -
                       (pflux(j + 1) - pflux(j)));
          }
        }
      }
