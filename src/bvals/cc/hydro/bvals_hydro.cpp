...
//! \brief implements boundary functions for Hydro variables and utilities to
//! manage primitive/conservative variable relationship in a derived class of
...
HydroBoundaryVariable::HydroBoundaryVariable(MeshBlock *pmb,
                                             AthenaArray<Real> *var_hydro,
...
//----------------------------------------------------------------------------------------
//! \fn void HydroBoundaryVariable::SelectCoarseBuffer(HydroBoundaryQuantity
//! type) \brief

void HydroBoundaryVariable::SelectCoarseBuffer(
    HydroBoundaryQuantity hydro_type) {
...
void HydroBoundaryVariable::SwapHydroQuantity(
    AthenaArray<Real> &var_hydro, HydroBoundaryQuantity hydro_type) {
  var_cc = &var_hydro;
...
#pragma omp simd linear(p)
  ...
  if (pbval_->shearing_box == 2) {
    ...
    for (int upper = 0; upper < 2; upper++) {
      if ((pmb->loc.lx1 == pbval_->loc_shear[upper]) &&
          (sign[upper] * nb.ni.ox1 < 0)) {
        for (int k = sk; k <= ek; ++k) {
          for (int j = sj; j <= ej; ++j) {
            for (int i = si; i <= ei; ++i) {
