...
//! \brief Set surface hydro flux buffers for sending to a block on the same
//! level

int HydroBoundaryVariable::LoadFluxBoundaryBufferSameLevel(
      ...
      for (int nn = nl_; nn <= nu_; nn++) {
        if (nn == IVY) {
          for (int k = pmb->ks; k <= pmb->ke; k++) {
            for (int j = pmb->js; j <= pmb->je; j++) {
              buf[p++] =
                  x1flux(nn, k, j, i) + sign * qomL * x1flux(IDN, k, j, i);
            }
          }
        } else if (nn == IEN) {
          for (int k = pmb->ks; k <= pmb->ke; k++) {
