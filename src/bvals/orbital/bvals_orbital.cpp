...
void OrbitalBoundaryCommunication::InitBoundaryData(OrbitalBoundaryData &bd,
  ...
  if (porb->orbital_refinement) {
    ...
    if (porb->orbital_direction == 1) {
      if (nx3 > 1) { // 3D
        ...
        if (type == BoundaryQuantity::orbital_cc) {
          ...
          ssize = (nx1 / 2 + 1) * (nx2 / 2 + 2) +
                  (nx1 / 2 + 2) * (nx2 / 2 + 1) + (nx1 / 2 + 2) * (nx2 / 2 + 2);
          lsize = (3 * nx1 + 1) * nx2;
        } else {
          std::stringstream msg;
          msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
      ...
      if (type == BoundaryQuantity::orbital_cc) {
        ssize =
            (NHYDRO + NSCALARS) * (nx1 / 2 + 2) * (nx2 / 2 + 2) * (nx3 / 2 + 2);
        lsize = (NHYDRO + NSCALARS) * nx1 * nx2 * nx3;
      } else if (type == BoundaryQuantity::orbital_fc) {
        ssize = (nx1 / 2 + 1) * (nx2 / 2 + 2) * (nx3 / 2 + 2) +
                (nx1 / 2 + 2) * (nx2 / 2 + 1) * (nx3 / 2 + 2) +
...
//----------------------------------------------------------------------------------------
//! \fn void
//! OrbitalBoundaryCommunication::DestroyBoundaryData(OrbitalBoundaryData &bd)
//! \brief destroy BoundaryData structure

void OrbitalBoundaryCommunication::DestroyBoundaryData(
    OrbitalBoundaryData &bd) {
  for (int n = 0; n < bd.nbmax; n++) {
    delete[] bd.send[n];
    delete[] bd.recv[n];
#ifdef MPI_PARALLEL
  ...
  if (porb->orbital_direction == 1) {
    for (int n = 0; n < pbval_->nneighbor; n++) {
      ...
      if (nb.ni.ox1 == 0 && nb.ni.ox3 == 0) {
        if (nb.ni.ox2 == -1) {
          SimpleNeighborBlock &snb1 =
              orbital_send_neighbor_[1][nb.ni.fi1 + nb.ni.fi2 * 2];
          SimpleNeighborBlock &snb2 =
              orbital_recv_neighbor_[0][nb.ni.fi1 + nb.ni.fi2 * 2];
          snb1.gid = nb.snb.gid;
          snb1.lid = nb.snb.lid;
          snb1.rank = nb.snb.rank;
          snb1.level = nb.snb.level;
          snb2.gid = nb.snb.gid;
          snb2.lid = nb.snb.lid;
          snb2.rank = nb.snb.rank;
          snb2.level = nb.snb.level;
        } else if (nb.ni.ox2 == 1) {
          SimpleNeighborBlock &snb1 =
              orbital_send_neighbor_[0][nb.ni.fi1 + nb.ni.fi2 * 2];
          SimpleNeighborBlock &snb2 =
              orbital_recv_neighbor_[1][nb.ni.fi1 + nb.ni.fi2 * 2];
          snb1.gid = nb.snb.gid;
          snb1.lid = nb.snb.lid;
          snb1.rank = nb.snb.rank;
          snb1.level = nb.snb.level;
...
void OrbitalBoundaryCommunication::ComputeOrbit(const Real dt) {
  ...
  if (porb->orbital_refinement) { // orbital refinement
    ...
    for (int upper = 0; upper < 2; upper++) {
      ...
      } else if (orbital_send_neighbor_[upper][0].level <
                 mylevel) { // to coarser
      ...
      } else if (orbital_recv_neighbor_[upper][0].level <
                 mylevel) { // from coarser
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
      ...
      int nco2 = (porb->orbital_direction == 1) ? pmb->block_size.nx3 / 2
        ...
        if (orbital_send_neighbor_[upper][0].level > mylevel) { // to finer
          for (int n = 0; n < orbital_bd_fc_[upper].nbmax; n++) {
            ...
            if (pmb->block_size.nx3 > 1) { // 3D
              ...
              size = (nco1 + 1) * (norb + 2) + (nco1 + 2) * (norb + 1) +
                     (nco1 + 2) * (norb + 2);
            }
            if (size > 0) {
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::StartReceiving(BoundaryCommSubset
//! phase) \brief StartReceiving function for the orbital communication
      ...
      for (int n = 0; n < orbital_bd_fc_[upper].nbmax; n++) {
        ...
        if (orbital_recv_fc_count_[upper][n] > 0) {
          ...
          if ((target_rank != Globals::my_rank) && (target_rank != -1)) {
            ...
            tag = pbval_->CreateBvalsMPITag(pmy_block_->lid,
                                            n + tag_offset[upper],
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalBoundaryCommunication::ClearBoundary(BoundaryCommSubset
//! phase) \brief ClearBoundary function for the orbital communication
void OrbitalBoundaryCommunication::ClearBoundary(BoundaryCommSubset phase) {
  for (int upper = 0; upper < 2; upper++) {
    for (int n = 0; n < orbital_bd_cc_[upper].nbmax; n++) {
      if (orbital_send_neighbor_[upper][n].rank == -1)
        continue;
      orbital_bd_cc_[upper].flag[n] = BoundaryStatus::waiting;
#ifdef MPI_PARALLEL
...
void OrbitalBoundaryCommunication::SendBoundaryBuffersCC() {
  ...
  for (int upper = 0; upper < 2; upper++) {
    for (int n = 0; n < orbital_bd_cc_[upper].nbmax; n++) {
      if (orbital_bd_cc_[upper].sflag[n] == BoundaryStatus::completed)
        continue;
      SimpleNeighborBlock &snb = orbital_send_neighbor_[upper][n];
      int p = 0;
      if (snb.level == mylevel) { // to same
        LoadHydroBufferSameLevel(orbital_bd_cc_[upper].send[n], p,
                                 n + offset[upper]);
      } else if (snb.level < mylevel) { // to coarser
        LoadHydroBufferToCoarser(orbital_bd_cc_[upper].send[n], p,
                                 n + offset[upper]);
      } else { // to finer
        LoadHydroBufferToFiner(orbital_bd_cc_[upper].send[n], p,
                               n + offset[upper]);
      }
      if (NSCALARS > 0) {
        if (snb.level == mylevel) { // to same
          LoadScalarBufferSameLevel(orbital_bd_cc_[upper].send[n], p,
                                    n + offset[upper]);
        } else if (snb.level < mylevel) { // to coarser
          LoadScalarBufferToCoarser(orbital_bd_cc_[upper].send[n], p,
                                    n + offset[upper]);
        } else { // to finer
          LoadScalarBufferToFiner(orbital_bd_cc_[upper].send[n], p,
                                  n + offset[upper]);
        }
      }
      ...
      if (snb.rank == Globals::my_rank) { // on the same process
        ...
        if (snb.level == mylevel) { // to same level
          std::memcpy(obd.recv[n], orbital_bd_cc_[upper].send[n],
                      p * sizeof(Real));
          ...
          if (porb->orbital_direction == 1) {
            ...
            std::memcpy(obd.recv[n1 + n3 * 2], orbital_bd_cc_[upper].send[n],
                        p * sizeof(Real));
            obd.flag[n1 + n3 * 2] = BoundaryStatus::arrived;
          } else {
            int n2 = static_cast<int>(pmb->loc.lx2 % 2);
            std::memcpy(obd.recv[n1 + n2 * 2], orbital_bd_cc_[upper].send[n],
                        p * sizeof(Real));
            obd.flag[n1 + n2 * 2] = BoundaryStatus::arrived;
          }
        } else { // to finer
          std::memcpy(obd.recv[0], orbital_bd_cc_[upper].send[n],
                      p * sizeof(Real));
        ...
        if (snb.level == mylevel) { // to same level
          ...
          if (porb->orbital_direction == 1) {
            int n3 = static_cast<int>(pmb->loc.lx3 % 2);
            tag =
                pbval_->CreateBvalsMPITag(snb.lid, n1 + n3 * 2 + offset[upper],
                                          orbital_advection_cc_phys_id_);
          } else {
            int n2 = static_cast<int>(pmb->loc.lx2 % 2);
            tag =
                pbval_->CreateBvalsMPITag(snb.lid, n1 + n2 * 2 + offset[upper],
                                          orbital_advection_cc_phys_id_);
          }
        } else { // to finer
...
bool OrbitalBoundaryCommunication::ReceiveBoundaryBuffersCC() {
  ...
  for (int upper = 0; upper < 2; upper++) {
    for (int n = 0; n < orbital_bd_cc_[upper].nbmax; n++) {
      if (orbital_bd_cc_[upper].flag[n] == BoundaryStatus::completed)
        continue;
      ...
      if (orbital_bd_cc_[upper].flag[n] == BoundaryStatus::waiting) {
        ...
        if (snb.rank == Globals::my_rank) {
          ...
          MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &test,
                     MPI_STATUS_IGNORE);
          MPI_Test(&orbital_bd_cc_[upper].req_recv[n], &test,
                   MPI_STATUS_IGNORE);
      ...
      if (snb.level == mylevel) { // from same level
        SetHydroBufferSameLevel(orbital_bd_cc_[upper].recv[n], p,
                                n + offset[upper]);
      } else if (snb.level < mylevel) { // from coarser
        SetHydroBufferFromCoarser(orbital_bd_cc_[upper].recv[n], p,
                                  n + offset[upper]);
      } else { // from finer
        SetHydroBufferFromFiner(orbital_bd_cc_[upper].recv[n], p,
                                n + offset[upper]);
      }
      if (NSCALARS > 0) {
        if (snb.level == mylevel) { // from same level
          SetScalarBufferSameLevel(orbital_bd_cc_[upper].recv[n], p,
                                   n + offset[upper]);
        } else if (snb.level < mylevel) { // from coarser
          SetScalarBufferFromCoarser(orbital_bd_cc_[upper].recv[n], p,
                                     n + offset[upper]);
        } else { // from finer
          SetScalarBufferFromFiner(orbital_bd_cc_[upper].recv[n], p,
                                   n + offset[upper]);
        }
      }
...
void OrbitalBoundaryCommunication::SendBoundaryBuffersFC() {
  ...
  for (int upper = 0; upper < 2; upper++) {
    for (int n = 0; n < orbital_bd_fc_[upper].nbmax; n++) {
      if (orbital_bd_fc_[upper].sflag[n] == BoundaryStatus::completed)
        continue;
      SimpleNeighborBlock &snb = orbital_send_neighbor_[upper][n];
      int p = 0;
      if (snb.level == mylevel) { // to same level
        LoadFieldBufferSameLevel(orbital_bd_fc_[upper].send[n], p,
                                 n + offset[upper]);
      } else if (snb.level < mylevel) { // to coarser
        LoadFieldBufferToCoarser(orbital_bd_fc_[upper].send[n], p,
                                 n + offset[upper]);
      } else { // to finer
        LoadFieldBufferToFiner(orbital_bd_fc_[upper].send[n], p,
                               n + offset[upper]);
      }
      ...
      if (snb.rank == Globals::my_rank) { // on the same process
        ...
        if (snb.level == mylevel) { // to same level
          std::memcpy(obd.recv[n], orbital_bd_fc_[upper].send[n],
                      p * sizeof(Real));
          obd.flag[n] = BoundaryStatus::arrived;
        } else if (snb.level < mylevel) { // to coarser
          int n1 = static_cast<int>(pmb->loc.lx1 % 2);
          if (porb->orbital_direction == 1) {
            int n3 = static_cast<int>(pmb->loc.lx3 % 2);
            std::memcpy(obd.recv[n1 + n3 * 2], orbital_bd_fc_[upper].send[n],
                        p * sizeof(Real));
            obd.flag[n1 + n3 * 2] = BoundaryStatus::arrived;
          } else {
            int n2 = static_cast<int>(pmb->loc.lx2 % 2);
            std::memcpy(obd.recv[n1 + n2 * 2], orbital_bd_fc_[upper].send[n],
                        p * sizeof(Real));
            obd.flag[n1 + n2 * 2] = BoundaryStatus::arrived;
          }
        } else { // to finer
          std::memcpy(obd.recv[0], orbital_bd_fc_[upper].send[n],
                      p * sizeof(Real));
          obd.flag[0] = BoundaryStatus::arrived;
        }
      } else {
...
bool OrbitalBoundaryCommunication::ReceiveBoundaryBuffersFC() {
  ...
  for (int upper = 0; upper < 2; upper++) {
    for (int n = 0; n < orbital_bd_fc_[upper].nbmax; n++) {
      if (orbital_bd_fc_[upper].flag[n] == BoundaryStatus::completed)
        continue;
      SimpleNeighborBlock &snb = orbital_recv_neighbor_[upper][n];
      if (orbital_bd_fc_[upper].flag[n] == BoundaryStatus::waiting) {
        ...
        if (snb.rank == Globals::my_rank) {
          ...
          MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &test,
                     MPI_STATUS_IGNORE);
          MPI_Test(&orbital_bd_fc_[upper].req_recv[n], &test,
                   MPI_STATUS_IGNORE);
          if (!static_cast<bool>(test)) {
            flag[upper] = false;
            continue;
      ...
      int p = 0;
      if (snb.level == mylevel) { // from same level
        SetFieldBufferSameLevel(orbital_bd_fc_[upper].recv[n], p,
                                n + offset[upper]);
      } else if (snb.level < mylevel) { // from coarser
        SetFieldBufferFromCoarser(orbital_bd_fc_[upper].recv[n], p,
                                  n + offset[upper]);
      } else { // from finer
        SetFieldBufferFromFiner(orbital_bd_fc_[upper].recv[n], p,
                                n + offset[upper]);
      }
...
void OrbitalBoundaryCommunication::LoadHydroBufferSameLevel(Real *buf, int &p,
                                                            int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::LoadHydroBufferToCoarser(Real *buf, int &p,
                                                            int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::LoadHydroBufferToFiner(Real *buf, int &p,
                                                          int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &ui = pmb->phydro->u;
  ...
  if (porb->orbital_uniform_mesh) { // uniform mesh
    if (porb->orbital_direction == 1) {
      ...
      BufferUtility::PackData(ui, buf, 0, NHYDRO - 1, il, iu, jl, ju, kl, ku,
                              p);
    } else if (porb->orbital_direction == 2) {
      int &onx = pmb->block_size.nx3;
      int il, iu, jl, ju, kl, ku;
...
void OrbitalBoundaryCommunication::SetHydroBufferSameLevel(Real *buf, int &p,
                                                           const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::SetHydroBufferFromCoarser(Real *buf, int &p,
                                                             const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &uo = porb->orbital_cons;
...
void OrbitalBoundaryCommunication::SetHydroBufferFromFiner(Real *buf, int &p,
                                                           const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &uo = porb->orbital_cons;
...
void OrbitalBoundaryCommunication::LoadFieldBufferSameLevel(Real *buf, int &p,
                                                            int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::LoadFieldBufferToCoarser(Real *buf, int &p,
                                                            int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  FaceField &bi = pmb->pfield->b;
...
void OrbitalBoundaryCommunication::LoadFieldBufferToFiner(Real *buf, int &p,
                                                          int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  FaceField &bi = pmb->pfield->b;
  ...
  if (porb->orbital_uniform_mesh) { // uniform mesh
    ...
    if (pmb->block_size.nx3 > 1) { // 3D
      ...
      // pack b1
      BufferUtility::PackData(bi.x1f, buf, il, iu + 1, jl - 1, ju + 1, kl - 1,
                              ku + 1, p);
      // pack b2
      BufferUtility::PackData(bi.x2f, buf, il - 1, iu + 1, jl, ju + 1, kl - 1,
                              ku + 1, p);
      // pack b3
      BufferUtility::PackData(bi.x3f, buf, il - 1, iu + 1, jl - 1, ju + 1, kl,
                              ku + 1, p);
    } else { // 2D
      if (porb->orbital_direction == 1) {
        onx = pmb->block_size.nx2;
        ...
        msg << "### FATAL ERROR in OrbitalBoundaryCommunication"
            << "::LoadFieldBufferToFiner" << std::endl
            << "2D Orbital Advection is not allowed in spherical_polar."
            << std::endl;
        ATHENA_ERROR(msg);
      }
      // pack b1
      BufferUtility::PackData(bi.x1f, buf, il, iu + 1, jl - 1, ju + 1, kl, ku,
                              p);
      // pack b2
      BufferUtility::PackData(bi.x2f, buf, il - 1, iu + 1, jl, ju + 1, kl, ku,
                              p);
      // pack b3
      BufferUtility::PackData(bi.x3f, buf, il - 1, iu + 1, jl - 1, ju + 1, kl,
                              ku, p);
...
void OrbitalBoundaryCommunication::SetFieldBufferSameLevel(Real *buf, int &p,
                                                           const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::SetFieldBufferFromCoarser(Real *buf, int &p,
                                                             const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &bo1 = porb->orbital_b1;
  ...
  if (porb->orbital_uniform_mesh) { // uniform mesh
    int onx = pmb->block_size.nx2;
    int il = pmb->cis, iu = pmb->cie, jl = pmb->cjs, ju = pmb->cje,
        kl = pmb->cks, ku = pmb->cke;
    if (pmb->block_size.nx3 > 1) { // 3D
      if (porb->orbital_direction == 1) {
        if (nb == 0) {
    ...
    pmb->pmr->ProlongateSharedFieldX1(bco.x1f, bto.x1f, il, iu + 1, jl, ju, kl,
                                      ku);
    pmb->pmr->ProlongateSharedFieldX2(bco.x2f, bto.x2f, il, iu, jl, ju + 1, kl,
                                      ku);
    pmb->pmr->ProlongateSharedFieldX3(bco.x3f, bto.x3f, il, iu, jl, ju, kl,
                                      ku + 1);
...
void OrbitalBoundaryCommunication::SetFieldBufferFromFiner(Real *buf, int &p,
                                                           const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &bo1 = porb->orbital_b1;
...
void OrbitalBoundaryCommunication::LoadScalarBufferSameLevel(Real *buf, int &p,
                                                             int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::LoadScalarBufferToCoarser(Real *buf, int &p,
                                                             int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::LoadScalarBufferToFiner(Real *buf, int &p,
                                                           int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &si = pmb->pscalars->s;
  ...
  if (porb->orbital_uniform_mesh) { // uniform mesh
    if (porb->orbital_direction == 1) {
      ...
      BufferUtility::PackData(si, buf, 0, NSCALARS - 1, il, iu, jl, ju, kl, ku,
                              p);
    } else if (porb->orbital_direction == 2) {
      int &onx = pmb->block_size.nx3;
      int il, iu, jl, ju, kl, ku;
...
void OrbitalBoundaryCommunication::SetScalarBufferSameLevel(Real *buf, int &p,
                                                            const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
...
void OrbitalBoundaryCommunication::SetScalarBufferFromCoarser(Real *buf, int &p,
                                                              const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &so = porb->orbital_scalar;
  ...
  if (porb->orbital_uniform_mesh) { // uniform mesh
    if (porb->orbital_direction == 1) {
      ...
      if (nb == 0) {
        ...
        pmb->pmr->ProlongateCellCenteredValues(sco, sto, 0, NSCALARS - 1,
                                               pmb->cis, pmb->cie, jl + 1,
                                               pmb->cje, pmb->cks, pmb->cke);
        for (int k = pmb->ks; k <= pmb->ke; k++) {
          for (int i = pmb->is; i <= pmb->ie; i++) {
...
void OrbitalBoundaryCommunication::SetScalarBufferFromFiner(Real *buf, int &p,
                                                            const int nb) {
  MeshBlock *pmb = pmy_block_;
  OrbitalAdvection *porb = pmy_orbital_;
  AthenaArray<Real> &so = porb->orbital_scalar;
