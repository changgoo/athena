...
//========================================================================================
//! \file bvals.hpp
//! \brief defines BoundaryBase, BoundaryValues classes used for setting BCs on
//! all data
...
//----------------------------------------------------------------------------------------
//! \class BoundaryBase
//! \brief Base class for all BoundaryValues classes (BoundaryValues and
//! MGBoundaryValues)
//!
//! \note
//! - 1x pair (neighbor index, buffer ID) per entire SET of separate variable
//! buffers
//!   (Hydro, Field, Passive Scalar, Gravity, etc.). Greedy allocation for
//!   worst-case of refined 3D; only 26 entries needed/initialized if unrefined
//!   3D, e.g.
...
public:
  ...
  // 1x pair (neighbor index, buffer ID) per entire SET of separate variable
  // buffers (Hydro, Field, Passive Scalar, Gravity, etc.). Greedy allocation
  // for worst-case of refined 3D; only 26 entries needed/initialized if
  // unrefined 3D, e.g.
...
protected:
  // 1D refined or unrefined=2
  // 2D refined=12, unrefined=8
  // 3D refined=56, unrefined=26. Refinement adds: 3*6 faces + 1*12 edges = +30
  // neighbors
...
private:
  // calculate 3x shared static data members when constructing only the 1st
  // class instance int maxneighbor_=BufferID() computes ni[] and then calls
  // bufid[]=CreateBufferID()
  static bool called_;
};

//----------------------------------------------------------------------------------------
//! \class BoundaryValues
//! \brief centralized class for interacting with each individual variable
//! boundary data
...
public:
  ...
  // non-inhertied / unique functions (do not exist in BoundaryVariable
  // objects): (these typically involve a coupled interaction of boundary
  // variable/quantities)
  // ------
  ...
  //! safety check of user's boundary fns in Mesh::Initialize before
  //! SetupPersistentMPI()
...
private:
  ...
  //! if a BoundaryPhysics or user fn should be applied at each MeshBlock
  //! boundary false --> e.g. block, polar, (shear-) periodic boundaries
  bool apply_bndry_fn_[6]{}; // C++11: in-class initializer of non-static member
  // C++11: direct-list-initialization -> value init of array -> zero init of
  // each scalar

  //! For spherical polar coordinates edge-case: if one MeshBlock wraps entirely
  //! around the pole (azimuthally), shift the k-axis by nx3/2 for cell- and
  //! face-centered variables, & emf, using this temporary 1D array.
  AthenaArray<Real> azimuthal_shift_;

  //! local counter for generating unique MPI tags for per-MeshBlock
  //! BoundaryVariable communication (subset of Mesh::next_phys_id_)
  ...
  std::int64_t
      loc_shear[2]; // x1 LogicalLocation of block(s) on inner/outer shear bndry
  ...
  // ProlongateBoundaries() wraps the following S/AMR-operations (within
  // nneighbor loop): (the next function is also called within 3x nested loops
  // over nk,nj,ni)
  void RestrictGhostCellsOnSameLevel(const NeighborBlock &nb, int nk, int nj,
                                     int ni);
  void ApplyPhysicalBoundariesOnCoarseLevel(
      ...
      std::vector<BoundaryVariable *> bvars_subset);
  void ProlongateGhostCells(const NeighborBlock &nb, int si, int ei, int sj,
  ...
  friend class Mesh;
  //! currently, this class friendship is required for copying send/recv buffers
  //! between BoundaryVariable objects within different MeshBlocks on the same
  //! MPI rank:
  friend class BoundaryVariable;
  friend class FaceCenteredBoundaryVariable; // needs nface_, nedge_,
                                             // num_north/south_...
  //! \todo (felker):
  ...
  friend class ParticleMeshBoundaryVariable; // needed for shearing box
                                             // quantities
