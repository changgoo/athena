...
// required definitions of static data members of BoundaryBase outside class
// definition (zero-initialization is performed for all static storage duration
// variables) scalar types: integral constant 0 is explicitly converted to type
...
// struct type: zero-initializes each non-static data member (this case: all
// scalar types)
...
//----------------------------------------------------------------------------------------
//! \fn BoundaryBase::BoundaryBase(Mesh *pm, LogicalLocation iloc, RegionSize
//! isize,
...
BoundaryBase::BoundaryBase(Mesh *pm, LogicalLocation iloc, RegionSize isize,
  ...
  // copy/set in class the input 6x BoundaryFlag for this local MeshBlock
  // boundaries
  for (int i = 0; i < 6; i++)
    block_bcs[i] = input_bcs[i];
  // count blocks around azimuth at each pole and setup structs for their
  // neighbor info:
...
int BoundaryBase::BufferID(int dim, bool multilevel) {
  ...
  for (int n = 0; n < b; n++)
    bufid[n] =
        CreateBufferID(ni[n].ox1, ni[n].ox2, ni[n].ox3, ni[n].fi1, ni[n].fi2);

  return b;
}

//----------------------------------------------------------------------------------------
//! \fn int BoundaryBase::FindBufferID(int ox1, int ox2, int ox3, int fi1, int
//! fi2) \brief find the boundary buffer ID from the direction
...
//!
//! \warning
//! The below procedure of generating unsigned integer bitfields from signed
//! integer types and converting output to signed integer tags (required by MPI)
//! is tricky and may lead to unsafe conversions (and overflows from built-in
//! types and MPI_TAG_UB). Note, the MPI standard requires signed int tag, with
//! MPI_TAG_UB>= 2^15-1 = 32,767 (inclusive)
//!
//! \todo (felker):
...
void BoundaryBase::SearchAndSetNeighbors(MeshBlockTree &tree, int *ranklist,
  ...
  if (block_size_.nx2 > 1)
    myox2 = ((loc.lx2 & 1LL) == 1LL) * 2 - 1;
  if (block_size_.nx3 > 1)
    myox3 = ((loc.lx3 & 1LL) == 1LL) * 2 - 1;
  ...
  for (int n = -1; n <= 1; n += 2) {
    ...
    if (neibt->pleaf_ != nullptr) { // neighbor at finer level
      int fface =
          1 - (n + 1) / 2; // 0 for BoundaryFace::outer_x1, 1 for inner_x1
      ...
      if (nlevel == loc.level) { // neighbor at same level
        ...
        if ((n == -1 && block_bcs[BoundaryFace::inner_x1] ==
  ...
  for (int n = -1; n <= 1; n += 2) {
    ...
    if (neibt->pleaf_ != nullptr) { // neighbor at finer level
      int fface =
          1 - (n + 1) / 2; // 0 for BoundaryFace::outer_x2, 1 for inner_x2
  ...
  if (block_size_.nx3 > 1) {
    for (int n = -1; n <= 1; n += 2) {
      ...
      if (neibt->pleaf_ != nullptr) { // neighbor at finer level
        int fface =
            1 - (n + 1) / 2; // 0 for BoundaryFace::outer_x3, 1 for inner_x3
        ...
        for (int f2 = 0; f2 < nf2; f2++) {
          for (int f1 = 0; f1 < nf1; f1++) {
            ...
            neighbor[nneighbor].SetNeighbor(
                ranklist[fid], nlevel, fid, fid - nslist[ranklist[fid]], 0, 0,
                n, NeighborConnect::face, bufid, tbid, false, false, f1, f2);
            bufid++;
            nneighbor++;
          }
        }
    ...
    for (int n = -1; n <= 1; n += 2) {
      ...
      if (neibt->pleaf_ != nullptr) { // neighbor at finer level
        int ff1 =
            1 - (n + 1) / 2; // 0 for BoundaryFace::outer_x1, 1 for inner_x1
        int ff2 =
            1 - (m + 1) / 2; // 0 for BoundaryFace::outer_x2, 1 for inner_x2
        if (polar) {
          ff2 = 1 - ff2;
        }
        ...
        for (int f1 = 0; f1 < nf2; f1++) {
          ...
          neighbor[nneighbor].SetNeighbor(
              ...
              NeighborConnect::edge, bufid, tbid, polar, false, f1, 0);
          bufid++;
          nneighbor++;
        }
      } else { // neighbor at same or coarser level
  ...
  if (block_bcs[BoundaryFace::inner_x2] == BoundaryFlag::polar ||
    ...
    int num_north_polar_blocks =
        static_cast<int>(pmy_mesh_->nrbx3 * (1 << level));
    for (int n = 0; n < num_north_polar_blocks; ++n) {
      ...
      // SMR+3D polar boundary check: all blocks around a pole are at same
      // refinement lvl:
      if (neibt == nullptr || neibt->gid_ < 0) {
        std::stringstream msg;
        msg << "### FATAL ERROR in BoundaryBase::SearchAndSetNeighbors"
            << std::endl
            << "The use of SMR with any 'polar' or 'polar_wedge' boundary in "
               "3D\n"
            << "requires that all MeshBlocks around a pole at the same radius "
               "\n"
            << "are at the same refinement level.\n"
            << "Current MeshBlock LogicalLocation = (" << loc.lx1 << ", "
            << loc.lx2 << ", " << loc.lx3
            << ")\nFindMeshBlock() returned nullptr or negative gid when "
               "finding\n"
            << "aziumthal neighbor n=" << n << "/" << num_north_polar_blocks
            << " total number of north polar blocks.\n"
            << "Azimuthal neighbor's LogicalLocation = (" << neighbor_loc.lx1
            << ", " << neighbor_loc.lx2 << ", " << neighbor_loc.lx3
            << ")\n----------------------------------\n"
            << "This MeshBlock's logical  level = " << loc.level << "\n"
            << "This MeshBlock's physical level = " << level << "\n"
            << "Aziumthal neighbor's level is ";
        if (neibt == nullptr) {
          // neighbor block is coarser or doesn't exist (physical boundary or
          // broken tree)
          msg << "coarser.\n";
        } else {
          // negative global ID in returned MeshBlockTree indicates that neibt
          // points to the parent MeshBlock of the actual neighbor block at a
          // finer refinement
          msg << "finer.\n";
        }
        msg << "----------------------------------" << std::endl;
    ...
    for (int n = 0; n < num_south_polar_blocks; ++n) {
      ...
      if (neibt == nullptr || neibt->gid_ < 0) {
        ...
        msg << "### FATAL ERROR in BoundaryBase::SearchAndSetNeighbors"
            ...
            << ")\nFindMeshBlock() returned nullptr or negative gid when "
               "finding\n"
            << "aziumthal neighbor n=" << n << "/" << num_south_polar_blocks
            << " total number of south polar blocks.\n"
            << "Azimuthal neighbor's LogicalLocation = (" << neighbor_loc.lx1
            << ", " << neighbor_loc.lx2 << ", " << neighbor_loc.lx3
            << ")\n----------------------------------\n"
            << "This MeshBlock's logical  level = " << loc.level << "\n"
    ...
    for (int n = -1; n <= 1; n += 2) {
      ...
      if (neibt->pleaf_ != nullptr) { // neighbor at finer level
        int ff1 =
            1 - (n + 1) / 2; // 0 for BoundaryFace::outer_x1, 1 for inner_x1
        int ff2 =
            1 - (m + 1) / 2; // 0 for BoundaryFace::outer_x3, 1 for inner_x3
        ...
        for (int f1 = 0; f1 < nf1; f1++) {
          ...
          neighbor[nneighbor].SetNeighbor(
              ...
              NeighborConnect::edge, bufid, tbid, false, false, f1, 0);
          bufid++;
          nneighbor++;
        }
      } else { // neighbor at same or coarser level
      ...
      for (int n = -1; n <= 1; n += 2) {
        ...
        if (neibt->pleaf_ != nullptr) { // neighbor at finer level
          ...
          int ff2 =
              1 - (m + 1) / 2; // 0 for BoundaryFace::outer_x2, 1 for inner_x2
          int ff3 =
              1 - (l + 1) / 2; // 0 for BoundaryFace::outer_x3, 1 for inner_x3
          if (polar) {
            ff2 = 1 - ff2;
          }
