...
//========================================================================================
//! \file bvals_fc.hpp
//! \brief handle boundaries for any FaceField type variable that represents a
//! physical
...
public:
  FaceCenteredBoundaryVariable(MeshBlock *pmb, FaceField *var,
                               FaceField &coarse_buf, EdgeField &var_flux);
  ~FaceCenteredBoundaryVariable();

  //! \note
  //! may want to rebind var_fc to b, b1, b2, etc. Hence ptr member, not
  //! reference
  FaceField *var_fc;
  ...
  //! bitfield \note must correspond to the # of "int *phys_id_" private
  //! members, below. Convert to array?
...
private:
  ...
  //! variable switch used in 2x functions, ReceiveFluxCorrection() and
  //! StartReceiving(): ready to recv flux from same level and apply correction?
  ...
  void LoadEMFShearingBoxBoundarySameLevel(EdgeField &src, Real *buf,
                                           const int nb);
  void SetEMFShearingBoxBoundarySameLevel(EdgeField &dst, Real *buf,
                                          const int nb);
