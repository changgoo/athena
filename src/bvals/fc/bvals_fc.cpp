...
int FaceCenteredBoundaryVariable::LoadBoundaryBufferSameLevel(
  ...
  // for SMR/AMR, always include the overlapping faces in edge and corner
  // boundaries
...
//!
//! \todo (felker):
//! * nearly identical to CellCenteredBoundaryVariable counterpart (extra call
//! to
//!   PolarFieldBoundaryAverage())
...
//----------------------------------------------------------------------------------------
//! \fn void FaceCenteredBoundaryVariable::StartReceiving(BoundaryCommSubset
...
void FaceCenteredBoundaryVariable::StartReceiving(BoundaryCommSubset phase) {
  ...
  for (int n = 0; n < pbval_->nneighbor; n++) {
    NeighborBlock &nb = pbval_->neighbor[n];
    if (nb.snb.rank != Globals::my_rank &&
        phase != BoundaryCommSubset::gr_amr) {
  ...
  if (pbval_->shearing_box == 1) {
    ...
    if (phase == BoundaryCommSubset::all) {
      for (int upper = 0; upper < 2; upper++) {
        if (pbval_->is_shear[upper]) {
          ...
          for (int n = 0; n < 3; n++) {
            if (counts1[n] > 0) {
              shear_send_count_emf_[upper][n] =
                  counts1[n] * (nx3 + 1) + (counts1[n] + 1) * nx3;
            ...
            if (counts2[n] > 0) {
              shear_recv_count_emf_[upper][n] =
                  counts2[n] * (nx3 + 1) + (counts2[n] + 1) * nx3;
              shear_bd_flux_[upper].flag[n] = BoundaryStatus::waiting;
            } else {
    ...
    for (int upper = 0; upper < 2; upper++) {
      if (pbval_->is_shear[upper]) {
        ...
        for (int n = 0; n < 4; n++) {
          if (counts1[n] > 0) {
            shear_send_count_fc_[upper][n] =
                NGHOST * (counts1[n] * (nc3 + 1) + (counts1[n] + 1) * nc3);
          ...
          if (counts2[n] > 0) {
            shear_recv_count_fc_[upper][n] =
                NGHOST * (counts2[n] * (nc3 + 1) + (counts2[n] + 1) * nc3);
            shear_bd_var_[upper].flag[n] = BoundaryStatus::waiting;
          } else {
...
//----------------------------------------------------------------------------------------
//! \fn void FaceCenteredBoundaryVariable::ClearBoundary(BoundaryCommSubset
