...
//----------------------------------------------------------------------------------------
//! \fn int FaceCenteredBoundaryVariable::LoadFluxBoundaryBufferSameLevel(Real
//! *buf,
...
int FaceCenteredBoundaryVariable::LoadFluxBoundaryBufferSameLevel(
  ...
  if (nb.ni.type == NeighborConnect::face) {
          ...
          if (porb->orbital_advection_defined) {
            ...
            if (nb.fid == BoundaryFace::inner_x1) {
              for (int k = pmb->ks; k <= pmb->ke; k++) {
                for (int j = pmb->js; j <= pmb->je + 1; j++)
                  buf[p++] = e3(k, j, i) -
                             0.5 * qomL * (bx1(k, j, i) + bx1(k, j - 1, i));
              }
    ...
    if (nb.eid >= 0 && nb.eid < 4) {
      ...
      // KGF: shearing box
      // shift azmuthal velocity if shearing boundary blocks
      if (pbval_->shearing_box == 1 && nb.shear && nb.ni.ox1 != 0 &&
          pmb->pmy_mesh->sts_loc == TaskType::main_int) {
        if (porb->orbital_advection_defined) {
          for (int k = pmb->ks; k <= pmb->ke; k++)
            buf[p++] = e3(k, j, i);
          ...
          if (nb.ni.ox1 == -1) {
            for (int k = pmb->ks; k <= pmb->ke; k++)
              buf[p++] =
                  e3(k, j, i) - 0.5 * qomL * (bx1(k, j, i) + bx1(k, j - 1, i));
          } else if (nb.ni.ox1 == 1) {
            for (int k = pmb->ks; k <= pmb->ke; k++)
              buf[p++] =
                  e3(k, j, i) + 0.5 * qomL * (bx1(k, j, i) + bx1(k, j - 1, i));
      ...
      // pack e2
      if (pbval_->shearing_box == 2 && nb.shear && nb.ni.ox1 != 0 &&
          pmb->pmy_mesh->sts_loc == TaskType::main_int) {
        if (nb.ni.ox1 == -1) {
          for (int j = pmb->js; j <= pmb->je; j++)
            buf[p++] = e2(k, j, i) + qomL * bx1(k, j, i);
...
//----------------------------------------------------------------------------------------
//! \fn int FaceCenteredBoundaryVariable::LoadFluxBoundaryBufferToCoarser(Real
//! *buf,
//!                                                        const NeighborBlock&
//!                                                        nb)
//! \brief Set EMF correction buffers for sending to a block on the coarser
//! level
...
//----------------------------------------------------------------------------------------
//! \fn int FaceCenteredBoundaryVariable::LoadFluxBoundaryBufferToPolar(Real
//! *buf,
//!                                                        const
//!                                                        SimpleNeighborBlock
//!                                                        &nb,
...
int FaceCenteredBoundaryVariable::LoadFluxBoundaryBufferToPolar(
  ...
  for (int i = pmb->is; i <= pmb->ie; ++i) {
    Real val = 0.0;
    for (int k = pmb->ks; k <= pmb->ke;
         ++k) { // avoid double counting right ends
...
void FaceCenteredBoundaryVariable::CopyPolarBufferSameProcess(
    const SimpleNeighborBlock &snb, int ssize, int polar_block_index,
    bool is_north) {
  ...
  if (is_north) {
    ...
    BoundaryStatus &target_flag =
        ptarget_pfbval->flux_north_flag_[pmy_block_->loc.lx3];
    ...
    BoundaryStatus &target_flag =
        ptarget_pfbval->flux_south_flag_[pmy_block_->loc.lx3];
    target_flag = BoundaryStatus::arrived;
  }
...
void FaceCenteredBoundaryVariable::SendFluxCorrection() {
  ...
  for (int n = 0; n < pbval_->nneighbor; n++) {
    ...
    if ((nb.ni.type != NeighborConnect::face) &&
      ...
      break;
    if (bd_var_flcor_.sflag[nb.bufid] == BoundaryStatus::completed)
      continue;
    int p = 0;
...
void FaceCenteredBoundaryVariable::SetFluxBoundarySameLevel(
    Real *buf, const NeighborBlock &nb) {
  MeshBlock *pmb = pmy_block_;
...
void FaceCenteredBoundaryVariable::SetFluxBoundaryFromFiner(
    Real *buf, const NeighborBlock &nb) {
  MeshBlock *pmb = pmy_block_;
  AthenaArray<Real> &e1 = pmb->pfield->e.x1e;
  AthenaArray<Real> &e2 = pmb->pfield->e.x2e;
...
//----------------------------------------------------------------------------------------
//! \fn void FaceCenteredBoundaryVariable::SetFluxBoundaryFromPolar(Real
//! **buf_list,
//!                                                           int num_bufs, bool
//!                                                           is_north)
//! \brief Overwrite EMF values along polar axis with azimuthal averages

void FaceCenteredBoundaryVariable::SetFluxBoundaryFromPolar(Real **buf_list,
                                                            int num_bufs,
                                                            bool is_north) {
  MeshBlock *pmb = pmy_block_;
...
void FaceCenteredBoundaryVariable::AverageFluxBoundary() {
  ...
  for (int n = 0; n < pbval_->nface_; n++) {
    if ((pbval_->block_bcs[n] != BoundaryFlag::block) &&
        ...
        (pbval_->block_bcs[n] != BoundaryFlag::polar))
      continue;
    if (n == BoundaryFace::inner_x1 || n == BoundaryFace::outer_x1) {
      Real div = 0.5;
      int i;
      ...
      if (nl == pmb->loc.level) {      // same ; divide all the face EMFs by 2
        if (pmb->block_size.nx3 > 1) { // 3D
          ...
          e3(pmb->ks, pmb->js, i) *= 0.5, e3(pmb->ks, pmb->js + 1, i) *= 0.5;
                 ...
                 pmb->loc.level) { // finer; divide the overlapping EMFs by 2
        if (pmb->block_size.nx3 > 1) { // 3D
...
//----------------------------------------------------------------------------------------
//! \fn void
//! FaceCenteredBoundaryVariable::PolarFluxBoundarySingleAzimuthalBlock() \brief
//! polar boundary edge-case:
//!
...
void FaceCenteredBoundaryVariable::PolarFluxBoundarySingleAzimuthalBlock() {
  ...
  if (pmb->loc.level == pmb->pmy_mesh->root_level &&
      pmb->pmy_mesh->nrbx3 == 1 && pmb->block_size.nx3 > 1) {
    AthenaArray<Real> &e1 = pmb->pfield->e.x1e;
    AthenaArray<Real> &e3 = pmb->pfield->e.x3e;
    if (pbval_->block_bcs[BoundaryFace::inner_x2] == BoundaryFlag::polar ||
        pbval_->block_bcs[BoundaryFace::inner_x2] ==
    ...
    if (pbval_->block_bcs[BoundaryFace::outer_x2] == BoundaryFlag::polar ||
        pbval_->block_bcs[BoundaryFace::outer_x2] ==
            BoundaryFlag::polar_wedge) {
      int j = pmb->je + 1;
      int nx3_half = (pmb->ke - pmb->ks + 1) / 2;
      for (int i = pmb->is; i <= pmb->ie; i++) {
...
bool FaceCenteredBoundaryVariable::ReceiveFluxCorrection() {
  ...
  if (recv_flx_same_lvl_) {
    for (int n = 0; n < pbval_->nneighbor;
         n++) { // first correct the same level
      NeighborBlock &nb = pbval_->neighbor[n];
      if (nb.ni.type != NeighborConnect::face &&
          nb.ni.type != NeighborConnect::edge)
        break;
      ...
      if ((nb.ni.type == NeighborConnect::face) ||
          ((nb.ni.type == NeighborConnect::edge) && (edge_flag_[nb.eid]))) {
        if (bd_var_flcor_.flag[nb.bufid] == BoundaryStatus::completed)
          continue;
        if (bd_var_flcor_.flag[nb.bufid] == BoundaryStatus::waiting) {
          if (nb.snb.rank == Globals::my_rank) { // on the same process
            ...
            int test;
            // probe MPI communications.  This is a bit of black magic that
            // seems to promote communications to top of stack and gets them to
            // complete more quickly
            MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &test,
                       MPI_STATUS_IGNORE);
            MPI_Test(&(bd_var_flcor_.req_recv[nb.bufid]), &test,
                     MPI_STATUS_IGNORE);
    ...
    if (!flag)
      return flag; // is this flag always false?
  ...
  if (flag) {
    ...
    if (pbval_->num_north_polar_blocks_ > 0)
      SetFluxBoundaryFromPolar(flux_north_recv_,
                               pbval_->num_north_polar_blocks_, true);
    for (int n = 0; n < pbval_->num_north_polar_blocks_; ++n)
      flux_north_flag_[n] = BoundaryStatus::completed;
    if (pbval_->num_south_polar_blocks_ > 0)
      SetFluxBoundaryFromPolar(flux_south_recv_,
                               pbval_->num_south_polar_blocks_, false);
    for (int n = 0; n < pbval_->num_south_polar_blocks_; ++n)
