...
//!              Real time, Real dt, int il, int jl, int ju, int kl, int ku, int
//!              ngh)
//! \brief REFLECTING boundary conditions, inner x1 boundary

void FaceCenteredBoundaryVariable::ReflectInnerX1(Real time, Real dt, int il,
...
#pragma omp simd
      for (int i = 1; i <= ngh; ++i) {
        (*var_fc).x1f(k, j, (il - i)) =
            -(*var_fc).x1f(k, j, (il + i)); // reflect 1-field
      }
    }
  }
...
//!              Real time, Real dt, int iu, int jl, int ju, int kl, int ku, int
//!              ngh)
//! \brief REFLECTING boundary conditions, outer x1 boundary

void FaceCenteredBoundaryVariable::ReflectOuterX1(Real time, Real dt, int iu,
                                                  int jl, int ju, int kl,
                                                  int ku, int ngh) {
  // copy face-centered magnetic fields into ghost zones, reflecting b1
  for (int k = kl; k <= ku; ++k) {
...
#pragma omp simd
      for (int i = 1; i <= ngh; ++i) {
        (*var_fc).x1f(k, j, (iu + i + 1)) =
            -(*var_fc).x1f(k, j, (iu - i + 1)); // reflect 1-field
      }
    }
  }
...
//!              Real time, Real dt, int il, int iu, int jl, int kl, int ku, int
//!              ngh)
//! \brief REFLECTING boundary conditions, inner x2 boundary

void FaceCenteredBoundaryVariable::ReflectInnerX2(Real time, Real dt, int il,
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        (*var_fc).x2f(k, (jl - j), i) =
            -(*var_fc).x2f(k, (jl + j), i); // reflect 2-field
      }
    }
  }
  for (int k = kl; k <= ku + 1; ++k) {
...
//!              Real time, Real dt, int il, int iu, int ju, int kl, int ku, int
//!              ngh)
//! \brief REFLECTING boundary conditions, outer x2 boundary

void FaceCenteredBoundaryVariable::ReflectOuterX2(Real time, Real dt, int il,
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        (*var_fc).x2f(k, (ju + j + 1), i) =
            -(*var_fc).x2f(k, (ju - j + 1), i); // reflect 2-field
      }
    }
  }
  for (int k = kl; k <= ku + 1; ++k) {
...
//!              Real time, Real dt, int il, int iu, int jl, int ju, int kl, int
//!              ngh)
//! \brief REFLECTING boundary conditions, inner x3 boundary

void FaceCenteredBoundaryVariable::ReflectInnerX3(Real time, Real dt, int il,
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        (*var_fc).x3f((kl - k), j, i) =
            -(*var_fc).x3f((kl + k), j, i); // reflect 3-field
      }
    }
  }
  return;
...
//!              Real time, Real dt, int il, int iu, int jl, int ju, int ku, int
//!              ngh)
//! \brief REFLECTING boundary conditions, outer x3 boundary

void FaceCenteredBoundaryVariable::ReflectOuterX3(Real time, Real dt, int il,
                                                  int iu, int jl, int ju,
                                                  int ku, int ngh) {
  // copy face-centered magnetic fields into ghost zones, reflecting b3
  for (int k = 1; k <= ngh; ++k) {
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        (*var_fc).x3f((ku + k + 1), j, i) =
            -(*var_fc).x3f((ku - k + 1), j, i); // reflect 3-field
      }
    }
  }
  return;
