...
//! \brief functions that apply BCs for face-centered flux corrections in
...
//----------------------------------------------------------------------------------------
//! \fn void
//! FaceCenteredBoundaryVariable::SendEMFShearingBoxBoundaryCorrection() \brief
//! Send shearing box boundary buffers for EMF correction
    ...
    if (pbval_->is_shear[upper]) {
      ...
      if (pmb->block_size.nx3 > 1) {
        ...
        if (pbval_->block_bcs[BoundaryFace::inner_x3] == BoundaryFlag::block ||
            pbval_->block_bcs[BoundaryFace::inner_x3] ==
        ...
        if (pbval_->block_bcs[BoundaryFace::outer_x3] == BoundaryFlag::block ||
            pbval_->block_bcs[BoundaryFace::outer_x3] ==
                BoundaryFlag::periodic) {
          for (int j = pmb->js; j <= pmb->je; j++)
      ...
      for (int n = 0; n < 3; n++) {
        SimpleNeighborBlock &snb =
            pbval_->sb_flux_data_[upper].send_neighbor[n];
        if (snb.rank != -1) {
          LoadEMFShearingBoxBoundarySameLevel(shear_var_emf_[upper],
                                              shear_bd_flux_[upper].send[n],
                                              n + offset[upper]);
          if (snb.rank == Globals::my_rank) {
            CopyShearFluxSameProcess(snb, shear_send_count_emf_[upper][n], n,
                                     upper);
          } else { // MPI
#ifdef MPI_PARALLEL
            ...
            MPI_Isend(shear_bd_flux_[upper].send[n],
                      shear_send_count_emf_[upper][n], MPI_ATHENA_REAL,
                      snb.rank, tag, MPI_COMM_WORLD,
...
//----------------------------------------------------------------------------------------
//! \fn bool
//! FaceCenteredBoundaryVariable::ReceiveEMFShearingBoxBoundaryCorrection()
...
bool FaceCenteredBoundaryVariable::ReceiveEMFShearingBoxBoundaryCorrection() {
  ...
  for (int upper = 0; upper < 2; upper++) {
    if (pbval_->is_shear[upper]) {
      for (int n = 0; n < 3; n++) {
        if (shear_bd_flux_[upper].flag[n] == BoundaryStatus::completed)
          continue;
        if (shear_bd_flux_[upper].flag[n] == BoundaryStatus::waiting) {
          if (pbval_->sb_flux_data_[upper].recv_neighbor[n].rank ==
              Globals::my_rank) {
            flag[upper] = false;
            continue;
          } else { // MPI boundary
            ...
            MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &test,
                       MPI_STATUS_IGNORE);
            MPI_Test(&shear_bd_flux_[upper].req_recv[n], &test,
                     MPI_STATUS_IGNORE);
        ...
        SetEMFShearingBoxBoundarySameLevel(shear_map_emf_[upper],
  ...
  for (int upper = 0; upper < 2; upper++) {
    if (pbval_->is_shear[upper]) {
      ...
      for (int k = ks; k <= ke + 1; k++) {
        ...
        for (int j = js; j <= je; j++) {
          e2(k, j, ii) = 0.5 * (e2(k, j, ii) + pe2(k, 0, j + shift) -
                                (pflux(j + 1) - pflux(j)));
        }
      }
      // ex3
      ...
      for (int k = ks; k <= ke; k++) {
        ...
        for (int j = js; j <= je + 1; j++) {
          e3(k, j, ii) = 0.5 * (e3(k, j, ii) + pe3(k, 0, j + shift) -
                                (pflux(j + 1) - pflux(j)));
        }
      }
