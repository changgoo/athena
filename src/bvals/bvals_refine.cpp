...
//!
//! SWITCHING BETWEEN PRIMITIVE VS. CONSERVED AND STANDARD VS. COARSE BUFFERS
//! HERE:
//! -----------
//!
...
//! BoundaryVariable objects with member pointers pointing to their CONSERVED
//! VARIABLE ARRAYS (standard and coarse buffers) by the time this function is
//! called.
//!
//! E.g. in time_integrator.cpp, the PROLONG task is called after SEND_HYD,
//! SETB_HYD, SEND_SCLR, SETB_SCLR, all of which indepedently switch to their
//! associated CONSERVED VARIABLE ARRAYS and before CON2PRIM which switches to
...
//! MeshRefinement::pvars_cc_, pvars_fc_ vectors, NOT the var_cc, coarse_buf ptr
//! members of CellCenteredBoundaryVariable objects, e.g. And the first step in
//! this function, RestrictGhostCellsOnSameLevel, by default operates on the
//! S/AMR-enrolled: (u, coarse_cons) for Hydro and (s, coarse_s) for
//! PassiveScalars (also on (w, coarse_prim) for Hydro if GR):
//!
//! __________
...
//!   - Used in ApplyPhysicalBoundariesOnCoarseLevel() and
//!   ProlongateGhostCells() where
...
//!     (also temporarily change to standard and coarse PRIMITIVE for GR
//!     simulations)
//! 2. ApplyPhysicalBoundariesOnCoarseLevel
//! (CellCenteredBoundaryVariable::var_cc)
//!   - ONLY var_cc (var_fc) is changed to = coarse_buf, PRIMITIVE
//!     (automatically switches var_cc to standard and coarse_buf to coarse
//!     primitive arrays after fn returns)
//!
...
//----------------------------------------------------------------------------------------
//! \fn void BoundaryValues::ProlongateBoundaries(const Real time, const Real
//! dt,
...
//! - This hardcoded technique is also used to manually specify the coupling
//! between
...
//!   RestrictGhostCellsOnSameLevel() (GR) and ProlongateGhostCells() (always)
//!   to switch between conserved and primitive tuples in
//!   MeshRefinement::pvars_cc_, but this does not require ph, pf due to
//!   MeshRefinement::SetHydroRefinement(hydro_type)
...
void BoundaryValues::ProlongateBoundaries(
  ...
  // TODO(KGF): temporarily hardcode Hydro and Field array access for the below
  // switch around ApplyPhysicalBoundariesOnCoarseLevel()

  // This hardcoded technique is also used to manually specify the coupling
  // between physical variables in:
  // - step 2, ApplyPhysicalBoundariesOnCoarseLevel(): calls to W(U) and user
  // BoundaryFunc
  // - step 3, ProlongateGhostCells(): calls to calculate bcc and U(W)

  // Additionally, pmr->SetHydroRefinement() is currently used in
  // RestrictGhostCellsOnSameLevel() (GR) and ProlongateGhostCells() (always) to
  // switch between conserved and primitive tuples in MeshRefinement::pvars_cc_,
  // but this does not require ph, pf due to
  // MeshRefinement::SetHydroRefinement(hydro_type)
  ...
  if (NSCALARS > 0) {
    ...
    // psbvar = dynamic_cast<CellCenteredBoundaryVariable
    // *>(bvars_main_int[???]);
  }
  ...
  // For each finer neighbor, to prolongate a boundary we need to fill one more
  // cell surrounding the boundary zone to calculate the slopes ("ghost-ghost
  // zone"). 3x steps:
  for (int n = 0; n < nneighbor; n++) {
    ...
    // Step 1. Apply necessary variable restrictions when ghost-ghost zone is on
    // same lvl
    for (int nk = nks; nk <= nke; nk++) {
      for (int nj = njs; nj <= nje; nj++) {
        for (int ni = nis; ni <= nie; ni++) {
          int ntype = std::abs(ni) + std::abs(nj) + std::abs(nk);
          // skip myself or coarse levels; only the same level must be
          // restricted
          if (ntype == 0 || nblevel[nk + 1][nj + 1][ni + 1] != mylevel)
            continue;
    ...
    // (temp workaround) to automatically call all BoundaryFunction_[] on
    // coarse_prim/b instead of previous targets var_cc=cons, var_fc=b
    ...
    // (temp workaround) swap BoundaryVariable var_cc/fc to standard primitive
    // variable arrays (not coarse) from coarse primitive variables arrays
    phbvar->var_cc = &(ph->w);
    if (MAGNETIC_FIELDS_ENABLED)
      pfbvar->var_fc = &(pf->b);
    if (NSCALARS > 0) {
...
//----------------------------------------------------------------------------------------
//! \fn void BoundaryValues::RestrictGhostCellsOnSameLevel(const NeighborBlock&
//! nb,
//!                                                        int nk, int nj, int
//!                                                        ni)
...
void BoundaryValues::RestrictGhostCellsOnSameLevel(const NeighborBlock &nb,
  ...
  for (auto cc_pair : pmr->pvars_cc_) {
    ...
    pmb->pmr->RestrictCellCenteredValues(*var_cc, *coarse_cc, 0, nu, ris, rie,
                                         rjs, rje, rks, rke);
  }
  ...
  for (auto fc_pair : pmr->pvars_fc_) {
    ...
    int &mylevel = loc.level;
    int rs = ris, re = rie + 1;
    if (rs == pmb->cis && nblevel[nk + 1][nj + 1][ni] < mylevel)
      rs++;
    if (re == pmb->cie + 1 && nblevel[nk + 1][nj + 1][ni + 2] < mylevel)
      re--;
    pmr->RestrictFieldX1((*var_fc).x1f, (*coarse_fc).x1f, rs, re, rjs, rje, rks,
                         rke);
    if (pmb->block_size.nx2 > 1) {
      rs = rjs, re = rje + 1;
      if (rs == pmb->cjs && nblevel[nk + 1][nj][ni + 1] < mylevel)
        rs++;
      if (re == pmb->cje + 1 && nblevel[nk + 1][nj + 2][ni + 1] < mylevel)
        re--;
      pmr->RestrictFieldX2((*var_fc).x2f, (*coarse_fc).x2f, ris, rie, rs, re,
                           rks, rke);
    } else { // 1D
      pmr->RestrictFieldX2((*var_fc).x2f, (*coarse_fc).x2f, ris, rie, rjs, rje,
                           rks, rke);
    ...
    if (pmb->block_size.nx3 > 1) {
      rs = rks, re = rke + 1;
      if (rs == pmb->cks && nblevel[nk][nj + 1][ni + 1] < mylevel)
        rs++;
      if (re == pmb->cke + 1 && nblevel[nk + 2][nj + 1][ni + 1] < mylevel)
        re--;
      pmr->RestrictFieldX3((*var_fc).x3f, (*coarse_fc).x3f, ris, rie, rjs, rje,
                           rs, re);
    } else { // 1D or 2D
      pmr->RestrictFieldX3((*var_fc).x3f, (*coarse_fc).x3f, ris, rie, rjs, rje,
                           rks, rke);
...
void BoundaryValues::ApplyPhysicalBoundariesOnCoarseLevel(
    const NeighborBlock &nb, const Real time, const Real dt, int si, int ei,
  ...
  //!   no longer members of MeshRefinement that always exist (even if not
  //!   allocated).

  // KGF: COUPLING OF QUANTITIES (must be manually specified)
  pmb->peos->ConservedToPrimitive(
      ph->coarse_cons_, ph->coarse_prim_, pf->coarse_b_, ph->coarse_prim_,
      pf->coarse_bcc_, pmr->pcoarsec, si - f1m, ei + f1p, sj - f2m, ej + f2p,
      sk - f3m, ek + f3p);
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
    pmb->peos->PassiveScalarConservedToPrimitive(
        ps->coarse_s_, ph->coarse_cons_, ps->coarse_r_, ps->coarse_r_,
        pmr->pcoarsec, si - f1m, ei + f1p, sj - f2m, ej + f2p, sk - f3m,
        ek + f3p);
  }

  if (nb.ni.ox1 == 0) {
    if (apply_bndry_fn_[BoundaryFace::inner_x1]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt, pmb->cis,
                                pmb->cie, sj, ej, sk, ek, 1, ph->coarse_prim_,
                                pf->coarse_b_, pf->coarse_bcc_, pcr->coarse_cr_,
  ...
  if (nb.ni.ox2 == 0 && pmb->block_size.nx2 > 1) {
    if (apply_bndry_fn_[BoundaryFace::inner_x2]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt, si, ei, pmb->cjs,
                                pmb->cje, sk, ek, 1, ph->coarse_prim_,
                                pf->coarse_b_, pf->coarse_bcc_, pcr->coarse_cr_,
  ...
  if (nb.ni.ox3 == 0 && pmb->block_size.nx3 > 1) {
    if (apply_bndry_fn_[BoundaryFace::inner_x3]) {
      DispatchBoundaryFunctions(pmb, pmr->pcoarsec, time, dt, si, ei, sj, ej,
                                pmb->cks, pmb->cke, 1, ph->coarse_prim_,
                                pf->coarse_b_, pf->coarse_bcc_, pcr->coarse_cr_,
...
//!                                               int si, int ei, int sj, int
//!                                               ej, int sk, int ek)
//! \brief Prolongate ghost cells

void BoundaryValues::ProlongateGhostCells(const NeighborBlock &nb, int si,
                                          int ei, int sj, int ej, int sk,
                                          int ek) {
  MeshBlock *pmb = pmy_block_;
  MeshRefinement *pmr = pmb->pmr;

  // prolongate cell-centered S/AMR-enrolled quantities (hydro, radiation,
  // scalars, ...)
  ...
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
    pmr->pvars_cc_[ps->refinement_idx] =
        std::make_tuple(&ps->r, &ps->coarse_r_);
  }
  ...
  for (auto cc_pair : pmr->pvars_cc_) {
    ...
    AthenaArray<Real> *coarse_cc = std::get<1>(cc_pair);
    int nu = var_cc->GetDim4() - 1;
    pmr->ProlongateCellCenteredValues(*coarse_cc, *var_cc, 0, nu, si, ei, sj,
                                      ej, sk, ek);
  }
  ...
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
    pmr->pvars_cc_[ps->refinement_idx] =
        std::make_tuple(&ps->s, &ps->coarse_s_);
  }
  ...
  il = si, iu = ei + 1;
  if ((nb.ni.ox1 >= 0) &&
      (nblevel[nb.ni.ox3 + 1][nb.ni.ox2 + 1][nb.ni.ox1] >= mylevel))
    il++;
  if ((nb.ni.ox1 <= 0) &&
  ...
  if (pmb->block_size.nx2 > 1) {
    jl = sj, ju = ej + 1;
    if ((nb.ni.ox2 >= 0) &&
        (nblevel[nb.ni.ox3 + 1][nb.ni.ox2][nb.ni.ox1 + 1] >= mylevel))
      jl++;
    if ((nb.ni.ox2 <= 0) &&
  ...
  for (auto fc_pair : pmr->pvars_fc_) {
    ...
    pmr->ProlongateSharedFieldX1((*coarse_fc).x1f, (*var_fc).x1f, il, iu, sj,
                                 ej, sk, ek);
    // step 2. calculate x2 outer surface fields and slopes
    pmr->ProlongateSharedFieldX2((*coarse_fc).x2f, (*var_fc).x2f, si, ei, jl,
                                 ju, sk, ek);
    // step 3. calculate x3 outer surface fields and slopes
    pmr->ProlongateSharedFieldX3((*coarse_fc).x3f, (*var_fc).x3f, si, ei, sj,
                                 ej, kl, ku);
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    pf = pmb->pfield;
    pf->CalculateCellCenteredField(pf->b, pf->bcc, pmb->pcoord, fsi, fei, fsj,
                                   fej, fsk, fek);
  }
  //! \note (felker):
  ...
  pmb->peos->PrimitiveToConserved(ph->w, pf->bcc, ph->u, pmb->pcoord, fsi, fei,
                                  fsj, fej, fsk, fek);
  if (NSCALARS > 0) {
    PassiveScalars *ps = pmb->pscalars;
    pmb->peos->PassiveScalarPrimitiveToConserved(
        ps->r, ph->w, ps->s, pmb->pcoord, fsi, fei, fsj, fej, fsk, fek);
  }
  return;
}
