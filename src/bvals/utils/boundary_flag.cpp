...
//! boundary condition. Typically called in Mesh() ctor and in pgen/*.cpp files.
...
//! \brief Parses enumerated type BoundaryFlag internal integer representation
//! to return string describing the boundary condition. Typically used to format
//! descriptive errors or diagnostics. Inverse of GetBoundaryFlag().
...
//!
//! Mesh() ctor only
//! checks the validity of user's input mesh/ixn_bc, oxn_bc string values
//! corresponding to a BoundaryFlag enumerator before passing it to a MeshBlock
//! and then BoundaryBase object. However, not all BoundaryFlag enumerators can
//! be used in all directions as a valid MeshBlock boundary.
...
void CheckBoundaryFlag(BoundaryFlag block_flag, CoordinateDirection dir) {
  ...
  msg << "### FATAL ERROR in CheckBoundaryFlag" << std::endl
      ...
      << GetBoundaryString(block_flag) << "\nin x" << dir + 1 << " direction"
      << std::endl;
  switch (dir) {
