...
//! \brief check compatibilty of user-selected 'polar' and/or 'polar_wedge'
//! boundary flags
...
//! \brief Check for any compatibility issues if polar-type boundary flags are
//! selected.
//!
//! Called after setting 6x boundary functions in MeshBlock's BoundaryValues()
//! constructor

void BoundaryValues::CheckPolarBoundaries() {
  // Check that spherical-like coordinates were specified at compile time (this
  // check must be manually updated when a new coordinate system is added to
  // coordinates/)
  ...
  if ((block_bcs[BoundaryFace::inner_x2] == BoundaryFlag::polar_wedge ||
    ...
    // PI, TWO_PI macro constants are specified to 16 digits after the decimal
    // pt.
    std::stringstream msg;
    msg << "### FATAL ERROR in BoundaryValues constructor" << std::endl
  ...
  // 3D spherical-like coordinates with at least one 'polar' or 'polar_wedge' x2
  // boundary
  if (pmy_block_->block_size.nx3 > 1) {
    // 'polar' vs. 'polar_wedge' selection in 3D: use the latter if and only if
    // the Mesh doesn't span exactly x3min=0.0,x3max=2*pi for lower,upper
    ...
    if (block_bcs[BoundaryFace::inner_x2] == BoundaryFlag::polar_wedge ||
      ...
      // in 3D, 'polar_wedge' only makes sense if each MeshBlock spans dx3=pi/n,
      // n=int
      Real mesh_dx3 =
          (pmy_mesh_->mesh_size.x3max - pmy_mesh_->mesh_size.x3min) /
          pmy_mesh_->nrbx3;
      // Real block_dx3 = pmy_block_->block_size.x3max -
      // pmy_block_->block_size.x3min; (accumulation of floating-point round off
      // makes it too dificult to impose this condition individually on each
      // MeshBlock's specific x3 limits)
      Real block_dx3 = mesh_dx3;
      // std::fmod() returns PI/dx3 with truncated fractional part, not actual
      // remainder
      Real remainder = std::abs(std::remainder(PI, block_dx3));
      // scale the machine precision by the # of azimuthal blocks due to
      // accumulation of error from multiple subtraction operations to compute
      // block_dx3
      Real fp_tol = std::numeric_limits<Real>::epsilon() * pmy_mesh_->nrbx3;
      if (remainder > fp_tol ||
          pmy_mesh_->mesh_size.x3rat !=
              1.0) { // nonuniform azimuth spacing forbidden
        std::stringstream msg;
        msg << "### FATAL ERROR in BoundaryValues constructor" << std::endl
            << "3D spherical-like coordinates with current x3 domain limits:\n"
            << std::setprecision(std::numeric_limits<Real>::max_digits10 - 1)
            << "x3min=" << std::scientific << pmy_mesh_->mesh_size.x3min << "\n"
            << "x3max=" << pmy_mesh_->mesh_size.x3max << "\n"
            << "Use 'polar_wedge' boundary flag(s) if and only if each "
               "MeshBlock\n"
            << "spans dx3=PI/n, for some integer n. Current MeshBlock "
               "azimuthal length:\n"
            << "dx3=" << block_dx3 << std::endl
            << "divides PI an int number of times, with |remainder|="
            << remainder << "\n"
            << "which exceeds floating-point rounding tolerance eps*nrbx3="
            << fp_tol << std::endl;
        ATHENA_ERROR(msg);
      }
    }
    // Azimuthal x3 boundaries (of Mesh) must both be periodic if 'polar' or
    // 'polar_wedge' flags are used even once
    ...
    // even number of cells in the azimuthal direction for the root grid of the
    // Mesh
    ...
    if (block_bcs[BoundaryFace::inner_x2] == BoundaryFlag::polar ||
      ...
      std::stringstream msg;
      msg << "### FATAL ERROR in BoundaryValues constructor" << std::endl
          << "Use 'polar_wedge', not 'polar' boundary flag for 2D "
             "spherical-like \n"
