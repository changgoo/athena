...
struct LogicalLocation { // aggregate and POD type
  // These values can exceed the range of std::int32_t even if the root grid has
  // only a single MeshBlock if >30 levels of AMR are used, since the
  // corresponding max index = 1*2^31 > INT_MAX = 2^31 -1 for most 32-bit signed
  // integer type impelementations
  std::int64_t lx1, lx2, lx3;
  int level;
  // comparison functions for sorting
  static bool Lesser(const LogicalLocation &left,
...
//! prototype for overloading the comparison operator (defined in
//! meshblock_tree.cpp)
...
struct RegionSize { // aggregate and POD type; do NOT reorder member
                    // declarations:
  ...
  RegionSize(Real x1min, Real x2min, Real x3min, Real x1max, Real x2max,
             Real x3max, Real x1rat, Real x2rat, Real x3rat, int nx1, int nx2,
             int nx3)
      : x1min(x1min), x2min(x2min), x3min(x3min), x1max(x1max), x2max(x2max),
        x3max(x3max), x1len(x1max - x1min), x2len(x2max - x2min),
        x3len(x3max - x3min), x1rat(x1rat), x2rat(x2rat), x3rat(x3rat),
...
//!   athena_fft.hpp, io_wrapper.hpp, bvals.hpp, hydro_diffusion.hpp,
//!   field_diffusion.hpp, task_list.hpp, ???
...
// and explicitly specified enumerator values aare unnecessary, but provided for
// clarity:
...
//------------------
// KGF: Except for the 2x MG* enums, these may be unnessary w/ the new class
// inheritance Now, only passed to BoundaryVariable::InitBoundaryData(); could
// replace w/ bool switch
...
using BValFunc = void (*)(MeshBlock *pmb, Coordinates *pco,
                          AthenaArray<Real> &prim, FaceField &b, Real time,
                          Real dt, int is, int ie, int js, int je, int ks,
                          int ke, int ngh);
...
using MGBoundaryFunc = void (*)(AthenaArray<Real> &dst, Real time, int nvar,
                                int is, int ie, int js, int je, int ks, int ke,
                                int ngh, Real x0, Real y0, Real z0, Real dx,
                                Real dy, Real dz);
using ViscosityCoeffFunc = void (*)(HydroDiffusion *phdif, MeshBlock *pmb,
                                    const AthenaArray<Real> &w,
                                    const AthenaArray<Real> &bc, int is, int ie,
                                    int js, int je, int ks, int ke);
using ConductionCoeffFunc = void (*)(HydroDiffusion *phdif, MeshBlock *pmb,
                                     const AthenaArray<Real> &w,
                                     const AthenaArray<Real> &bc, int is,
                                     int ie, int js, int je, int ks, int ke);
using FieldDiffusionCoeffFunc = void (*)(FieldDiffusion *pfdif, MeshBlock *pmb,
                                         const AthenaArray<Real> &w,
                                         const AthenaArray<Real> &bmag, int is,
                                         int ie, int js, int je, int ks,
                                         int ke);
...
using CRBoundaryFunc = void (*)(MeshBlock *pmb, Coordinates *pco,
                                CosmicRay *pcr, const AthenaArray<Real> &w,
                                const AthenaArray<Real> &bc,
                                AthenaArray<Real> &u_cr, Real time, Real dt,
                                int is, int ie, int js, int je, int ks, int ke,
                                int ngh);
using CRSrcTermFunc = void (*)(MeshBlock *pmb, const Real time, const Real dt,
                               const AthenaArray<Real> &prim,
                               const AthenaArray<Real> &bcc,
                               AthenaArray<Real> &u_cr,
                               AthenaArray<Real> &CRInjectionRate);
using OrbitalVelocityFunc = Real (*)(OrbitalAdvection *porb, Real x1, Real x2,
                                     Real x3);
