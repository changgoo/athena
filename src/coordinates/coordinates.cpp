...
//----------------------------------------------------------------------------------------
//! Coordinates constructor: sets coordinates and coordinate spacing of cell
//! FACES
...
Coordinates::Coordinates(MeshBlock *pmb, ParameterInput *pin, bool flag)
  ...
  if (coarse_flag) {
    ...
    ku = pmb->ke;
    ng = NGHOST;
    nc1 = pmy_block->ncells1, nc2 = pmy_block->ncells2,
    nc3 = pmy_block->ncells3;
  }
  ...
  // allocate arrays for volume- and face-centered geometry coefficients of
  // cells (only for spherical-polar, cylindrical, cartesian coordinates, for
  // now)
  if (!GENERAL_RELATIVITY) { // exclude: minkowski, gr_user, schwarzschild,
                             // kerr-schild
  ...
  //--- X1-DIRECTION: initialize coordinates and spacing of cell FACES
  //(x1f,dx1f)
  ...
  if (!(pm->use_uniform_meshgen_fn_[X1DIR])) {
    for (int i = il - ng; i <= iu + ng + 1; ++i) {
      ...
      if (!coarse_flag) {
        ...
        noffset =
            static_cast<std::int64_t>((i - il) * 2 + lx1 * block_size.nx1);
      }
    ...
    if (!coarse_flag) {
      ...
      if (rmax > 1.1 || rmin < 1.0 / 1.1) {
        std::cout << "### Warning in Coordinates constructor" << std::endl
                  << "Neighboring cell sizes differ by more than 10% in the x1 "
                     "direction."
                  << std::endl;
      }
    }
  ...
  // correct cell face coordinates in ghost zones for reflecting boundary
  // condition
  ...
  if (pmy_block->pbval->block_bcs[BoundaryFace::outer_x1] ==
      BoundaryFlag::reflect) {
    for (int i = 1; i <= ng; ++i) {
  ...
  //--- X2-DIRECTION: initialize coordinates and spacing of cell FACES
  //(x2f,dx2f)
  ...
  if (nc2 > 1) {
    ...
    if (!(pm->use_uniform_meshgen_fn_[X2DIR])) {
      for (int j = jl - ng; j <= ju + ng + 1; ++j) {
        ...
        if (!coarse_flag) {
          ...
          noffset =
              static_cast<std::int64_t>((j - jl) * 2 + lx2 * block_size.nx2);
        }
        Real rx = ComputeMeshGeneratorX(noffset, nrootmesh, false);
      ...
      if (!coarse_flag) {
        ...
        if (rmax > 1.1 || rmin < 1.0 / 1.1) {
          std::cout << "### Warning in Coordinates constructor" << std::endl
                    << "Neighboring cell sizes differ by more than 10% in the "
                       "x2 direction."
                    << std::endl;
        }
      }
    ...
    // correct cell face coordinates in ghost zones for reflect and polar bndry
    // condition
    if (pmy_block->pbval->block_bcs[BoundaryFace::inner_x2] ==
            BoundaryFlag::reflect ||
    ...
    if (pmy_block->pbval->block_bcs[BoundaryFace::outer_x2] ==
            BoundaryFlag::reflect ||
  ...
  //--- X3-DIRECTION: initialize coordinates and spacing of cell FACES
  //(x3f,dx3f)
  ...
  if (nc3 > 1) {
    ...
    if (!(pm->use_uniform_meshgen_fn_[X3DIR])) {
      for (int k = kl - ng; k <= ku + ng + 1; ++k) {
        ...
        if (!coarse_flag) {
          ...
          noffset =
              static_cast<std::int64_t>((k - kl) * 2 + lx3 * block_size.nx3);
        }
        Real rx = ComputeMeshGeneratorX(noffset, nrootmesh, false);
    ...
    // correct cell face coordinates in ghost zones for reflecting boundary
    // condition
    if (pmy_block->pbval->block_bcs[BoundaryFace::inner_x3] ==
        BoundaryFlag::reflect) {
      for (int k = 1; k <= ng; ++k) {
    ...
    if (pmy_block->pbval->block_bcs[BoundaryFace::outer_x3] ==
        BoundaryFlag::reflect) {
      for (int k = 1; k <= ng; ++k) {
...
void Coordinates::Edge1Length(const int k, const int j, const int il,
...
void Coordinates::Edge2Length(const int k, const int j, const int il,
                              const int iu, AthenaArray<Real> &len) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    len(i) = dx2f(j);
...
void Coordinates::Edge3Length(const int k, const int j, const int il,
                              const int iu, AthenaArray<Real> &len) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    len(i) = dx3f(k);
...
// x3v(k))
void Coordinates::VolCenter1Length(const int k, const int j, const int il,
                                   const int iu, AthenaArray<Real> &len) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    len(i) = dx1v(i);
...
void Coordinates::CenterWidth1(const int k, const int j, const int il,
...
void Coordinates::CenterWidth2(const int k, const int j, const int il,
...
void Coordinates::CenterWidth3(const int k, const int j, const int il,
...
void Coordinates::Face1Area(const int k, const int j, const int il,
...
void Coordinates::Face2Area(const int k, const int j, const int il,
                            const int iu, AthenaArray<Real> &area) {
#pragma nounroll
  for (int i = il; i <= iu; ++i) {
    // area2 = dx dz
...
void Coordinates::Face3Area(const int k, const int j, const int il,
                            const int iu, AthenaArray<Real> &area) {
#pragma nounroll
  for (int i = il; i <= iu; ++i) {
    // area3 = dx dy
...
void Coordinates::CellVolume(const int k, const int j, const int il,
...
//-------------------------------------------------------------------------------------
// Laplacian: calculate total Laplacian of 4D scalar array s() to second order
// accuracy may need to replace dx*f with dx*v for nonuniform coordinates for
// some applications

void Coordinates::Laplacian(const AthenaArray<Real> &s,
                            AthenaArray<Real> &delta_s, const int il,
                            const int iu, const int jl, const int ju,
...
void Coordinates::Metric(Real x1, Real x2, Real x3, ParameterInput *pin,
  ...
  pmy_block->pmy_mesh->UserMetric_(x1, x2, x3, pin, g, g_inv, dg_dx1, dg_dx2,
                                   dg_dx3);
  return;
}

//--------------------------------------------------------------------------------------
//! \fn void Coordinates::MeshCoordsToIndices(
//          Real x1, Real x2, Real x3, Real& xi1, Real& xi2, Real& xi3) const
//  \brief returns in index coordinates (xi1, xi2, xi3) with respect to the
//  local
//         grid of MeshBlock pmb from the physical coordinates (x1, x2, x3).
// TODO(ccyang): User-defined mesh generator is not supported yet.

void Coordinates::MeshCoordsToIndices(Real x1, Real x2, Real x3, Real &xi1,
  ...
  if (pm->use_uniform_meshgen_fn_[X1DIR]) {
    ...
    xi1 = is + std::log(((x1max - rn * x1min) - (1.0 - rn) * x1) /
                        block_size.x1len) /
                   std::log(r);
  }
  ...
  if (pm->use_uniform_meshgen_fn_[X2DIR]) {
    ...
    xi2 = js + std::log(((x2max - rn * x2min) - (1.0 - rn) * x2) /
                        block_size.x2len) /
                   std::log(r);
  }
  ...
  if (pm->use_uniform_meshgen_fn_[X3DIR]) {
    ...
    xi3 = ks + std::log(((x3max - rn * x3min) - (1.0 - rn) * x3) /
                        block_size.x3len) /
                   std::log(r);
  }
}
...
//--------------------------------------------------------------------------------------
//! \fn void Coordinates::IndicesToMeshCoords(
...
void Coordinates::IndicesToMeshCoords(Real xi1, Real xi2, Real xi3, Real &x1,
...
void Coordinates::CartesianToMeshCoords(Real x, Real y, Real z, Real &x1,
                                        Real &x2, Real &x3) const {
  std::stringstream msg;
  msg << "### FATAL ERROR in function [Coordinates::CartesianToMeshCoords]"
      << std::endl
...
void Coordinates::MeshCoordsToCartesian(Real x1, Real x2, Real x3, Real &x,
                                        Real &y, Real &z) const {
  std::stringstream msg;
  msg << "### FATAL ERROR in function [Coordinates::MeshCoordsToCartesian]"
      << std::endl
      << "not implemented yet. " << std::endl;
  ATHENA_ERROR(msg);
  return;
...
void Coordinates::CartesianToMeshCoordsVector(Real x, Real y, Real z, Real vx,
                                              Real vy, Real vz, Real &vx1,
                                              Real &vx2, Real &vx3) const {
  std::stringstream msg;
  msg << "### FATAL ERROR in function "
         "[Coordinates::CartesianToMeshCoordsVector]"
      << std::endl
      << "not implemented yet. " << std::endl;
  ATHENA_ERROR(msg);
  return;
...
void Coordinates::MeshCoordsToCartesianVector(Real x1, Real x2, Real x3,
                                              Real vx1, Real vx2, Real vx3,
                                              Real &vx, Real &vy,
                                              Real &vz) const {
  std::stringstream msg;
  msg << "### FATAL ERROR in function "
         "[Coordinates::MeshCoordsToCartesianVector]"
      << std::endl
      << "not implemented yet. " << std::endl;
  ATHENA_ERROR(msg);
  return;
