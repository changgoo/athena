...
//! \brief implements functions for Kerr spacetime and Kerr-Schild
//! (t,r,theta,phi) coordinates in a derived class of the Coordinates abstract
//! base class.
//!
...
//!          & &  + \Sigma d\theta^2
//!          & &  + (r^2 + a^2 + (2 M a^2 r/\Sigma) \sin^2\theta) \sin^2\theta
//!          d\phi^2
//!   \f}
//!     where
//!   \f{eqnarray*}{
...
KerrSchild::KerrSchild(MeshBlock *pmb, ParameterInput *pin, bool flag)
  ...
  // Allocate and compute arrays for intermediate geometric quantities that are
  // only needed if object is NOT a coarse mesh
  if (!coarse_flag) {
    // Allocate arrays for intermediate geometric quantities: r-direction
    ...
    for (int i = il - ng; i <= iu + ng; ++i) {
      ...
      coord_width1_i1_(i) =
          rm_p - rm_m + m * std::log((rm_p + r_p) / (rm_m + r_m));
    ...
    for (int j = jll; j <= juu; ++j) {
      for (int i = il - ng; i <= iu + ng; ++i) {
        ...
        trans_face1_ji1_(j, i) =
            1.0 / std::sqrt(1.0 + 2.0 * m * r_m / sigma_mc);
        ...
        trans_face1_ji5_(j, i) =
            2.0 * m * r_m *
            std::sqrt(sigma_mc / ((sigma_mc + 2.0 * m * r_m) *
        ...
        trans_face1_ji7_(j, i) =
            ...
            (r_m_qu + a2 * r_m_sq + 2.0 * m * a2 * r_m +
             delta_m * a2 * cos_c_sq);
        if (i == (iu + ng)) {
          trans_face1_ji1_(j, i + 1) =
              1.0 / std::sqrt(1.0 + 2.0 * m * r_p / sigma_pc);
          trans_face1_ji2_(j, i + 1) = std::sqrt(
              (sigma_pc + 2.0 * m * r_p) /
        ...
        trans_face2_ji1_(j, i) =
            1.0 / std::sqrt(1.0 + 2.0 * m * r_c / sigma_cm);
        ...
        trans_face2_ji5_(j, i) =
            2.0 * m * r_c /
            (sigma_cm * std::sqrt(1.0 + 2.0 * m * r_c / sigma_cm));
        trans_face2_ji6_(j, i) =
            -a * sin_m_sq * std::sqrt(1.0 + 2.0 * m * r_c / sigma_cm);
        if (j == juu) {
          trans_face2_ji1_(j + 1, i) =
              1.0 / std::sqrt(1.0 + 2.0 * m * r_c / sigma_cp);
          trans_face2_ji2_(j + 1, i) =
              std::sqrt(1.0 + 2.0 * m * r_c / sigma_cp);
          trans_face2_ji3_(j + 1, i) = std::sqrt(sigma_cp);
          trans_face2_ji4_(j + 1, i) = std::sqrt(sigma_cp) * std::abs(sin_p);
          trans_face2_ji5_(j + 1, i) =
              2.0 * m * r_c /
              (sigma_cp * std::sqrt(1.0 + 2.0 * m * r_c / sigma_cp));
          trans_face2_ji6_(j + 1, i) =
              -a * sin_p_sq * std::sqrt(1.0 + 2.0 * m * r_c / sigma_cp);
        }
        trans_face3_ji1_(j, i) =
            1.0 / std::sqrt(1.0 + 2.0 * m * r_c / sigma_cc);
        ...
        trans_face3_ji5_(j, i) =
            2.0 * m * r_c /
            (sigma_cc * std::sqrt(1.0 + 2.0 * m * r_c / sigma_cc));
        trans_face3_ji6_(j, i) =
            -a * sin_c_sq * std::sqrt(1.0 + 2.0 * m * r_c / sigma_cc);
      }
    }
  }
}
...
void KerrSchild::Edge1Length(const int k, const int j, const int il,
                             const int iu, AthenaArray<Real> &lengths) {
...
void KerrSchild::Edge2Length(const int k, const int j, const int il,
                             const int iu, AthenaArray<Real> &lengths) {
  // \Delta L = 1/3 * |\cos\theta_- - \cos\theta_+|
  //     * (3 r_-^2 + a^2 (\cos^2\theta_- + \cos\theta_- \cos\theta_+ +
  //     \cos^2\theta_+))
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    lengths(i) =
        coord_len2_j1_(j) * (coord_len2_i1_(i) + ONE_3RD * coord_len2_j2_(j));
  }
  return;
}

void KerrSchild::Edge3Length(const int k, const int j, const int il,
                             const int iu, AthenaArray<Real> &lengths) {
...
void KerrSchild::CenterWidth1(const int k, const int j, const int il,
...
void KerrSchild::CenterWidth2(const int k, const int j, const int il,
...
void KerrSchild::CenterWidth3(const int k, const int j, const int il,
                              const int iu, AthenaArray<Real> &dx3) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    // \Delta W = |\sin\theta| (\phi_+ - \phi_-)
    //     * \sqrt{r^2 + a^2 + 2 M a^2 r \sin^2\theta / (r^2 + a^2
    //     \cos^2\theta)}
    dx3(i) =
        coord_width3_j1_(j) * coord_width3_k1_(k) * coord_width3_ji1_(j, i);
  }
  return;
}
...
// Notes:
//   \Delta V = 1/3 * (r_+ - r_-) |\cos\theta_- - \cos\theta_+| (\phi_+ -
//   \phi_-)
//       * (r_-^2 + r_- r_+ + r_+^2
//       + a^2 (\cos^2\theta_- + \cos\theta_- \cos\theta_+ + \cos^2\theta_+))

void KerrSchild::CellVolume(const int k, const int j, const int il,
                            const int iu, AthenaArray<Real> &volumes) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    volumes(i) = ONE_3RD * coord_vol_i1_(i) * coord_vol_j1_(j) *
                 coord_vol_k1_(k) * (coord_vol_i2_(i) + coord_vol_j2_(j));
  }
  return;
}
...
void KerrSchild::AddCoordTermsDivergence(const Real dt,
...
#pragma omp simd
      for (int i = pmy_block->is; i <= pmy_block->ie; ++i) {
        ...
        Real d2_g_13 =
            -2.0 * (1.0 + 2.0 * m * r * r2_a2 / sigma_sq) * a * sincos;
        Real d2_g_22 = -2.0 * a2 * sincos;
        Real d2_g_33 =
            2.0 * (delta + 2.0 * m * r * SQR(r2_a2) / sigma_sq) * sincos;
        ...
        Real uu_sq = g_11 * uu1 * uu1 + 2.0 * g_13 * uu1 * uu3 +
                     g_22 * uu2 * uu2 + g_33 * uu3 * uu3;
        ...
        Real s_1 =
            0.5 * (d1_g_00 * tt00 + 2.0 * d1_g_01 * tt01 +
                   2.0 * d1_g_03 * tt03 + d1_g_11 * tt11 +
                   2.0 * d1_g_13 * tt13 + d1_g_22 * tt22 + d1_g_33 * tt33);
        Real s_2 =
            0.5 * (d2_g_00 * tt00 + 2.0 * d2_g_01 * tt01 +
                   2.0 * d2_g_03 * tt03 + d2_g_11 * tt11 +
                   2.0 * d2_g_13 * tt13 + d2_g_22 * tt22 + d2_g_33 * tt33);
...
void KerrSchild::CellMetric(const int k, const int j, const int il,
...
void KerrSchild::Face1Metric(const int k, const int j, const int il,
                             const int iu, AthenaArray<Real> &g,
                             AthenaArray<Real> &g_inv) {
  // Extract useful quantities that do not depend on r
  const Real &m = bh_mass_;
...
void KerrSchild::Face2Metric(const int k, const int j, const int il,
                             const int iu, AthenaArray<Real> &g,
                             AthenaArray<Real> &g_inv) {
  // Extract useful quantities that do not depend on r
  const Real &m = bh_mass_;
...
void KerrSchild::Face3Metric(const int k, const int j, const int il,
                             const int iu, AthenaArray<Real> &g,
                             AthenaArray<Real> &g_inv) {
  // Extract useful quantities that do not depend on r
  const Real &m = bh_mass_;
...
//   k,j: phi- and theta-indices
//   il,iu: r-index bounds
//   bb1: 1D array of normal components B^1 of magnetic field, in global
//   coordinates prim_l: 1D array of left primitives, using global coordinates
...
void KerrSchild::PrimToLocal1(const int k, const int j, const int il,
...
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    ...
    if (MAGNETIC_FIELDS_ENABLED) {
      ...
      Real tmp = g_11 * uu1_l * uu1_l + 2.0 * g_13 * uu1_l * uu3_l +
                 g_22 * uu2_l * uu2_l + g_33 * uu3_l * uu3_l;
      Real gamma_l = std::sqrt(1.0 + tmp);
      Real u0_l = gamma_l / alpha;
      Real u1_l = uu1_l - alpha * gamma_l * g01;
      Real u2_l = uu2_l;
      Real u3_l = uu3_l;
      tmp = g_11 * uu1_r * uu1_r + 2.0 * g_13 * uu1_r * uu3_r +
            g_22 * uu2_r * uu2_r + g_33 * uu3_r * uu3_r;
      ...
      Real b0_l = g_10 * bb1_l * u0_l + g_11 * bb1_l * u1_l +
                  g_13 * bb1_l * u3_l + g_22 * bb2_l * u2_l +
                  g_30 * bb3_l * u0_l + g_31 * bb3_l * u1_l +
                  g_33 * bb3_l * u3_l;
      Real b1_l = (bb1_l + b0_l * u1_l) / u0_l;
      Real b2_l = (bb2_l + b0_l * u2_l) / u0_l;
      Real b3_l = (bb3_l + b0_l * u3_l) / u0_l;
      Real b0_r = g_10 * bb1_r * u0_r + g_11 * bb1_r * u1_r +
                  g_13 * bb1_r * u3_r + g_22 * bb2_r * u2_r +
                  g_30 * bb3_r * u0_r + g_31 * bb3_r * u1_r +
                  g_33 * bb3_r * u3_r;
...
//   k,j: phi- and theta-indices
//   il,iu: r-index bounds
//   bb2: 1D array of normal components B^2 of magnetic field, in global
//   coordinates prim_l: 1D array of left primitives, using global coordinates
//   prim_r: 1D array of right primitives, using global coordinates
// Outputs:
//   prim_l: values overwritten in local coordinates
...
void KerrSchild::PrimToLocal2(const int k, const int j, const int il,
                              const int iu, const AthenaArray<Real> &bb2,
                              AthenaArray<Real> &prim_l,
                              AthenaArray<Real> &prim_r,
                              AthenaArray<Real> &bbx) {
  // Calculate metric coefficients
...
//   k,j: phi- and theta-indices
//   il,iu: r-index bounds
//   bb3: 1D array of normal components B^3 of magnetic field, in global
//   coordinates prim_l: 1D array of left primitives, using global coordinates
//   prim_r: 1D array of right primitives, using global coordinates
// Outputs:
//   prim_l: values overwritten in local coordinates
...
void KerrSchild::PrimToLocal3(const int k, const int j, const int il,
                              const int iu, const AthenaArray<Real> &bb3,
                              AthenaArray<Real> &prim_l,
                              AthenaArray<Real> &prim_r,
                              AthenaArray<Real> &bbx) {
  // Calculate metric coefficients
...
void KerrSchild::FluxToGlobal1(const int k, const int j, const int il,
                               const int iu, const AthenaArray<Real> &cons,
...
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    ...
    Real t11 = m1_tm * m1_tm * ttt + m1_tm * m1_x * ttx + m1_x * m1_tm * txt +
               m1_x * m1_x * txx;
    Real t12 = m1_tm * m2_y * tty + m1_x * m2_y * txy;
    Real t13 = m1_tm * m3_x * ttx + m1_tm * m3_z * ttz + m1_x * m3_x * txx +
               m1_x * m3_z * txz;
...
void KerrSchild::FluxToGlobal2(const int k, const int j, const int il,
                               const int iu, const AthenaArray<Real> &cons,
                               const AthenaArray<Real> &bbx,
                               AthenaArray<Real> &flux, AthenaArray<Real> &ey,
                               AthenaArray<Real> &ez) {
...
void KerrSchild::FluxToGlobal3(const int k, const int j, const int il,
                               const int iu, const AthenaArray<Real> &cons,
                               const AthenaArray<Real> &bbx,
                               AthenaArray<Real> &flux, AthenaArray<Real> &ey,
                               AthenaArray<Real> &ez) {
...
void KerrSchild::RaiseVectorCell(Real a_0, Real a_1, Real a_2, Real a_3, int k,
                                 int j, int i, Real *pa0, Real *pa1, Real *pa2,
                                 Real *pa3) {
  // Extract geometric quantities
  const Real &m = bh_mass_;
  const Real &a = bh_spin_;
...
void KerrSchild::LowerVectorCell(Real a0, Real a1, Real a2, Real a3, int k,
                                 int j, int i, Real *pa_0, Real *pa_1,
                                 Real *pa_2, Real *pa_3) {
  // Extract geometric quantities
  const Real &m = bh_mass_;
  const Real &a = bh_spin_;
