...
//! \brief implements functions for spherical polar (r-theta-phi) coordinates in
//! a derived class of the Coordinates abstract base class.
...
SphericalPolar::SphericalPolar(MeshBlock *pmb, ParameterInput *pin, bool flag)
  ...
  // check that Mesh's polar coordinate range does not exceed [0, pi], even in
  // 2D (use 2D cylindrical coordinates to create a circular Mesh)
  if (block_size.nx2 > 1 && (pm->mesh_size.x2min < static_cast<Real>(0.0) ||
  ...
  if (!coarse_flag) {
    ...
    // Compute and store constant coefficients needed for face-areas,
    // cell-volumes, etc. This helps improve performance.
#pragma omp simd
    for (int i = il - ng; i <= iu + ng; ++i) {
      Real rm = x1f(i);
      Real rp = x1f(i + 1);
...
#pragma omp simd
      ...
      if (IsPole(jl)) // inner polar boundary
        coord_area1vc_j_(jl - 1) =
            2.0 - std::cos(x2v(jl - 1)) - std::cos(x2v(jl));
...
void SphericalPolar::Edge2Length(const int k, const int j, const int il,
...
void SphericalPolar::Edge3Length(const int k, const int j, const int il,
                                 const int iu, AthenaArray<Real> &len) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
...
// x3v(k)) VolCenter2(i,j,k) located at (i,j+1/2,k), i.e. (x1v(i), x2f(j+1),
// x3v(k))
...
void SphericalPolar::CenterWidth2(const int k, const int j, const int il,
...
void SphericalPolar::CenterWidth3(const int k, const int j, const int il,
...
void SphericalPolar::Face1Area(const int k, const int j, const int il,
...
void SphericalPolar::Face2Area(const int k, const int j, const int il,
                               const int iu, AthenaArray<Real> &area) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
...
void SphericalPolar::Face3Area(const int k, const int j, const int il,
                               const int iu, AthenaArray<Real> &area) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
...
void SphericalPolar::CellVolume(const int k, const int j, const int il,
...
void SphericalPolar::AddCoordTermsDivergence(const Real dt,
  ...
  bool do_hydro_diffusion =
      (hd.hydro_diffusion_defined && (hd.nu_iso > 0.0 || hd.nu_aniso > 0.0));
...
#pragma omp simd
      for (int i = pmy_block->is; i <= pmy_block->ie; ++i) {
        ...
        Real m_ii = prim(IDN, k, j, i) *
                    (SQR(prim(IM2, k, j, i)) + SQR(prim(IM3, k, j, i)));
        ...
        if (!STS_ENABLED) {
          if (do_hydro_diffusion) {
            m_ii += 0.5 * (hd.visflx[X2DIR](IM2, k, j + 1, i) +
                           hd.visflx[X2DIR](IM2, k, j, i));
            m_ii += 0.5 * (hd.visflx[X3DIR](IM3, k + 1, j, i) +
        ...
        if (use_x2_fluxes) {
          u(IM3, k, j, i) -=
              dt * coord_src1_i_(i) * coord_src2_j_(j) *
...
#pragma omp simd
        for (int i = pmy_block->is; i <= pmy_block->ie; ++i) {
          // src_1 = < M_{theta theta} + M_{phi phi} ><1/r>
          Real m_ii = 0.5 * (hd.visflx[X2DIR](IM2, k, j + 1, i) +
                             hd.visflx[X2DIR](IM2, k, j, i));
          m_ii += 0.5 * (hd.visflx[X3DIR](IM3, k + 1, j, i) +
                         hd.visflx[X3DIR](IM3, k, j, i));
          ...
          if (stage == 1 && pmy_block->pmy_mesh->sts_integrator == "rkl2") {
            flux_div(IM1, k, j, i) +=
                0.5 * pmy_block->pmy_mesh->dt * coord_src1_i_(i) * m_ii;

            // src_2 = -< M_{theta r} ><1/r>
            flux_div(IM2, k, j, i) -=
                0.5 * pmy_block->pmy_mesh->dt * coord_src2_i_(i) *
                (coord_area1_i_(i) * flux[X1DIR](IM2, k, j, i) +
                 coord_area1_i_(i + 1) * flux[X1DIR](IM2, k, j, i + 1));
          ...
          // src_2 = < M_{phi phi} ><cot theta/r>
          Real m_pp = 0.5 * (hd.visflx[X3DIR](IM3, k + 1, j, i) +
                             hd.visflx[X3DIR](IM3, k, j, i));
          ...
          if (stage == 1 && pmy_block->pmy_mesh->sts_integrator == "rkl2") {
            ...
            flux_div(IM2, k, j, i) += 0.5 * pmy_block->pmy_mesh->dt *
                                      coord_src1_i_(i) * coord_src1_j_(j) *
                                      m_pp;
          }
          ...
          if (use_x2_fluxes) {
            ...
            if (stage == 1 && pmy_block->pmy_mesh->sts_integrator == "rkl2") {
              // src_2 = -< M_{theta r} ><1/r>
              flux_div(IM3, k, j, i) -=
                  0.5 * pmy_block->pmy_mesh->dt * coord_src1_i_(i) *
                  coord_src2_j_(j) *
                  (coord_area2_j_(j) * flux[X2DIR](IM3, k, j, i) +
                   coord_area2_j_(j + 1) * flux[X2DIR](IM3, k, j + 1, i));
            }
          } else {
            Real m_ph = 0.5 * (hd.visflx[X2DIR](IM3, k, j + 1, i) +
                               hd.visflx[X2DIR](IM3, k, j, i));
            ...
            if (stage == 1 && pmy_block->pmy_mesh->sts_integrator == "rkl2") {
              ...
              flux_div(IM3, k, j, i) -= 0.5 * pmy_block->pmy_mesh->dt *
                                        coord_src1_i_(i) * coord_src3_j_(j) *
                                        m_ph;
            }
          }
        }
...
void SphericalPolar::CartesianToMeshCoords(Real x, Real y, Real z, Real &x1,
...
void SphericalPolar::MeshCoordsToCartesian(Real x1, Real x2, Real x3, Real &x,
                                           Real &y, Real &z) const {
...
void SphericalPolar::CartesianToMeshCoordsVector(Real x, Real y, Real z,
...
void SphericalPolar::MeshCoordsToCartesianVector(Real x1, Real x2, Real x3,
                                                 Real vx1, Real vx2, Real vx3,
                                                 Real &vx, Real &vy,
                                                 Real &vz) const {
