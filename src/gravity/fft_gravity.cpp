...
FFTGravityDriver::~FFTGravityDriver() { delete gtlist_; }
...
void FFTGravityDriver::Solve(int stage, int mode) {
  ...
  for (int nb = 0; nb < pmy_mesh_->nblocal; ++nb) {
    ...
    if (pmy_mesh_->particle_gravity) {
      ...
      for (Particles *ppar : pmb->ppars_grav) {
  ...
  for (int igid = nbs; igid <= nbe; igid++) {
    ...
    if (pmb != nullptr) {
      pfb->RetrieveResult(pmb->pgrav->phi, 0, NGHOST, pmb->loc,
                          pmb->block_size);
    }
    //    else { // on another process
    //    }
      ...
      for (int i = 0; i < knx[0]; i++) {
        ...
        if (gidx == 0) {
          ...
          if (mode == 0) { // Discrete FT
            ...
            if (dim_ > 1)
              pcoeff += ((2.0 * std::cos(ky) - 2.0) / dx2sq);
            if (dim_ > 2)
              pcoeff += ((2.0 * std::cos(kz) - 2.0) / dx3sq);
