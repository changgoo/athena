...
// helper function to compute positive modulus (consistent with python %
// operator)
...
BlockFFTGravity::BlockFFTGravity(MeshBlock *pmb, ParameterInput *pin)
    : BlockFFT(pmb), SHEAR_PERIODIC(pmb->pmy_mesh->shear_periodic),
      PHASE_SHIFT(false), dx1_(pmb->pcoord->dx1v(NGHOST)),
      dx2_(pmb->pcoord->dx2v(NGHOST)), dx3_(pmb->pcoord->dx3v(NGHOST)),
      ...
      roll_buf(pmb->ncells3, pmb->ncells1, pmb->ncells2), send_buf(nx3 * nx2),
      recv_buf(nx3 * nx2), pflux(pmb->ncells2 + 1),
  ...
  grfflag = GetGreenFuncFlag(
      pin->GetOrAddString("self_gravity", "green_function", "cell_averaged"));
  ...
  pf3dgrf_->setup(2 * Nx1, 2 * Nx2, 2 * Nx3, 2 * in_ilo, 2 * in_ihi + 1,
                  2 * in_jlo, 2 * in_jhi + 1, 2 * in_klo, 2 * in_khi + 1,
                  2 * slow_ilo, 2 * slow_ihi + 1, 2 * slow_jlo,
                  2 * slow_jhi + 1, 2 * slow_klo, 2 * slow_khi + 1, permute,
...
void BlockFFTGravity::ExecuteForward() {
  ...
  if (gbflag == GravityBoundaryFlag::periodic) {
      ...
      for (int i = 0; i < mid_nx1; i++) {
        for (int k = 0; k < mid_nx3; k++) {
          for (int j = 0; j < mid_nx2; j++) {
            ...
            in_[idx] *= std::exp(-TWO_PI * I_ * rshear_ * (Real)(mid_jlo + j) *
                                 (Real)(mid_ilo + i) / (Real)Nx1);
          }
        }
      }
      // mid2fast
    ...
    pf3d->remap(data, data, pf3d->remap_fastslow);
    // even term (l=2p)
    std::memcpy(in_e_, in_,
                sizeof(std::complex<Real>) * slow_nx1 * slow_nx2 * slow_nx3);
    // odd term (l=2p+1)
    std::memcpy(in_o_, in_,
                sizeof(std::complex<Real>) * slow_nx1 * slow_nx2 * slow_nx3);
    ...
    // slow_forward
    pf3d->perform_ffts(reinterpret_cast<FFT_DATA *>(in_e_), FFTW_FORWARD,
                       pf3d->fft_slow);
    pf3d->perform_ffts(reinterpret_cast<FFT_DATA *>(in_o_), FFTW_FORWARD,
                       pf3d->fft_slow);
...
void BlockFFTGravity::ExecuteBackward() {
  ...
  if (gbflag == GravityBoundaryFlag::periodic) {
    ...
    FFT_SCALAR *data = reinterpret_cast<FFT_SCALAR *>(in_);
    // slow_backward
    pf3d->perform_ffts(reinterpret_cast<FFT_DATA *>(in_e_), FFTW_BACKWARD,
                       pf3d->fft_slow);
    pf3d->perform_ffts(reinterpret_cast<FFT_DATA *>(in_o_), FFTW_BACKWARD,
                       pf3d->fft_slow);
    ...
    for (int j = 0; j < slow_nx2; j++) {
      for (int i = 0; i < slow_nx1; i++) {
        for (int k = 0; k < slow_nx3; k++) {
          int idx = k + slow_nx3 * (i + slow_nx1 * j);
          in_[idx] =
              in_e_[idx] + std::exp(PI * I_ * (Real)k / (Real)Nx3) * in_o_[idx];
        }
      }
    }
    // slow2fast
    ...
    for (int j = 0; j < slow_nx2; j++) {
      for (int i = 0; i < slow_nx1; i++) {
        for (int k = 0; k < slow_nx3; k++) {
          ...
          if (SHEAR_PERIODIC & !PHASE_SHIFT) {
            kxy =
                std::sqrt((2. - 2. * std::cos(kx)) / dx1sq_ +
                          2 * qomt * std::sin(kx) * std::sin(ky) / dx1_ / dx2_ +
                          (SQR(qomt) + 1.) * (2. - 2. * std::cos(ky)) / dx2sq_);
          } else {
          ...
          if (grfflag == GreenFuncFlag::cell_averaged) {
            ...
            Real exp_kxydz3 =
                exp_kxydz1 * exp_kxydz2; // for std::exp(-3*kxy*dx3_)
            ...
            if ((slow_ilo + i == 0) && (slow_jlo + j == 0)) {
              ...
              kernel_o =
                  -four_pi_G * dx3_ / (1. - cos_o) + 0.125 * four_pi_G * dx3_;
            } else {
              kernel_e =
                  -0.5 * four_pi_G / SQR(kxy) / dx3_ *
                  (2 * (1. - exp_kxydz) +
            ...
            if ((slow_ilo + i == 0) && (slow_jlo + j == 0)) {
              ...
              kernel_e = -0.5 * four_pi_G / kxy * (1. - exp_kxyLz) *
                         (1. - exp_kxydz2) /
                         (1. + exp_kxydz2 - 2. * exp_kxydz1 * cos_e);
              kernel_o = -0.5 * four_pi_G / kxy * (1. + exp_kxyLz) *
                         (1. - exp_kxydz2) /
                         (1. + exp_kxydz2 - 2. * exp_kxydz1 * cos_o);
            }
          } else {
            std::stringstream msg;
            msg << "### FATAL ERROR in BlockFFTGravity::ApplyKernel"
                << std::endl
...
//! The steps are different for different gravity boundary conditions. This
//! function is an umbrella function that executes required steps depending on
...
void BlockFFTGravity::Solve(int stage) {
  ...
  if (is_particle_gravity) {
    rhosum.NewAthenaArray(pmy_block_->ncells3, pmy_block_->ncells2,
                          pmy_block_->ncells1);
    for (Particles *ppar : pmy_block_->ppars_grav) {
  ...
  if (SHEAR_PERIODIC & PHASE_SHIFT) {
    ...
    // It was found that the phase shift method introduces spurious error when
    // the sheared distance at the boundary is not an integer multiple of the
    // cell width. To cure this, we solve the Poisson equation at the nearest
    // two 'integer times' when the sheared distance is an integer multiple of
    // the cell width, and then linearly interpolate the solution in time.
    Real time = pmy_block_->pmy_mesh->time;
    Real qomL = qshear_ * Omega_0_ * Lx1_;
    Real dt = time - (static_cast<int>(qomL * time / Lx2_)) * Lx2_ / qomL;
    Real qomt = qshear_ * Omega_0_ * dt;
    ...
    // The shearing-periodic boundary condition is equivalent to the usual
    // periodic BC, (x'+Lx, y') = (x', y'+Ly) = (x', y'), in this new coordinate
    // system. Since Poisson equation is not covarient for this transform, we
    // need to modify the equation and therefore the kernel used in ApplyKernel.
    ...
    for (int j = 0; j < 2 * nx2; j++) {
      for (int i = 0; i < 2 * nx1; i++) {
        ...
        if (grfflag == GreenFuncFlag::cell_averaged) {
          ...
          grf_[idx] = _GetIGF((-gi + 0.5) * dx1_, (-gj + 0.5) * dx2_,
                              (-gk + 0.5) * dx3_);
          grf_[idx] -= _GetIGF((-gi + 0.5) * dx1_, (-gj + 0.5) * dx2_,
                               (-gk - 0.5) * dx3_);
          grf_[idx] -= _GetIGF((-gi + 0.5) * dx1_, (-gj - 0.5) * dx2_,
          ...
          if ((gi == 0) && (gj == 0) && (gk == 0)) {
            ...
            grf_[idx] =
                1. /
                std::sqrt(SQR(gi * dx1_) + SQR(gj * dx2_) + SQR(gk * dx3_)) *
                dvol;
          }
        } else {
          std::stringstream msg;
...
//----------------------------------------------------------------------------------------
//! \fn BlockFFTGravity::LoadOBCSource(const AthenaArray<Real> &src, int px, int
//! py,
//                                     int pz)
//! \brief Load source and multiply phase shift term for the open boundary
//! condition.
void BlockFFTGravity::LoadOBCSource(const AthenaArray<Real> &src, int px,
                                    int py, int pz) {
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        ...
        int gj = in_jlo + j - js;
        int gk = in_klo + k - ks;
        Real phase = PI * (gi * px / (Real)Nx1 + gj * py / (Real)Nx2 +
                           gk * pz / (Real)Nx3);
...
//----------------------------------------------------------------------------------------
//! \fn BlockFFTGravity::RetrieveOBCResult(AthenaArray<Real> &dst, int px, int
//! py,
//                                         int pz)
//! \brief Retrieve result and multiply phase shift term for the open boundary
//! condition.
void BlockFFTGravity::RetrieveOBCResult(AthenaArray<Real> &dst, int px, int py,
                                        int pz) {
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
  ...
  for (int j = 0; j < slow_nx2; j++) {
    for (int i = 0; i < slow_nx1; i++) {
      for (int k = 0; k < slow_nx3; k++) {
        ...
        int grfidx =
            (2 * k + pz) +
            (2 * slow_nx3) * ((2 * i + px) + (2 * slow_nx1) * (2 * j + py));
        in_[idx] *= grf_[grfidx];
      }
    }
  }
...
void BlockFFTGravity::RollUnroll(AthenaArray<Real> &dat, Real dt) {
  ...
  // SMOON: currently, messages are received one at a time, so there is no
  // confusion
  int remapvar_tag = 0;
  MPI_Request rq;
  ...
  cnt = nx1 * NGHOST * nx3;
  ierr = MPI_Irecv(recv_gbuf.data(), cnt, MPI_DOUBLE, lower_id, remapvar_tag,
                   MPI_COMM_WORLD, &rq);
  ...
  // Send data
  ierr = MPI_Send(send_gbuf.data(), cnt, MPI_DOUBLE, upper_id, remapvar_tag,
                  MPI_COMM_WORLD);
  ...
  // Post a non-blocking receive for the data to be sent
  cnt = nx1 * NGHOST * nx3;
  ierr = MPI_Irecv(recv_gbuf.data(), cnt, MPI_DOUBLE, upper_id, remapvar_tag,
                   MPI_COMM_WORLD, &rq);
  ...
  // Send data
  ierr = MPI_Send(send_gbuf.data(), cnt, MPI_DOUBLE, lower_id, remapvar_tag,
                  MPI_COMM_WORLD);
  ...
  for (int k = ks; k <= ke; k++) {
    for (int i = is; i <= ie; i++) {
      ...
      if (xorder <= 2) {
        pmy_block_->porb->RemapFluxPlm(pflux, dat, eps, osgn, k, i, js, je + 1,
                                       shift0);
      } else {
        pmy_block_->porb->RemapFluxPpm(pflux, dat, eps, osgn, k, i, js, je + 1,
                                       shift0);
      }
      for (int j = js; j <= je; j++) {
  ...
  for (int i = is; i <= ie; i++) {
    ...
    if (joffset >= 0) {
      ...
      // Step A.1: Send density at [je-(joverlap-1):je] to [js:js+(joverlap-1)]
      // of the target MeshBlock. Skip this step if joverlap = 0.
      if (joverlap != 0) {
        // Find ids of procs that data in [je-(joverlap-1):je] is sent to, and
        // data in [js:js+(joverlap-1)] is received from. Only execute if
        // joverlap>0
        loc = my_loc;
      ...
      if (Ngrids == 0) {
        // If the target MeshBlock is of my own, do local copy instead of MPI
        // calls.
        for (int k = ks; k <= ke; k++) {
        ...
        // Find the id of the MeshBlock that data in [js:je-joverlap] is sent
        // to, and data in [js+joverlap:je] is received from.
        loc = my_loc;
      ...
      // Here, the steps are identical to the case A, but with different array
      // indices.
      joffset = -joffset;
      Ngrids = static_cast<int>(joffset / nx2);
      joverlap = joffset - Ngrids * nx2;
      // Step B.1: Send density at [js:js+(joverlap-1)] to [je-(joverlap-1):je]
      // of the target MeshBlock. Skip this step if joverlap = 0.
      if (joverlap != 0) {
        // Find ids of procs that data in [js:js+(joverlap-1)] is sent to, and
        // data in [je-(joverlap-1):je] is received from. Only execute if
        // joverlap>0
        loc = my_loc;
        loc.lx2 = positive_modulo(loc.lx2 - Ngrids - 1, Nx2 / nx2);
        pleaf = proot->FindMeshBlock(loc);
      ...
      if (Ngrids == 0) {
        ...
        // Find the id of the MeshBlock that data in [js+joverlap:je] is sent
        // to, and data in [js:je-joverlap] is received from.
        loc = my_loc;
        loc.lx2 = positive_modulo(loc.lx2 - Ngrids, Nx2 / nx2);
        pleaf = proot->FindMeshBlock(loc);
        sendto_id = pleaf->GetGid();
...
void BlockFFTGravity::SetPhysicalBoundaries() {
  // TODO(SMOON) use BoundaryFace rather than loc.lx3, etc, and write
  // Boundaryfunctions
  if (gbflag == GravityBoundaryFlag::disk) {
      ...
      for (int k = 1; k <= NGHOST; k++) {
        for (int j = js - NGHOST; j <= je + NGHOST; j++) {
          for (int i = is - NGHOST; i <= ie + NGHOST; i++) {
            ...
            pmy_block_->pgrav->phi(ks - k, j, i) =
                pmy_block_->pgrav->phi(ks, j, i);
            // linear extrapolation
            //            pmy_block_->pgrav->phi(ks-k,j,i) =
            //            pmy_block_->pgrav->phi(ks,j,i)
            //                + k*(pmy_block_->pgrav->phi(ks,j,i) -
            //                pmy_block_->pgrav->phi(ks+1,j,i));
          }
        }
      }
    }
    if (pmy_block_->loc.lx3 == Nx3 / nx3 - 1) {
      for (int k = 1; k <= NGHOST; k++) {
        for (int j = js - NGHOST; j <= je + NGHOST; j++) {
          for (int i = is - NGHOST; i <= ie + NGHOST; i++) {
            // constant extrapolation
            pmy_block_->pgrav->phi(ke + k, j, i) =
                pmy_block_->pgrav->phi(ke, j, i);
            // linear extrapolation
            //            pmy_block_->pgrav->phi(ke+k,j,i) =
            //            pmy_block_->pgrav->phi(ke,j,i)
            //                + k*(pmy_block_->pgrav->phi(ke,j,i) -
            //                pmy_block_->pgrav->phi(ke-1,j,i));
          }
        }
      }
    }
    ...
    if (pmy_block_->loc.lx2 == 0) {
      for (int k = ks - NGHOST; k <= ke + NGHOST; k++) {
        for (int j = 1; j <= NGHOST; j++) {
          for (int i = is - NGHOST; i <= ie + NGHOST; i++) {
            pmy_block_->pgrav->phi(k, js - j, i) =
                pmy_block_->pgrav->phi(k, js, i);
          }
        }
      }
    }
    if (pmy_block_->loc.lx2 == Nx2 / nx2 - 1) {
      for (int k = ks - NGHOST; k <= ke + NGHOST; k++) {
        for (int j = 1; j <= NGHOST; j++) {
          for (int i = is - NGHOST; i <= ie + NGHOST; i++) {
            pmy_block_->pgrav->phi(k, je + j, i) =
                pmy_block_->pgrav->phi(k, je, i);
          }
        }
      }
    }
      ...
      for (int k = ks - NGHOST; k <= ke + NGHOST; k++) {
        for (int j = js - NGHOST; j <= je + NGHOST; j++) {
          for (int i = 1; i <= NGHOST; i++) {
            pmy_block_->pgrav->phi(k, j, is - i) =
                pmy_block_->pgrav->phi(k, j, is);
          }
        }
      }
    }
    if (pmy_block_->loc.lx1 == Nx1 / nx1 - 1) {
      for (int k = ks - NGHOST; k <= ke + NGHOST; k++) {
        for (int j = js - NGHOST; j <= je + NGHOST; j++) {
          for (int i = 1; i <= NGHOST; i++) {
            pmy_block_->pgrav->phi(k, j, ie + i) =
                pmy_block_->pgrav->phi(k, j, ie);
          }
        }
      }
    }
...
//! \brief Parses input string to return scoped enumerator flag specifying
//! gravity boundary condition. Typically called in BlockFFTGravity() ctor.
...
//! \brief Parses input string to return scoped enumerator flag specifying
//! Green's function. Typically called in BlockFFTGravity() ctor.
...
Real _GetIGF(Real x, Real y, Real z) {
  ...
  return y * z * std::log(x + r) + z * x * std::log(y + r) +
         x * y * std::log(z + r) - 0.5 * SQR(x) * std::atan(y * z / x / r) -
