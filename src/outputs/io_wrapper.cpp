...
//! IOWrapperSizeT cnt) \brief wrapper for {MPI_File_write} versus {std::fwrite}
...
std::size_t IOWrapper::Write(const void *buf, IOWrapperSizeT size,
  ...
  MPI_Status status;
  int nwrite;
  if (MPI_File_write(fh_, const_cast<void *>(buf), cnt * size, MPI_BYTE,
                     &status) != MPI_SUCCESS)
    return -1;
...
std::size_t IOWrapper::Write_at_all(const void *buf, IOWrapperSizeT size,
  ...
  MPI_Status status;
  int nwrite;
  if (MPI_File_write_at_all(fh_, offset, const_cast<void *>(buf), cnt * size,
