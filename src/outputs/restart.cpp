...
//----------------------------------------------------------------------------------------
//! \fn void RestartOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool
//! flag) \brief Cycles over all MeshBlocks and writes data to a single restart
//! file.
...
void RestartOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin,
  ...
  fname.append(".");
  // add file number to name, unless write is forced by terminate signal, in
  // which case replace number in the name by the string "final".  This keeps
  // the restart file numbers consistent with output.dt when a job is restarted
  // many times.
  if (!force_write)
    fname.append(number);
  else
  ...
  if (!force_write) {
    ...
    pin->SetInteger(output_params.block_name, "file_number",
                    output_params.file_number);
    pin->SetReal(output_params.block_name, "next_time",
                 output_params.next_time);
  }
  ...
  headeroffset = sbuf.size() * sizeof(char) + 3 * sizeof(int) +
                 2 * sizeof(Real) + sizeof(RegionSize) + udsize;
  ...
  MPI_Allgatherv(MPI_IN_PLACE, mynb, MPI_UINT64_T, offset, pm->nblist,
                 pm->nslist, MPI_UINT64_T, MPI_COMM_WORLD);
#endif
  ...
  for (int b = 0; b < mynb; ++b) {
    ...
    // NEW_OUTPUT_TYPES: add output of additional physics to restarts here also
    // update MeshBlock::GetBlockSizeInBytes accordingly and MeshBlock
    ...
    if (GENERAL_RELATIVITY) {
      std::memcpy(pdata, pmb->phydro->w.data(),
                  pmb->phydro->w.GetSizeInBytes());
      pdata += pmb->phydro->w.GetSizeInBytes();
      std::memcpy(pdata, pmb->phydro->w1.data(),
                  pmb->phydro->w1.GetSizeInBytes());
      pdata += pmb->phydro->w1.GetSizeInBytes();
    }

    // Longitudinal, face-centered magnetic field components:
    if (MAGNETIC_FIELDS_ENABLED) {
      std::memcpy(pdata, pmb->pfield->b.x1f.data(),
                  pmb->pfield->b.x1f.GetSizeInBytes());
      pdata += pmb->pfield->b.x1f.GetSizeInBytes();
      std::memcpy(pdata, pmb->pfield->b.x2f.data(),
                  pmb->pfield->b.x2f.GetSizeInBytes());
      pdata += pmb->pfield->b.x2f.GetSizeInBytes();
      std::memcpy(pdata, pmb->pfield->b.x3f.data(),
                  pmb->pfield->b.x3f.GetSizeInBytes());
    ...
    // CR variables:
    if (CR_ENABLED) {
      std::memcpy(pdata, pmb->pcr->u_cr.data(),
                  pmb->pcr->u_cr.GetSizeInBytes());
