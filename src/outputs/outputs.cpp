  ...
  while (pib != nullptr) {
    if (pib->block_name.compare(0, 6, "output") == 0) {
      ...
      if (op.dt > 0.0) { // only add output if dt>0
        ...
        op.orbital_system_output =
            pin->GetOrAddBoolean(op.block_name, "orbital_system", false);
...
void OutputType::LoadOutputData(MeshBlock *pmb) {
  ...
  if (output_params.variable.compare("m2") == 0) {
    ...
    pod->name = "mom2";
    if (porb->orbital_advection_defined &&
        !output_params.orbital_system_output && porb->orbital_direction == 1) {
      porb->ConvertOrbitalSystem(phyd->w, phyd->u, OrbitalTransform::cons);
  ...
  if (output_params.variable.compare("m3") == 0) {
    ...
    pod->name = "mom3";
    if (porb->orbital_advection_defined &&
        !output_params.orbital_system_output && porb->orbital_direction == 2) {
      porb->ConvertOrbitalSystem(phyd->w, phyd->u, OrbitalTransform::cons);
  ...
  //      pod->data.InitWithShallowSlice(pmb->ppars[ipar]->ppm->weight, 4, 0,
  //      1); AppendOutputDataNode(pod); num_vars_++;
  //    }
  //  }
  ...
  if (output_params.variable.compare("vp") == 0 ||
    ...
    for (Particles *ppar : pmb->ppars) {
      ...
      pod->type = "VECTORS";
      pod->name = "vp";
      pod->name += std::to_string(ppar->ipar);
      pod->data = ppar->ppm->GetVelocityField();
      AppendOutputDataNode(pod);
      num_vars_ += 3;
    }
  }
  ...
  if (output_params.variable.compare("rhop") == 0 ||
    ...
    for (Particles *ppar : pmb->ppars) {
      ...
      pod->name = "rhop";
      pod->name += std::to_string(ppar->ipar);
      pod->data = ppar->ppm->GetMassDensity();
      AppendOutputDataNode(pod);
      num_vars_++;
    }
  }
  ...
  if (output_params.variable.compare(0, 3, "uov") == 0 ||
    ...
    if (sscanf(output_params.variable.c_str(), "uov%d", &iv) > 0) {
      if (iv >= 0 && iv < pmb->nuser_out_var)
        ns = iv, ne = iv;
    } else if (sscanf(output_params.variable.c_str(), "user_out_var%d", &iv) >
               0) {
      if (iv >= 0 && iv < pmb->nuser_out_var)
        ns = iv, ne = iv;
    }
...
//----------------------------------------------------------------------------------------
//! \fn void OutputType::ReplaceOutputDataNode(OutputData *pold, OutputData
//! *pnew) \brief
...
//! \brief scans through singly linked list of OutputTypes and makes any outputs
//! needed.
  ...
  while (ptype != nullptr) {
    if ((pm->time == pm->start_time) ||
      ...
      if (first && (ptype->output_params.file_type != "hst" || wtflag)) {
        ...
        if (pm->particle)
          Particles::ComputePMDensityAndCommunicate(pm, true);
        first = false;
      }
