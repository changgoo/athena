...
//! \brief writes output data as a formatted table.  Should not be used to
//! output large 3D data sets as this format is very slow and memory intensive.
//! Most useful for 1D slices and/or sums.  Writes one file per Meshblock.
...
//----------------------------------------------------------------------------------------
//! \fn void FormattedTableOutput::WriteOutputFile(Mesh *pm, ParameterInput
//! *pin,
//!                                                bool flag)
//! \brief writes OutputData to file in tabular format using C style
//! std::fprintf
...
void FormattedTableOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin,
  ...
  for (int b = 0; b < pm->nblocal; ++b) {
    ...
    // build doubly linked list of OutputData nodes (setting data ptrs to
    // appropriate quantity on MeshBlock for each node), then slice/sum as
    // needed
    LoadOutputData(pmb);
    ...
    // "."+"blockid"+"."+"file_id"+"."+XXXXX+".tab", where XXXXX = 5-digit
    // file_number
    std::string fname;
    char number[6];
    ...
    if ((pfile = std::fopen(fname.c_str(), "w")) == nullptr) {
      msg << "### FATAL ERROR in function "
             "[FormattedTableOutput::WriteOutputFile]"
          << std::endl
    ...
    std::fprintf(pfile, "#");
    if (out_is != out_ie)
      std::fprintf(pfile, " i       x1v     ");
    if (out_js != out_je)
      std::fprintf(pfile, " j       x2v     ");
    if (out_ks != out_ke)
      std::fprintf(pfile, " k       x3v     ");
    // write data col headers from "name" stored in OutputData nodes of doubly
    // linked list
    ...
    for (int k = out_ks; k <= out_ke; ++k) {
      for (int j = out_js; j <= out_je; ++j) {
        for (int i = out_is; i <= out_ie; ++i) {
          ...
          // step through doubly linked list of OutputData's and write each on
          // same line
    ...
    // don't forget to close the output file and clean up ptrs to data in
    // OutputData
    std::fclose(pfile);
    ClearOutputData(); // required when LoadOutputData() is used.
...
//----------------------------------------------------------------------------------------
//! \fn void ParticleFormattedTableOutput::WriteOutputFile(Mesh *pm,
//! ParameterInput *pin,
//!                                                bool flag)
//! \brief writes OutputData to file in tabular format using C style
//! std::fprintf
...
void ParticleFormattedTableOutput::WriteOutputFile(Mesh *pm,
  ...
  if (pm->particle)
    Particles::FormattedTableOutput(pm, output_params);
