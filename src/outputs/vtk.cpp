...
//!
//! Data is written in RECTILINEAR_GRID geometry, in BINARY format, and in FLOAT
//! type Writes one file per MeshBlock.
...
//----------------------------------------------------------------------------------------
//! Functions to detect big endian machine, and to byte-swap 32-bit words.  The
//! vtk legacy format requires data to be stored as big-endian.
...
//----------------------------------------------------------------------------------------
//! \fn void VTKOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool
//! flag) \brief Cycles over all MeshBlocks and writes OutputData in (legacy)
//! vtk format, one
//!        MeshBlock per file
...
void VTKOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag) {
  ...
  for (int b = 0; b < pm->nblocal; ++b) {
    ...
    // "."+"blockid"+"."+"file_id"+"."+XXXXX+".vtk", where XXXXX = 5-digit
    // file_number
    std::string fname;
    char number[6];
    ...
    // Specify the type of data, dimensions, and coordinates.  If N>1, then
    // write N+1 cell faces as binary floats.  If N=1, then write 1 cell center
    // position.
    ...
    //  (every node in the OutputData doubly linked lists), all in binary floats
    //  format
        ...
        for (int j = out_js; j <= out_je; ++j) {
          for (int i = out_is; i <= out_ie; ++i) {
            for (int n = 0; n < nvar; ++n) {
              data[nvar * (i - out_is) + n] =
                  static_cast<float>(pdata->data(n, k, j, i));
            }
          }
          ...
          std::fwrite(data, sizeof(float),
                      static_cast<std::size_t>(nvar * ncells1), pfile);
        }
      }
      pdata = pdata->pnext;
    }
