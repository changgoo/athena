...
//========================================================================================
//! \file history.cpp
//! \brief writes history output data, volume-averaged quantities that are
//! output
...
HistoryOutput::HistoryOutput(OutputParameters oparams) : OutputType(oparams) {
  // NEW_OUTPUT_TYPES:
...
//----------------------------------------------------------------------------------------
//! \fn void HistoryOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool
//! flag) \brief Writes a history file
...
void HistoryOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag) {
  ...
  for (int n = 0; n < num_vars_; ++n)
    hst_data[n] = 0.0;
  // initialize user-defined history outputs depending on the requested
  // operation
  for (int n = 0; n < pm->nuser_history_output_; n++) {
    ...
    case UserHistoryOperation::max:
      hst_data[num_vars_ + n] = real_lowest;
      break;
  ...
  for (int b = 0; b < pm->nblocal; ++b) {
    ...
    // Sum history variables over cells. Note ghost cells are never included in
    // sums
      ...
      for (int k = pmb->ks; k <= pmb->ke; ++k) {
        for (int j = pmb->js; j <= pmb->je; ++j) {
          ...
          for (int i = pmb->is; i <= pmb->ie; ++i) {
            ...
            // Cell-centered magnetic energy, partitioned by coordinate
            // direction:
      ...
      for (int k = pmb->ks; k <= pmb->ke; ++k) {
        for (int j = pmb->js; j <= pmb->je; ++j) {
          ...
          for (int i = pmb->is; i <= pmb->ie; ++i) {
            ...
            if (CR_ENABLED) {
              constexpr int prev_out = NHYDRO + 3 + NGRAV + NFIELD + NSCALARS;
    ...
    for (int n = 0; n < pm->nuser_history_output_;
         n++) { // user-defined history outputs
      if (pm->user_history_func_[n] != nullptr) {
        Real usr_val = pm->user_history_func_[n](pmb, n);
        switch (pm->user_history_ops_[n]) {
        case UserHistoryOperation::sum:
          // TODO(felker): this should automatically volume-weight the sum, like
          // the built-in variables. But existing user-defined .hst fns are
          // currently weighting their returned values.
  ...
  if (Globals::my_rank == 0) {
    MPI_Reduce(MPI_IN_PLACE, hst_data.get(), num_vars_, MPI_ATHENA_REAL,
               MPI_SUM, 0, MPI_COMM_WORLD);
  } else {
    MPI_Reduce(hst_data.get(), hst_data.get(), num_vars_, MPI_ATHENA_REAL,
               MPI_SUM, 0, MPI_COMM_WORLD);
  }
  ...
  if (Globals::my_rank == 0) {
    ...
    if (output_params.file_number == 0) {
      ...
      if (SELF_GRAVITY_ENABLED)
        std::fprintf(pfile, "[%d]=grav-E   ", iout++);
      if (MAGNETIC_FIELDS_ENABLED) {
