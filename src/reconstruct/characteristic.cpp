...
//! \brief Functions to transform vectors between primitive and characteristic
//! variables
...
//! \brief Computes inner-product of left-eigenmatrix of Roe's matrix A in the
//! primitive variables and an input vector.  This operation converts primitive
//! to characteristic variables.  The result is returned in the input vector,
//! with the components of the characteristic field stored such that vect(1,i)
//! is in the direction of the sweep.
//!
//! The order of the components in the input vector should be:
//!    (IDN,IVX,IVY,IVZ,[IPR],[IBY,IBZ])
//! and these are permuted according to the direction specified by the input
//! flag "ivx".
//!
//! REFERENCES:
...
void Reconstruction::LeftEigenmatrixDotVector(const int ivx, const int il,
                                              const int iu,
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        ...
        // Multiply row of L-eigenmatrix with vector using matrix elements from
        // eq. A18
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        ...
        // eq. A22
        Real v_0 =
            0.5 * alpha_f * (vect(IDN, i) * id - cf * vect(ivx, i) / iso_cs2) +
            qs * (bet2 * vect(ivy, i) + bet3 * vect(ivz, i)) +
            as_prime * (bet2 * vect(IBY, i) + bet3 * vect(IBZ, i));
        Real v_1 = 0.5 * (bet2 * (vect(IBZ, i) * s * isqrtd + vect(ivz, i)) -
                          bet3 * (vect(IBY, i) * s * isqrtd + vect(ivy, i)));
        Real v_2 =
            0.5 * alpha_s * (vect(IDN, i) * id - cs * vect(ivx, i) / iso_cs2) -
            qf * (bet2 * vect(ivy, i) + bet3 * vect(ivz, i)) -
            af_prime * (bet2 * vect(IBY, i) + bet3 * vect(IBZ, i));
        Real v_3 =
            0.5 * alpha_s * (vect(IDN, i) * id + cs * vect(ivx, i) / iso_cs2) +
...
//! \brief Computes inner-product of right-eigenmatrix of Roe's matrix A in the
//! primitive variables and an input vector.  This operation converts
//! characteristic to primitive variables.  The result is returned in the input
//! vector.
//!
//! The order of the components in the input vector (characteristic fields)
//! should be:
//!    (IDN,ivx,ivy,ivz,[IPR],[IBY,IBZ])
//! where the lower-case indices indicate that the characteristic field in the
//! direction of the sweep (designated by the input flag "ivx") is stored first.
//! On output, the components of velocity are in the standard order used for
//! primitive variables.
//!
//! REFERENCES:
//! - J. Stone, T. Gardiner, P. Teuben, J. Hawley, & J. Simon "Athena: A new
...
void Reconstruction::RightEigenmatrixDotVector(const int ivx, const int il,
                                               const int iu,
                                               const AthenaArray<Real> &b1,
                                               const AthenaArray<Real> &w,
                                               AthenaArray<Real> &vect) {
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        ...
        // Multiply row of R-eigenmatrix with vector using matrix elements from
        // eq. A12 Components of vect() are addressed directly as they are input
        // in permuted order
        Real v_0 = w(IDN, i) * (alpha_f * (vect(0, i) + vect(6, i)) +
                                alpha_s * (vect(2, i) + vect(4, i))) +
                   vect(3, i);
        Real v_1 = cf * alpha_f * (vect(6, i) - vect(0, i)) +
                   cs * alpha_s * (vect(4, i) - vect(2, i));
        Real v_2 = bet2 * (qs * (vect(0, i) - vect(6, i)) +
                           qf * (vect(4, i) - vect(2, i))) +
                   bet3 * (vect(5, i) - vect(1, i));
        Real v_3 = bet3 * (qs * (vect(0, i) - vect(6, i)) +
                           qf * (vect(4, i) - vect(2, i))) +
                   bet2 * (vect(1, i) - vect(5, i));
        Real v_4 = w(IDN, i) * asq *
                   (alpha_f * (vect(0, i) + vect(6, i)) +
                    alpha_s * (vect(2, i) + vect(4, i)));
        Real v_5 = bet2 * (as * (vect(0, i) + vect(6, i)) -
                           af * (vect(2, i) + vect(4, i))) -
                   bet3 * s * sqrtd * (vect(5, i) + vect(1, i));
        Real v_6 = bet3 * (as * (vect(0, i) + vect(6, i)) -
                           af * (vect(2, i) + vect(4, i))) +
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        ...
        Real v_1 = cf * alpha_f * (vect(5, i) - vect(0, i)) +
                   cs * alpha_s * (vect(3, i) - vect(2, i));
        Real v_2 = bet2 * (qs * (vect(0, i) - vect(5, i)) +
                           qf * (vect(3, i) - vect(2, i))) +
                   bet3 * (vect(4, i) - vect(1, i));
        Real v_3 = bet3 * (qs * (vect(0, i) - vect(5, i)) +
                           qf * (vect(3, i) - vect(2, i))) +
                   bet2 * (vect(1, i) - vect(4, i));
        Real v_4 = bet2 * (as * (vect(0, i) + vect(5, i)) -
                           af * (vect(2, i) + vect(3, i))) -
                   bet3 * s * sqrtd * (vect(4, i) + vect(1, i));
        Real v_5 = bet3 * (as * (vect(0, i) + vect(5, i)) -
                           af * (vect(2, i) + vect(3, i))) +
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        ...
        // Multiply row of R-eigenmatrix with vector using matrix elements from
        // eq. A3 Components of vect() are addressed directly as they are input
        // in permuted order
