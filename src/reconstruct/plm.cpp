...
//========================================================================================
//! \file plm.cpp
//! \brief  piecewise linear reconstruction for both uniform and non-uniform
//! meshes
...
#pragma omp simd
  ...
  // Apply simplified van Leer (VL) limiter expression for a Cartesian-like
  // coordinate with uniform mesh spacing
...
#pragma omp simd simdlen(SIMD_WIDTH)
    ...
    // Cartesian-like coordinate with nonuniform mesh spacing or for any
    // curvilinear coordinate spacing
  } else {
    for (int n = 0; n < NWAVE; ++n) {
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // curvature (may not exactly hold for nonuniform meshes, but converges
        // w/ smooth nonuniformity)
        Real cf =
            pco->dx1v(i) / (pco->x1f(i + 1) - pco->x1v(i)); // (Mignone eq 33)
        Real cb = pco->dx1v(i - 1) / (pco->x1v(i) - pco->x1f(i));
        // (modified) VL limiter (Mignone eq 37)
        // (dQ^F term from eq 31 pulled into eq 37, then multiply by
        // (dQ^F/dQ^F)^2)
        dwm(n, i) = (dq2 * (cf * dqB + cb * dqF) /
                     (SQR(dqB) + SQR(dqF) + dq2 * (cf + cb - 2.0)));
        if (dq2 <= 0.0)
          dwm(n, i) = 0.0; // ---> no concern for divide-by-0 in above line

        // Real v = dqB/dqF;
        // monotoniced central (MC) limiter (Mignone eq 38)
        // (std::min calls should avoid issue if divide-by-zero causes v=Inf)
        // dwm(n,i) = dqF*std::max(0.0, std::min(0.5*(1.0 + v), std::min(cf,
        // cb*v)));
      }
    }
  }
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int i = il; i <= iu; ++i) {
      wl(n, i + 1) =
          wc(n, i) +
          ((pco->x1f(i + 1) - pco->x1v(i)) / pco->dx1f(i)) * dwm(n, i);
      wr(n, i) =
          wc(n, i) - ((pco->x1v(i) - pco->x1f(i)) / pco->dx1f(i)) * dwm(n, i);
    }
  }
...
#pragma omp simd
    for (int i = il; i <= iu; ++i) {
      ...
      // TODO(felker): check if fused loop with NWAVE redundant application is
      // slower
...
#pragma omp simd simdlen(SIMD_WIDTH)
    ...
    Real dxF = pco->dx2f(j) /
               pco->dx2v(j); // dimensionless, not technically a dx quantity
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // dq2 > 0 ---> dqF, dqB are nonzero and have the same sign ----> no
        // risk for (dqF + dqB) = 0 cancellation causing a divide-by-0 in the
        // above line
        if (dq2 <= 0.0)
          dwm(n, i) = 0.0;
      }
