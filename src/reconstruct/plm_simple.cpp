...
//! meshes Operates on the entire nx4 range of a single AthenaArray<Real> input
//! (no MHD). No assumptions of hydrodynamic fluid variable input; no
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // dqm(n,i) = dqF*std::max(0.0, std::min(0.5*(1.0 + v), std::min(cf,
        // cb*v)));
      }
    }
  }
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int i = il; i <= iu; ++i) {
      ...
      ql(n, i + 1) =
          qc(n, i) +
          ((pco->x1f(i + 1) - pco->x1v(i)) / pco->dx1f(i)) * dqm(n, i);
      qr(n, i) =
          qc(n, i) - ((pco->x1v(i) - pco->x1f(i)) / pco->dx1f(i)) * dqm(n, i);
    }
  }
  return;
}
