...
//========================================================================================
//! \file ppm.cpp
//! \brief piecewise parabolic reconstruction with modified McCorquodale/Colella
//! limiter
//!        for a Cartesian-like coordinate with uniform spacing, Mignone
//!        modified original PPM limiter for nonuniform and/or curvilinear
//!        coordinate.
/// 1
//! REFERENCES:
...
//!                              const AthenaArray<Real> &w, const
//!                              AthenaArray<Real> &bcc, AthenaArray<Real> &wl,
//!                              AthenaArray<Real> &wr)
//! \brief Returns L/R interface values in X1-dir constructed using fourth-order
//! PPM and
...
void Reconstruction::PiecewiseParabolicX1(const int k, const int j,
  ...
  AthenaArray<Real> &bx = scr01_i_, &wc = scr1_ni_, &q_im2 = scr2_ni_,
                    &q_im1 = scr3_ni_, &q = scr4_ni_, &q_ip1 = scr5_ni_,
                    &q_ip2 = scr6_ni_, &qr_imh = scr7_ni_, &ql_iph = scr8_ni_;
  ...
  AthenaArray<Real> &d2qc_im1 = scr07_i_, &d2qc = scr08_i_,
                    &d2qc_ip1 = scr09_i_, &d2qf = scr10_i_;
...
#pragma omp simd
  ...
  // Note order of characteristic fields in output vect corresponds to
  // (IVX,IVY,IVZ)
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int i = il; i <= iu; ++i) {
      // nonuniform or uniform Cartesian-like coord reconstruction from volume
      // averages:
      if (!curvilinear[X1DIR]) {
        ...
        // Approximate interface average at i-1/2 and i+1/2 using PPM (CW
        // eq 1.6) KGF: group the biased stencil quantities to preserve FP
        // symmetry
        ...
        dph(i) = c1i(i) * q_im2(n, i) + c2i(i) * q_im1(n, i) +
                 c3i(i) * q(n, i) + c4i(i) * q_ip1(n, i);
        dph_ip1(i) = c1i(i + 1) * q_im1(n, i) + c2i(i + 1) * q(n, i) +
                     c3i(i + 1) * q_ip1(n, i) + c4i(i + 1) * q_ip2(n, i);
      }
    }
    ...
    if (uniform[X1DIR] && !curvilinear[X1DIR]) {
      // approximate second derivative at interfaces for smooth extrema
      // preservation
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu + 1; ++i) {
        ...
        d2qc(i) =
            q_im1(n, i) + q_ip1(n, i) - 2.0 * q(n, i); // (CD eq 85a) (no 1/2)
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= (iu + 1); ++i) {
        ...
        // KGF: add the off-centered quantities first to preserve FP symmetry
        Real qa =
            3.0 * (q(n, i) + q_ip1(n, i) - 2.0 * dph_ip1(i)); // (CD eq 85b)
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        // KGF: add the off-centered quantities first to preserve FP symmetry
        d2qf(i) =
            6.0 * (dph(i) + dph_ip1(i) - 2.0 * q(n, i)); // a6 coefficient * -2
      }

      //--- Step 2b.
      //---------------------------------------------------------------------
      // Nonuniform and/or curvilinear coordinate: apply strict monotonicity
      // constraints (Mignone eq 45)
    } else {
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
...
#pragma omp simd simdlen(SIMD_WIDTH)
      ...
      //--- Step 4b.
      //---------------------------------------------------------------------
      // Nonuniform and/or curvilinear coordinate: apply Mignone limiters to
      // parabolic interpolant. Note, the Mignone limiter does not check for
      // cell-averaged extrema:
    } else {
      for (int i = il; i <= iu; ++i) {
        ...
        if (qa <= 0.0) { // Local extrema detected
          ...
          // Overshoot i-1/2,R / i,(-) state
          if (std::abs(dqf_minus(i)) >=
              hplus_ratio_i(i) * std::abs(dqf_plus(i))) {
            qminus(i) = q(n, i) - hplus_ratio_i(i) * dqf_plus(i);
          }
          // Overshoot i+1/2,L / i,(+) state
          if (std::abs(dqf_plus(i)) >=
              hminus_ratio_i(i) * std::abs(dqf_minus(i))) {
    ...
    //--- Step 5.
    //------------------------------------------------------------------------
    // Convert limited cell-centered values to interface-centered L/R Riemann
    // states both L/R values defined over [il,iu]
#pragma omp simd
    for (int i = il; i <= iu; ++i) {
      ql_iph(n, i) = qplus(i);
      qr_imh(n, i) = qminus(i);
...
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    ...
    // TODO(felker): check that fused loop with NWAVE redundant application is
    // slower
...
//!                              AthenaArray<Real> &bcc, AthenaArray<Real> &wl,
//!                              AthenaArray<Real> &wr)
//! \brief Returns L/R interface values in X2-dir constructed using fourth-order
//! PPM and
//!        Colella-Sekora or Mignone limiting over [kl,ku][jl,ju][il,iu]
...
void Reconstruction::PiecewiseParabolicX2(const int k, const int j,
  ...
  // set work arrays used for primitive/characterstic cell-averages to scratch
  AthenaArray<Real> &bx = scr01_i_, &wc = scr1_ni_, &q_jm2 = scr2_ni_,
                    &q_jm1 = scr3_ni_, &q = scr4_ni_, &q_jp1 = scr5_ni_,
                    &q_jp2 = scr6_ni_, &qr_jmh = scr7_ni_, &ql_jph = scr8_ni_;

  // set work PPM work arrays to shallow copies of scratch arrays:
  AthenaArray<Real> &dd = scr02_i_, &dd_jm1 = scr03_i_, &dd_jp1 = scr04_i_,
                    &dph = scr05_i_, &dph_jp1 = scr06_i_;

  AthenaArray<Real> &d2qc_jm1 = scr07_i_, &d2qc = scr08_i_,
                    &d2qc_jp1 = scr09_i_, &d2qf = scr10_i_;
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int i = il; i <= iu; ++i) {
      ...
      if (!curvilinear[X2DIR]) {
        ...
        // Approximate interface average at j-1/2 and j+1/2 using PPM (CW
        // eq 1.6) KGF: group the biased stencil quantities to preserve FP
        // symmetry
        ...
        dph(i) = c1j(j) * q_jm2(n, i) + c2j(j) * q_jm1(n, i) +
                 c3j(j) * q(n, i) + c4j(j) * q_jp1(n, i);
        dph_jp1(i) = c1j(j + 1) * q_jm1(n, i) + c2j(j + 1) * q(n, i) +
                     c3j(j + 1) * q_jp1(n, i) + c4j(j + 1) * q_jp2(n, i);
      }
    }
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        d2qc(i) =
            q_jm1(n, i) + q_jp1(n, i) - 2.0 * q(n, i); // (CD eq 85a) (no 1/2)
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // KGF: add the off-centered quantities first to preserve FP symmetry
        Real qa =
            3.0 * (q(n, i) + q_jp1(n, i) - 2.0 * dph_jp1(i)); // (CD eq 85b)
        Real qb = d2qc(i);     // (CD eq 85a) (no 1/2)
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        // KGF: add the off-centered quantities first to preserve FP symmetry
        d2qf(i) =
            6.0 * (dph(i) + dph_jp1(i) - 2.0 * q(n, i)); // a6 coefficient * -2
      }
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        qb = std::max(std::max(std::abs(q(n, i)), std::abs(q_jp1(n, i))),
                      std::abs(q_jp2(n, i)));
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        if (qa <= 0.0) { // Local extrema detected
          ...
          // Overshoot j-1/2,R / j,(-) state
          if (std::abs(dqf_minus(i)) >=
              hplus_ratio_j(j) * std::abs(dqf_plus(i))) {
            qminus(i) = q(n, i) - hplus_ratio_j(j) * dqf_plus(i);
          }
          // Overshoot j+1/2,L / j,(+) state
          if (std::abs(dqf_plus(i)) >=
              hminus_ratio_j(j) * std::abs(dqf_minus(i))) {
...
//!                              AthenaArray<Real> &bcc, AthenaArray<Real> &wl,
//!                              AthenaArray<Real> &wr)
//! \brief Returns L/R interface values in X3-dir constructed using fourth-order
//! PPM and
//!        Colella-Sekora or Mignone limiting over [kl,ku][jl,ju][il,iu]
...
void Reconstruction::PiecewiseParabolicX3(const int k, const int j,
  ...
  // set work arrays used for primitive/characterstic cell-averages to scratch
  AthenaArray<Real> &bx = scr01_i_, &wc = scr1_ni_, &q_km2 = scr2_ni_,
                    &q_km1 = scr3_ni_, &q = scr4_ni_, &q_kp1 = scr5_ni_,
                    &q_kp2 = scr6_ni_, &qr_kmh = scr7_ni_, &ql_kph = scr8_ni_;
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        d2qc(i) =
            q_km1(n, i) + q_kp1(n, i) - 2.0 * q(n, i); // (CD eq 85a) (no 1/2)
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // KGF: add the off-centered quantities first to preserve FP symmetry
        Real qa =
            3.0 * (q(n, i) + q_kp1(n, i) - 2.0 * dph_kp1(i)); // (CD eq 85b)
        Real qb = d2qc(i);     // (CD eq 85a) (no 1/2)
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        // KGF: add the off-centered quantities first to preserve FP symmetry
        d2qf(i) =
            6.0 * (dph(i) + dph_kp1(i) - 2.0 * q(n, i)); // a6 coefficient * -2
      }
      //--- Step 2b.
      //---------------------------------------------------------------------
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        qb = std::max(std::max(std::abs(q(n, i)), std::abs(q_kp1(n, i))),
                      std::abs(q_kp2(n, i)));
      ...
      //--- Step 4b.
      //---------------------------------------------------------------------
      // Nonuniform coordinate spacing: apply Mignone limiters to parabolic
      // interpolant Note, the Mignone limiter does not check for cell-averaged
      // extrema:
    } else {
      for (int i = il; i <= iu; ++i) {
        Real qa = dqf_minus(i) * dqf_plus(i);
        if (qa <= 0.0) { // Local extrema detected
          ...
          // Overshoot k-1/2,R / k,(-) state
          if (std::abs(dqf_minus(i)) >=
              hplus_ratio_k(k) * std::abs(dqf_plus(i))) {
            qminus(i) = q(n, i) - hplus_ratio_k(k) * dqf_plus(i);
          }
          // Overshoot k+1/2,L / k,(+) state
          if (std::abs(dqf_plus(i)) >=
              hminus_ratio_k(k) * std::abs(dqf_minus(i))) {
