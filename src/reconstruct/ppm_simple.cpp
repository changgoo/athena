...
//!        coordinate.
//! Operates on the entire nx4 range of a single AthenaArray<Real> input (no
//! MHD). No assumptions of hydrodynamic fluid variable input; no characteristic
//! projection.
//!
//! REFERENCES:
...
void Reconstruction::PiecewiseParabolicX1(const int k, const int j,
  ...
  AthenaArray<Real> &q_im2 = scr2_ni_, &q_im1 = scr3_ni_, &q_i = scr4_ni_,
    ...
    for (int i = il; i <= iu; ++i) {
      ...
      if (!curvilinear[X1DIR]) {
        ...
        dph(i) = c1i(i) * q_im2(n, i) + c2i(i) * q_im1(n, i) +
                 c3i(i) * q_i(n, i) + c4i(i) * q_ip1(n, i);
        dph_ip1(i) = c1i(i + 1) * q_im1(n, i) + c2i(i + 1) * q_i(n, i) +
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu + 1; ++i) {
        ...
        d2qc(i) =
            q_im1(n, i) + q_ip1(n, i) - 2.0 * q_i(n, i); // (CD eq 85a) (no 1/2)
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= (iu + 1); ++i) {
        ...
        // KGF: add the off-centered quantities first to preserve FP symmetry
        Real qa =
            3.0 * (q_i(n, i) + q_ip1(n, i) - 2.0 * dph_ip1(i)); // (CD eq 85b)
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        // KGF: add the off-centered quantities first to preserve FP symmetry
        d2qf(i) = 6.0 * (dph(i) + dph_ip1(i) -
                         2.0 * q_i(n, i)); // a6 coefficient * -2
      }
...
#pragma omp simd
    for (int i = il; i <= iu; ++i) {
      dqf_minus(i) =
          q_i(n, i) - qminus(i); // (CS eq 25) = -dQ^- in Mignone's notation
...
void Reconstruction::PiecewiseParabolicX2(const int k, const int j,
  ...
  // set work arrays used for primitive/characterstic cell-averages to scratch
  AthenaArray<Real> &q_jm2 = scr2_ni_, &q_jm1 = scr3_ni_, &q_j = scr4_ni_,
    ...
    for (int i = il; i <= iu; ++i) {
      ...
      if (!curvilinear[X2DIR]) {
        ...
        dph(i) = c1j(j) * q_jm2(n, i) + c2j(j) * q_jm1(n, i) +
                 c3j(j) * q_j(n, i) + c4j(j) * q_jp1(n, i);
        dph_jp1(i) = c1j(j + 1) * q_jm1(n, i) + c2j(j + 1) * q_j(n, i) +
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        d2qc(i) =
            q_jm1(n, i) + q_jp1(n, i) - 2.0 * q_j(n, i); //(CD eq 85a) (no 1/2)
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // KGF: add the off-centered quantities first to preserve FP symmetry
        Real qa =
            3.0 * (q_j(n, i) + q_jp1(n, i) - 2.0 * dph_jp1(i)); // (CD eq 85b)
        Real qb = d2qc(i);     // (CD eq 85a) (no 1/2)
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        // KGF: add the off-centered quantities first to preserve FP symmetry
        d2qf(i) = 6.0 * (dph(i) + dph_jp1(i) -
                         2.0 * q_j(n, i)); // a6 coefficient * -2
      }
...
void Reconstruction::PiecewiseParabolicX3(const int k, const int j,
  ...
  // set work arrays used for primitive/characterstic cell-averages to scratch
  AthenaArray<Real> &q_km2 = scr2_ni_, &q_km1 = scr3_ni_, &q_k = scr4_ni_,
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        d2qc(i) =
            q_km1(n, i) + q_kp1(n, i) - 2.0 * q_k(n, i); // (CD eq 85a) (no 1/2)
...
#pragma omp simd simdlen(SIMD_WIDTH)
      for (int i = il; i <= iu; ++i) {
        ...
        // KGF: add the off-centered quantities first to preserve FP symmetry
        Real qa =
            3.0 * (q_k(n, i) + q_kp1(n, i) - 2.0 * dph_kp1(i)); // (CD eq 85b)
        Real qb = d2qc(i);     // (CD eq 85a) (no 1/2)
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        // KGF: add the off-centered quantities first to preserve FP symmetry
        d2qf(i) = 6.0 * (dph(i) + dph_kp1(i) -
                         2.0 * q_k(n, i)); // a6 coefficient * -2
      }
      //--- Step 2b.
      //---------------------------------------------------------------------
...
#pragma omp simd simdlen(SIMD_WIDTH)
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        if (qa <= 0.0) { // Local extrema detected
          ...
          // TODO(felker): could delete hplus_ratio_k() arrays for curvilinear
          // PPMx3 Overshoot k-1/2,R / k,(-) state
          if (std::abs(dqf_minus(i)) >=
