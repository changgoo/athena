...
namespace {
// TODO(felker): replace these hand-rolled linear algebra routines with a real
// library
...
Reconstruction::Reconstruction(MeshBlock *pmb, ParameterInput *pin)
  ...
  if (input_recon == "1") {
    ...
    // PPM approximates interfaces with 4th-order accurate stencils, but use
    // xorder=3 to denote that the overall scheme is "between 2nd and 4th" order
    // w/o flux terms
    xorder = 3;
  ...
  if (GENERAL_EOS && characteristic_projection) {
    std::stringstream msg;
    msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
        << "General EOS does not support characteristic reconstruction."
        << std::endl;
    ATHENA_ERROR(msg);
  }
  ...
  // check for necessary number of ghost zones for PPM w/o fourth-order flux
  // corrections
  if (xorder == 3) {
    int req_nghost = 3;
    if (NGHOST < req_nghost) {
      ...
      msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
          << "xorder=" << input_recon
          << " (PPM) reconstruction selected, but nghost=" << NGHOST
          << std::endl
          << "Reconfigure with --nghost=XXX with XXX > " << req_nghost - 1
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
  ...
  if (xorder == 4) {
    ...
    if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
      if (pmb->block_size.x1rat != 1.0 || pmb->block_size.x2rat != 1.0 ||
        ...
        msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
            ...
            << "Carteisan mesh with square cells. Rerun with uniform cell "
               "spacing "
            << std::endl
      ...
      // Note, probably want to make the following condition less strict (signal
      // warning for small differences due to floating-point issues) but upgrade
      // to error for large deviations from a square mesh. Currently signals a
      // warning for each MeshBlock with non-square cells.
      if ((pmb->block_size.nx2 > 1 && dx_i != dx_j) ||
          (pmb->block_size.nx3 > 1 && dx_j != dx_k)) {
        // It is possible for small floating-point differences to arise despite
        // equal analytic values for grid spacings in the coordinates.cpp
        // calculation of: Real
        // dx=(block_size.x1max-block_size.x1min)/(ie-is+1); due to the 3x
        // rounding operations in numerator, e.g. float(float(x1max) -
        // float((x1min)) if mesh/x1max != mesh/x2max, etc. and/or if an
        // asymmetric MeshBlock decomposition is used
        if (Globals::my_rank == 0) {
          // std::stringstream msg;
          std::cout << "### Warning in Reconstruction constructor" << std::endl
                    << "Selected time/xorder=" << input_recon
                    << " flux calculations"
                    << " require a uniform, Carteisan mesh with" << std::endl
                    << "square cells (dx1f=dx2f=dx3f). "
                    << "Change mesh limits and/or number of cells for equal "
                       "spacings\n"
                    << "Current values are:" << std::endl
                    << std::scientific
                    << std::setprecision(
                           std::numeric_limits<Real>::max_digits10 - 1)
                    << "dx1f=" << dx_i << std::endl
                    << "dx2f=" << dx_j << std::endl
                    << "dx3f=" << dx_k << std::endl;
          // ATHENA_ERROR(msg);
      ...
      msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
          ...
          << "is incompatible with selected time/xorder=" << input_recon
          << std::endl
    ...
    if (pmb->pmy_mesh->shear_periodic) {
      ...
      msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
          << "Selected time/xorder=" << input_recon << " flux calculations"
          << "currently does not support shearing box boundary conditions "
          << std::endl;
      ATHENA_ERROR(msg);
      return;
    }

    // check for necessary number of ghost zones for PPM w/ fourth-order flux
    // corrections
    int req_nghost = 4;
    // until new algorithm for face-averaged Field->bf to cell-averaged
    // Hydro->bcc conversion is added, NGHOST>=6
  ...
  // switch to secondary PLM and PPM variants for nonuniform and/or curvilinear
  // meshes
  ...
  if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
    // spherical_polar: x1=r and x2=theta require special treatment. x3=phi does
    // not
    curvilinear[X1DIR] = true;
  ...
  // TODO(c-white): use modified version of curvilinear PPM reconstruction
  // weights and limiter formulations for Schwarzschild, Kerr metrics instead of
  // Cartesian-like wghts
  ...
  if ((xorder == 3) || (xorder == 4)) {
    ...
    // Greedily allocate tiny 4x4 matrix + 4x1 vectors (RHS, solution, and
    // permutation indices) in case PPMx1 and/or PPMx2 require them for
    // computing the curvilinear coorddinate reconstruction weights. Same data
    // structures are reused at each spatial index (i or j) and for both PPMx1
    // and PPMx2 weight calculations:
    ...
    int permute[kNrows];
    int m_coord =
        0; // power of simple Jacobian term;  0 ---> Cartesian coordinates
    if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
      m_coord = 1;
    ...
    if (curvilinear[X1DIR]) {
        ...
        if (!uniform[X1DIR]) { // nonuniform spacing in curvilinear coordinate
          for (int row = 0; row < kNrows; row++) {
            ...
            for (int col = 0; col < kNcols; col++) {
              // Mignone equation 23: simplification of entries in \beta matrix
              // when Jacobian is a simple power of the coordinate, e.g. radial
              // coordinate:
              ...
              // TODO(kfelker): add NaN detection to matrix entries. They are
              // challenging to compute accurately w/ finite-precision
              // arithmetic. Both the numerator and denominator consist of
              // differences of nearby x1f values (that get closer the as
              // resolution increases) raised to the same power, where the
              // numerator has pow_num=3:6 and pow_denom=3 in spherical-polar
              // coordinates, e.g.
            }
          }

          if (DoolittleLUPDecompose(beta, 4, permute)) {
            // LUP decomposition succeeded; solve the linear system w/
            // forward/back sub:
            ...
            msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                << "Failure in LUP decomposition step for computing PPMx1 "
                   "curvilinear\n"
                << "reconstruction weights at cell index i=" << i
                << " for nx1=" << pmb->block_size.nx1 << std::endl;
            std::cout << msg.str();
            ATHENA_ERROR(msg);
          }
          // TODO(felker): probably need to change the following condition to
          // ensure that the ghost cells have negative radius. Add conditional
          // for r < 0
          if (i < pmb->is) {
            // cant simply take abs(r) for negative radius ghost cells when we
            // have to solve a matrix-vector system of equations to get the
            // coefficients, since that introduces a singular matrix. Highest
            // indexed ghost cell (with upper face at r=0) appear to have same
            // radial face locations (reversed) as the first real cell. instead,
          ...
          // Mignone section 2.2: conservative reconstruction from volume
          // averages
          Real io = std::abs(i - pmy_block_->is); // il=is-1 ---> io = 2
          // Notes:
          // - io (i offset) must be floating-point, not integer type. io^4 and
          // io^8 terms in below lines quickly cause overflows of 32-bit and
          // 64-bit integer limtis in the intermediate calculations of RHS
          // expressions
          // - io=1 should correspond to "is" (first real cell face)
          // - take absolute value to handle lower x1 ghost zone cell faces
          // properly
          if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
            ...
            // w_im1
            c1i(i) = -(2.0 * io - 3.0) *
                     (5.0 * SQR(io) * io + 8.0 * SQR(io) - 3.0 * io - 4.0) /
                     delta;
            // w_i
            c2i(i) = (2.0 * io - 1.0) *
                     (35.0 * SQR(io) * io + 24.0 * SQR(io) - 93.0 * io - 60.0) /
                     delta;
            // w_ip1
            c3i(i) = (2.0 * io + 1.0) *
                     (35.0 * SQR(io) * io - 24.0 * SQR(io) - 93.0 * io + 60.0) /
            ...
            Real delta = 36.0 * (15.0 * SQR(SQR(SQR(io))) -
                                 85.0 * SQR(SQR(io)) * SQR(io) +
                                 150.0 * SQR(SQR(io)) - 60.0 * SQR(io) + 16);
            c1i(i) = -(3.0 * SQR(io) - 9.0 * io + 7.0) *
                     (15.0 * SQR(SQR(io)) * SQR(io) + 48.0 * SQR(SQR(io)) * io +
                      23.0 * SQR(SQR(io)) - 48.0 * SQR(io) * io -
                      30.0 * SQR(io) + 16.0 * io + 12.0) /
                     delta;
            c2i(i) =
                (3.0 * SQR(io) - 3.0 * io + 1.0) *
          ...
          // (typical deviations of sum(wghts)!=1.0 are around machine
          // precision)
        } // end "uniform[X1DIR]"
      }   // end loop over i

      // Compute curvilinear geometric factors for limiter (Mignone eq 48):
      // radial direction in cylindrical and spherical-polar coordinates. Same
      // formulas for nonuniform and uniform radial mesh spacings.
      for (int i = (pmb->is) - 1; i <= (pmb->ie) + 1; ++i) {
        ...
        if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
          ...
          h_plus = 3.0 + (2.0 * dx_i * (10.0 * xv_i + dx_i)) /
                             (20.0 * SQR(xv_i) + SQR(dx_i));
          h_minus = 3.0 + (2.0 * dx_i * (-10.0 * xv_i + dx_i)) /
                              (20.0 * SQR(xv_i) + SQR(dx_i));
        }
...
#pragma omp simd
        for (int i = (pmb->is) - NGHOST; i <= (pmb->ie) + NGHOST; ++i) {
          // reducing general formula in ppm.cpp corresonds to Mignone eq B.4
          // weights:
          // (-1/12, 7/12, 7/12, -1/12)
          c1i(i) = 0.5;
          c2i(i) = 0.5;
      ...
      } else { // coeffcients along Cartesian-like x1 with nonuniform mesh
               // spacing
#pragma omp simd
      ...
      if (curvilinear[X2DIR]) {
        ...
        for (int j = (pmb->js) - NGHOST + 2; j <= (pmb->je) + NGHOST - 1; ++j) {
          ...
          for (int row = 0; row < kNrows; row++) {
            ...
            int s = row - kNrows / 2; // rescale index from -2 to +1
            Real coeff = 1.0 / (std::cos(pco->x2f(j + s)) -
                                std::cos(pco->x2f(j + s + 1)));
            for (int col = 0; col < kNcols; col++) {
              beta[col][row] = 0.0;
              for (int k = 0; k <= col; k++) {
                // Mignone equation 27:
          ...
          if (DoolittleLUPDecompose(beta, 4, permute)) {
            ...
            msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                << "Failure in LUP decomposition step for computing PPMx2 "
                   "curvilinear\n"
                << "reconstruction weights at cell index j=" << j
                << " for nx2=" << pmb->block_size.nx2 << std::endl;
            std::cout << msg.str();
            ATHENA_ERROR(msg);
          }
          // TODO(felker): possibly transform weights at theta <0 and/or theta
          // >pi
          c1j(j) = w_sol[0];
          c2j(j) = w_sol[1];
          c3j(j) = w_sol[2];
        ...
        for (int j = (pmb->js) - 1; j <= (pmb->je) + 1; ++j) {
          ...
          // if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
          Real h_plus, h_minus;
          // note: x2 may become negative at lower boundary ghost cells and may
          // exceedc pi at upper boundary ghost cells
          // TODO(felker): may need to take abs() or change signs of terms in
          // ghost zone
        ...
        for (int j = (pmb->js) - 1; j <= (pmb->je) + 1; ++j) {
          // h_plus = 3.0;
          // h_minus = 3.0;
          // Ratios are = 2 for Cartesian coords, as in the original PPM
          // limiter's overshoot conditions
          hplus_ratio_j(j) = 2.0;
...
#pragma omp simd
        ...
        } else { // coeffcients along Cartesian-like x2 with nonuniform mesh
                 // spacing
#pragma omp simd
          for (int j = (pmb->js) - NGHOST + 2; j <= (pmb->je) + NGHOST - 1;
               ++j) {
...
//! \brief perform LU decomposition with partial (row) pivoting using
//! Doolittle's algorithm. Partial pivoting is required for stability.
//!
//! Let D be a diagonal matrix, L be a unit lower triangular matrix (main
//! diagonal is all 1's), and U be a unit upper triangular matrix Crout = (LD)U
//! ---> unit upper triangular U and L'=LD non-unit lower triangular Doolittle =
//! L(DU) ---> unit lower triangular L and U'=DU non-unit upper triangular
//!
//! INPUT:
//!   - a: square nxn matrix A of real numbers. Must be a mutable
//!   pointer-to-pointer/rows.
//!   - n: number of rows and columns in "a", rows in pivot output
//!
//! Also expects "const Real lu_tol >=0" file-scope variable to be defined =
//! criterion for detecting degenerate input "a" (or nearly-degenerate).
//!
//! OUTPUT:
//!   - a: modified in-place to contain both lower- and upper-triangular
//!   matrices L, U
//!     as A <- L + U (the 1's on the diagonal of L are not stored) in the
//!     decomposition PA=LU. See NR pg 50; even though they claim to use Crout,
//!     they are probably use Doolittle. They assume unit diagonal in Lx=Pb
//!     forward substitution.
//!   - pivot: nx1 int vector that is a sparse representation of the nxn
//!   permutation
//!     matrix P. For each row/vector entry, the value = the column # of the
//!     nonzero pivot element
//!
//! RETURN:
//!  - failure=0: routine detected that "a" matrix was nearly-singular
//!  - success=1: LUP decomposition completed
//!
//! Both "a", "pivot" can then be passed with RHS vector "b" to
//! DoolittleLUPSolve in order to solve Ax=b system of linear equations
//!
//! REFERENCES:
//!   - Numerical Recipes, 3rd ed. (NR) section 2.3 "LU Decomposition & its
//!   Applications"
...
int DoolittleLUPDecompose(Real **a, int n, int *pivot) {
  ...
  for (int i = 0; i < n; i++)
    pivot[i] = i;
  // In our sparse representation, could let pivot be (n+1)x1 and init.
  // pivot[n]=n, increment for each pivot so that it equals n+s upon return, s=#
  // permutations Useful for determinant calculation.
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (a_max < lu_tol) { // 0.0) { // see NR comment in ludcmp.h
      ...
      std::cout << std::scientific
                ...
                << "pivot element=" << a_max << " < tolerance=" << lu_tol
                << std::endl;
      return failure;
    }
    ...
    // these lines are the only difference from Crout's in-place approach w/
    // pivoting
    for (int j = i + 1; j < n;
         j++) { // loop over rows; NR has the same approach as here
...
//! \brief after DoolittleLUPDecompose() function has transformed input the LHS
//! of Ax=b system to partially-row pivoted, LUP decomposed equivalent
//! PAx=LUx=Pb, solve for x
//!
//! INPUT:
//!  - lu: square nxn matrix of real numbers containing output "a" of successful
//!    DoolittleLUPDecompose() function call. See notes in that function for
//!    details.
//!  - pivot: nx1 vector of integers produced by DoolittleLUPDecompose()
//!  - b: RHS column vector of real numbers in original Ax=b system of linear
//!  equations
//!
//! OUTPUT:
//!  - x: nx1 column vector of real numbers containing solution in original Ax=b
//!  system

void DoolittleLUPSolve(Real **lu, int *pivot, Real *b, int n, Real *x) {
  // forward substitution, Ly=Pb (L must be a UNIT lower-triangular matrix)
  for (int i = 0; i < n; i++) {
    // initialize the solution to the RHS values, repeating permutation from LU
    // decomp.
    x[i] = b[pivot[i]];
    for (int j = 0; j < i; j++)
      x[i] -= lu[i][j] * x[j];
