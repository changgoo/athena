...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (iprob == 0) {
    ...
    for (int k = ks; k <= ke; k++) {
      for (int j = js; j <= je + 1; j++) {
        for (int i = is; i <= ie; i++) {
          ...
          pfield->b.x2f(k, j, i) =
              amp / std::pow(std::sqrt(4. * PI * eta_ohm * t0), 1.0) *
              std::exp(-(std::pow(x1 - x10, 2.)) / (4. * eta_ohm * t0));
        }
      }
    }
    ...
    if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
      ...
      for (int k = ks; k <= ke + 1; k++) {
        for (int j = js; j <= je; j++) {
          for (int i = is; i <= ie; i++) {
            ...
            x2 = pcoord->x2v(j);
            pfield->b.x3f(k, j, i) =
                amp / std::pow(std::sqrt(4. * PI * eta_ohm * t0), 2.0) *
      ...
      std::stringstream msg;
      msg << "### FATAL ERROR in resist.cpp ProblemGenerator" << std::endl
          << "2-d diffusion test only compatible with cartesian or" << std::endl
  ...
  if (f2) {
    ...
    for (int j = pmb->js - 1; j <= pmb->je + 1; j++) {
      for (int i = pmb->is - 1; i <= pmb->ie + 1; i++) {
        Real eps = std::sqrt(
            SQR(0.5 * (bcc(IB3, k, j, i + 1) - bcc(IB3, k, j, i - 1))) +
    ...
    for (int i = pmb->is - 1; i <= pmb->ie + 1; i++) {
      Real eps =
          std::sqrt(SQR(0.5 * (bcc(IB2, k, j, i + 1) - bcc(IB2, k, j, i - 1))));
      maxeps = std::max(maxeps, eps);
    }
  }
