...
//! \brief Advection of a field loop THROUGH the poles in spherical_polar
//! coordinates.
//!
//! Originally developed by ZZ.  Sets up constant uniform-density flow in
//! x-direction through poles, and follows advection of loop.  Set xz>0 (xz<0)
//! for loop through upper (lower) pole.  Works in 2D and 3D.
//========================================================================================
...
namespace {
void VelProfileCyl(const Real rad, const Real phi, const Real z, Real &v1,
                   Real &v2, Real &v3);
...
void LoopInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                 FaceField &b, Real time, Real dt, int il, int iu, int jl,
                 int ju, int kl, int ku, int ngh);
void LoopOuterX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                 FaceField &b, Real time, Real dt, int il, int iu, int jl,
                 int ju, int kl, int ku, int ngh);
void LoopInnerX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                 FaceField &b, Real time, Real dt, int il, int iu, int jl,
                 int ju, int kl, int ku, int ngh);
void LoopOuterX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                 FaceField &b, Real time, Real dt, int il, int iu, int jl,
                 int ju, int kl, int ku, int ngh);
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    for (int k = ks; k <= ke; ++k) {
      ...
      for (int j = jl; j <= ju; ++j) {
        ...
        for (int i = is; i <= ie; ++i) {
          pfield->b.x2f(k, j, i) =
              -1.0 * (len(i + 1) * a3(k, j, i + 1) - len(i) * a3(k, j, i)) /
              area(i);
        }
      }
    }
    for (int k = ks; k <= ke + 1; ++k) {
      for (int j = js; j <= je; ++j) {
        ...
        for (int i = is; i <= ie; ++i) {
          pfield->b.x3f(k, j, i) =
              (len(i + 1) * a2(k, j, i + 1) - len(i) * a2(k, j, i)) / area(i);
        }
      }
    }
    ...
    if (block_size.nx3 > 1) {
      for (int k = ks; k <= ke; ++k) {
        for (int j = js; j <= je; ++j) {
          ...
          for (int i = is; i <= ie + 1; ++i) {
            pfield->b.x1f(k, j, i) -=
                (len_p1(i) * a2(k + 1, j, i) - len(i) * a2(k, j, i)) / area(i);
          }
        }
      }
      for (int k = ks; k <= ke; ++k) {
  ...
  for (int k = ks; k <= ke; ++k) {
    for (int j = js; j <= je; ++j) {
      for (int i = is; i <= ie; ++i) {
        ...
        VelProfileCyl(pcoord->x1v(i), pcoord->x2v(j), pcoord->x3v(k), v1, v2,
                      v3);
        ...
        if (NON_BAROTROPIC_EOS) {
          phydro->u(IEN, k, j, i) =
              isocs2 * phydro->u(IDN, k, j, i) / (gamma_gas - 1.0);
          phydro->u(IEN, k, j, i) +=
              0.5 *
          ...
          if (MAGNETIC_FIELDS_ENABLED) {
            phydro->u(IEN, k, j, i) +=
                0.5 *
                (SQR(0.5 *
                     (pfield->b.x1f(k, j, i + 1) + pfield->b.x1f(k, j, i))) +
                 SQR(0.5 *
                     (pfield->b.x2f(k, j + 1, i) + pfield->b.x2f(k, j, i))) +
                 SQR(0.5 *
                     (pfield->b.x3f(k + 1, j, i) + pfield->b.x3f(k, j, i))));
          }
        }
      }
    }
...
void VelProfileCyl(const Real x1, const Real x2, const Real x3, Real &v1,
                   Real &v2, Real &v3) {
  ...
  for (int k = pmb->ks - 1; k <= pmb->ke + 1; k++) {
    for (int j = pmb->js - 1; j <= pmb->je + 1; j++) {
      for (int i = pmb->is - 1; i <= pmb->ie + 1; i++) {
        Real b = std::sqrt(SQR(bc(IB1, k, j, i)) + SQR(bc(IB2, k, j, i)) +
                           SQR(bc(IB3, k, j, i)));
        maxb = std::max(maxb, b);
      }
    }
