...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (pmy_mesh->particle && (rho0_star > 0.0)) {
    ...
    Real mpar =
        Surf * mesh_size.x1len * mesh_size.x2len / static_cast<Real>(npartot);
    ...
    //    std::cout << "[Problem IsoSheet] nparmax: " << ppar->nparmax_ << "
    //    npar: "
...
Real DeltaRho(MeshBlock *pmb, int iout) {
  ...
  if ((!pmb->pmy_mesh->particle || (rho0_star == 0)) && (iout == 1))
    return l1_err;
    ...
    for (int j = js; j <= je; ++j) {
      ...
      for (int i = is; i <= ie; ++i) {
        ...
        if (iout == 0) { // drho_gas
          ...
          std::stringstream msg;
          msg << "### FATAL ERROR in function [DeltaRho]" << std::endl
              << "iout: " << iout << "not allowed" << std::endl;
          ATHENA_ERROR(msg);
        }
