...
//!
//! The center of the grid is assumed to have coordinates (x1,x2) = [0,0]; the
//! grid initialization must be consistent with this
//!
    ...
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        ...
        if (rad <= r0) {
          ...
          // smooth solution between r0 and r1.  For no smoothing, set r1<0 in
          // input
          if (rad <= r1) {
