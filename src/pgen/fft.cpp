...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
...
#ifdef MPI_PARALLEL
  MPI_Allreduce(MPI_IN_PLACE, &zc_cpus, 1, MPI_ATHENA_REAL, MPI_MIN,
                MPI_COMM_WORLD);
#endif
  ...
  MPI_Allreduce(MPI_IN_PLACE, &err1, 1, MPI_ATHENA_REAL, MPI_SUM,
                MPI_COMM_WORLD);
  MPI_Allreduce(MPI_IN_PLACE, &err2, 1, MPI_ATHENA_REAL, MPI_SUM,
                MPI_COMM_WORLD);
#endif
  ...
  if (Globals::my_rank == 0) {
    ...
    std::cout << "====================================================="
              << std::endl;
    std::cout << "Error for Real: " << err1 << " Imaginary: " << err2
              << std::endl;
    std::cout << "====================================================="
              << std::endl;
    ...
    if ((pfile = std::fopen(fname.c_str(), "r")) != nullptr) {
      ...
      std::fprintf(pfile,
                   "# Nx1  Nx2  Nx3  Ncycle ZCS-min Real-Error Imag-Error\n");
    }
