...
//!
//! PURPOSE:  Problem generator for non-linear 3D MHD in shearing sheet. Based
//! on the
//!  initial conditions described in "Local Three-dimensional
//!  Magnetohydrodynamic Simulations of Accretion Disks" by Hawley, Gammie &
//!  Balbus, or HGB.
//!
//! Several different field configurations and perturbations are possible:
//!
//! - ifield = 1 - Bz=B0 sin(kx*x1) field with zero-net-flux [default] (kx
//! input)
//! - ifield = 2 - uniform Bz
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real kx =
      (TWO_PI / Lx) * (static_cast<Real>(nwx)); // nxw=-ve for leading wave
  ...
  if (ipert == 1) {
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      Real px_hat =
          -d0 * iso_cs * (ky * iso_cs * aa + 2.0 * Omega_0 * bb) * amp / denom;
      Real py_hat =
          d0 * (amp + ky * px_hat + (2.0 - qshear) * Omega_0 * rd_hat) / kx;
      for (int k = ks; k <= ke; k++) {
        for (int j = js; j <= je; j++) {
          for (int i = is; i <= ie; i++) {
            x1 = pcoord->x1v(i);
            ...
            // Initialize (d, M, P)
            rd = d0 * (1.0 +
                       rd_hat * std::cos(static_cast<Real>(kx * x1 + ky * x2)));
  ...
  // ifield = 2 - uniform Bz
  // ifield = 3 - B=(0,B0std::cos(kx*x1),B0std::sin(kx*x1))=zero-net flux w
  // helicity ifield = 4 - B=(0,B0/std::sqrt(2),B0/std::sqrt(2))= net
  // toroidal+vertical field ifield = 5 - uniform By
  if (ifield != 3) {
    if (ifield == 1) {
      for (int k = ks; k <= ke; k++) {
        for (int j = js; j <= je; j++) {
          for (int i = is; i <= ie; i++) {
            ...
            pfield->b.x3f(k, j, i) =
                B0 * (std::sin(static_cast<Real>(kx) * x1));
            if (i == ie)
              pfield->b.x1f(k, j, ie + 1) = 0.0;
            if (j == je)
              pfield->b.x2f(k, je + 1, i) = 0.0;
            if (k == ke)
              pfield->b.x3f(ke + 1, j, i) =
                  B0 * (std::sin(static_cast<Real>(kx) * x1));
          }
        }
      }
        ...
        for (int j = js; j <= je; j++) {
          for (int i = is; i <= ie; i++) {
            ...
            if (j == je)
              pfield->b.x2f(k, je + 1, i) = 0.0;
            if (k == ke)
              pfield->b.x3f(ke + 1, j, i) = B0 * dir_sgn;
          }
        }
      }
    } else if (ifield == 4) {
        ...
        for (int j = js; j <= je; j++) {
          for (int i = is; i <= ie; i++) {
            ...
            if (i == ie)
              pfield->b.x1f(k, j, ie + 1) = 0.0;
            if (j == je)
              pfield->b.x2f(k, je + 1, i) = B0 / std::sqrt(2);
            if (k == ke)
              pfield->b.x3f(ke + 1, j, i) = B0 / std::sqrt(2);
          }
        }
      }
    } else { // ifield = 5
        ...
        for (int j = js; j <= je; j++) {
          for (int i = is; i <= ie; i++) {
            ...
            if (i == ie)
              pfield->b.x1f(k, j, ie + 1) = 0.0;
            if (j == je)
              pfield->b.x2f(k, je + 1, i) = B0;
            if (k == ke)
              pfield->b.x3f(ke + 1, j, i) = 0.0;
          }
        }
      }
    }
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      ...
      for (int i = is; i <= ie; i++) {
        ...
        dvxvy += volume(i) * w(IDN, k, j, i) * w(IVX, k, j, i) *
                 (w(IVY, k, j, i) + vshear);
      }
    }
  }
