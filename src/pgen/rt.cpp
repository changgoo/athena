...
//!
//! Note the gravitational acceleration is hardwired to be 0.1. Density
//! difference is hardwired to be 2.0 in 2D, and is set by the input parameter
//! `problem/rhoh` in 3D (default value is 3.0). This reproduces 2D results of
//! Liska & Wendroff, 3D results of Dimonte et al.
//!
//! FOR 2D HYDRO:
...
//! Gravity acts in y-dirn. Special reflecting boundary conditions added in x2
//! to improve hydrostatic eqm (prevents launching of weak waves) Atwood number
//! A=(d2-d1)/(d2+d1)=1/3. Options:
//!    - iprob = 1  -- Perturb V2 using single mode
//!    - iprob != 1 -- Perturb V2 using multiple mode
//!
//! FOR 3D:
//! Problem domain should be -.05 < x < .05; -.05 < y < .05, -.1 < z < .1,
//! gamma=5/3 to match Dimonte et al.  Interface is at z=0; perturbation added
//! to Vz. Gravity acts in z-dirn. Special reflecting boundary conditions added
//! in x3.  A=1/2.  Options:
...
void ProjectPressureInnerX2(MeshBlock *pmb, Coordinates *pco,
                            AthenaArray<Real> &prim, FaceField &b, Real time,
                            Real dt, int il, int iu, int jl, int ju, int kl,
                            int ku, int ngh);
void ProjectPressureOuterX2(MeshBlock *pmb, Coordinates *pco,
                            AthenaArray<Real> &prim, FaceField &b, Real time,
                            Real dt, int il, int iu, int jl, int ju, int kl,
                            int ku, int ngh);
void ProjectPressureInnerX3(MeshBlock *pmb, Coordinates *pco,
                            AthenaArray<Real> &prim, FaceField &b, Real time,
                            Real dt, int il, int iu, int jl, int ju, int kl,
                            int ku, int ngh);
void ProjectPressureOuterX3(MeshBlock *pmb, Coordinates *pco,
                            AthenaArray<Real> &prim, FaceField &b, Real time,
                            Real dt, int il, int iu, int jl, int ju, int kl,
                            int ku, int ngh);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real kx =
      2.0 * (PI) / (pmy_mesh->mesh_size.x1max - pmy_mesh->mesh_size.x1min);
  Real ky =
      2.0 * (PI) / (pmy_mesh->mesh_size.x2max - pmy_mesh->mesh_size.x2min);
  Real kz =
      2.0 * (PI) / (pmy_mesh->mesh_size.x3max - pmy_mesh->mesh_size.x3min);
      ...
      for (int j = js; j <= je; j++) {
        for (int i = is; i <= ie; i++) {
          ...
          if (iprob == 1) {
            ...
            phydro->u(IM2, k, j, i) =
                (ran2(&iseed) - 0.5) * (1.0 + std::cos(ky * pcoord->x2v(j)));
          }
          ...
          if (NON_BAROTROPIC_EOS) {
            phydro->u(IEN, k, j, i) =
                (1.0 / gamma + grav_acc * den * (pcoord->x2v(j))) / gm1;
    ...
    for (int k = ks; k <= ke; k++) {
      for (int j = js; j <= je; j++) {
        for (int i = is; i <= ie; i++) {
          ...
          if (iprob == 1) {
            phydro->u(IM3, k, j, i) = (1.0 + std::cos(kx * (pcoord->x1v(i)))) /
                                      8.0 *
                                      (1.0 + std::cos(ky * pcoord->x2v(j))) *
          ...
          if (NON_BAROTROPIC_EOS) {
            phydro->u(IEN, k, j, i) =
                (1.0 / gamma + grav_acc * den * (pcoord->x3v(k))) / gm1;
...
//!                             AthenaArray<Real> &prim, FaceField &b, Real
//!                             time, Real dt, int il, int iu, int jl, int ju,
...
#pragma omp simd
        for (int i = il; i <= iu; ++i) {
          b.x2f(k, (ju + j + 1), i) =
              -b.x2f(k, (ju - j + 1), i); // reflect 2-field
        }
      }
    }
...
#pragma omp simd
          for (int i = il; i <= iu; ++i) {
            prim(IVZ, kl - k, j, i) =
                -prim(IVZ, kl + k - 1, j, i); // reflect 3-vel
          }
        } else if (n == (IPR)) {
#pragma omp simd
          for (int i = il; i <= iu; ++i) {
            prim(IPR, kl - k, j, i) = prim(IPR, kl + k - 1, j, i) -
                                      prim(IDN, kl + k - 1, j, i) * grav_acc *
                                          (2 * k - 1) * pco->dx3f(k);
          }
        } else {
#pragma omp simd
...
#pragma omp simd
          for (int i = il; i <= iu; ++i) {
            prim(IVZ, ku + k, j, i) =
                -prim(IVZ, ku - k + 1, j, i); // reflect 3-vel
          }
        } else if (n == (IPR)) {
#pragma omp simd
          for (int i = il; i <= iu; ++i) {
            prim(IPR, ku + k, j, i) = prim(IPR, ku - k + 1, j, i) +
                                      prim(IDN, ku - k + 1, j, i) * grav_acc *
                                          (2 * k - 1) * pco->dx3f(k);
          }
        } else {
#pragma omp simd
...
#pragma omp simd
        for (int i = il; i <= iu; ++i) {
          b.x3f((ku + k + 1), j, i) =
              -b.x3f((ku - k + 1), j, i); // reflect 3-field
        }
      }
    }
  }
  ...
  for (int k = pmb->ks - f3; k <= pmb->ke + f3; k++) {
    for (int j = pmb->js - f2; j <= pmb->je + f2; j++) {
      for (int i = pmb->is - 1; i <= pmb->ie + 1; i++) {
        if (w(IDN, k, j, i - 1) / w(IDN, k, j, i) > drmax)
          drmax = w(IDN, k, j, i - 1) / w(IDN, k, j, i);
        if (w(IDN, k, j, i + 1) / w(IDN, k, j, i) > drmax)
          drmax = w(IDN, k, j, i + 1) / w(IDN, k, j, i);
        if (w(IDN, k, j, i) / w(IDN, k, j, i - 1) > drmax)
          drmax = w(IDN, k, j, i) / w(IDN, k, j, i - 1);
        if (w(IDN, k, j, i) / w(IDN, k, j, i + 1) > drmax)
          drmax = w(IDN, k, j, i) / w(IDN, k, j, i + 1);
        if (f2) {
          if (w(IDN, k, j - 1, i) / w(IDN, k, j, i) > drmax)
            drmax = w(IDN, k, j - 1, i) / w(IDN, k, j, i);
          if (w(IDN, k, j + 1, i) / w(IDN, k, j, i) > drmax)
            drmax = w(IDN, k, j + 1, i) / w(IDN, k, j, i);
          if (w(IDN, k, j, i) / w(IDN, k, j - 1, i) > drmax)
            drmax = w(IDN, k, j, i) / w(IDN, k, j - 1, i);
          if (w(IDN, k, j, i) / w(IDN, k, j + 1, i) > drmax)
            drmax = w(IDN, k, j, i) / w(IDN, k, j + 1, i);
        }
        if (f3) {
          if (w(IDN, k - 1, j, i) / w(IDN, k, j, i) > drmax)
            drmax = w(IDN, k - 1, j, i) / w(IDN, k, j, i);
          if (w(IDN, k + 1, j, i) / w(IDN, k, j, i) > drmax)
            drmax = w(IDN, k + 1, j, i) / w(IDN, k, j, i);
          if (w(IDN, k, j, i) / w(IDN, k - 1, j, i) > drmax)
            drmax = w(IDN, k, j, i) / w(IDN, k - 1, j, i);
          if (w(IDN, k, j, i) / w(IDN, k + 1, j, i) > drmax)
            drmax = w(IDN, k, j, i) / w(IDN, k + 1, j, i);
        }
      }
