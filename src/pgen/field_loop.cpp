...
//!  -  problem/drat  = density ratio in loop, to test density advection and
//!  conduction
...
//!
//! REFERENCE: T. Gardiner & J.M. Stone, "An unsplit Godunov method for ideal
//! MHD via constrined transport", JCP, 205, 509 (2005)
//========================================================================================
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  // nxN != ncellsN, in general. Allocate to extend through 2*ghost, regardless
  // # dim
    ...
    for (int j = js; j <= je + 1; j++) {
      for (int i = is; i <= ie + 1; i++) {
        ...
        if (iprob == 1) {
          ax(k, j, i) = 0.0;
          ay(k, j, i) = 0.0;
          if ((SQR(pcoord->x1f(i) - x0) + SQR(pcoord->x2f(j) - y0)) <
              rad * rad) {
        ...
        if (iprob == 2) {
          if ((SQR(pcoord->x2f(j)) + SQR(pcoord->x3f(k))) < rad * rad) {
            ax(k, j, i) =
                amp *
                (rad - std::sqrt(SQR(pcoord->x2f(j)) + SQR(pcoord->x3f(k))));
          } else {
            ax(k, j, i) = 0.0;
          }
          ay(k, j, i) = 0.0;
        ...
        if (iprob == 3) {
          if ((SQR(pcoord->x1f(i)) + SQR(pcoord->x3f(k))) < rad * rad) {
            ay(k, j, i) =
                amp *
                (rad - std::sqrt(SQR(pcoord->x1f(i)) + SQR(pcoord->x3f(k))));
          } else {
            ay(k, j, i) = 0.0;
          }
          ax(k, j, i) = 0.0;
        ...
        // with a rotation about the x2-axis.  Define coordinate systems
        // (x1,x2,x3) and (x,y,z) with the following transformation rules:
        ...
        if (iprob == 5) {
          ...
          if ((SQR(pcoord->x1f(i)) + SQR(pcoord->x2v(j)) +
            ...
            ay(k, j, i) = amp * (rad - std::sqrt(SQR(pcoord->x1f(i)) +
                                                 SQR(pcoord->x2v(j)) +
                                                 SQR(pcoord->x3f(k))));
          } else {
            ay(k, j, i) = 0.0;
          ...
          if ((SQR(pcoord->x1f(i)) + SQR(pcoord->x2f(j)) +
            ...
            az(k, j, i) = amp * (rad - std::sqrt(SQR(pcoord->x1f(i)) +
                                                 SQR(pcoord->x2f(j)) +
                                                 SQR(pcoord->x3v(k))));
          } else {
            az(k, j, i) = 0.0;
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        ...
        phydro->u(IM1, k, j, i) =
            phydro->u(IDN, k, j, i) * vflow * x1size / diag;
        phydro->u(IM2, k, j, i) =
            phydro->u(IDN, k, j, i) * vflow * x2size / diag;
        phydro->u(IM3, k, j, i) =
            phydro->u(IDN, k, j, i) * vflow * x3size / diag;
        if ((SQR(pcoord->x1v(i)) + SQR(pcoord->x2v(j)) + SQR(pcoord->x3v(k))) <
            rad * rad) {
        ...
        if (pmy_mesh->shear_periodic) {
          ...
          if (!porb->orbital_advection_defined)
            phydro->u(IM2, k, j, i) -=
                qshear * omega0 * x1 * phydro->u(IDN, k, j, i);
        }
      }
    }
  }
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie + 1; i++) {
        pfield->b.x1f(k, j, i) =
            (az(k, j + 1, i) - az(k, j, i)) / pcoord->dx2f(j) -
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je + 1; j++) {
      for (int i = is; i <= ie; i++) {
        pfield->b.x2f(k, j, i) =
            (ax(k + 1, j, i) - ax(k, j, i)) / pcoord->dx3f(k) -
  ...
  for (int k = ks; k <= ke + 1; k++) {
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        pfield->b.x3f(k, j, i) =
            (ay(k, j, i + 1) - ay(k, j, i)) / pcoord->dx1f(i) -
