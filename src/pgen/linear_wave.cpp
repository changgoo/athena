...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  // primarily used for tests of decaying linear waves (might conditionally
  // enroll):
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
  ...
  for (int b = 0; b < nblocal; ++b) {
    ...
    for (int k = kl; k <= ku; k++) {
      for (int j = jl; j <= ju; j++) {
        for (int i = il; i <= iu; i++) {
          Real x = cos_a2 * (pmb->pcoord->x1v(i) * cos_a3 +
                             pmb->pcoord->x2v(j) * sin_a3) +
    ...
    if (pmb->precon->correct_err) {
      ...
      il = pmb->is, iu = pmb->ie, jl = pmb->js, ju = pmb->je, kl = pmb->ks,
      ku = pmb->ke;

      // Compute and store Laplacian of cell-centered conserved variables, Hydro
      // and Bcc
      AthenaArray<Real> delta_cons_;
      delta_cons_.NewAthenaArray(ncells4, pmb->ncells3, pmb->ncells2,
                                 pmb->ncells1);
      pmb->pcoord->Laplacian(cons_, delta_cons_, il, iu, jl, ju, kl, ku, nl,
                             nu);

      // TODO(felker): assuming uniform mesh with dx1f=dx2f=dx3f, so this
      // factors out
      ...
      for (int n = nl; n <= nu; ++n) {
        for (int k = kl; k <= ku; ++k) {
          for (int j = jl; j <= ju; ++j) {
            for (int i = il; i <= iu; ++i) {
              cons_(n, k, j, i) =
                  cons_(n, k, j, i) + C * delta_cons_(n, k, j, i);
            }
          }
        }
      }
  ...
  if (Globals::my_rank == 0) {
    MPI_Reduce(MPI_IN_PLACE, &l1_err, (NHYDRO + NFIELD), MPI_ATHENA_REAL,
               MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(MPI_IN_PLACE, &max_err, (NHYDRO + NFIELD), MPI_ATHENA_REAL,
               MPI_MAX, 0, MPI_COMM_WORLD);
  } else {
    MPI_Reduce(&l1_err, &l1_err, (NHYDRO + NFIELD), MPI_ATHENA_REAL, MPI_SUM, 0,
               MPI_COMM_WORLD);
    MPI_Reduce(&max_err, &max_err, (NHYDRO + NFIELD), MPI_ATHENA_REAL, MPI_MAX,
               0, MPI_COMM_WORLD);
  }
#endif
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    // nxN != ncellsN, in general. Allocate to extend through 2*ghost,
    // regardless # dim
        ...
        for (int j = js; j <= je + 1; j++) {
          for (int i = is; i <= ie + 1; i++) {
            if ((pbval->nblevel[1][0][1] > level && j == js) ||
                ...
                (pbval->nblevel[2][2][1] > level && j == je + 1 &&
                 k == ke + 1)) {
            ...
            if ((pbval->nblevel[1][1][0] > level && i == is) ||
                ...
                (pbval->nblevel[2][1][2] > level && i == ie + 1 &&
                 k == ke + 1)) {
            ...
            if ((pbval->nblevel[1][1][0] > level && i == is) ||
                ...
                (pbval->nblevel[1][2][2] > level && i == ie + 1 &&
                 j == je + 1)) {
...
void Eigensystem(const Real d, const Real v1, const Real v2, const Real v3,
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      Real bt, bt_star, bet2, bet3, bet2_star, bet3_star, bet_starsq, vbet,
          alpha_f, alpha_s;
      Real isqrtd, sqrtd, s, twid_a, qf, qs, af_prime, as_prime, afpbb, aspbb,
          vax;
      ...
      hp = h - (vaxsq + btsq / d);
      twid_asq =
          std::max((gm1 * (hp - 0.5 * vsq) - (gm1 - 1.0) * x), TINY_NUMBER);
      ...
      left_eigenmatrix[0][0] =
          alpha_f * (vsq - hp) + cff * (cf + v1) - qs * vqstr - aspb;
      ...
      left_eigenmatrix[2][0] =
          alpha_s * (vsq - hp) + css * (cs + v1) + qf * vqstr + afpb;
      ...
      Real bt, bt_star, bet2, bet3, bet2_star, bet3_star, bet_starsq, alpha_f,
          alpha_s;
...
int RefinementCondition(MeshBlock *pmb) {
  ...
  if (dmax - d0 > 0.9 * amp * rem[0][wave_flag])
    return 1;
