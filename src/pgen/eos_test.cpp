...
//!
//! Problem generator designed to test, debug, and verify EOS tables and
//! functions
//
//========================================================================================
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (GENERAL_EOS) {
    ...
    if (EOS_TABLE_ENABLED) {
      ...
      std::cout << "Using table '" << pin->GetString("hydro", "eos_file_name")
                << "'.\n";
      std::cout << "Shape (nVar, nEgas, nRho): " << ptable->nVar << ", "
                << ptable->nEgas << ", " << ptable->nRho << '\n';
      std::cout << "logEgasMin, logEgasMax: " << ptable->logEgasMin << ", "
                << ptable->logEgasMax << '\n';
      std::cout << "logRhoMin, logRhoMax: " << ptable->logRhoMin << ", "
                << ptable->logRhoMax << '\n';
      std::cout << "eUnit, rhoUnit, hUnit: " << ptable->eUnit << ", "
                << ptable->rhoUnit << ", " << ptable->hUnit << '\n';
      std::cout << "EosRatios: ";
      for (int i = 0; i < ptable->nVar; ++i)
        std::cout << ptable->EosRatios(i) << ", ";
      std::cout << '\n';
    }
  ...
  if (eos_loop) {
    ...
    while (rho > 0 && std::isfinite(rho) && egas > 0 && std::isfinite(egas)) {
      ...
      u(IDN, 0, 0, 0) = rho;
      u(IEN, 0, 0, 0) = egas;
      peos->ConservedToPrimitive(u, zeros, f, w, zeros, pcoord, 0, 0, 0, 0, 0,
                                 0);
