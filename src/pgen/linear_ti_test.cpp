...
//! \brief Problem generator for thermal instability test using TIGRESS classic
//! cooling function. A small perturbation is given to a uniform medium in
//! equilibrium but which is thermally unstable. We then track thr growth rate
...
static Real OmegaG(MeshBlock *pmb, const Real rho, const Real Press,
                   const Real k);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (cooling) {
    ...
    if (cooling_type.compare("enroll") == 0) {
      ...
      std::cout << "[Problem] Cooling solver is set to operator split"
                << std::endl;
    }
  } else {
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  while ((pgas_high - pgas_low) / pgas_mid > tol) {
    ...
    // << " "
    //           << tcool_low << " " << tcool_mid << " " << tcool_high <<
    //           std::endl;
  ...
  if (Globals::my_rank == 0) {
    std::cout << "============== Check Initialization ==============="
              << std::endl
              << " Input (nH, P/k, T) in cgs = " << rho_0 << " " << pgas_0
              << " " << T << std::endl
              << " nH to code den = " << pcf->nH_to_code_den << "  mu = " << mu
              << " muH = " << muH << std::endl;
  }
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      for (int i = il; i <= iu; ++i) {
        Real x = pcoord->x1v(i);
        Real pdev =
            -1 * alpha * rho_0 * (om / kx) * (om / kx) * std::cos(kx * x);
...
//========================================================================================
//! \fn static Real SolveCubic(const Real a, const Real b, const Real c, const
//! Real d) \brief solve a cubic equation \note
...
//! \brief Maximum over density as a history variable, used to track the growth
//! rate of
