...
//!
//! REFERENCE:
//! - Stone, J., Hawley, J., Gammie, C.F. & Balbus, S. A., ApJ 463, 656-673
//! (1996)
...
void StratOutflowInnerX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &a,
                         FaceField &b, Real time, Real dt, int il, int iu,
                         int jl, int ju, int kl, int ku, int ngh);
void StratOutflowOuterX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &a,
                         FaceField &b, Real time, Real dt, int il, int iu,
                         int jl, int ju, int kl, int ku, int ngh);
namespace {
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  // TODO(felker): tons of unused variables in this file: xmin, xmax, rbx, rby,
  // Ly, ky,...
  Real x1, x2, x3;
  // Real xmin, xmax;
  Real x1f, x2f, x3f;
  ...
  Real kx =
      (2.0 * PI / Lx) * (static_cast<Real>(nwx)); // nxw=-ve for leading wave
        ...
        if (MAGNETIC_FIELDS_ENABLED) {
          ...
          if (ifield == 2) {
            ...
            if (j == je)
              pfield->b.x2f(k, je + 1, i) = 0.0;
            if (k == ke)
              pfield->b.x3f(ke + 1, j, i) = B0;
          }
          if (ifield == 3) {
            pfield->b.x1f(k, j, i) = 0.0;
            pfield->b.x2f(k, j, i) = 0.0;
            pfield->b.x3f(k, j, i) =
                B0 * (1.0 + 0.5 * std::sin(static_cast<Real>(kx) * x1));
            if (i == ie)
              pfield->b.x1f(k, j, ie + 1) = 0.0;
            if (j == je)
              pfield->b.x2f(k, je + 1, i) = 0.0;
          ...
          if (ifield == 4) {
            pfield->b.x1f(k, j, i) = 0.0;
            pfield->b.x2f(k, j, i) =
                B0 * (std::cos(static_cast<Real>(kx) * x1));
            pfield->b.x3f(k, j, i) =
                B0 * (std::sin(static_cast<Real>(kx) * x1));
            if (i == ie)
              pfield->b.x1f(k, j, ie + 1) = 0.0;
            if (j == je)
              pfield->b.x2f(k, je + 1, i) =
                  B0 * (std::cos(static_cast<Real>(kx) * x1));
            if (k == ke)
              pfield->b.x3f(ke + 1, j, i) =
                  B0 * (std::sin(static_cast<Real>(kx) * x1));
          ...
          if (ifield == 6) {
            ...
            pfield->b.x1f(k, j, i) = 0.0;
            pfield->b.x2f(k, j, i) =
                std::sqrt(den * std::exp(-x3 * x3) * SQR(Omega_0) / beta);
            pfield->b.x3f(k, j, i) = 0.0;
            if (i == ie)
              pfield->b.x1f(k, j, ie + 1) = 0.0;
            if (j == je)
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) { return; }
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        if (NON_BAROTROPIC_EOS) {
          cons(IEN, k, j, i) -=
              dt * den * SQR(Omega_0) * prim(IVZ, k, j, i) * x3 * fsmooth;
        }
      }
    }
  }
  ...
  for (int k = 1; k <= ngh; k++) {
    for (int j = jl; j <= ju; j++) {
      for (int i = il; i <= iu; i++) {
        ...
        prim(IDN, kl - k, j, i) =
            den * std::exp(-(SQR(x3) - SQR(x3b)) / (2.0 * Tkl / SQR(Omega_0)));
  ...
  for (int k = 1; k <= ngh; k++) {
    for (int j = jl; j <= ju; j++) {
      for (int i = il; i <= iu; i++) {
        ...
        // assuming a constant temperature in the ghost zones
        prim(IDN, ku + k, j, i) =
            den * std::exp(-(SQR(x3) - SQR(x3b)) / (2.0 * Tku / SQR(Omega_0)));
        // Copy the velocities, but not the momenta ---
        // important because of the density extrapolation above
