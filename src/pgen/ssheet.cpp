...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (ipert == 3) {
    ...
    if (error_output) {
      ...
      EnrollUserHistoryOutput(1, Historyvxs, "vxs", UserHistoryOperation::sum);
    ...
    for (int j = jl; j <= ju; j++) {
      for (int i = il; i <= iu; i++) {
        ...
        if (ipert == 1) {
          ...
          std::stringstream msg;
          msg << "### FATAL ERROR in ssheet.cpp ProblemGenerator" << std::endl
              << "Shearing wave sheet ipert=" << ipert << " is unrecognized"
              << std::endl;
          ATHENA_ERROR(msg);
        }
        if (NON_BAROTROPIC_EOS) {
  ...
  if (flag) {
        ...
        if (pmb->block_size.nx3 == 1) {         // 2D
          ...
          for (int j = pmb->cjs; j <= pmb->cje; j++) {
            for (int i = pmb->cis; i <= pmb->cie; i++) {
              ...
              Real vx_vol =
                  vx00 * vol00 + vx01 * vol01 + vx10 * vol10 + vx11 * vol11;
              Real dvy_vol =
                  dvy00 * vol00 + dvy01 * vol01 + dvy10 * vol10 + dvy11 * vol11;
          ...
          for (int k = pmb->cks; k <= pmb->cke; k++) {
            for (int j = pmb->cjs; j <= pmb->cje; j++) {
              for (int i = pmb->cis; i <= pmb->cie; i++) {
                ...
                Real SN = std::sin(kx * pmb->pcoord->x1f(ii + 1) +
                                   ky * pmb->pcoord->x2f(jj + 1));
                Real vx_vol = vx000 * vol000 + vx001 * vol001 + vx010 * vol010 +
                              vx011 * vol011 + vx100 * vol100 + vx101 * vol101 +
    ...
    if (Globals::nranks > 1) {
      ...
      if (Globals::my_rank == 0) {
        MPI_Reduce(MPI_IN_PLACE, vxs.data(), ntot, MPI_ATHENA_REAL, MPI_SUM, 0,
                   MPI_COMM_WORLD);
      } else {
        MPI_Reduce(vxs.data(), vxs.data(), ntot, MPI_ATHENA_REAL, MPI_SUM, 0,
                   MPI_COMM_WORLD);
      }
      // dvys
      if (Globals::my_rank == 1) {
        MPI_Reduce(MPI_IN_PLACE, dvys.data(), ntot, MPI_ATHENA_REAL, MPI_SUM, 1,
                   MPI_COMM_WORLD);
      } else {
        MPI_Reduce(dvys.data(), dvys.data(), ntot, MPI_ATHENA_REAL, MPI_SUM, 1,
                   MPI_COMM_WORLD);
      }
    }
#endif
...
Real Historydvyc(MeshBlock *pmb, int iout) {
  ...
  Real dvy0 =
      iso_cs * amp * std::fabs(static_cast<Real>(nwy) / static_cast<Real>(nwx));
...
Real Historydvys(MeshBlock *pmb, int iout) {
  ...
  if (Globals::my_rank != exe_rank_dvy)
    return 0.0;
  if (pmb->lid != 0)
    return 0.0;
