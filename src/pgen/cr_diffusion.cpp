...
void Diffusion(MeshBlock *pmb, AthenaArray<Real> &u_cr, AthenaArray<Real> &prim,
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
    ...
    for (int k = ks; k <= ke; ++k) {
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie; ++i) {
          ...
          Real sol = exp(-40.0 * dist_sq / (4 * diff_coef * time * 40.0 + 1)) /
                     std::sqrt(4 * diff_coef * time * 40 + 1);
          sum_error += std::fabs(pmb->pcr->u_cr(CRE, k, j, i) - sol);
          num_cell++;
        } // end i
      }   // end j
    }     // end k
  ...
  if (Globals::my_rank == 0) {
    ...
    if ((pfile = fopen(fname.c_str(), "r")) != NULL) {
      ...
      fprintf(pfile,
              "# Nx1  Nx2  Nx3  direction vx  vy  vz  sigma  time  error");
      fprintf(pfile, "\n");
    }

    // write errors
    fprintf(pfile, "%d  %d  %d  %d", mesh_size.nx1, mesh_size.nx2,
            mesh_size.nx3, direction);
    ...
    for (int j = js; j <= je; ++j) {
      for (int i = is; i <= ie; ++i) {
        ...
        if (NON_BAROTROPIC_EOS) {
          phydro->u(IEN, k, j, i) =
              0.5 * (vx * vx + vy * vy + vz * vz) + 1.0 / (gamma - 1.0);
        }
        ...
        if (CR_ENABLED) {
          ...
          if (direction == 0) {
            pcr->u_cr(CRF1, k, j, i) =
                vx * 4.0 * exp(-40.0 * dist_sq) / (3.0 * pcr->vmax) +
                80 * x1 * exp(-40.0 * dist_sq) / pcr->sigma;
            pcr->u_cr(CRF2, k, j, i) = 0.0;
            pcr->u_cr(CRF3, k, j, i) = 0.0;
          } else if (direction == 1) {
            pcr->u_cr(CRF2, k, j, i) =
                vy * 4.0 * exp(-40.0 * dist_sq) / (3.0 * pcr->vmax) +
                80 * x2 * exp(-40.0 * dist_sq) / pcr->sigma;
            pcr->u_cr(CRF1, k, j, i) = 0.0;
            pcr->u_cr(CRF3, k, j, i) = 0.0;
          } else if (direction == 2) {
            pcr->u_cr(CRF3, k, j, i) =
                vz * 4.0 * exp(-40.0 * dist_sq) / (3.0 * pcr->vmax) +
                80 * x3 * exp(-40.0 * dist_sq) / pcr->sigma;
            pcr->u_cr(CRF1, k, j, i) = 0.0;
            pcr->u_cr(CRF2, k, j, i) = 0.0;
          }
        }
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    for (int k = kl; k <= ku; ++k) {
      for (int j = jl; j <= ju; ++j) {
        ...
        for (int i = il; i <= iu; ++i) {
          Real distance =
              0.5 * (pcr->cwidth(i - 1) + pcr->cwidth(i + 1)) + pcr->cwidth(i);
        ...
        if (pmb->block_size.nx2 > 1) {
          ...
          for (int i = il; i <= iu; ++i) {
            Real distance =
                0.5 * (pcr->cwidth1(i) + pcr->cwidth2(i)) + pcr->cwidth(i);
            Real dprdy =
                (u_cr(CRE, k, j + 1, i) - u_cr(CRE, k, j - 1, i)) / 3.0;
            dprdy /= distance;
        ...
        if (pmb->block_size.nx3 > 1) {
          ...
          for (int i = il; i <= iu; ++i) {
            Real distance =
                0.5 * (pcr->cwidth1(i) + pcr->cwidth2(i)) + pcr->cwidth(i);
            Real dprdz =
                (u_cr(CRE, k + 1, j, i) - u_cr(CRE, k - 1, j, i)) / 3.0;
            dprdz /= distance;
        ...
        // streaming velocity is calculated with respect to the current
        // coordinate
        //  system
          ...
          if (pcr->stream_flag) {
            ...
            if (va < TINY_NUMBER) {
              ...
              pcr->sigma_adv(0, k, j, i) =
                  std::fabs(pcr->b_grad_pc(k, j, i)) /
                  (std::sqrt(pb) * va * (1.0 + 1.0 / 3.0) * invlim *
                   u_cr(CRE, k, j, i));
            }
      ...
      for (int j = jl; j <= ju; ++j) {
        ...
        for (int i = il; i <= iu; ++i) {
          Real distance =
              0.5 * (pcr->cwidth(i - 1) + pcr->cwidth(i + 1)) + pcr->cwidth(i);
          Real grad_pr =
              (u_cr(CRE, k, j, i + 1) - u_cr(CRE, k, j, i - 1)) / 3.0;
          grad_pr /= distance;
