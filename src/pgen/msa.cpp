...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  scaleH = 1.0 / std::sqrt(TWO_PI *
                           nJ); // scale height for hyperbolic secant^2 profile
    ...
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        ...
        if (strat) {
          ...
          rd = amp * SQR(1.0 / std::cosh(x3 / scaleH)) *
               std::cos(kx * x1 + ky * x2);
        } else {
          den = d0;
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
  ...
  if ((tf != 2.0) || (Q != 2.0) || (nJ != 2.5) || (nwx != -3.0) || (nwy != 1) ||
      (amp != 1e-6)) {
    std::stringstream msg;
    msg << "### FATAL ERROR in msa.cpp ProblemGenerator" << std::endl
        << "The errors can be computed only with the following input "
           "parameters:"
        << std::endl
    ...
    for (int k = ks; k <= ke; k++) {
      for (int j = js; j <= je; j++) {
        for (int i = is; i <= ie; i++) {
          ...
          m2 = d1 * (-qshear * Omega0 * x1 +
                     2.0 * v2a * std::sin(kxt * x1 + ky * x2));
          m3 = 0.0;
          if (NON_BAROTROPIC_EOS) {
            // (background pressure) = (sound speed)^2*(background
            // density)/(Gamma) (perturbed pressure) = (sound
            // speed)^2*(perturbed density)
