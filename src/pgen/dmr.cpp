...
//!  Only works for genuinely 2D hydro problems in X1-X2 plane with adiabatic
//!  EOS.
//!
...
void DMRInnerX1(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                FaceField &b, Real time, Real dt, int il, int iu, int jl,
                int ju, int kl, int ku, int ngh);
void DMRInnerX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                FaceField &b, Real time, Real dt, int il, int iu, int jl,
                int ju, int kl, int ku, int ngh);
void DMROuterX2(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
                FaceField &b, Real time, Real dt, int il, int iu, int jl,
                int ju, int kl, int ku, int ngh);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (block_size.nx3 > 1) {
    msg << "### FATAL ERROR in Problem Generator" << std::endl
        << "nx3=" << block_size.nx3 << " but this test only works for 2D"
        << std::endl;
    ATHENA_ERROR(msg);
  }

  // Initialize shock using parameters defined in Woodward & Colella.  Note we
  // smooth the shock according to the volume fraction of the
  ...
  for (int j = js; j <= je; ++j) {
    for (int i = is; i <= ie; ++i) {
      ...
      Real shock_xpos_btm =
          0.1666666666 + pcoord->x2f(j) / std::sqrt(static_cast<Real>(3.0));
      Real shock_xpos_top =
          0.1666666666 + pcoord->x2f(j + 1) / std::sqrt(static_cast<Real>(3.0));
      ...
      if (pcoord->x1f(i) > shock_xpos_top) {
        ...
        Real fracl = 0.5 * std::sqrt(3.0) * dx * dx /
                     (pcoord->dx1f(i) * pcoord->dx2f(j));
        ...
        phydro->u(IEN, ks, j, i) +=
            0.5 *
            (SQR(phydro->u(IM1, ks, j, i)) + SQR(phydro->u(IM2, ks, j, i))) /
            phydro->u(IDN, ks, j, i);
...
//----------------------------------------------------------------------------------------
//! \fn void DMROuterX2()
//! \brief Sets TIME-DEPENDENT boundary condition on upper Y boundary (ojb) for
//! dmr test Quantaties at this boundary are held fixed at the downstream state
//! for x1 < 0.16666666+v1_shock*time, and at the upstream state for x1 >
//! 0.16666666+v1_shock*time
    ...
    for (int i = pmb->is; i <= pmb->ie; i++) {
      Real epsr = (std::abs(w(IDN, k, j, i + 1) - 2.0 * w(IDN, k, j, i) +
                   ...
                   std::abs(w(IDN, k, j + 1, i) - 2.0 * w(IDN, k, j, i) +
                            w(IDN, k, j - 1, i))) /
                  w(IDN, k, j, i);
      Real epsp = (std::abs(w(IPR, k, j, i + 1) - 2.0 * w(IPR, k, j, i) +
                            w(IPR, k, j, i - 1)) +
