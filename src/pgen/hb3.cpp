...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (ShBoxCoord != 2) {
    std::stringstream msg;
    msg << "### FATAL ERROR in hb3.cpp ProblemGenerator" << std::endl
        << "Shearing sheet only works for x-z plane with shboxcoord=2"
        << std::endl
  ...
  std::cout << "[hb3.cpp]: [Lx,Lz,Ly] = [" << x1size << "," << x2size << ","
            << x3size << "]" << std::endl;
    ...
    for (int i = is; i <= ie; i++) {
      ...
      if (MAGNETIC_FIELDS_ENABLED) {
        if (ifield == 1) {
          ...
          if (i == ie)
            pfield->b.x1f(ks, j, ie + 1) = 0.0;
          if (j == je)
            pfield->b.x2f(ks, je + 1, i) =
          ...
          if (i == ie)
            pfield->b.x1f(ks, j, ie + 1) = 0.0;
          if (j == je)
            pfield->b.x2f(ks, je + 1, i) = B0;
        } else {
          std::stringstream msg;
          msg << "### FATAL ERROR in hb3.cpp ProblemGenerator" << std::endl
              << "Shearing sheet ifield=" << ifield << " is unrecognized."
              << std::endl;
          ATHENA_ERROR(msg);
        }
        if (NON_BAROTROPIC_EOS) {
          phydro->u(IEN, ks, j, i) +=
              0.5 * (SQR(0.5 * (pfield->b.x1f(ks, j, i) +
                                pfield->b.x1f(ks, j, i + 1))) +
                     SQR(0.5 * (pfield->b.x2f(ks, j, i) +
                                pfield->b.x2f(ks, j + 1, i))) +
