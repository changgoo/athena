...
//!
//! PURPOSE:  Problem generator for linear MHD in shearing sheet. Based on the
//! initial
//!   conditions described in "ORBITAL ADVECTION BY INTERPOLATION: A FAST AND
//!   ACCURATE NUMERICAL SCHEME FOR SUPER-FAST MHD FLOWS" by Johnson, Guan, &
//!   Gammie, or JGG.
//!
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (error_output) {
    if (ipert == 1) {
      ...
      ruser_mesh_data[0].NewAthenaArray(mesh_size.nx3, mesh_size.nx2,
                                        mesh_size.nx1);
      ...
      if (Globals::my_rank == 0 ||
          (Globals::nranks > 1 && Globals::my_rank == 1)) {
      ...
      ruser_mesh_data[0].NewAthenaArray(mesh_size.nx3, mesh_size.nx2,
                                        mesh_size.nx1);
      ruser_mesh_data[1].NewAthenaArray(mesh_size.nx3, mesh_size.nx2,
                                        mesh_size.nx1);
  ...
  if (ipert != 1 && ipert != 2) {
    std::stringstream msg;
    msg << "### FATAL ERROR in jgg.cpp ProblemGenerator" << std::endl
        << "Parameter problem/ipert should be chosen from 1  or 2."
        << std::endl;
    ATHENA_ERROR(msg);
  }
  return;
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (porb->shboxcoord != 1) {
    std::stringstream msg;
    msg << "### FATAL ERROR in jgg.cpp ProblemGenerator" << std::endl
        << "This problem generator requires shearing box in x-y plane."
        << std::endl;
    ATHENA_ERROR(msg);
  }
  ...
  if (ipert == 1) {
    ...
    for (int k = ks; k <= ke; k++) {
      for (int j = js; j <= je; j++) {
        for (int i = is; i <= ie + 1; i++) {
          pfield->b.x1f(k, j, i) =
              rbx + (raz(k, j + 1, i) - raz(k, j, i)) / pcoord->dx2f(j) -
    ...
    for (int k = ks; k <= ke; k++) {
      for (int j = js; j <= je + 1; j++) {
        for (int i = is; i <= ie; i++) {
          pfield->b.x2f(k, j, i) =
              rby + (rax(k + 1, j, i) - rax(k, j, i)) / pcoord->dx3f(k) -
    ...
    for (int k = ks; k <= ke + 1; k++) {
      for (int j = js; j <= je; j++) {
        for (int i = is; i <= ie; i++) {
          pfield->b.x3f(k, j, i) =
              rbz + (ray(k, j, i + 1) - ray(k, j, i)) / pcoord->dx1f(i) -
        ...
        for (int k = pmb->ks; k <= pmb->ke; k++) {
          for (int j = pmb->js; j <= pmb->je; j++) {
            for (int i = pmb->is; i <= pmb->ie; i++) {
              int ti = static_cast<int>(loc.lx1) * pmb->block_size.nx1 +
                       (i - pmb->is);
              int tj = static_cast<int>(loc.lx2) * pmb->block_size.nx2 +
                       (j - pmb->js);
              int tk = static_cast<int>(loc.lx3) * pmb->block_size.nx3 +
                       (k - pmb->ks);
              if (ipert == 1) {
                ...
                if (mesh_size.nx3 > 1) { // 3D
                  ruser_mesh_data[0](tk, tj, ti) =
                      2.0 * pmb->pfield->bcc(IB3, k, j, i) * vol * SN;
                } else { // 2D
                  ruser_mesh_data[0](tk, tj, ti) =
                      2.0 * pmb->pfield->bcc(IB1, k, j, i) * vol * SN;
                }
              } else { // ipert == 2
                ruser_mesh_data[0](tk, tj, ti) =
                    (pmb->pfield->bcc(IB1, k, j, i) - rbx) / dbx0;
                ruser_mesh_data[1](tk, tj, ti) =
                    (pmb->pfield->bcc(IB2, k, j, i) - rby) / dby0;
              }
            }
          }
        }
        ...
        if (pmb->block_size.nx3 == 1) {         // 2D
          ...
          for (int j = pmb->cjs; j <= pmb->cje; j++) {
            for (int i = pmb->cis; i <= pmb->cie; i++) {
              ...
              int kk = k;
              int ti =
                  static_cast<int>(loc.lx1 >> 1) * pmb->block_size.nx1 +
                  static_cast<int>(loc.lx1 % 2) * (pmb->block_size.nx1 / 2) +
                  (i - pmb->cis);
              int tj =
                  static_cast<int>(loc.lx2 >> 1) * pmb->block_size.nx2 +
              ...
              if (ipert == 1) {
                ...
                Real bx_vol =
                    bx00 * vol00 + bx01 * vol01 + bx10 * vol10 + bx11 * vol11;
                ...
                ruser_mesh_data[0](tk, tj, ti) =
                    0.25 * (dbx00 + dbx01 + dbx10 + dbx11) / dbx0;
                ...
                ruser_mesh_data[1](tk, tj, ti) =
                    0.25 * (dby00 + dby01 + dby10 + dby11) / dby0;
              }
            }
          }
        } else { // 3D
            ...
            for (int j = pmb->cjs; j <= pmb->cje; j++) {
              for (int i = pmb->cis; i <= pmb->cie; i++) {
                ...
                int tj =
                    ...
                    (j - pmb->cjs);
                int tk =
                    static_cast<int>(loc.lx3 >> 1) * pmb->block_size.nx3 +
                ...
                if (ipert == 1) {
                  ...
                  Real vol111 =
                      pmb->pcoord->GetCellVolume(kk + 1, jj + 1, ii + 1);
                  Real SN = std::sin(kx * pmb->pcoord->x1f(ii + 1) +
                                     ky * pmb->pcoord->x2f(jj + 1) +
                  ...
                  Real dbx111 =
                      pmb->pfield->bcc(IB1, kk + 1, jj + 1, ii + 1) - rbx;
                  ruser_mesh_data[0](tk, tj, ti) =
                      0.125 *
                      (dbx000 + dbx001 + dbx010 + dbx011 + dbx100 + dbx101 +
                       dbx110 + dbx111) /
                      dbx0;
                  ...
                  Real dby111 =
                      pmb->pfield->bcc(IB2, kk + 1, jj + 1, ii + 1) - rby;
                  ruser_mesh_data[1](tk, tj, ti) =
                      0.125 *
                      (dby000 + dby001 + dby010 + dby011 + dby100 + dby101 +
                       dby110 + dby111) /
                      dby0;
                }
              }
        ...
        for (int k = 0; k < mesh_size.nx3; k++) {
          for (int i = 0; i < mesh_size.nx1; i++) {
            ...
            for (int j = 0; j < mesh_size.nx2 / 2; j++) {
              if (j == abs_nwy) {
                amp_wave =
                    std::sqrt(SQR(fft_data[j][0]) + SQR(fft_data[j][1])) *
                    inv_half_nx2;
              } else {
                amp_others +=
                    std::sqrt(SQR(fft_data[j][0]) + SQR(fft_data[j][1])) *
...
Real HistoryBxc(MeshBlock *pmb, int iout) {
  ...
  Real kx =
      (TWO_PI / Lx) * (static_cast<Real>(nwx) + qom * pmb->pmy_mesh->time);
  Real ky = (TWO_PI / Ly) * (static_cast<Real>(nwy));
  Real kz = (TWO_PI / Lz) * (static_cast<Real>(nwz));
