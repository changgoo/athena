...
//! finite volume methods in cylindrical and spherical coordinates. Journal of
//! Computational Physics, 270, 784-814.
//========================================================================================
...
// pointwise analytic exact solution at any t >=0 for a linear x1 velocity
// profile
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
  ...
  if (NSCALARS > 0) { // TODO(felker): error-out at compile time if NSCALARS < 1
    ...
    // (temp. workaround for zero-length array prohibition from ISO C++; replace
    ...
    // recalculate initial condition from ProblemGenerator on final Mesh
    // configuration: (may have changed due to AMR)
            ...
            for (int i = il; i <= iu; i++) {
              ...
              if (iprob == 1) {
                if (use_gl_quadrature) {
                  ...
                  // GL implementation returns total integral, not ave. Divide
                  // by delta_r
                  Real cell_quad =
                      GaussLegendre::integrate(N_gl, IntegrandFinal, xl, xu);
                  cell_ave = cell_quad / vol(i);
                  // assuming that the Gaussian profile is 1D in radial
                  // coordinate to pull out 2x integrals from the triple volume
                  // integral
                  if (std::strcmp(COORDINATE_SYSTEM, "cartesian") ==
                          0 || // dy*dz
                      std::strcmp(COORDINATE_SYSTEM, "cylindrical") ==
                          0) { // dz*dphi
                    cell_ave *= pmb->pcoord->dx2f(j);
                    cell_ave *= pmb->pcoord->dx3f(k);
                  } else { // if (std::strcmp(COORDINATE_SYSTEM,
                           // "spherical_polar") == 0)
                  ...
                  // Use standard midpoint approximation with cell-centered
                  // coords:
                ...
                if (use_gl_quadrature) {
                  ...
                  cell_ave *=
                      ONE_3RD *
                      (SQR(pmb->pcoord->x1f(i + 1)) * pmb->pcoord->x1f(i + 1) -
    ...
    if (Globals::my_rank == 0) {
      ...
      if ((pfile = std::fopen(fname.c_str(), "r")) != nullptr) {
        ...
        // The file does not exist -- open the file in write mode and add
        // headers
      } else {
  ...
  for (int k = ks; k <= ke; k++) {
    for (int j = js; j <= je; j++) {
      ...
      for (int i = is; i <= ie; i++) {
        ...
        //--- iprob=1
        if (iprob == 1) {
          // Note: GL quadrature is only ever used for the passive scalar
          // profile (even though it would be straightforward to compute the
          // analytic integral for the specific Gaussian profile) for generality
          // with any initial condition
          Real xl, xu;
          xl = pcoord->x1f(i);
          xu = pcoord->x1f(i + 1);
          phydro->u(IM2, k, j, i) = 0.0;
          // The cell-averaged linear velocity profile is always computed from
          // the analytic integral expression:
          if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0 ||   // dy*dz
              std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) { // dz*dphi
            phydro->u(IM1, k, j, i) =
                d0 * alpha * ONE_3RD *
                (pcoord->x1f(i + 1) * SQR(pcoord->x1f(i + 1)) -
                 pcoord->x1f(i) * SQR(pcoord->x1f(i)));
            phydro->u(IM1, k, j, i) *=
                pcoord->dx2f(j) * pcoord->dx3f(k) / vol(i);
          } else { // if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") ==
                   // 0)
            // sin(theta)*dtheta*dphi
            phydro->u(IM1, k, j, i) =
                d0 * alpha * 0.25 *
                (SQR(SQR(pcoord->x1f(i + 1))) - SQR(SQR(pcoord->x1f(i))));
            phydro->u(IM1, k, j, i) *= pcoord->dx3f(k) / vol(i);
            phydro->u(IM1, k, j, i) *=
                std::cos(pcoord->x2f(j)) - std::cos(pcoord->x2f(j + 1));
          }
          // vs. midpoint approximation for intiialization of linear velocity
          // profile: phydro->u(IM1,k,j,i) = d0*alpha*pcoord->x1v(i);

          if (use_gl_quadrature) {
            // Use Gauss-Legendre quadrature rules to compute cell-averaged
            // passive scalar initial condition based on the pointwise analytic
            // formula GL implementation returns total integral, not average.
            // Divide by delta_r
            Real cell_quad =
                GaussLegendre::integrate(N_gl, IntegrandInitial, xl, xu);
            cell_ave = cell_quad / vol(i);
            // assume that the Gaussian profile is 1D in radial coordinate to
            // pull out 2x integrals from the triple volume integral
            if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0 ||   // dy*dz
                std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) { // dz*dphi
          ...
          phydro->u(IM2, k, j, i) = d0 * alpha *
                                    (std::sin(xu) - xu * std::cos(xu) -
                                     (std::sin(xl) - xl * std::cos(xl)));
          phydro->u(IM2, k, j, i) *= pcoord->dx3f(k) / vol(i);
          phydro->u(IM2, k, j, i) *=
              0.25 * (SQR(SQR(pcoord->x1f(i + 1))) - SQR(SQR(pcoord->x1f(i))));
          // vs. midpoint approximation for intiialization of linear velocity
          // profile: (note, Mignone assumes r=1 for the 1D variant of this
          // problem; need to scale v_theta with radius in this 2D variant)
          ...
          if (use_gl_quadrature) {
            ...
            cell_ave *=
                ONE_3RD * (SQR(pcoord->x1f(i + 1)) * pcoord->x1f(i + 1) -
