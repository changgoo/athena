...
//!
//! Problem generator for shock tube (1-D Riemann) problems. Initializes
//! plane-parallel shock along x1 (in 1D, 2D, 3D), along x2 (in 2D, 3D), and
//! along x3 (in 3D).
//========================================================================================
...
//! \brief Calculate pressure as a function of density and temperature for H
//! EOS.
//========================================================================================
...
Real press(Real rho, Real T) {
  // Ionization fraction
  Real x =
      2. / (1 + std::sqrt(1 + 4. * rho * std::exp(1. / T) * std::pow(T, -1.5)));
  return rho * T * (1. + x);
}
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
      ...
      for (int j = pmb->js; j <= pmb->je; j++) {
        for (int i = pmb->is; i <= pmb->ie; i++) {
          ...
          if (r > xfp) {
            ...
            by = 5.0074 / std::sqrt(4.0 * PI);
            bz = 1.5517 / std::sqrt(4.0 * PI);
            e0 = 1.9317 / gm1 + 0.5 * ((mx * mx + my * my + mz * mz) / d0 +
                                       (bx * bx + by * by + bz * bz));
          } else if (r > xrm) {
            d0 = 1.4903;
            mx = 0.60588 * d0;
      ...
      for (int j = pmb->js; j <= pmb->je; j++) {
        for (int i = pmb->is; i <= pmb->ie; i++) {
          ...
          if (r > xs) {
            ...
            m0 = v0 * d0;
            e0 = (0.30313 * std::pow((1.0 + 0.20046 * (0.92745 - v0)), 7)) /
                     0.4 +
                 0.5 * d0 * v0 * v0;
          } else {
            d0 = 1.0;
  ...
  std::fprintf(pfile, "  %e  %e  %e  %e  %e", err[IDN], err[IM1], err[IM2],
               err[IM3], err[IEN]);
  if (MAGNETIC_FIELDS_ENABLED) {
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (shk_dir == 1 && (xshock < pmy_mesh->mesh_size.x1min ||
    ...
    msg << "### FATAL ERROR in Problem Generator" << std::endl
        << "xshock=" << xshock
        << " lies outside x1 domain for shkdir=" << shk_dir << std::endl;
    ATHENA_ERROR(msg);
  }
  if (shk_dir == 2 && (xshock < pmy_mesh->mesh_size.x2min ||
    ...
    msg << "### FATAL ERROR in Problem Generator" << std::endl
        << "xshock=" << xshock
        << " lies outside x2 domain for shkdir=" << shk_dir << std::endl;
    ATHENA_ERROR(msg);
  }
  if (shk_dir == 3 && (xshock < pmy_mesh->mesh_size.x3min ||
    ...
    msg << "### FATAL ERROR in Problem Generator" << std::endl
        << "xshock=" << xshock
        << " lies outside x3 domain for shkdir=" << shk_dir << std::endl;
    ATHENA_ERROR(msg);
  }
        ...
        for (int i = is; i <= ie; ++i) {
          if (pcoord->x1v(i) < xshock) {
            ...
            if (NON_BAROTROPIC_EOS) {
              ...
              phydro->u(IEN, k, j, i) +=
                  0.5 * wl[IDN] *
                  (wl[IVX] * wl[IVX] + wl[IVY] * wl[IVY] + wl[IVZ] * wl[IVZ]);
            }
          } else {
            ...
            if (NON_BAROTROPIC_EOS) {
              ...
              phydro->u(IEN, k, j, i) +=
                  0.5 * wr[IDN] *
                  (wr[IVX] * wr[IVX] + wr[IVY] * wr[IVY] + wr[IVZ] * wr[IVZ]);
            }
          }
        }
      }
    ...
    for (int n = 0; n < NSCALARS; ++n) {
      for (int k = ks; k <= ke; ++k) {
        for (int j = js; j <= je; ++j) {
          for (int i = is; i <= ie; ++i) {
            pscalars->s(n, k, j, i) =
                1.0 / scalar_norm * phydro->u(IDN, k, j, i);
          }
        }
      }
    }
