...
//! \brief Problem generator to study the propagation of cosmic rays in the
//! galactic environment reproduced in TIGRESS. MHD quantities are initialised
//! by reading in athena vtk files.
//======================================================================================
...
static int
    inj_cells; // parameter to calculate the distribution of injected CR energy
...
void TempCalculation(Units *punit, Real rho, Real Press, Real &Temp, Real &mu,
                     Real &muH);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (CR_ENABLED) {
    ...
    EnrollUserHistoryOutput(0, hst_Injected_CRenergy, "CRInjEn",
...
void CalculateInjectionRate(ParameterInput *pin, MeshBlock *pmb,
  ...
  pList.resize(Nstars);
  ierr = MPI_Bcast(&pList[0], Nstars * sizeof(struct StarParS), MPI_BYTE, 0,
                   MPI_COMM_WORLD);
#endif
  ...
  Real sigma =
      std::pow(inj_volume / (2. * PI), 1. / 3.) * std::pow(2 * PI, -1. / 6.);
  ...
  for (size_t s = 0; s < Nstars; ++s) {
    ...
    if (tstar < age_th && pList[s].m > 0.) {
      ...
      InjectionRate_in_code =
          InjectionRate / punit->Myr_in_code; // in code units
      tot_InjectionRate += InjectionRate_in_code;

      // Calculate the cell-centered grid position associated to each star
      // cluster
      ...
      Real grid_xs = static_cast<int>(pList[s].x1 / cell_x) * cell_x +
                     sign_xs * 0.5 * cell_x;
      Real grid_ys = static_cast<int>(pList[s].x2 / cell_y) * cell_y +
                     sign_ys * 0.5 * cell_y;
      Real grid_zs = static_cast<int>(pList[s].x3 / cell_z) * cell_z +
                     sign_zs * 0.5 * cell_z;
      ...
      for (int k = ks; k <= ke; ++k) {
        for (int j = js; j <= je; ++j) {
          for (int i = is; i <= ie; ++i) {
            ...
            Real R = std::sqrt(R2);
            CRInjectionRate(k, j, i) +=
                InjectionRate_in_code / inj_volume *
            ...
            if (pmb->pmy_mesh->mesh_bcs[BoundaryFace::inner_x1] ==
              ...
              if (grid_xs < 0)
                Rper1 =
                    std::pow(-2 * Bound_x1 + pmb->pcoord->x1v(i) - grid_xs, 2) +
                    std::pow(pmb->pcoord->x2v(j) - grid_ys, 2) +
                    std::pow(pmb->pcoord->x3v(k) - grid_zs, 2);
              if (grid_xs >= 0)
                Rper1 =
            ...
            if (pmb->pmy_mesh->mesh_bcs[BoundaryFace::inner_x2] ==
              ...
              if (grid_ys < 0)
                Rper2 =
                    std::pow(pmb->pcoord->x1v(i) - grid_xs, 2) +
...
//!
//! \note This Problem Generator can be used only to read vtk files
//! corresponding to times t that are integer multiples of 1/Omega, where Omega
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    for (int k = ks; k <= ke; ++k)
      for (int j = js; j <= je; ++j)
        for (int i = is; i <= ie + 1; ++i)
          pfield->b.x1f(k, j, i) =
              0.5 * (data_B(k - ks + gks, j - js + gjs, i - is + gis) +
    ...
    pfield->CalculateCellCenteredField(pfield->b, pfield->bcc, pcoord, is, ie,
                                       js, je, ks, ke);
  } // End MHD */
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    ...
    peos->PrimitiveToConserved(phydro->w, b, phydro->u, pcoord, is, ie, js, je,
                               ks, ke);
  }
  b.DeleteAthenaArray();
    ...
    for (int k = ks; k <= ke; ++k) {
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie; ++i) {
          ...
          Real v2 = phydro->w(IVX, k, j, i) * phydro->w(IVX, k, j, i) +
                    phydro->w(IVY, k, j, i) * phydro->w(IVY, k, j, i) +
                    phydro->w(IVZ, k, j, i) * phydro->w(IVZ, k, j, i);
...
static void read_vtk(MeshBlock *mb, std::string vtkdir, std::string vtkfile0,
                     std::string field, int component,
  ...
  const std::string athena_header =
      "# vtk DataFile Version 2.0"; // athena4.2 header
  const std::string athena_header3 =
      "# vtk DataFile Version 3.0"; // athena4.2 header
  ...
  if (SHOW_OUTPUT) {
    printf("Number of meshbloks %d %d %d \n", tigress_xmeshblocks,
           tigress_ymeshblocks, tigress_zmeshblocks);
  }
    ...
    for (int j = 0; j < tigress_ymeshblocks; ++j) {
      for (int i = 0; i < tigress_xmeshblocks; ++i) {
        ...
        if (k == 0)
          ...
          vtk_name =
              vtk_name0.substr(0, pos1) + "-" + id_str + vtk_name0.substr(pos1);
        ...
        while (true) {
          ...
          if (strcmp(type, "SCALARS") == 0) {
            ...
            if (strcmp(t_type, "LOOKUP_TABLE") != 0 ||
              ...
              msg << "### FATAL ERROR in Problem Generator [read_vtk]"
                  << std::endl
                  << "Expected \"LOOKUP_TABLE default, found " << t_type << " "
                  << t_format << std::endl;
              throw std::runtime_error(msg.str().c_str());
            }
            if (SHOW_OUTPUT) {
...
static void read_starpar_vtk(MeshBlock *mb, std::string filename,
  ...
  std::stringstream msg;
  FILE *fp = NULL;
  char cline[256], type[256], variable[256], format[256], t_type[256],
      t_format[256];
  ...
  if (SHOW_OUTPUT)
    std::cout << "POINTS" << nstars << std::endl;
  ...
  fscanf(fp, "CELLS %d %d\n", &itmp1, &itmp2);
  if (SHOW_OUTPUT)
    std::cout << "CELLS" << itmp1 << " " << itmp2 << std::endl;
  ...
  if (SHOW_OUTPUT)
    std::cout << "CELL_TYPES" << itmp1 << std::endl;
  if (itmp1 != nstars) {
    fclose(fp);
    msg << "### FATAL ERROR in Problem Generator [read_vtk]" << std::endl
  ...
  if (SHOW_OUTPUT)
    std::cout << "FIELD FieldData " << itmp1 << std::endl;
  if (itmp1 != 2) {
    fclose(fp);
    msg << "### FATAL ERROR in Problem Generator [read_vtk]" << std::endl
  ...
  if (SHOW_OUTPUT)
    std::cout << "CELL_DATA " << itmp1 << std::endl;
  if (itmp1 != nstars) {
    fclose(fp);
    msg << "### FATAL ERROR in Problem Generator [read_vtk]" << std::endl
  ...
  if (SHOW_OUTPUT)
    std::cout << "POINT_DATA " << itmp1 << std::endl;
  if (itmp1 != nstars) {
    fclose(fp);
    msg << "### FATAL ERROR in Problem Generator [read_vtk]" << std::endl
...
Real set_CR_Luminosity(Real tage) {
  ...
  Real SNR[Narray] = {
      ...
      0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00,
      0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00,
      0.00000000e+00, 4.75335226e-05, 5.52077439e-04, 5.22396189e-04,
      5.48276965e-04, 5.33334895e-04, 5.44502653e-04, 5.79428696e-04,
      5.58470195e-04, 5.94292159e-04, 6.10942025e-04, 5.98411595e-04,
      6.39734835e-04, 6.50129690e-04, 6.28058359e-04, 5.79428696e-04,
      5.22396189e-04, 5.06990708e-04, 4.72063041e-04, 4.77529274e-04,
      4.83058802e-04, 4.75335226e-04, 4.50816705e-04, 4.42588372e-04,
      4.34510224e-04, 4.27562886e-04, 4.19758984e-04, 4.13047502e-04,
      4.07380278e-04, 4.00866718e-04, 3.99024902e-04, 3.99024902e-04,
      3.94457302e-04, 3.89941987e-04, 3.85478358e-04, 3.81944271e-04,
      3.77572191e-04, 3.74110588e-04, 3.69828180e-04, 3.66437575e-04,
      3.63078055e-04, 3.59749335e-04, 3.55631319e-04, 3.52370871e-04,
      3.49945167e-04, 3.46736850e-04, 3.43557948e-04, 3.40408190e-04,
      3.37287309e-04, 3.34195040e-04, 3.31894458e-04, 3.26587832e-04,
      3.22849412e-04, 3.19889511e-04, 3.16956746e-04, 3.14774831e-04,
      3.11888958e-04, 3.09741930e-04, 3.06902199e-04, 3.04789499e-04,
      3.01995172e-04, 2.99916252e-04, 2.97851643e-04, 2.95801247e-04,
      2.93089325e-04, 2.91071712e-04, 2.89067988e-04, 2.87078058e-04,
      2.85101827e-04, 2.83139200e-04, 2.81190083e-04, 2.79254384e-04,
      2.77332010e-04, 2.75422870e-04, 2.73526873e-04, 2.71643927e-04,
      2.69773943e-04, 2.68534445e-04, 2.66685866e-04, 2.66072506e-04,
      2.64240876e-04, 2.63026799e-04, 2.61216135e-04, 2.60015956e-04,
      2.58226019e-04, 2.57039578e-04, 2.55270130e-04, 2.54097271e-04,
      2.52348077e-04, 2.51188643e-04, 2.50034536e-04, 2.48885732e-04,
      2.47172415e-04, 2.46036760e-04, 2.44906324e-04, 2.43220401e-04,
      2.42102905e-04, 2.40990543e-04, 2.39883292e-04, 2.38781128e-04,
      2.37137371e-04, 2.36047823e-04, 2.34963282e-04, 2.33883724e-04,
      2.32809126e-04, 2.31739465e-04, 2.30674719e-04, 2.29614865e-04,
      2.28034207e-04, 2.27509743e-04, 2.26464431e-04, 2.25423921e-04,
      2.24388192e-04, 2.23357222e-04, 2.22330989e-04, 2.21309471e-04,
      2.20292646e-04, 2.19280494e-04, 2.18272991e-04, 2.17270118e-04,
      2.16271852e-04, 2.15774441e-04, 2.14783047e-04, 2.13796209e-04,
      2.12813905e-04, 2.12324446e-04, 2.10862815e-04, 2.10377844e-04,
      2.09411246e-04, 2.08449088e-04, 2.07491352e-04, 2.07014135e-04,
      2.06062991e-04, 2.05116218e-04, 2.04644464e-04, 2.03704208e-04,
      2.02768272e-04, 2.02301918e-04, 2.01372425e-04, 2.00447203e-04,
      1.99986187e-04, 1.99067334e-04, 1.98609492e-04, 1.97696964e-04,
      1.90107828e-04, 1.89670592e-04, 1.88799135e-04, 1.87931682e-04,
      1.87068214e-04, 1.86208714e-04, 1.85353162e-04, 1.84926862e-04,
      1.83653834e-04, 1.83231442e-04, 1.82389570e-04, 1.81551566e-04,
      1.80717413e-04, 1.80301774e-04, 1.79473363e-04, 1.78648757e-04,
      1.78237877e-04, 1.77418948e-04, 1.76603782e-04, 1.75792361e-04,
      1.75388050e-04, 1.74582215e-04, 1.73780083e-04, 1.72981636e-04,
      1.72583789e-04, 1.71790839e-04, 1.71395731e-04, 1.70608239e-04,
      1.69824365e-04, 1.69044093e-04, 1.68655303e-04, 1.68267406e-04,
      1.67494288e-04, 1.66724721e-04, 1.65958691e-04, 1.65576996e-04,
      1.64816239e-04, 1.64437172e-04, 1.63681652e-04, 1.63305195e-04,
      1.62554876e-04,
  };
  ...
  if (SNR[iage] == SNR[iage + 1])
    return 0;

  // instanteneous rate of supernove per cluster mass
  N_SNe = lin_interpol(tage, iage * dage, (iage + 1) * dage, SNR[iage],
                       SNR[iage + 1]);
    ...
    for (int k = 1; k <= ngh; ++k) {
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie; ++i) {
          u_cr(CRE, ks - k, j, i) =
              u_cr(CRE, ks, j, i) -
              k * (u_cr(CRE, ks + 1, j, i) - u_cr(CRE, ks, j, i));
    ...
    for (int k = 1; k <= ngh; ++k) {
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie; ++i) {
          u_cr(CRE, ke + k, j, i) =
              u_cr(CRE, ke, j, i) +
              k * (u_cr(CRE, ke, j, i) - u_cr(CRE, ke - 1, j, i));
