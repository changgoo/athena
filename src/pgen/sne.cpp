...
void get_values_for_injection_region(Mesh *pm, Real data_vals[]) {
  ...
  for (int i=0; i<pm->nblocal; ++i) {
    ...
    if (pmb->pcoord->x1v(pmb->is) > rblast) continue;
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin) {
  if (kappa_flag > 0) {
    HydroDiffusion &hd = phydro->hdif;
    for(int k=ks; k<=ke; k++) {
      for(int j=js; j<=je; j++) {
        for(int i=is; i<=ie; i++) {
  		    Real kappa_primef = 0.5*(hd.kappa(HydroDiffusion::DiffProcess::iso,k,j,i) +
              hd.kappa(HydroDiffusion::DiffProcess::iso,k,j,i-1));
  ...
  for (int k=ks; k<=ke; ++k) {
    for (int j=js; j<=je; ++j) {
      for (int i=is; i<=ie; ++i) {
        ...
        phdif->kappa(HydroDiffusion::DiffProcess::iso,k,j,i) = kappa_eff; //  dimensions [mass]/([length]*[time])
      }
    }
  }
  return;
...
static Real cond_timestep(MeshBlock *pmb, int iout){
  ...
  Real min_dt = 1e20;
  HydroDiffusion &hd = pmb->phydro->hdif;

  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        min_dt = std::min(min_dt,
                          0.5*SQR(pmb->pcoord->dx1v(i))*pmb->phydro->u(IDN,k,j,i)/
                          hd.kappa(HydroDiffusion::DiffProcess::iso,k,j,i));
        }
      }
    }
