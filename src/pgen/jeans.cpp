...
//========================================================================================
//! \file jeans.cpp
//! \brief Linear wave problem generator for 1D/2D/3D problems including
//! self-gravity
//!
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  lambda = x1;
  if (mesh_size.nx2 > 1 && ang_3 != 0.0)
    lambda = std::min(lambda, x2);
  if (mesh_size.nx3 > 1 && ang_2 != 0.0)
  ...
  if (Globals::my_rank == 0) {
    ...
    std::cout << "angle2 " << ang_2 * 180. / PI << " " << sin_a2 << " "
              << cos_a2 << std::endl;
    std::cout << "angle3 " << ang_3 * 180. / PI << " " << sin_a3 << " "
              << cos_a3 << std::endl;
  }
  return;
}
  ...
  for (int k = ks; k <= ke; ++k) {
    for (int j = js; j <= je; ++j) {
      for (int i = is; i <= ie; ++i) {
        ...
        phydro->u(IDN, k, j, i) =
            d0 *
            (1.0 + amp * sinkx + amp * amp * std::sin(pcoord->x1v(i) * kwave));
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          phydro->u(IEN, k, j, i) +=
              0.5 * SQR(phydro->u(IM2, k, j, i)) / phydro->u(IDN, k, j, i);
          phydro->u(IEN, k, j, i) +=
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
  ...
  if (omega2 < 0) {
    if (Globals::my_rank == 0)
      std::cout << "This problem is Jeans unstable, njeans = " << njeans
                << std::endl;
    //    return;
  }
  ...
  for (int b = 0; b < nblocal; ++b) {
    ...
    for (int k = ks; k <= ke; ++k) {
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie; ++i) {
          Real x =
              cos_a2 * (pcoord->x1v(i) * cos_a3 + pcoord->x2v(j) * sin_a3) +
              pcoord->x3v(k) * sin_a2;
          ...
          max_err[IDN] = std::max(
              static_cast<Real>(std::abs(den - phydro->u(IDN, k, j, i))),
              max_err[IDN]);
          ...
          max_err[IM1] = std::max(
              static_cast<Real>(std::abs(m1 - phydro->u(IM1, k, j, i))),
              max_err[IM1]);
          max_err[IM2] = std::max(
              static_cast<Real>(std::abs(m2 - phydro->u(IM2, k, j, i))),
              max_err[IM2]);
          max_err[IM3] = std::max(
              static_cast<Real>(std::abs(m3 - phydro->u(IM3, k, j, i))),
              max_err[IM3]);
          if (NON_BAROTROPIC_EOS) {
            Real e0 =
                p0 * (1 + gam * amp * sinkx * cosot); /// gm1 + 0.5*m*m/den;
            l1_err[IEN] += std::abs(e0 - phydro->w(IEN, k, j, i));
            max_err[IEN] = std::max(
                static_cast<Real>(std::abs(e0 - phydro->w(IEN, k, j, i))),
                max_err[IEN]);
          }
        }
      }
