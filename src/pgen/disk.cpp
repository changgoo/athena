...
//! and spherical polar coordinates.  Initial conditions are in vertical
//! hydrostatic eqm.
...
namespace {
void GetCylCoord(Coordinates *pco, Real &rad, Real &phi, Real &z, int i, int j,
                 int k);
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k = ks; k <= ke; ++k) {
    ...
    for (int j = js; j <= je; ++j) {
      ...
      for (int i = is; i <= ie; ++i) {
        x1 = pcoord->x1v(i);
        GetCylCoord(pcoord, rad, phi, z, i, j,
                    k); // convert to cylindrical coordinates
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          phydro->u(IEN, k, j, i) =
              p_over_r * phydro->u(IDN, k, j, i) / (gamma_gas - 1.0);
          phydro->u(IEN, k, j, i) +=
              0.5 *
...
Real DenProfileCyl(const Real rad, const Real phi, const Real z) {
  ...
  Real dentem =
      denmid *
      std::exp(gm0 / p_over_r * (1. / std::sqrt(SQR(rad) + SQR(z)) - 1. / rad));
  den = dentem;
...
//----------------------------------------------------------------------------------------
//! User-defined boundary Conditions: sets solution in ghost zones to initial
//! values
    ...
    for (int k = kl; k <= ku; ++k) {
      for (int j = jl; j <= ju; ++j) {
        for (int i = 1; i <= ngh; ++i) {
          ...
          if (NON_BAROTROPIC_EOS)
            prim(IEN, k, j, il - i) =
                PoverR(rad, phi, z) * prim(IDN, k, j, il - i);
        }
      }
    }
  } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
    ...
    for (int k = kl; k <= ku; ++k) {
      for (int j = jl; j <= ju; ++j) {
        for (int i = 1; i <= ngh; ++i) {
          ...
          if (NON_BAROTROPIC_EOS)
            prim(IEN, k, j, iu + i) =
                PoverR(rad, phi, z) * prim(IDN, k, j, iu + i);
        }
      }
    }
  } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
    ...
    for (int k = kl; k <= ku; ++k) {
      for (int j = 1; j <= ngh; ++j) {
        for (int i = il; i <= iu; ++i) {
          ...
          if (NON_BAROTROPIC_EOS)
            prim(IEN, k, jl - j, i) =
                PoverR(rad, phi, z) * prim(IDN, k, jl - j, i);
        }
      }
    }
  } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
    ...
    for (int k = kl; k <= ku; ++k) {
      for (int j = 1; j <= ngh; ++j) {
        for (int i = il; i <= iu; ++i) {
          ...
          if (NON_BAROTROPIC_EOS)
            prim(IEN, k, ju + j, i) =
                PoverR(rad, phi, z) * prim(IDN, k, ju + j, i);
        }
      }
    }
  } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
    ...
    for (int k = 1; k <= ngh; ++k) {
      for (int j = jl; j <= ju; ++j) {
        for (int i = il; i <= iu; ++i) {
          ...
          if (NON_BAROTROPIC_EOS)
            prim(IEN, kl - k, j, i) =
                PoverR(rad, phi, z) * prim(IDN, kl - k, j, i);
        }
      }
    }
  } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
    ...
    for (int k = 1; k <= ngh; ++k) {
      for (int j = jl; j <= ju; ++j) {
        for (int i = il; i <= iu; ++i) {
          ...
          if (NON_BAROTROPIC_EOS)
            prim(IEN, ku + k, j, i) =
                PoverR(rad, phi, z) * prim(IDN, ku + k, j, i);
        }
      }
    }
  } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
