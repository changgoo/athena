...
//! \brief Problem generator for magnetized equatorial inflow around Kerr black
//! hole.
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  if (MAGNETIC_FIELDS_ENABLED) {
    pfield->CalculateCellCenteredField(pfield->b, bb, pcoord, is - NGHOST,
  ...
  peos->PrimitiveToConserved(phydro->w, bb, phydro->u, pcoord, is - NGHOST,
                             ie + NGHOST, js, je, ks, ke);
  return;
}
...
//   put,pur,puphi: values set to interpolated u^\mu in Boyer-Lindquist
//   coordinates pbt,pbr,pbphi: values set to interpolated b^\mu in
...
void CalculateFromTable(Real r, Real theta, Real *prho, Real *put, Real *pur,
  ...
  if (r < interp_values(0, 0)) {
    ...
    for (n = 0; n < num_lines - 1; ++n) {
      if (r < interp_values(0, n + 1)) {
        fraction = (r - interp_values(0, n)) /
                   (interp_values(0, n + 1) - interp_values(0, n));
        break;
      }
    }
  }
  ...
  *prho = (1.0 - fraction) * interp_values(1, n) +
          fraction * interp_values(1, n + 1);
  Real ur = (1.0 - fraction) * interp_values(2, n) +
            fraction * interp_values(2, n + 1);
  Real uphi = (1.0 - fraction) * interp_values(3, n) +
              fraction * interp_values(3, n + 1);
  Real bbr = (1.0 - fraction) * interp_values(4, n) +
             fraction * interp_values(4, n + 1);
  Real bbphi = (1.0 - fraction) * interp_values(5, n) +
               fraction * interp_values(5, n + 1);
  ...
  Real g_phiphi =
      (SQR(r) + SQR(a) + 2.0 * m * SQR(a) * r / sigma * SQR(sin)) * SQR(sin);
  Real var_a = g_tt;
