    ...
    for (int j = js; j <= je; j++) {
      for (int i = is; i <= ie; i++) {
        ...
        if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
          ...
          Real x = pcoord->x1v(i) * std::sin(pcoord->x2v(j)) *
                   std::cos(pcoord->x3v(k));
          Real y = pcoord->x1v(i) * std::sin(pcoord->x2v(j)) *
                   std::sin(pcoord->x3v(k));
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          if (rad < rout) {
            if (rad < rin) {
              ...
              Real f = (rad - rin) / (rout - rin);
              Real log_pres =
                  (1.0 - f) * std::log(prat * pa) + f * std::log(pa);
              pres = std::exp(log_pres);
            }
          }
          phydro->u(IEN, k, j, i) = pres / gm1;
          if (RELATIVISTIC_DYNAMICS) // this should only ever be SR with this
                                     // file
      ...
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie + 1; ++i) {
          if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
            ...
            pfield->b.x1f(k, j, i) = b0 * (std::cos(angle) * std::cos(phi) +
      ...
      for (int j = js; j <= je + 1; ++j) {
        for (int i = is; i <= ie; ++i) {
          if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
            ...
            Real phi = pcoord->x2v(j);
            pfield->b.x2f(k, j, i) = b0 * (std::sin(angle) * std::cos(phi) -
        ...
        for (int i = is; i <= ie; ++i) {
          if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0 ||
            ...
            pfield->b.x3f(k, j, i) = 0.0;
                   ...
                   // 0) {
            Real phi = pcoord->x3v(k);
            pfield->b.x3f(k, j, i) = b0 * (std::sin(angle) * std::cos(phi) -
                                           std::cos(angle) * std::sin(phi));
          }
        }
      }
...
void Mesh::UserWorkAfterLoop(ParameterInput *pin) {
  ...
  int ic, jc, kc;
  for (ic = is; ic <= ie; ic++)
    if (pmb->pcoord->x1f(ic) > x1_0)
      break;
  ic--;
  for (jc = pmb->js; jc <= pmb->je; jc++)
    if (pmb->pcoord->x2f(jc) > x2_0)
      break;
  jc--;
  for (kc = pmb->ks; kc <= pmb->ke; kc++)
    if (pmb->pcoord->x3f(kc) > x3_0)
      break;
  kc--;
...
int RefinementCondition(MeshBlock *pmb) {
  ...
  if (pmb->pmy_mesh->f3) {
    for (int k = pmb->ks - 1; k <= pmb->ke + 1; k++) {
      for (int j = pmb->js - 1; j <= pmb->je + 1; j++) {
        for (int i = pmb->is - 1; i <= pmb->ie + 1; i++) {
          Real eps =
              std::sqrt(
                  SQR(0.5 * (w(IPR, k, j, i + 1) - w(IPR, k, j, i - 1))) +
    ...
    for (int j = pmb->js - 1; j <= pmb->je + 1; j++) {
      for (int i = pmb->is - 1; i <= pmb->ie + 1; i++) {
        Real eps =
            std::sqrt(SQR(0.5 * (w(IPR, k, j, i + 1) - w(IPR, k, j, i - 1))) +
                      SQR(0.5 * (w(IPR, k, j + 1, i) - w(IPR, k, j - 1, i)))) /
            w(IPR, k, j, i);
        maxeps = std::max(maxeps, eps);
      }
