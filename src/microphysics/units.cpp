...
//! \brief default unit constructor from the parameter input (default is for ISM
//! problems)
...
Units::Units(Real dunit, Real lunit, Real vunit)
    : Density(dunit), Length(lunit), Velocity(vunit) {
  SetUnitsConstants();
}
...
void Units::SetUnitsConstants() {
  ...
  aR_in_code =
      Constants::aR * erg / (cm * cm * cm * Kelvin * Kelvin * Kelvin * Kelvin);
...
void Units::PrintConstantsInCodeUnits() {
  std::cout << std::scientific
            << "===== Constants  in Code Units =====" << std::endl;
