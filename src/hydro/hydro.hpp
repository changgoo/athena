...
// TODO(felker): consider adding a struct FaceFlux w/ overloaded ctor in
// athena.hpp, or: using FaceFlux = AthenaArray<Real>[3];
...
#endif
  void CalculateVelocityDifferences(const int k, const int j, const int il,
                                    const int iu, const int ivx,
                                    AthenaArray<Real> &dvn,
                                    AthenaArray<Real> &dvt);
