...
//! \brief Computes divergence of the Hydro fluxes and adds that to a conserved
//! variable register
...
//! \brief Adds diffusive flux divergence to weighted average of conservative
//! variables from previous step of time integration algorithm.  RKL2 registers
//! are set to flux divergence update if first stage of RKL2 STS.  Only a subset
//! of ph->u indices are integrated, dependent on which diffusive processes are
//! integrated.
...
#pragma omp simd
          for (int i = is; i <= ie; ++i) {
            ...
            if (stage == 1 && pmb->pmy_mesh->sts_integrator == "rkl2") {
              fl_div_out(n, k, j, i) =
                  -0.5 * pmb->pmy_mesh->dt * dflx(n, i) / vol(i);
            }
          }
        }
      }
