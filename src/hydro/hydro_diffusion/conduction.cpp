...
#pragma omp simd private(kappaf, denf, dTdy, dTdz)
        for (int i=is; i<=ie+1; ++i) {
          ...
          dTdy = FourLimiter(Tjp1i  -Tkji  ,Tkji  -Tjm1i,
                             Tjp1im1-Tkjim1,Tkjim1-Tjm1im1);
          dTdy /= dy;
          ...
          dTdz = FourLimiter(Tkp1i  -Tkji  ,Tkji  -Tkm1i,
                             Tkp1im1-Tkjim1,Tkjim1-Tkm1im1);
          dTdz /= dz;
...
#pragma omp simd private(kappaf, denf, dTdx, dTdz)
          for (int i=il; i<=iu; ++i) {
            ...
            Real Tjm1im1 = p(IPR,k,j-1,i-1)/p(IDN,k,j-1,i-1);
            dTdx = FourLimiter(Tjip1  -Tkji  ,Tkji  -Tjim1,
                               Tjm1ip1-Tkjm1i,Tkjm1i-Tjm1im1);
            dTdx /= dx;
            ...
            dTdz = FourLimiter(Tkp1j  -Tkji  ,Tkji  -Tkm1j,
                               Tkp1jm1-Tkjm1i,Tkjm1i-Tkm1jm1);
            dTdz /= dz;
...
#pragma omp simd private(kappaf, denf, dTdy, dTdx)
        for (int i=il; i<=iu; ++i) {
          ...
          Real Tkm1im1 = p(IPR,k-1,j,i-1)/p(IDN,k-1,j,i-1);
          dTdx = FourLimiter(Tkip1  -Tkji  ,Tkji  -Tkim1,
                             Tkm1ip1-Tkm1ji,Tkm1ji-Tkm1im1);
          dTdx /= dx;
          ...
          Real Tkm1jm1 = p(IPR,k-1,j-1,i)/p(IDN,k-1,j-1,i);
          dTdy = FourLimiter(Tkjp1  -Tkji  ,Tkji  -Tkjm1,
                             Tkm1jp1-Tkm1ji,Tkm1ji-Tkm1jm1);
          dTdy /= dy;
