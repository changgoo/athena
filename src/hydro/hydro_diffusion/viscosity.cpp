...
#pragma omp simd private(nu1, denf, flx1, flx2, flx3)
      for (int i = is; i <= ie + 1; ++i) {
        nu1 = 0.5 * (nu(DiffProcess::iso, k, j, i) +
                     nu(DiffProcess::iso, k, j, i - 1));
        denf = 0.5 * (p_i(IDN, k, j, i) + p_i(IDN, k, j, i - 1));
        flx1 = -denf * nu1 *
               (fx_(i) +
                nuiso2 * 0.5 * (div_vel_(k, j, i) + div_vel_(k, j, i - 1)));
        flx2 = -denf * nu1 * fy_(i);
        flx3 = -denf * nu1 * fz_(i);
        ...
        if (NON_BAROTROPIC_EOS)
          x1flux(IEN, k, j, i) +=
              0.5 * ((p(IM1, k, j, i - 1) + p(IM1, k, j, i)) * flx1 +
...
#pragma omp simd private(nu1, denf, flx1, flx2, flx3)
        for (int i = il; i <= iu; i++) {
          nu1 = 0.5 * (nu(DiffProcess::iso, k, j, i) +
                       nu(DiffProcess::iso, k, j - 1, i));
          denf = 0.5 * (p_i(IDN, k, j - 1, i) + p_i(IDN, k, j, i));
          flx1 = -denf * nu1 * fx_(i);
          flx2 = -denf * nu1 *
                 (fy_(i) +
                  nuiso2 * 0.5 * (div_vel_(k, j - 1, i) + div_vel_(k, j, i)));
          flx3 = -denf * nu1 * fz_(i);
          ...
          if (NON_BAROTROPIC_EOS)
            x2flux(IEN, k, j, i) +=
                0.5 * ((p(IM1, k, j, i) + p(IM1, k, j - 1, i)) * flx1 +
...
#pragma omp simd private(nu1, denf, flx1, flx2, flx3)
        for (int i = il; i <= iu; i++) {
          nu1 = 0.5 * (nu(DiffProcess::iso, k, j, i) +
                       nu(DiffProcess::iso, k - 1, j, i));
          denf = 0.5 * (p_i(IDN, k - 1, j, i) + p_i(IDN, k, j, i));
          flx1 = -denf * nu1 * fx_(i);
          flx2 = -denf * nu1 * fy_(i);
          flx3 = -denf * nu1 *
                 (fz_(i) +
                  nuiso2 * 0.5 * (div_vel_(k - 1, j, i) + div_vel_(k, j, i)));
          x3flux(IM1, k, j, i) += flx1;
          x3flux(IM2, k, j, i) += flx2;
          x3flux(IM3, k, j, i) += flx3;
          if (NON_BAROTROPIC_EOS)
            x3flux(IEN, k, j, i) +=
                0.5 * ((p(IM1, k, j, i) + p(IM1, k - 1, j, i)) * flx1 +
...
void HydroDiffusion::FaceXdx(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    len(i) =
        2.0 * (prim(IM1, k, j, i) - prim(IM1, k, j, i - 1)) / pco_->dx1v(i - 1);
  }
  return;
}

// v_{x2;x1}+v_{x1;x2}  covariant derivative at x1 interface
void HydroDiffusion::FaceXdy(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f2) {
#pragma omp simd
...
void HydroDiffusion::FaceXdz(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f3) {
#pragma omp simd
      ...
      len(i) =
          ...
          + 0.5 *
                ((prim(IM1, k + 1, j, i) + prim(IM1, k + 1, j, i - 1)) -
                 (prim(IM1, k - 1, j, i) + prim(IM1, k - 1, j, i - 1))) /
                pco_->h31f(i) /
                pco_->h32v(j) // note, more terms than FaceXdy() line
...
void HydroDiffusion::FaceYdx(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f2) {
#pragma omp simd
    for (int i = il; i <= iu; ++i) {
      len(i) = (prim(IM1, k, j, i) - prim(IM1, k, j - 1, i)) / pco_->h2v(i) /
                   pco_->dx2v(j - 1) +
               pco_->h2v(i) * 0.5 *
...
void HydroDiffusion::FaceYdy(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f2) {
#pragma omp simd
    for (int i = il; i <= iu; ++i) {
      len(i) = 2.0 * (prim(IM2, k, j, i) - prim(IM2, k, j - 1, i)) /
                   pco_->h2v(i) / pco_->dx2v(j - 1) +
               (prim(IM1, k, j, i) + prim(IM1, k, j - 1, i)) / pco_->h2v(i) *
                   pco_->dh2vd1(i);
    }
  } else {
#pragma omp simd
...
void HydroDiffusion::FaceYdz(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f3) {
#pragma omp simd
...
void HydroDiffusion::FaceZdx(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f3) {
#pragma omp simd
    for (int i = il; i <= iu; ++i) {
      len(i) =
          (prim(IM1, k, j, i) - prim(IM1, k - 1, j, i)) / pco_->dx3v(k - 1) +
          0.5 * pco_->h31v(i) *
...
void HydroDiffusion::FaceZdy(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f3) {
#pragma omp simd
    for (int i = il; i <= iu; ++i) {
      len(i) = (prim(IM2, k, j, i) - prim(IM2, k - 1, j, i)) / pco_->h31v(i) /
                   pco_->h32v(j) / pco_->dx3v(k - 1) +
               0.5 * pco_->h32v(j) *
...
void HydroDiffusion::FaceZdz(const int k, const int j, const int il,
                             const int iu, const AthenaArray<Real> &prim,
                             AthenaArray<Real> &len) {
  if (pmb_->pmy_mesh->f3) {
#pragma omp simd
...
void ConstViscosity(HydroDiffusion *phdif, MeshBlock *pmb,
