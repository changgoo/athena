...
// currently must be free functions for compatibility with user-defined fn via
// fn pointers
...
public:
  ...
  // array indices for hydro diffusion (conduction & viscosity) variants:
  // directionality should not be scoped (C++11) since enumerators are only used
  // as "int" to index arrays
  enum DiffProcess { iso = 0, aniso = 1 };

  // functions
  // iprim is primitives in the inertial system
  void CalcDiffusionFlux(const AthenaArray<Real> &prim,
                         const AthenaArray<Real> &iprim, const FaceField &b,
                         const AthenaArray<Real> &bcc);
  // TODO(felker): Rename+move out of this class. Confusing w/
  // Hydro::AddDiffusionFluxes() See note in hydro_diffusion.cpp.
  ...
  void AddDiffusionFluxSingleCell(AthenaArray<Real> *flx_src,
                                  AthenaArray<Real> *flx_des, int i, int j,
                                  int k);
  void AddDiffusionEnergyFluxSingleCell(AthenaArray<Real> *flux_src,
                                        AthenaArray<Real> *flux_des, int i,
                                        int j, int k);
  ...
  void ViscousFluxAniso(const AthenaArray<Real> &p,
