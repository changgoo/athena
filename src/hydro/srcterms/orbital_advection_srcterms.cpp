...
//                           const AthenaArray<Real> &prim, AthenaArray<Real>
//                           &cons)
//  \brief Add source terms for orbital advection
void HydroSourceTerms::OrbitalAdvectionSourceTerms(
  ...
  if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
    if (pmb->pmy_mesh->OrbitalVelocity_ ==
        nullptr) { // default orbital velocity
...
#pragma omp simd
          for (int i = pmb->is; i <= pmb->ie; ++i) {
            ...
            const Real &vy = prim(IVY, k, j, i);
            const Real vc =
                vKc(k, i) + qshear_ * Omega_0_ * pmb->pcoord->x1v(i);
            // 2D components
            cons(IM1, k, j, i) += dt * 2.0 * Omega_0_ * (den * (vy + vc));
            cons(IM2, k, j, i) -= dt * (2.0 * Omega_0_ + dvk_i) * mom1;
            if (NON_BAROTROPIC_EOS) {
              const Real vm =
                  vKf1(k, i) + qshear_ * Omega_0_ * pmb->pcoord->x1f(i);
              const Real vp =
                  vKf1(k, i + 1) + qshear_ * Omega_0_ * pmb->pcoord->x1f(i + 1);
              ...
              if (dvk_k != 0.0) {
                ...
                if (NON_BAROTROPIC_EOS) {
                  const Real rho_v3 = 0.25 * (flux[X3DIR](IDN, k + 1, j, i) +
                                              flux[X3DIR](IDN, k, j, i)) +
                                      0.5 * mom3;
                  Real temp = -rho_v3 * vy;
                  if (MAGNETIC_FIELDS_ENABLED) {
                    cons(IEN, k, j, i) +=
                        pf->bcc(IB2, k, j, i) * pf->bcc(IB3, k, j, i);
                  }
...
#pragma omp simd
            for (int i = pmb->is; i <= pmb->ie; ++i) {
              ...
              const Real mom3 = den * prim(IVZ, k, j, i);
              const Real flux_xc = 0.5 * (flux[X1DIR](IDN, k, j, i + 1) +
                                          flux[X1DIR](IDN, k, j, i));
              const Real flux_zc = 0.5 * (flux[X3DIR](IDN, k + 1, j, i) +
                                          flux[X3DIR](IDN, k, j, i));
              cons(IM1, k, j, i) += 2.0 * dt * vc * ri * (den * vp);
              cons(IM2, k, j, i) -=
                  0.5 * dt *
                  ((2.0 * vc * ri + rdok_i) * (mom1 + flux_xc) +
                   dvk_k * (mom3 + flux_zc));
              ...
              if (NON_BAROTROPIC_EOS) {
                ...
                cons(IEN, k, j, i) +=
                    dt * (temp1 * rdok_i + temp2 * dvk_k + src_k * flux_zc);
              }
            }
          }
        }
...
#pragma omp simd
          for (int i = pmb->is; i <= pmb->ie; ++i) {
            ...
            const Real flux_c = 0.5 * (flux[X1DIR](IDN, ks, j, i + 1) +
                                       flux[X1DIR](IDN, ks, j, i));
            cons(IM1, ks, j, i) += 2.0 * dt * vc * ri * (den * vp);
            cons(IM2, ks, j, i) -=
                dt * (vc * ri + 0.5 * rdok_i) * (mom1 + flux_c);
            if (NON_BAROTROPIC_EOS) {
...
#pragma omp simd
        ...
        std::cout << "[OribtalAdvectionSourceTerm]: not compatible to 3D with "
                  << "user-defined vK in cylindrical coordinates ."
                  << std::endl;
        return;
      }
    }
...
#pragma omp simd
        for (int i = pmb->is; i <= pmb->ie; ++i) {
          ...
          const Real sdvk_j =
              (dvk2 - cv3 * vk) * ri; // (dvk/dtheta - vk cot(\theta))/r
          const Real mom1 = den * prim(IVX, k, j, i);
          ...
          const Real flux_xc =
              0.5 * (flux[X1DIR](IDN, k, j, i + 1) + flux[X1DIR](IDN, k, j, i));
          const Real flux_yc =
              0.5 * (flux[X2DIR](IDN, k, j + 1, i) + flux[X2DIR](IDN, k, j, i));
          cons(IM1, k, j, i) += dt * ri * den * (2.0 * vc * vp + (src - grav));
          cons(IM2, k, j, i) += dt * ri * den * cv1 * (2.0 * vc * vp + src);
          cons(IM3, k, j, i) -=
              0.5 * dt *
              (((vc + Omega_0_ * rv * sv) * ri + dvk1) * (mom1 + flux_xc) +
               ((vc + Omega_0_ * rv * sv) * cv3 + dvk2) * (mom2 + flux_yc) *
                   ri);
          // energy
          if (NON_BAROTROPIC_EOS) {
            // This is consistent with the pointmass.
