...
//!
//! We add shearing box source term via operator splitting method. The source
//! terms are added after the fluxes are computed in each step of the
//! integration (in FluxDivergence) to give predictions of the conservative
//! variables for either the next step or the final update.
...
#pragma omp simd
        for (int i = pmb->is; i <= pmb->ie; ++i) {
          ...
          cons(IM1, k, j, i) +=
              2.0 * dt *
              (Omega_0_ * (den * prim(IVY, k, j, i)) + qO2 * den * xc);
          ...
          if (NON_BAROTROPIC_EOS) {
            ...
            cons(IEN, k, j, i) +=
                dt *
                (flux[X1DIR](IDN, k, j, i) * (phic - phil) +
...
#pragma omp simd
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        cons(IM1, ks, j, i) +=
            2.0 * dt *
            (Omega_0_ * (den * prim(IVZ, ks, j, i)) + qO2 * den * xc);
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          cons(IEN, ks, j, i) +=
              dt *
              (flux[X1DIR](IDN, ks, j, i) * (phic - phil) +
