...
void Hydro::RiemannSolver(const int k, const int j, const int il, const int iu,
  ...
  if (GENERAL_RELATIVITY && ivx == IVY && pmy_block->pcoord->IsPole(j)) {
    LLFNonTransforming(pmy_block, k, j, il, iu, bb, g_, gi_, prim_l, prim_r,
                       flux, ey, ez);
  } else {
    LLFTransforming(pmy_block, k, j, il, iu, ivx, bb, bb_normal_, g_, gi_,
  ...
  for (int i = il; i <= iu; ++i) {
    ...
    if (GENERAL_RELATIVITY) {
      for (int n = 0; n < NWAVE; ++n) {
        cons(n, i) =
            0.5 * (cons_r[n] + cons_l[n] + (flux_l[n] - flux_r[n]) / lambda);
      }
    }
    ...
    ey(k, j, i) = -0.5 * (flux_l[IBY] + flux_r[IBY] -
                          lambda * (cons_r[IBY] - cons_l[IBY]));
    ez(k, j, i) = 0.5 * (flux_l[IBZ] + flux_r[IBZ] -
                         lambda * (cons_r[IBZ] - cons_l[IBZ]));
  }
...
// Notes:
//   implements LLF algorithm similar to that of fluxcalc() in step_ch.c in Harm
//   derived from RiemannSolver() in llf_mhd_rel_no_transform.cpp assuming ivx =
//   IVY
