...
namespace {
// Declarations
void HLLDTransforming(
    MeshBlock *pmb, const int k, const int j, const int il, const int iu,
    const int ivx, const AthenaArray<Real> &bb, AthenaArray<Real> &bb_normal,
    AthenaArray<Real> &lambdas_p_l, AthenaArray<Real> &lambdas_m_l,
    AthenaArray<Real> &lambdas_p_r, AthenaArray<Real> &lambdas_m_r,
    AthenaArray<Real> &g, AthenaArray<Real> &gi, AthenaArray<Real> &prim_l,
    AthenaArray<Real> &prim_r, AthenaArray<Real> &cons, AthenaArray<Real> &flux,
    AthenaArray<Real> &ey, AthenaArray<Real> &ez);
Real EResidual(Real w_guess, Real dd, Real ee, Real m_sq, Real bb_sq,
               Real ss_sq, Real gamma_prime);
...
//   tries to implement HLLD algorithm from Mignone, Ugliano, & Bodo 2009, MNRAS
//   393
//       1141 (MUB)
//   otherwise implements HLLE algorithm similar to that of fluxcalc() in
//   step_ch.c in
//       Harm
...
void Hydro::RiemannSolver(const int k, const int j, const int il, const int iu,
  ...
  if (GENERAL_RELATIVITY && ivx == IVY && pmy_block->pcoord->IsPole(j)) {
    HLLENonTransforming(pmy_block, k, j, il, iu, bb, g_, gi_, prim_l, prim_r,
                        flux, ey, ez);
  } else {
    HLLDTransforming(pmy_block, k, j, il, iu, ivx, bb, bb_normal_, lambdas_p_l_,
                     lambdas_m_l_, lambdas_p_r_, lambdas_m_r_, g_, gi_, prim_l,
...
//   lambdas_p_l,lambdas_m_l,lambdas_p_r,lambdas_m_r: 1D scratch arrays for
//   wavespeeds g,gi: 1D scratch arrays for metric coefficients prim_l,prim_r:
//   1D arrays of left and right primitive states cons: 1D scratch array for
...
// Notes:
//   prim_l, prim_r overwritten
//   implements HLLD algorithm from Mignone, Ugliano, & Bodo 2009, MNRAS 393
//   1141 (MUB) references Mignone & Bodo 2006, MNRAS 368 1040 (MB) references
//   Mignone & McKinney 2007, MNRAS 378 1118 (MM) follows Athena 4.2, hlld_sr.c,
//   in variable choices and magic numbers
...
void HLLDTransforming(
  ...
  const Real vc_extension =
      1.0e-6; // use contact region if Alfven speeds smaller
...
#pragma omp simd simdlen(SIMD_WIDTH)
  ...
  Real cons_interface[NWAVE][SIMD_WIDTH]
      __attribute__((aligned(CACHELINE_BYTES)));
  Real flux_interface[NWAVE][SIMD_WIDTH]
      __attribute__((aligned(CACHELINE_BYTES)));
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int m = 0; m < std::min(SIMD_WIDTH, iu - i + 1); m++) {
      ...
      pmb->peos->FastMagnetosonicSpeedsGR(wgas_l, pgas_l, u_l[0], u_l[1],
                                          b_sq_l, -1.0, 0.0, 1.0,
                                          &lambdas_p_l(ipm), &lambdas_m_l(ipm));

      // Calculate right wavespeeds
      Real wgas_r = rho_r + gamma_adi / (gamma_adi - 1.0) * pgas_r;
      pmb->peos->FastMagnetosonicSpeedsGR(wgas_r, pgas_r, u_r[0], u_r[1],
                                          b_sq_r, -1.0, 0.0, 1.0,
                                          &lambdas_p_r(ipm), &lambdas_m_r(ipm));
      ...
      for (int n = 0; n < NWAVE; ++n) {
        flux_hll[n][m] =
            (lambda_l * r_r[n][m] - lambda_r * r_l[n][m]) * lambda_diff_inv;
      }
    }
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int m = 0; m < std::min(SIMD_WIDTH, iu - i + 1); m++) {
      ...
      // Calculate total pressure in HLL region
      {
        ...
        Real m_sq = SQR(cons_hll[ivx][m]) + SQR(cons_hll[ivy][m]) +
                    SQR(cons_hll[ivz][m]);
        Real bb_sq = SQR(bbx) + SQR(cons_hll[IBY][m]) + SQR(cons_hll[IBZ][m]);
        Real m_dot_bb = cons_hll[ivx][m] * bbx +
                        cons_hll[ivy][m] * cons_hll[IBY][m] +
        ...
        Real w_init =
            (s2 >= 0.0 && a1 >= 0.0) ? -2.0 * a0 / (a1 + s) : 0.5 * (-a1 + s);
        ...
        for (int n = 0; n < num_nr; ++n) {
          ...
          Real derivative = EResidualPrime(w_old, cons_hll[IDN][m], m_sq, bb_sq,
                                           ss_sq, gamma_prime);
          Real delta = -res_old / derivative;
          w_new = w_old + delta;
          res_new = EResidual(w_new, cons_hll[IDN][m], cons_hll[IEN][m], m_sq,
                              bb_sq, ss_sq, gamma_prime);
        }
        Real w = w_new;
        ...
        Real chi =
            (1.0 - v_sq) * (w - gamma_lor * cons_hll[IDN][m]); // (MM A11)
        Real pgas = 1.0 / gamma_prime * chi;                   // (MM A17)
        Real vx =
            (cons_hll[ivx][m] + m_dot_bb / w * bbx) / (w + bb_sq); // (MM A10)
      ...
      if (SQR(bbx) / ptot_hll < p_transition) { // weak magnetic field
        ...
        Real a0 = cons_hll[ivx][m] * flux_hll[IEN][m] -
                  flux_hll[ivx][m] * cons_hll[IEN][m];
        Real s2 = SQR(a1) - 4.0 * a0;
        Real s = (s2 < 0.0) ? 0.0 : std::sqrt(s2);
        ptot_init[m] = (s2 >= 0.0 && a1 >= 0.0) ? -2.0 * a0 / (a1 + s)
                                                : (-a1 + s) / 2.0; // (MUB 55)
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int m = 0; m < std::min(SIMD_WIDTH, iu - i + 1); m++) {
      ...
      ptot_0[m] = ptot_init[m];
      {
        ...
        Real cl =
            r_l[ivy][m] * r_l[IBY][m] + r_l[ivz][m] * r_l[IBZ][m]; // (MUB 28)
        Real ql = -al - gl + SQR(bbx) * (1.0 - SQR(lambda_l));     // (MUB 29)
        Real xl_inv =
            1.0 /
            (bbx * (al * lambda_l * bbx + cl) -
             (al + gl) * (lambda_l * ptot_0[m] + r_l[IEN][m])); // (MUB 30)
        vx_al = (bbx * (al * bbx + lambda_l * cl) -
                 (al + gl) * (ptot_0[m] + r_l[ivx][m])) *
                xl_inv; // (MUB 23)
        vy_al = (ql * r_l[ivy][m] +
                 r_l[IBY][m] *
                     (cl + bbx * (lambda_l * r_l[ivx][m] - r_l[IEN][m]))) *
                xl_inv; // (MUB 24)
        vz_al = (ql * r_l[ivz][m] +
                 r_l[IBZ][m] *
                     (cl + bbx * (lambda_l * r_l[ivx][m] - r_l[IEN][m]))) *
        ...
        Real cr =
            r_r[ivy][m] * r_r[IBY][m] + r_r[ivz][m] * r_r[IBZ][m]; // (MUB 28)
        Real qr = -ar - gr + SQR(bbx) * (1.0 - SQR(lambda_r));     // (MUB 29)
        Real xr_inv =
            1.0 /
            (bbx * (ar * lambda_r * bbx + cr) -
             (ar + gr) * (lambda_r * ptot_0[m] + r_r[IEN][m])); // (MUB 30)
        vx_ar = (bbx * (ar * bbx + lambda_r * cr) -
                 (ar + gr) * (ptot_0[m] + r_r[ivx][m])) *
                xr_inv; // (MUB 23)
        vy_ar = (qr * r_r[ivy][m] +
                 r_r[IBY][m] *
                     (cr + bbx * (lambda_r * r_r[ivx][m] - r_r[IEN][m]))) *
                xr_inv; // (MUB 24)
        vz_ar = (qr * r_r[ivz][m] +
                 r_r[IBZ][m] *
                     (cr + bbx * (lambda_r * r_r[ivx][m] - r_r[IEN][m]))) *
        ...
        Real v_rm_l =
            vx_al * r_l[ivx][m] + vy_al * r_l[ivy][m] + vz_al * r_l[ivz][m];
        Real wtot_al = ptot_0[m] + (r_l[IEN][m] - v_rm_l) / (lambda_l - vx_al);
        Real v_rm_r =
            vx_ar * r_r[ivx][m] + vy_ar * r_r[ivy][m] + vz_ar * r_r[ivz][m];
        ...
        Real denom_al_inv =
            1.0 / (lambda_l * ptot_0[m] + r_l[IEN][m] + bbx * eta_l);
        ...
        Real denom_ar_inv =
            1.0 / (lambda_r * ptot_0[m] + r_r[IEN][m] + bbx * eta_r);
        ...
        Real by_c_delta_kx = cons_ar[IBY][m] * (lambda_ar[m] - vx_ar) -
                             cons_al[IBY][m] * (lambda_al[m] - vx_al) +
                             bbx * (vy_ar - vy_al);
        Real bz_c_delta_kx = cons_ar[IBZ][m] * (lambda_ar[m] - vx_ar) -
                             cons_al[IBZ][m] * (lambda_al[m] - vx_al) +
                             bbx * (vz_ar - vz_al);

        // Calculate residual
        Real k_sq_l = SQR(kx_l) + SQR(ky_l) + SQR(kz_l);
        Real k_dot_bc_delta_kx =
            kx_l * bbx_c_delta_kx + ky_l * by_c_delta_kx + kz_l * bz_c_delta_kx;
        Real y_l =
            (1.0 - k_sq_l) / (eta_l * delta_kx - k_dot_bc_delta_kx); // (MUB 49)
        Real k_sq_r = SQR(kx_r) + SQR(ky_r) + SQR(kz_r);
        k_dot_bc_delta_kx =
            kx_r * bbx_c_delta_kx + ky_r * by_c_delta_kx + kz_r * bz_c_delta_kx;
        Real y_r =
            (1.0 - k_sq_r) / (eta_r * delta_kx - k_dot_bc_delta_kx); // (MUB 49)
      ...
      ptot_1[m] = ptot_init[m] * (1.0 + initial_offset);
      {
        ...
        // Calculate K_L and K_R (MUB 43)
        Real denom_al_inv =
            1.0 / (lambda_l * ptot_1[m] + r_l[IEN][m] + bbx * eta_l);
        kx_l = (r_l[ivx][m] + ptot_1[m] + lambda_l * bbx * eta_l) *
               denom_al_inv; // R_{B^x} = \lambda B^x
        ky_l = (r_l[ivy][m] + r_l[IBY][m] * eta_l) * denom_al_inv;
        kz_l = (r_l[ivz][m] + r_l[IBZ][m] * eta_l) * denom_al_inv;
        Real denom_ar_inv =
            1.0 / (lambda_r * ptot_1[m] + r_r[IEN][m] + bbx * eta_r);
...
#pragma omp simd simdlen(SIMD_WIDTH)
    for (int m = 0; m < std::min(SIMD_WIDTH, iu - i + 1); m++) {
      ...
      for (int n = 0; n < num_secant; ++n) {
        ...
        if (need_to_iterate) {
          ptot_n = (res_last * ptot_old - res_old * ptot_last) /
                   (res_last - res_old);
        } else {
          ptot_n = ptot_last;
        }
        ...
        Real ql = -al - gl + SQR(bbx) * (1.0 - SQR(lambda_l));     // (MUB 29)
        Real xl_inv =
            1.0 / (bbx * (al * lambda_l * bbx + cl) -
                   (al + gl) * (lambda_l * ptot_n + r_l[IEN][m])); // (MUB 30)
        vx_al = (bbx * (al * bbx + lambda_l * cl) -
                 (al + gl) * (ptot_n + r_l[ivx][m])) *
                xl_inv; // (MUB 23)
        vy_al = (ql * r_l[ivy][m] +
                 r_l[IBY][m] *
                     (cl + bbx * (lambda_l * r_l[ivx][m] - r_l[IEN][m]))) *
        ...
        Real qr = -ar - gr + SQR(bbx) * (1.0 - SQR(lambda_r));     // (MUB 29)
        Real xr_inv =
            1.0 / (bbx * (ar * lambda_r * bbx + cr) -
                   (ar + gr) * (lambda_r * ptot_n + r_r[IEN][m])); // (MUB 30)
        vx_ar = (bbx * (ar * bbx + lambda_r * cr) -
                 (ar + gr) * (ptot_n + r_r[ivx][m])) *
                xr_inv; // (MUB 23)
        vy_ar = (qr * r_r[ivy][m] +
                 r_r[IBY][m] *
                     (cr + bbx * (lambda_r * r_r[ivx][m] - r_r[IEN][m]))) *
        ...
        // Calculate K_L and K_R (MUB 43)
        Real denom_al_inv =
            1.0 / (lambda_l * ptot_n + r_l[IEN][m] + bbx * eta_l);
        kx_l = (r_l[ivx][m] + ptot_n + lambda_l * bbx * eta_l) *
               denom_al_inv; // R_{B^x} = \lambda B^x
        ky_l = (r_l[ivy][m] + r_l[IBY][m] * eta_l) * denom_al_inv;
        kz_l = (r_l[ivz][m] + r_l[IBZ][m] * eta_l) * denom_al_inv;
        Real denom_ar_inv =
            1.0 / (lambda_r * ptot_n + r_r[IEN][m] + bbx * eta_r);
      ...
      // Calculate v_c (MUB 47), averaging left and right values in case of
      // disagreement
      Real k_sq_l = SQR(kx_l) + SQR(ky_l) + SQR(kz_l);
      ...
      if (vx_c >= 0.0) { // cL region
        ...
        cons_c[IEN][m] = (lambda_al[m] * cons_al[IEN][m] - cons_al[ivx][m] +
                          ptot_c[m] * vx_c - v_bb_c * bbx) /
                         (lambda_al[m] - vx_c); // (MUB 51)
      } else {                                  // cR region
        cons_c[IDN][m] = cons_ar[IDN][m] * (lambda_ar[m] - vx_ar) /
                         (lambda_ar[m] - vx_c); // (MUB 50)
        cons_c[IEN][m] = (lambda_ar[m] * cons_ar[IEN][m] - cons_ar[ivx][m] +
                          ptot_c[m] * vx_c - v_bb_c * bbx) /
                         (lambda_ar[m] - vx_c); // (MUB 51)
      }
      cons_c[ivx][m] =
          (cons_c[IEN][m] + ptot_c[m]) * vx_c - v_bb_c * bbx; // (MUB 52)
      ...
      for (int n = 0; n < NWAVE; ++n) {
        if (vx_c >= 0.0) { // cL region
          flux_c[n][m] =
              flux_al[n][m] + lambda_al[m] * (cons_c[n][m] - cons_al[n][m]);
        } else { // cR region
          flux_c[n][m] =
              flux_ar[n][m] + lambda_ar[m] * (cons_c[n][m] - cons_ar[n][m]);
        }
      }
    }
    ...
    for (int m = 0; m < std::min(SIMD_WIDTH, iu - i + 1); m++) {
      ...
      if (GENERAL_RELATIVITY) {
        v_interface = gi(i01, ipm) / std::sqrt(SQR(gi(i01, ipm)) -
                                               gi(I00, ipm) * gi(i11, ipm));
      }
...
Real EResidualPrime(Real w_guess, Real dd, Real m_sq, Real bb_sq, Real ss_sq,
  ...
  Real dv_sq_dw = -2.0 / (w_cu * w_b_cu) // (MM A16)
                  * (ss_sq * (3.0 * w_guess * (w_guess + bb_sq) + SQR(bb_sq)) +
                     m_sq * w_cu);
  Real dchi_dw =
      1.0 - v_sq // (cf. MM A14)
...
//   ey,ez: 3D arrays of magnetic fluxes (electric fields) across interfaces
// Notes:
//   implements HLLE algorithm similar to that of fluxcalc() in step_ch.c in
//   Harm derived from RiemannSolver() in hlle_mhd_rel_no_transform.cpp assuming
//   ivx = IVY same function as in hlle_mhd_rel.cpp
...
#pragma omp simd
  for (int i = il; i <= iu; ++i) {
    // Extract metric
    const Real &g_00 = g(I00, i), &g_01 = g(I01, i), &g_02 = g(I02, i),
               &g_03 = g(I03, i), &g_10 = g(I01, i), &g_11 = g(I11, i),
               &g_12 = g(I12, i), &g_13 = g(I13, i), &g_20 = g(I02, i),
               &g_21 = g(I12, i), &g_22 = g(I22, i), &g_23 = g(I23, i),
               &g_30 = g(I03, i), &g_31 = g(I13, i), &g_32 = g(I23, i),
               &g_33 = g(I33, i);
    const Real &g00 = gi(I00, i), &g01 = gi(I01, i), &g02 = gi(I02, i),
               &g03 = gi(I03, i), &g10 = gi(I01, i), &g11 = gi(I11, i),
               &g12 = gi(I12, i), &g13 = gi(I13, i), &g20 = gi(I02, i),
               &g21 = gi(I12, i), &g22 = gi(I22, i), &g23 = gi(I23, i),
               &g30 = gi(I03, i), &g31 = gi(I13, i), &g32 = gi(I23, i),
               &g33 = gi(I33, i);
    ...
    Real tmp = g_11 * SQR(uu1_l) + 2.0 * g_12 * uu1_l * uu2_l +
               2.0 * g_13 * uu1_l * uu3_l + g_22 * SQR(uu2_l) +
               2.0 * g_23 * uu2_l * uu3_l + g_33 * SQR(uu3_l);
    ...
    ucov_l[0] = g_00 * ucon_l[0] + g_01 * ucon_l[1] + g_02 * ucon_l[2] +
                g_03 * ucon_l[3];
    ucov_l[1] = g_10 * ucon_l[0] + g_11 * ucon_l[1] + g_12 * ucon_l[2] +
                g_13 * ucon_l[3];
    ucov_l[2] = g_20 * ucon_l[0] + g_21 * ucon_l[1] + g_22 * ucon_l[2] +
                g_23 * ucon_l[3];
    ucov_l[3] = g_30 * ucon_l[0] + g_31 * ucon_l[1] + g_32 * ucon_l[2] +
                g_33 * ucon_l[3];

    // Calculate 4-velocity in right state
    Real ucon_r[4], ucov_r[4];
    tmp = g_11 * SQR(uu1_r) + 2.0 * g_12 * uu1_r * uu2_r +
          2.0 * g_13 * uu1_r * uu3_r + g_22 * SQR(uu2_r) +
          2.0 * g_23 * uu2_r * uu3_r + g_33 * SQR(uu3_r);
    ...
    ucov_r[0] = g_00 * ucon_r[0] + g_01 * ucon_r[1] + g_02 * ucon_r[2] +
                g_03 * ucon_r[3];
    ucov_r[1] = g_10 * ucon_r[0] + g_11 * ucon_r[1] + g_12 * ucon_r[2] +
                g_13 * ucon_r[3];
    ucov_r[2] = g_20 * ucon_r[0] + g_21 * ucon_r[1] + g_22 * ucon_r[2] +
                g_23 * ucon_r[3];
    ucov_r[3] = g_30 * ucon_r[0] + g_31 * ucon_r[1] + g_32 * ucon_r[2] +
                g_33 * ucon_r[3];
    ...
    bcov_l[0] = g_00 * bcon_l[0] + g_01 * bcon_l[1] + g_02 * bcon_l[2] +
                g_03 * bcon_l[3];
    bcov_l[1] = g_10 * bcon_l[0] + g_11 * bcon_l[1] + g_12 * bcon_l[2] +
                g_13 * bcon_l[3];
    bcov_l[2] = g_20 * bcon_l[0] + g_21 * bcon_l[1] + g_22 * bcon_l[2] +
                g_23 * bcon_l[3];
    bcov_l[3] = g_30 * bcon_l[0] + g_31 * bcon_l[1] + g_32 * bcon_l[2] +
                g_33 * bcon_l[3];
    Real b_sq_l = bcon_l[0] * bcov_l[0] + bcon_l[1] * bcov_l[1] +
                  bcon_l[2] * bcov_l[2] + bcon_l[3] * bcov_l[3];
    ...
    bcov_r[0] = g_00 * bcon_r[0] + g_01 * bcon_r[1] + g_02 * bcon_r[2] +
                g_03 * bcon_r[3];
    bcov_r[1] = g_10 * bcon_r[0] + g_11 * bcon_r[1] + g_12 * bcon_r[2] +
                g_13 * bcon_r[3];
    bcov_r[2] = g_20 * bcon_r[0] + g_21 * bcon_r[1] + g_22 * bcon_r[2] +
                g_23 * bcon_r[3];
    bcov_r[3] = g_30 * bcon_r[0] + g_31 * bcon_r[1] + g_32 * bcon_r[2] +
                g_33 * bcon_r[3];
    Real b_sq_r = bcon_r[0] * bcov_r[0] + bcon_r[1] * bcov_r[1] +
                  bcon_r[2] * bcov_r[2] + bcon_r[3] * bcov_r[3];
    ...
    cons_l[IEN] =
        wtot_l * ucon_l[0] * ucov_l[0] - bcon_l[0] * bcov_l[0] + ptot_l;
    ...
    cons_r[IEN] =
        wtot_r * ucon_r[0] * ucov_r[0] - bcon_r[0] * bcov_r[0] + ptot_r;
