...
#pragma omp simd private(flxi, wli, wri, spd)
  for (int i = il; i <= iu; ++i) {
    ...
    Real bxsq = bxi * bxi;
    Real ptl =
        SQR(cs) * wli[IDN] + 0.5 * (bxsq + SQR(wli[IBY]) + SQR(wli[IBZ]));
    Real ptr =
        SQR(cs) * wri[IDN] + 0.5 * (bxsq + SQR(wri[IBY]) + SQR(wri[IBZ]));
    ...
    // rho component of U^{hll} from Mignone eqn. (15); uses F_L and F_R from
    // eqn. (6)
    ...
    Real fdhll =
        (spd[4] * fl.d - spd[0] * fr.d + spd[4] * spd[0] * (ur.d - ul.d)) *
        idspd;
    Real fmxhll =
        (spd[4] * fl.mx - spd[0] * fr.mx + spd[4] * spd[0] * (ur.mx - ul.mx)) *
        idspd;
    ...
    // Uc*
    Real x =
        sqrtdhll * (bxi > 0.0 ? 1.0 : -1.0); // from below eqn. (37) of Mignone
    ucst.d = dhll;                           // eqn. (20) of Mignone
    ucst.mx = mxhll;                         // eqn. (24) of Mignone
    ucst.my = 0.5 * (ulst.my + urst.my +
                     (urst.by - ulst.by) * x); // eqn. (34) of Mignone
    ucst.mz = 0.5 * (ulst.mz + urst.mz +
                     (urst.bz - ulst.bz) * x); // eqn. (35) of Mignone
    ucst.by = 0.5 * (ulst.by + urst.by +
                     (urst.my - ulst.my) / x); // eqn. (36) of Mignone
    ucst.bz = 0.5 * (ulst.bz + urst.bz +
                     (urst.mz - ulst.mz) / x); // eqn. (37) of Mignone
