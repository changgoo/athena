...
void HLLETransforming(MeshBlock *pmb, const int k, const int j, const int il,
                      ...
                      AthenaArray<Real> &gi, AthenaArray<Real> &prim_l,
                      AthenaArray<Real> &prim_r, AthenaArray<Real> &cons,
...
//   tries to implement HLLE algorithm from Mignone & Bodo 2005, MNRAS 364 126
//   (MB2005) otherwise implements HLLE algorithm similar to that of fluxcalc()
//   in step_ch.c in
//       Harm
...
void Hydro::RiemannSolver(const int k, const int j, const int il, const int iu,
  ...
  if (GENERAL_RELATIVITY && ivx == IVY && pmy_block->pcoord->IsPole(j)) {
    ...
    HLLETransforming(pmy_block, k, j, il, iu, ivx, bb, bb_normal_, g_, gi_,
                     prim_l, prim_r, cons_, flux, ey, ez);
  }
  for (int i = il; i <= iu; ++i) {
...
#pragma omp simd simdlen(SIMD_WIDTH)
  for (int i = il; i <= iu; ++i) {
    ...
    pmb->peos->FastMagnetosonicSpeedsGR(wgas_l, pgas_l, u_l[0], u_l[1], b_sq_l,
                                        -1.0, 0.0, 1.0, &lambda_p_l,
                                        &lambda_m_l);

    // Calculate right wavespeeds
    Real wgas_r = rho_r + gamma_adi / (gamma_adi - 1.0) * pgas_r;
    Real lambda_m_r, lambda_p_r;
    pmb->peos->FastMagnetosonicSpeedsGR(wgas_r, pgas_r, u_r[0], u_r[1], b_sq_r,
                                        -1.0, 0.0, 1.0, &lambda_p_r,
                                        &lambda_m_r);
    ...
    if (GENERAL_RELATIVITY) {
      for (int n = 0; n < NWAVE; ++n) {
        cons_hll[n] = (lambda_r * cons_r[n] - lambda_l * cons_l[n] + flux_l[n] -
                       flux_r[n]) *
                      lambda_diff_inv;
      }
    }
    ...
    if (GENERAL_RELATIVITY) {
      v_interface =
          gi(i01, i) / std::sqrt(SQR(gi(i01, i)) - gi(I00, i) * gi(i11, i));
    }
...
//   ivx = IVY same function as in hlld_rel.cpp
void HLLENonTransforming(MeshBlock *pmb, const int k, const int j, const int il,
                         const int iu, const AthenaArray<Real> &bb,
                         AthenaArray<Real> &g, AthenaArray<Real> &gi,
                         AthenaArray<Real> &prim_l, AthenaArray<Real> &prim_r,
