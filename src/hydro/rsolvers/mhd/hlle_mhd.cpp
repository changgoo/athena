  ...
  for (int i = il; i <= iu; ++i) {
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      er = wri[IPR] / gm1 +
           ...
           pbr;
      hroe = ((el + wli[IPR] + pbl) / sqrtdl + (er + wri[IPR] + pbr) / sqrtdr) *
             isdlpdr;
    }

    //--- Step 3. Compute fast magnetosonic speed in L,R, and Roe-averaged
    // states

    Real cl = pmy_block->peos->FastMagnetosonicSpeed(wli, bxi);
    Real cr = pmy_block->peos->FastMagnetosonicSpeed(wri, bxi);

    // Compute fast-magnetosonic speed using eq. B18 (adiabatic) or B39
    // (isothermal)
    ...
    //--- Step 4. Compute the max/min wave speeds based on L/R and Roe-averaged
    // values
