...
#pragma omp simd simdlen(SIMD_WIDTH) private(wli, wri, spd, flxi)
  for (int i = il; i <= iu; ++i) {
    ...
    spd[2] =
        (sdr * ur.mx - sdl * ul.mx + (ptl - ptr)) / (sdr * ur.d - sdl * ul.d);
    ...
    // Real ptstl = ptl + ul.d*sdl*(sdl-sdml); // these equations had issues
    // when averaged Real ptstr = ptr + ur.d*sdr*(sdr-sdmr);
