...
//!
//! Computes 1D fluxes using Roe's linearization.  When Roe's method fails
//! because of negative density in the intermediate states, LLF fluxes are used
//! instead (only density, not pressure, is checked in this version).
//!
//! REFERENCES:
...
inline void RoeFlux(const Real wroe[], const Real b1, const Real x,
                    const Real y, const Real du[], const Real wli[], Real flx[],
                    Real eigenvalues[], int &flag);

// (gamma-1) and isothermal sound speed made global so can be shared with
// eigensystem fns
Real gm1, iso_cs;
} // namespace
  ...
  for (int i = il; i <= iu; ++i) {
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      er = wri[IPR] / gm1 +
           ...
           pbr;
      wroe[IPR] =
          ((el + wli[IPR] + pbl) / sqrtdl + (er + wri[IPR] + pbr) / sqrtdr) *
          isdlpdr;
    }
    ...
    if (llf_flag != 0) {
      ...
      Real a = 0.5 *
               std::max((std::abs(wli[IVX]) + cfl), (std::abs(wri[IVX]) + cfr));
...
inline void RoeFlux(const Real wroe[], const Real b1, const Real x,
  ...
  if (NON_BAROTROPIC_EOS) {
    ...
    // eq. B29 Normalize by (gamma-1)/2a^{2}: quantities denoted by \bar{f}
    Real a[(NWAVE)];
    ...
    a[0] = du[0] *
           (alpha_f_bar * (vsq - hp) + cff * (cf + v1) - qs_hat * vqstr - aspb);
    ...
    a[2] = du[0] *
           (alpha_s_bar * (vsq - hp) + css * (cs + v1) + qf_hat * vqstr + afpb);
    ...
    // flag This uses the [0][*] components of the right-eigenmatrix
    ...
    // Now multiply projection with R-eigenvectors from eq. B21 and SUM into
    // output fluxes
    ...
    if (NON_BAROTROPIC_EOS)
      coeff[IEN] = 0.5 * std::abs(ev[IEN]) * a[IEN];
