...
//----------------------------------------------------------------------------------------
//! \fn void Hydro::SingleStateLLF_MHD
//! \brief The LLF Riemann solver for MHD (both adiabatic and isothermal)

void Hydro::SingleStateLLF_MHD(Real wli[], Real wri[], Real bxi, Real flx[]) {
  Real fl[NWAVE], fr[NWAVE], du[NWAVE];
  ...
  flx[IBY] = 0.5 * (fl[IBY] + fr[IBY]) - a * du[IBY];
  flx[IBZ] = 0.5 * (fl[IBZ] + fr[IBZ]) - a * du[IBZ];
