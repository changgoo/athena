...
// container to store (density, momentum, total energy, tranverse magnetic
// field) minimizes changes required to adopt athena4.2 version of this solver
struct Cons1D {
...
#pragma omp simd simdlen(SIMD_WIDTH) private(wli, wri, spd, flxi)
  for (int i = il; i <= iu; ++i) {
    ...
    Real pbl =
        0.5 *
        (bxsq + (SQR(wli[IBY]) + SQR(wli[IBZ]))); // magnetic pressure (l/r)
    ...
    //--- Step 2.  Compute L & R wave speeds according to Miyoshi & Kusano, eqn.
    //(67)
    ...
    fl.e = wli[IVX] * (ul.e + ptl - bxsq) -
           bxi * (wli[IVY] * ul.by + wli[IVZ] * ul.bz);
    ...
    fr.e = wri[IVX] * (ur.e + ptr - bxsq) -
           bxi * (wri[IVY] * ur.by + wri[IVZ] * ur.bz);
    ...
    // shock detector
    Real th1 = std::min(1.0, (cfmax - std::min(dvn(i), 0.0)) /
                                 (cfmax - std::min(dvt(i), 0.0)));
    ...
    Real clsq =
        ((pbl + kel) + std::sqrt(SQR(pbl + kel) - 2.0 * kel * bxsq)) / ul.d;
    Real crsq =
        ((pbr + ker) + std::sqrt(SQR(pbr + ker) - 2.0 * ker * bxsq)) / ur.d;
    Real chi = std::min(1.0, std::sqrt(std::max(clsq, crsq)) / cfmax);
    Real phi = chi * (2.0 - chi);
    Real ptst =
        (sdrd * ptl - sdld * ptr + phi * sdrd * sdld * (wri[IVX] - wli[IVX])) /
        (sdrd - sdld);
    ...
    // v_i* dot B_i*
    // (KGF): group transverse momenta terms for floating-point associativity
    // symmetry
    Real vbstl =
        (ulst.mx * bxi + (ulst.my * ulst.by + ulst.mz * ulst.bz)) * ulst_d_inv;
    // eqn (48) of M&K
    // (KGF): group transverse by, bz terms for floating-point associativity
    // symmetry
    ulst.e = (sdl * ul.e - ptl * wli[IVX] + ptst * spd[2] +
              bxi * (wli[IVX] * bxi + (wli[IVY] * ul.by + wli[IVZ] * ul.bz) -
                     vbstl)) *
             sdml_inv;
    ...
    urst.e = (sdr * ur.e - ptr * wri[IVX] + ptst * spd[2] +
              bxi * (wri[IVX] * bxi + (wri[IVY] * ur.by + wri[IVZ] * ur.bz) -
                     vbstr)) *
             sdmr_inv;
    ...
    if (0.5 * bxsq < (SMALL_NUMBER)*ptst) {
      ...
      // eqn (59) of M&K
      Real tmp = invsumd * (sqrtdl * (ulst.my * ulst_d_inv) +
      ...
      // eqn (60) of M&K
      tmp = invsumd *
            (sqrtdl * (ulst.mz * ulst_d_inv) + sqrtdr * (urst.mz * urst_d_inv) +
      ...
      // eqn (63) of M&K
      tmp =
          spd[2] * bxi + (uldst.my * uldst.by + uldst.mz * uldst.bz) / uldst.d;
