...
//!
//!  Computes 1D fluxes using the Harten-Lax-van Leer (HLL) Riemann solver. This
//!  flux is very diffusive, especially for contacts, and so it is not
//!  recommended for use in applications.  However, as shown by Einfeldt et
//!  al.(1991), it is positively conservative (cannot return negative densities
//!  or pressure), so it is a useful option when other approximate solvers fail
//!  and/or when extra dissipation is needed.
//!
//! REFERENCES:
//! - E.F. Toro, "Riemann Solvers and numerical methods for fluid dynamics", 2nd
//! ed.,
//!   Springer-Verlag, Berlin, (1999) chpt. 10.
//! - Einfeldt et al., "On Godunov-type methods near low densities", JCP, 92,
//! 273 (1991)
...
//! \brief The HLLE Riemann solver for hydrodynamics (both adiabatic and
//! isothermal)
...
#pragma omp simd private(wli, wri, wroe, fl, fr, flxi)
  for (int i = il; i <= iu; ++i) {
    ...
    if (GENERAL_EOS) {
      ...
      if (NON_BAROTROPIC_EOS) {
        Real q =
            hroe - 0.5 * (SQR(wroe[IVX]) + SQR(wroe[IVY]) + SQR(wroe[IVZ]));
        a = (q < 0.0) ? 0.0 : std::sqrt(gm1 * q);
      }

      //--- Step 4. Compute the max/min wave speeds based on L/R and
      // Roe-averaged values
