...
void HLLCTransforming(MeshBlock *pmb, const int k, const int j, const int il,
                      const int iu, const int ivx, AthenaArray<Real> &g,
...
//----------------------------------------------------------------------------------------
//! \fn void Hydro::RiemannSolver(const int k, const int j, const int il, const
//! int iu,
//!                           const int ivx,
//!                           AthenaArray<Real> &prim_l, AthenaArray<Real>
//!                           &prim_r, AthenaArray<Real> &flux, const
...
//! - tries to implement HLLC algorithm from Mignone & Bodo 2005, MNRAS 364 126
//! (MB2005)
...
void Hydro::RiemannSolver(const int k, const int j, const int il, const int iu,
                          const int ivx, AthenaArray<Real> &prim_l,
                          AthenaArray<Real> &prim_r, AthenaArray<Real> &flux,
  ...
  if (GENERAL_RELATIVITY && ivx == IVY && pmy_block->pcoord->IsPole(j)) {
    ...
    HLLCTransforming(pmy_block, k, j, il, iu, ivx, g_, gi_, prim_l, prim_r,
                     cons_, flux);
  }
  return;
}
...
namespace {
//----------------------------------------------------------------------------------------
//! \fn void HLLCTransforming(MeshBlock *pmb, const int k, const int j, const
//! int il,
...
//! \brief Frame-transforming HLLC implementation
//!
//! Inputs:
...
//! Notes:
//!  - prim_l, prim_r overwritten
//!  - implements HLLC algorithm from Mignone & Bodo 2005, MNRAS 364 126
//!  (MB2005)
...
#pragma omp simd simdlen(SIMD_WIDTH)
  for (int i = il; i <= iu; ++i) {
    ...
    if (std::abs(flux_hll[IEN]) > TINY_NUMBER) { // use quadratic formula
      // Follows algorithm in Numerical Recipes (section 5.6) for avoiding
      // cancellations
      lambda_star =
          -2.0 * cons_hll[ivx] /
    ...
    // Note: Could also use (MB2005 17), but note the first minus sign there is
    // wrong.
...
//!  Harm
//!  - derived from RiemannSolver() in hlle_rel_no_transform.cpp assuming ivx =
//!  IVY
