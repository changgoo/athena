...
//========================================================================================
//! \file lhllc.cpp
//! \brief Low-dissipation HLLC (LHLLC) Riemann solver for adiabatic
//! hydrodynamics.
...
//! \brief The Low-dissipation HLLC (LHLLC) Riemann solver for adiabatic
//! hydrodynamics
...
#pragma omp simd private(wli, wri, flxi, fl, fr)
  for (int i = il; i <= iu; ++i) {
    ...
    Real th1 = std::min(1.0, (cmax - std::min(dvn(i), 0.0)) /
                                 (cmax - std::min(dvt(i), 0.0)));
    Real th =
        th1 * th1 * th1 * th1; // this 4th power is empirical (see Minoshima+)

    // Determine the contact wave speed...
    Real am = (mr * wri[IVX] - ml * wli[IVX] - th * (wri[IPR] - wli[IPR])) /
              (mr - ml);
    ...
    Real cp = (mr * wli[IPR] - ml * wri[IPR] +
               phi * mr * ml * (wri[IVX] - wli[IVX])) /
              (mr - ml);
    cp = cp > 0.0 ? cp : 0.0;
