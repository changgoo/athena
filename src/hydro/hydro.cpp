...
Hydro::Hydro(MeshBlock *pmb, ParameterInput *pin)
    : pmy_block(pmb), u(NHYDRO, pmb->ncells3, pmb->ncells2, pmb->ncells1),
      ...
      w1(NHYDRO, pmb->ncells3, pmb->ncells2, pmb->ncells1), dvn(pmb->ncells1),
      dvt(pmb->ncells1),
      ...
      hsrc(this, pin), hdif(this, pin),
      fofc_enabled(pin->GetBoolean("hydro", "fofc")) {
  int nc1 = pmb->ncells1, nc2 = pmb->ncells2, nc3 = pmb->ncells3;
  Mesh *pm = pmy_block->pmy_mesh;

  pmb->RegisterMeshBlockData(u);

  // Allocate optional memory primitive/conserved variable registers for
  // time-integrator
  ...
  if (MAGNETIC_FIELDS_ENABLED &&
      RELATIVISTIC_DYNAMICS) { // only used in (SR/GR)MHD
  ...
  if (fofc_enabled) {
    ...
    if (MAGNETIC_FIELDS_ENABLED)
      bcctest_.NewAthenaArray(nc3, nc2, nc1);
  }
...
//! Real dt) \brief Calculate the weighting factor for the constrained transport
//! method
