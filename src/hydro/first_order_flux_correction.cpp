...
// TODO(felker): consider combining with PassiveScalars implementation + (see
// 57cfe28b) (may rename to AddPhysicalFluxDivergence or
// AddQuantityFluxDivergence to explicitly distinguish from CoordTerms) (may
// rename to AddHydroFluxDivergence and AddScalarsFluxDivergence, if the
// implementations remain completely independent / no inheritance is used)
void Hydro::FirstOrderFluxCorrection(Real delta, Real gam0, Real gam1,
                                     Real beta) {
...
#pragma omp simd
        for (int i = is; i <= ie; ++i) {
          ...
          utest_(n, k, j, i) = gam0 * u(n, k, j, i) +
                               gam1 * (u1(n, k, j, i) + delta * u(n, k, j, i));
        }
      }
    }
  }
...
#pragma omp simd
      for (int i = is; i <= ie; ++i) {
        ...
        bcctest_(IB1, k, j, i) = gam0 * bcc_(IB1, k, j, i) +
                                 gam1 * (b1old + delta * bcc_(IB1, k, j, i));
        bcctest_(IB2, k, j, i) = gam0 * bcc_(IB2, k, j, i) +
                                 gam1 * (b2old + delta * bcc_(IB2, k, j, i));
        bcctest_(IB3, k, j, i) = gam0 * bcc_(IB3, k, j, i) +
                                 gam1 * (b3old + delta * bcc_(IB3, k, j, i));

        bcctest_(IB2, k, j, i) +=
            dtodx1 * (e3x1_(k, j, i + 1) - e3x1_(k, j, i));
        bcctest_(IB3, k, j, i) -=
            dtodx1 * (e2x1_(k, j, i + 1) - e2x1_(k, j, i));
        if (pmb->pmy_mesh->f2) {
          bcctest_(IB1, k, j, i) -=
              dtodx2 * (e3x2_(k, j + 1, i) - e3x2_(k, j, i));
          bcctest_(IB3, k, j, i) +=
              dtodx2 * (e1x2_(k, j + 1, i) - e1x2_(k, j, i));
        }
        if (pmb->pmy_mesh->f3) {
          bcctest_(IB1, k, j, i) +=
              dtodx3 * (e2x3_(k + 1, j, i) - e2x3_(k, j, i));
          bcctest_(IB2, k, j, i) -=
              dtodx3 * (e1x3_(k + 1, j, i) - e1x3_(k, j, i));
        }
      }
    }
  }
