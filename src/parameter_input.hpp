...
//!
//! Contains data structures used to store, and functions used to access,
//! parameters read from the input file.  See comments at start of
//! parameter_input.cpp for more information on the Athena++ input file format.
...
struct InputLine {
  ...
  InputLine
      *pnext; // pointer to the next node in this nested singly linked list
};
...
public:
  ...
  std::size_t
      max_len_parname; // length of longest param_name, for nice-looking output
  std::size_t
      max_len_parvalue; // length of longest param_value, to format outputs
  InputBlock
      *pnext; // pointer to the next node in InputBlock singly linked list

  InputLine *pline; // pointer to head node in nested singly linked list (in
                    // this block)
...
public:
  ...
  // data
  InputBlock
      *pfirst_block; // pointer to head node in singly linked list of InputBlock
  // (not storing a reference to the tail node)
...
private:
  std::string
      last_filename_; // last input file opened, to prevent duplicate reads
