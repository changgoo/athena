...
void FFT3d::setup(int user_nfast, int user_nmid, int user_nslow,
                  ...
                  int user_permute, int &user_fftsize, int &user_sendsize,
                  int &user_recvsize) {
  int flag, allflag;

  if (setupflag)
    error->all("FFT is already setup");
  setupflag = 1;
  ...
  in_klo = user_in_klo;
  in_khi = user_in_khi;
  out_ilo = user_out_ilo;
  out_ihi = user_out_ihi;
  out_jlo = user_out_jlo;
  out_jhi = user_out_jhi;
  out_klo = user_out_klo;
  out_khi = user_out_khi;
  ...
  if (!prime_factorable(nfast))
    error->all("Invalid nfast");
  if (!prime_factorable(nmid))
    error->all("Invalid nmid");
  if (!prime_factorable(nslow))
    error->all("Invalid nslow");

  // error checks in indices and tiling

  flag = 0;
  if (in_ilo > in_ihi + 1 || in_jlo > in_jhi + 1 || in_klo > in_khi + 1)
    flag = 1;
  if (in_ilo < 0 || in_jlo < 0 || in_klo < 0)
    flag = 1;
  if (in_ihi >= nfast || in_jhi >= nmid || in_khi >= nslow)
    flag = 1;

  MPI_Allreduce(&flag, &allflag, 1, MPI_INT, MPI_MAX, world);

  if (allflag)
    error->all("FFT setup in/out indices are invalid");
  ...
  if (collective == 0)
    collective_bp = collective_pp = 0;
  else if (collective == 1)
    collective_bp = collective_pp = 1;
  else {
    collective_bp = 0;
    collective_pp = 1;
  }
  ...
  flag = 0;
  if (in_ilo != out_ilo || in_ihi != out_ihi || in_jlo != out_jlo ||
      in_jhi != out_jhi || in_klo != out_klo || in_khi != out_khi)
    flag = 1;
  if (permute)
    flag = 1;
  MPI_Allreduce(&flag, &allflag, 1, MPI_INT, MPI_MAX, world);
  ...
  factor(nprocs);
  if (nfactor > NFACTOR)
    error->all("Nprocs is too large to factor");

  procfactors(1, nmid, nslow, npfast1, npfast2, npfast3, ipfast1, ipfast2,
              ipfast3);
  procfactors(nfast, 1, nslow, npmid1, npmid2, npmid3, ipmid1, ipmid2, ipmid3);
  procfactors(nfast, nmid, 1, npslow1, npslow2, npslow3, ipslow1, ipslow2,
              ipslow3);

  if (exchange)
    procfactors(nfast, nmid, nslow, npbrick1, npbrick2, npbrick3, ipbrick1,
                ipbrick2, ipbrick3);
  else
    npbrick1 = npbrick2 = npbrick3 = 0;
  ...
  if (permute == 2 && out_ilo == slow_ilo && out_ihi == slow_ihi &&
      out_jlo == slow_jlo && out_jhi == slow_jhi && out_klo == slow_klo &&
      out_khi == slow_khi)
    flag = 0;
  else
    flag = 1;
  ...
  if (!inout_layout_same)
    remap_inverse_create(sendsize, recvsize);
  ...
  insize =
      (in_ihi - in_ilo + 1) * (in_jhi - in_jlo + 1) * (in_khi - in_klo + 1);
...
void FFT3d::setup_memory(FFT_SCALAR *user_sendbuf, FFT_SCALAR *user_recvbuf) {
  if (!setupflag)
    error->all("Cannot setup FFT memory before setup");
  if (memoryflag)
    error->all("Cannot setup FFT memory with memoryflag set");
...
void FFT3d::compute(FFT_SCALAR *in, FFT_SCALAR *out, int flag) {
  if (!setupflag)
    error->all("Cannot compute FFT before setup");
  if (!setup_memory_flag)
    error->all("Cannot compute FFT before setup_memory");
  ...
  if (flag == 1 || inout_layout_same) {
    ...
    if (remap_prefast)
      ...
      memcpy(out, in, insize * sizeof(FFT_SCALAR));
    ...
    if (remap_preslow)
      ...
      memcpy(out, in, outsize * sizeof(FFT_SCALAR));
...
void FFT3d::only_1d_ffts(FFT_SCALAR *in, int flag) {
  if (!setupflag)
    error->all("Cannot compute 1d FFTs before setup");
...
void FFT3d::only_remaps(FFT_SCALAR *in, FFT_SCALAR *out, int flag) {
  if (!setupflag)
    error->all("Cannot perform FFT remap before setup");
  if (!setup_memory_flag)
  ...
  if (flag == 1 || inout_layout_same) {
    ...
    if (remap_postfast)
      remap(data, data, remap_postfast);
  }
}
...
void FFT3d::only_one_remap(FFT_SCALAR *in, FFT_SCALAR *out, int flag,
                           int which) {
  if (!setupflag)
    error->all("Cannot perform an FFT remap before setup");
  if (!setup_memory_flag)
  ...
  if (flag == 1 || inout_layout_same) {
    if (which == 1) {
      ...
      if (remap_fastmid)
        remap(in, out, remap_fastmid);
    } else if (which == 3) {
      if (remap_midslow)
        remap(in, out, remap_midslow);
    } else if (which == 4) {
      if (remap_postslow)
        remap(in, out, remap_postslow);
    }
    ...
    if (which == 4) {
      ...
      if (remap_slowmid)
        remap(in, out, remap_slowmid);
    } else if (which == 2) {
      if (remap_midfast)
        remap(in, out, remap_midfast);
    } else if (which == 1) {
      if (remap_postfast)
        remap(in, out, remap_postfast);
    }
...
void FFT3d::tune(int user_nfast, int user_nmid, int user_nslow, int user_in_ilo,
                 int user_in_ihi, int user_in_jlo, int user_in_jhi,
                 int user_in_klo, int user_in_khi, int user_out_ilo,
                 int user_out_ihi, int user_out_jlo, int user_out_jhi,
                 int user_out_klo, int user_out_khi, int user_permute,
                 int &user_fftsize, int &user_sendsize, int &user_recvsize,
                 int flag, int niter, double tmax, int tflag) {
  if (setupflag)
    error->all("FFT is already setup");
  if (flag < -1 || flag > 1)
    error->all("Invalid flag arg for FFT tune");
  if (niter <= 0 || tmax < 0.0)
    error->all("Invalid niter/tmax args for FFT tune");
  if (tflag < 0 || tflag > 1)
    error->all("Invalid tflag arg for FFT tune");
  ...
  setup(user_nfast, user_nmid, user_nslow, user_in_ilo, user_in_ihi,
        user_in_jlo, user_in_jhi, user_in_klo, user_in_khi, user_out_ilo,
        user_out_ihi, user_out_jlo, user_out_jhi, user_out_klo, user_out_khi,
  ...
  if (user_fftsize > maxfftsize) {
    data = (FFT_SCALAR *)memory->srealloc(data, user_fftsize * 2 *
                                                    sizeof(FFT_SCALAR));
    for (int i = 2 * maxfftsize; i < 2 * user_fftsize; i++)
      data[i] = 0.0;
  ...
  int nruns = 10;
  if (tflag)
    timetotal = nruns * (timesetup + 3 * niter * timefft);
  else
    timetotal = nruns * (timesetup + niter * timefft);

  if (tmax > 0.0 && timetotal > tmax) {
    if (tflag)
      niter = static_cast<int>((tmax / nruns - timesetup) / timefft / 3);
    else
  ...
  if (cstart >= 0) {
    ...
    for (int cflag = cstart; cflag < cstop; cflag++) {
      for (int eflag = estart; eflag < estop; eflag++) {
        ...
        setup(user_nfast, user_nmid, user_nslow, user_in_ilo, user_in_ihi,
              user_in_jlo, user_in_jhi, user_in_klo, user_in_khi, user_out_ilo,
              user_out_ihi, user_out_jlo, user_out_jhi, user_out_klo,
              user_out_khi, user_permute, user_fftsize, user_sendsize,
              user_recvsize);
        if (user_fftsize > maxfftsize) {
          data = (FFT_SCALAR *)memory->srealloc(data, user_fftsize * 2 *
                                                          sizeof(FFT_SCALAR));
        ...
        tune_trial(data, niter, flag, tflag, tfft[ntrial], t1d[ntrial],
                   tremap[ntrial], tremap1[ntrial], tremap2[ntrial],
...
void FFT3d::tune_trial(FFT_SCALAR *data, int nper, int flag, int tflag,
  ...
  if (tflag) {
    ...
    for (int which = 1; which <= 4; which++) {
      ...
      time1 = MPI_Wtime();
      if (which == 1)
        timeremap1 = time1 - time2;
      else if (which == 2)
        timeremap2 = time1 - time2;
      else if (which == 3)
        timeremap3 = time1 - time2;
      else if (which == 4)
        timeremap4 = time1 - time2;
      time2 = time1;
    }

  } else
    time1d = timeremap = timeremap1 = timeremap2 = timeremap3 = timeremap4 =
        0.0;
}
...
void FFT3d::remap_forward_create(int &sendsize, int &recvsize) {
  ...
  if (remap_preflag) {
    ...
    remap_prefast->remap3d->setup(
        in_ilo, in_ihi, in_jlo, in_jhi, in_klo, in_khi, fast_ilo, fast_ihi,
        fast_jlo, fast_jhi, fast_klo, fast_khi, 2, 0, 0, ssize, rsize);
  ...
  if (exchange == 0) {
    ...
    remap_fastmid->remap3d->setup(
        fast_ilo, fast_ihi, fast_jlo, fast_jhi, fast_klo, fast_khi, mid_ilo,
        mid_ihi, mid_jlo, mid_jhi, mid_klo, mid_khi, 2, 1, 0, ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
    ...
    remap_fastmid->remap3d->packflag = packflag;
    remap_fastmid->remap3d->setup(fast_ilo, fast_ihi, fast_jlo, fast_jhi,
                                  fast_klo, fast_khi, brick_ilo, brick_ihi,
                                  brick_jlo, brick_jhi, brick_klo, brick_khi, 2,
                                  0, 0, ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
    ...
    remap_fastmid->remap3d_extra->setup(
        brick_ilo, brick_ihi, brick_jlo, brick_jhi, brick_klo, brick_khi,
        mid_ilo, mid_ihi, mid_jlo, mid_jhi, mid_klo, mid_khi, 2, 1, 0, ssize,
        rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
  }
  ...
  if (exchange == 0) {
    ...
    remap_midslow->remap3d->setup(mid_jlo, mid_jhi, mid_klo, mid_khi, mid_ilo,
                                  mid_ihi, slow_jlo, slow_jhi, slow_klo,
                                  slow_khi, slow_ilo, slow_ihi, 2, 1, 0, ssize,
                                  rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
    ...
    remap_midslow->remap3d->packflag = packflag;
    remap_midslow->remap3d->setup(mid_jlo, mid_jhi, mid_klo, mid_khi, mid_ilo,
                                  mid_ihi, brick_jlo, brick_jhi, brick_klo,
                                  brick_khi, brick_ilo, brick_ihi, 2, 0, 0,
                                  ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
    ...
    remap_midslow->remap3d_extra->setup(
        brick_jlo, brick_jhi, brick_klo, brick_khi, brick_ilo, brick_ihi,
        slow_jlo, slow_jhi, slow_klo, slow_khi, slow_ilo, slow_ihi, 2, 1, 0,
        ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
  }
  ...
  if (remap_postflag) {
    ...
    remap_postslow->remap3d->setup(slow_klo, slow_khi, slow_ilo, slow_ihi,
                                   slow_jlo, slow_jhi, out_klo, out_khi,
                                   out_ilo, out_ihi, out_jlo, out_jhi, 2,
                                   newpermute, 0, ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
  ...
  remap_premid->remap3d->setup(in_ilo, in_ihi, in_jlo, in_jhi, in_klo, in_khi,
                               mid_ilo, mid_ihi, mid_jlo, mid_jhi, mid_klo,
                               mid_khi, 2, 1, 0, ssize, rsize);
  sendsize = MAX(sendsize, ssize);
  recvsize = MAX(recvsize, rsize);
  ...
  remap_fastslow->remap3d->setup(
      fast_ilo, fast_ihi, fast_jlo, fast_jhi, fast_klo, fast_khi, slow_ilo,
      slow_ihi, slow_jlo, slow_jhi, slow_klo, slow_khi, 2, 2, 0, ssize, rsize);
  sendsize = MAX(sendsize, ssize);
  recvsize = MAX(recvsize, rsize);
...
void FFT3d::remap_inverse_create(int &sendsize, int &recvsize) {
  ...
  if (remap_postflag) {
    ...
    if (permute == 0) {
      ...
      remap_preslow->remap3d->setup(out_ilo, out_ihi, out_jlo, out_jhi, out_klo,
                                    out_khi, slow_ilo, slow_ihi, slow_jlo,
                                    slow_jhi, slow_klo, slow_khi, 2, 2, 0,
                                    ssize, rsize);
      sendsize = MAX(sendsize, ssize);
      recvsize = MAX(recvsize, rsize);
    } else if (permute == 1) {
      ...
      remap_preslow->remap3d->collective = collective_bp;
      remap_preslow->remap3d->packflag = packflag;
      remap_preslow->remap3d->setup(out_jlo, out_jhi, out_klo, out_khi, out_ilo,
                                    out_ihi, slow_jlo, slow_jhi, slow_klo,
                                    slow_khi, slow_ilo, slow_ihi, 2, 1, 0,
                                    ssize, rsize);
      sendsize = MAX(sendsize, ssize);
      recvsize = MAX(recvsize, rsize);
    } else if (permute == 2) {
      ...
      remap_preslow->remap3d->collective = collective_bp;
      remap_preslow->remap3d->packflag = packflag;
      remap_preslow->remap3d->setup(out_klo, out_khi, out_ilo, out_ihi, out_jlo,
                                    out_jhi, slow_klo, slow_khi, slow_ilo,
                                    slow_ihi, slow_jlo, slow_jhi, 2, 0, 0,
                                    ssize, rsize);
      sendsize = MAX(sendsize, ssize);
      recvsize = MAX(recvsize, rsize);
    }
  ...
  if (exchange == 0) {
    ...
    remap_slowmid->remap3d->setup(
        slow_klo, slow_khi, slow_ilo, slow_ihi, slow_jlo, slow_jhi, mid_klo,
        mid_khi, mid_ilo, mid_ihi, mid_jlo, mid_jhi, 2, 2, 0, ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
    ...
    remap_slowmid->remap3d->packflag = packflag;
    remap_slowmid->remap3d->setup(slow_klo, slow_khi, slow_ilo, slow_ihi,
                                  slow_jlo, slow_jhi, brick_klo, brick_khi,
                                  brick_ilo, brick_ihi, brick_jlo, brick_jhi, 2,
                                  0, 0, ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
    ...
    remap_slowmid->remap3d_extra->setup(
        brick_klo, brick_khi, brick_ilo, brick_ihi, brick_jlo, brick_jhi,
        mid_klo, mid_khi, mid_ilo, mid_ihi, mid_jlo, mid_jhi, 2, 2, 0, ssize,
        rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
  }
  ...
  if (exchange == 0) {
    ...
    remap_midfast->remap3d->setup(mid_jlo, mid_jhi, mid_klo, mid_khi, mid_ilo,
                                  mid_ihi, fast_jlo, fast_jhi, fast_klo,
                                  fast_khi, fast_ilo, fast_ihi, 2, 2, 0, ssize,
                                  rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
    ...
    remap_midfast->remap3d_extra->setup(
        brick_jlo, brick_jhi, brick_klo, brick_khi, brick_ilo, brick_ihi,
        fast_jlo, fast_jhi, fast_klo, fast_khi, fast_ilo, fast_ihi, 2, 2, 0,
        ssize, rsize);
  }
  ...
  if (remap_preflag) {
    ...
    remap_postfast->remap3d->setup(
        fast_ilo, fast_ihi, fast_jlo, fast_jhi, fast_klo, fast_khi, in_ilo,
        in_ihi, in_jlo, in_jhi, in_klo, in_khi, 2, 0, 0, ssize, rsize);
    sendsize = MAX(sendsize, ssize);
    recvsize = MAX(recvsize, rsize);
  ...
  remap_slowfast->remap3d->setup(
      slow_klo, slow_khi, slow_ilo, slow_ihi, slow_jlo, slow_jhi, fast_klo,
      fast_khi, fast_ilo, fast_ihi, fast_jlo, fast_jhi, 2, 1, 0, ssize, rsize);
  sendsize = MAX(sendsize, ssize);
  recvsize = MAX(recvsize, rsize);
  ...
  remap_postmid->remap3d->setup(mid_jlo, mid_jhi, mid_klo, mid_khi, mid_ilo,
                                mid_ihi, in_jlo, in_jhi, in_klo, in_khi, in_ilo,
                                in_ihi, 2, 2, 0, ssize, rsize);
  sendsize = MAX(sendsize, ssize);
  recvsize = MAX(recvsize, rsize);
...
void FFT3d::setup_ffts() {
  ...
  fft_fast->plan_forward =
      FFTW_API(plan_many_dft)(1, &nfast, n, NULL, &nfast, 1, nfast, NULL,
                              &nfast, 1, nfast, FFTW_FORWARD, FFTW_ESTIMATE);
  fft_fast->plan_backward =
      FFTW_API(plan_many_dft)(1, &nfast, n, NULL, &nfast, 1, nfast, NULL,
                              &nfast, 1, nfast, FFTW_BACKWARD, FFTW_ESTIMATE);
  n = fft_mid->n;
  fft_mid->plan_forward =
      FFTW_API(plan_many_dft)(1, &nmid, n, NULL, &nmid, 1, nmid, NULL, &nmid, 1,
                              nmid, FFTW_FORWARD, FFTW_ESTIMATE);
  fft_mid->plan_backward =
      FFTW_API(plan_many_dft)(1, &nmid, n, NULL, &nmid, 1, nmid, NULL, &nmid, 1,
                              nmid, FFTW_BACKWARD, FFTW_ESTIMATE);
  n = fft_slow->n;
  fft_slow->plan_forward =
      FFTW_API(plan_many_dft)(1, &nslow, n, NULL, &nslow, 1, nslow, NULL,
                              &nslow, 1, nslow, FFTW_FORWARD, FFTW_ESTIMATE);
  fft_slow->plan_backward =
      FFTW_API(plan_many_dft)(1, &nslow, n, NULL, &nslow, 1, nslow, NULL,
                              &nslow, 1, nslow, FFTW_BACKWARD, FFTW_ESTIMATE);
}

void FFT3d::perform_ffts(FFT_DATA *data, int flag, FFT1d *plan) {
  if (flag == -1)
    FFTW_API(execute_dft)(plan->plan_forward, data, data);
  else
    FFTW_API(execute_dft)(plan->plan_backward, data, data);
}
...
void FFT3d::procfactors(int nx, int ny, int nz, int &npx, int &npy, int &npz,
                        int &ipx, int &ipy, int &ipz) {
  int i, j, jk, ifac, jfac, kfac;
  double newarea;

  int sqroot = (int)std::sqrt(nprocs) + 1;
  if (sqroot * sqroot > nprocs)
    sqroot--;
  ...
  for (i = 0; i < nfactor; i++) {
    ...
    for (j = i; j < nfactor; j++) {
      jfac = factors[j];
      kfac = jk / jfac;
      if (ifac * jfac * kfac != nprocs)
        continue;
      if (ifac > jfac || jfac > kfac)
        continue;
