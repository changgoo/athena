...
// destructor
PerturbationBlock::~PerturbationBlock() { vec.DeleteAthenaArray(); }

//----------------------------------------------------------------------------------------
//! \fn PerturbationGenerator::PerturbationGenerator(Mesh *pm, ParameterInput
//! *pin) \brief PerturbationGenerator constructor
//!
//! \note
...
PerturbationGenerator::PerturbationGenerator(Mesh *pm, ParameterInput *pin)
    : FFTDriver(pm, pin),
      ...
      nhigh(
          pin->GetOrAddInteger("perturbation", "nhigh", pm->mesh_size.nx1 / 2)),
      f_shear(pin->GetOrAddReal("perturbation", "f_shear",
                                -1)), // ratio of shear component
      expo(pin->GetOrAddReal("perturbation", "expo", 2)) { // power-law exponent
  if (f_shear > 1) {
    std::stringstream msg;
    msg << "### FATAL ERROR in PerturbationGenerator" << std::endl
        << "The ratio between shear and compressible components should be less "
           "than one"
        << std::endl;
    ATHENA_ERROR(msg);
    return;
  ...
  if (rseed < 0) {
    ...
    // This would make perturbation identical irrespective of number of MPI
    // ranks, but the cost of the PowerSpectrum() function call is huge. Not
    // recommended with turb_flag = 3 or turb_flag = with small dtdrive
    global_ps_ = true;
  }
...
void PerturbationGenerator::GenerateVector() { GenerateVector(fv_); }
...
void PerturbationGenerator::AssignVector() { AssignVector(fv_); }
...
void PerturbationGenerator::GenerateScalar() { GenerateScalar(fv_[0]); }

void PerturbationGenerator::GenerateScalar(std::complex<Real> *fv) {
  PowerSpectrum(fv);
}

void PerturbationGenerator::AssignScalar() { AssignScalar(fv_[0]); }
...
void PerturbationGenerator::PowerSpectrum(std::complex<Real> *amp) {
  ...
  std::normal_distribution<Real> ndist(0.0,
      ...
      for (int gj = 0; gj < kNx2; gj++) {
        for (int gi = 0; gi < kNx1; gi++) {
          ...
          if ((nmag > nlow) && (nmag < nhigh)) {
            ...
            } else { // if it is not in FFTBlock, just burn unused random
                     // numbers
              Real A = ndist(rng_generator);
              Real ph = udist(rng_generator) * TWO_PI;
            }
  ...
  Real dkx = pfb->dkx[0];
  if (knx2 > 1)
    dkx = dkx < pfb->dkx[1] ? dkx : pfb->dkx[1];
  if (knx3 > 2)
    dkx = dkx < pfb->dkx[2] ? dkx : pfb->dkx[2];
    ...
    for (int j = 0; j < knx2; j++) {
      for (int i = 0; i < knx1; i++) {
        ...
        if (global_ps_) {
          ...
          Real ph = udist(rng_generator) * TWO_PI;
          amp[kidx] =
              pcoeff * A * std::complex<Real>(std::cos(ph), std::sin(ph));
        }
      }
    }
  }
...
//----------------------------------------------------------------------------------------
//! \fn void PerturbationGenerator::Project(std::complex<Real> **fv, Real
//! f_shear) \brief calculate velocity field with a given ratio of shear to
//! comp.
...
void PerturbationGenerator::Project(std::complex<Real> **fv, Real f_shear) {
  ...
  for (int nv = 0; nv < 3; nv++) {
    for (int kidx = 0; kidx < pfb->cnt_; kidx++) {
      fv[nv][kidx] =
          (1 - f_shear) * fv_co_[nv][kidx] + f_shear * fv_sh_[nv][kidx];
    }
  }
}
...
void PerturbationGenerator::Project(std::complex<Real> **fv,
                                    std::complex<Real> **fv_sh,
                                    std::complex<Real> **fv_co) {
  FFTBlock *pfb = pmy_fb;
  AthenaFFTIndex *idx = pfb->b_in_;
    ...
    for (int j = 0; j < knx2; j++) {
      for (int i = 0; i < knx1; i++) {
        ...
        if (gidx == 0.0) {
          ...
          kz /= kmag;
          // Form (khat.f)
          std::complex<Real> kdotf =
              kx * fv[0][kidx] + ky * fv[1][kidx] + kz * fv[2][kidx];
...
//! \brief Get k index, which runs from 0, 1, ... Nx/2-1, -Nx/2, -Nx/2+1, ...,
//! -1.
