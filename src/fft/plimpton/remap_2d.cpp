...
void remap_2d(double *in, double *out, double *buf, struct remap_plan_2d *plan)
...
remap_2d_create_plan(MPI_Comm comm, int in_ilo, int in_ihi, int in_jlo,
                     int in_jhi, int out_ilo, int out_ihi, int out_jlo,
                     int out_jhi, int nqty, int permute, int memory,
                     int precision)

{
  ...
  MPI_Allgather(&out, sizeof(struct extent_2d), MPI_BYTE, array,
                sizeof(struct extent_2d), MPI_BYTE, comm);
  ...
  if (nsend) {
    ...
    plan->packplan =
        (struct pack_plan_2d *)malloc(nsend * sizeof(struct pack_plan_2d));
  ...
  MPI_Allgather(&in, sizeof(struct extent_2d), MPI_BYTE, array,
                sizeof(struct extent_2d), MPI_BYTE, comm);
  ...
  if (nrecv) {
    ...
    plan->unpackplan =
        (struct pack_plan_2d *)malloc(nrecv * sizeof(struct pack_plan_2d));
