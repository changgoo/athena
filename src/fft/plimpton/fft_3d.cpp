...
struct fft_plan_3d *
fft_3d_create_plan(MPI_Comm comm, int nfast, int nmid, int nslow, int in_ilo,
                   int in_ihi, int in_jlo, int in_jhi, int in_klo, int in_khi,
                   int out_ilo, int out_ihi, int out_jlo, int out_jhi,
                   int out_klo, int out_khi, int scaled, int permute, int *nbuf)

{
  ...
  if (remapflag == 0) {
    ...
    plan->pre_plan =
        remap_3d_create_plan(comm, in_ilo, in_ihi, in_jlo, in_jhi, in_klo,
                             in_khi, first_ilo, first_ihi, first_jlo, first_jhi,
                             first_klo, first_khi, FFT_PRECISION, 0, 0, 2);
    if (plan->pre_plan == NULL)
      return NULL;

    if (Globals::my_rank == 0)
      std::cout
          << "### WARNING in MPIFFT: " << std::endl
  ...
  plan->length1 = nfast;
  plan->total1 =
      nfast * (first_jhi - first_jlo + 1) * (first_khi - first_klo + 1);
  ...
  plan->mid1_plan = remap_3d_create_plan(
      comm, first_ilo, first_ihi, first_jlo, first_jhi, first_klo, first_khi,
      second_ilo, second_ihi, second_jlo, second_jhi, second_klo, second_khi,
      FFT_PRECISION, 1, 0, 2);
  if (plan->mid1_plan == NULL)
    return NULL;
  ...
  plan->length2 = nmid;
  plan->total2 =
      (second_ihi - second_ilo + 1) * nmid * (second_khi - second_klo + 1);
  ...
  plan->mid2_plan = remap_3d_create_plan(
      comm, second_jlo, second_jhi, second_klo, second_khi, second_ilo,
      second_ihi, third_jlo, third_jhi, third_klo, third_khi, third_ilo,
      third_ihi, FFT_PRECISION, 1, 0, 2);
  if (plan->mid2_plan == NULL)
    return NULL;
  ...
  plan->length3 = nslow;
  plan->total3 =
      (third_ihi - third_ilo + 1) * (third_jhi - third_jlo + 1) * nslow;

  /* remap from 3rd FFT to final distribution
     not needed if permute = 2 and third indices = out indices on all procs */

  if (permute == 2 && out_ilo == third_ilo && out_ihi == third_ihi &&
      out_jlo == third_jlo && out_jhi == third_jhi && out_klo == third_klo &&
      out_khi == third_khi)
    flag = 0;
  else
    flag = 1;
  ...
  if (remapflag == 0)
    ...
    plan->post_plan = remap_3d_create_plan(
        comm, third_klo, third_khi, third_ilo, third_ihi, third_jlo, third_jhi,
        out_klo, out_khi, out_ilo, out_ihi, out_jlo, out_jhi, FFT_PRECISION,
        (permute + 1) % 3, 0, 2);
    if (plan->post_plan == NULL)
      return NULL;
  ...
  /* configure plan memory pointers and allocate work space
     ...
     first/second/third_size = amount of memory needed after pre,mid1,mid2
     remaps copy_size = amount needed internally for extra copy of data
  ...
  *nbuf = copy_size + scratch_size;
  ...
  if (plan->length2 == plan->length1) {
    ...
    plan->plan_mid_backward = fftw_plan_many_dft(
        1, &(plan->length2), plan->total2 / plan->length2, plan->scratch, NULL,
        1, plan->length2, plan->scratch, NULL, 1, plan->length2, FFTW_BACKWARD,
        FFTW_ESTIMATE);
  }
  ...
  if (plan->length3 == plan->length1) {
    ...
    plan->plan_slow_backward = fftw_plan_many_dft(
        1, &(plan->length3), plan->total3 / plan->length3, plan->scratch, NULL,
        1, plan->length3, plan->scratch, NULL, 1, plan->length3, FFTW_BACKWARD,
        FFTW_ESTIMATE);
  }
...

{
  ...
  if (plan->mid1_plan)
    remap_3d_destroy_plan(plan->mid1_plan);
  if (plan->mid2_plan)
    remap_3d_destroy_plan(plan->mid2_plan);
  if (plan->post_plan)
    remap_3d_destroy_plan(plan->post_plan);
