...
void remap_3d(double *in, double *out, double *buf, struct remap_plan_3d *plan)

{
  ...
  for (irecv = 0; irecv < plan->nrecv; irecv++)
    MPI_Irecv(&scratch[plan->recv_bufloc[irecv]], plan->recv_size[irecv],
              MPI_DOUBLE, plan->recv_proc[irecv], 0, plan->comm,
              &plan->request[irecv]);

  /* send all messages to other procs */

  for (isend = 0; isend < plan->nsend; isend++) {
    plan->pack(&in[plan->send_offset[isend]], plan->sendbuf,
               &plan->packplan[isend]);
  ...
  if (plan->self) {
    ...
    plan->pack(&in[plan->send_offset[isend]],
               &scratch[plan->recv_bufloc[irecv]], &plan->packplan[isend]);
...
struct remap_plan_3d *remap_3d_create_plan(
    MPI_Comm comm, int in_ilo, int in_ihi, int in_jlo, int in_jhi, int in_klo,
    int in_khi, int out_ilo, int out_ihi, int out_jlo, int out_jhi, int out_klo,
    int out_khi, int nqty, int permute, int memory, int precision)

{
  ...
  if (precision == 1) {
    if (me == 0)
      printf("Single precision not supported\n");
    return NULL;
  }
  ...
  MPI_Allgather(&out, sizeof(struct extent_3d), MPI_BYTE, array,
                sizeof(struct extent_3d), MPI_BYTE, comm);
  ...
  if (nsend) {
    ...
    plan->packplan =
        (struct pack_plan_3d *)malloc(nsend * sizeof(struct pack_plan_3d));

    if (plan->send_offset == NULL || plan->send_size == NULL ||
        plan->send_proc == NULL || plan->packplan == NULL)
      return NULL;
  }
  ...
  for (i = 0; i < nprocs; i++) {
    ...
    if (remap_3d_collide(&in, &array[iproc], &overlap)) {
      ...
      plan->send_size[nsend] =
          nqty * overlap.isize * overlap.jsize * overlap.ksize;
      nsend++;
    }
  }
  ...
  MPI_Allgather(&in, sizeof(struct extent_3d), MPI_BYTE, array,
                sizeof(struct extent_3d), MPI_BYTE, comm);
  ...
  if (nrecv) {
    ...
    plan->unpackplan =
        (struct pack_plan_3d *)malloc(nrecv * sizeof(struct pack_plan_3d));

    if (plan->recv_offset == NULL || plan->recv_size == NULL ||
        plan->recv_proc == NULL || plan->recv_bufloc == NULL ||
        plan->request == NULL || plan->unpackplan == NULL)
      return NULL;
  }
  ...
  if (memory == 1) {
    if (nrecv > 0) {
      if (precision == 1)
        ...
        plan->scratch = (double *)malloc(nqty * out.isize * out.jsize *
                                         out.ksize * sizeof(double));
