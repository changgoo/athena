...
FFTBlock::FFTBlock(FFTDriver *pfd, LogicalLocation iloc, int igid,
    ...
    : // public:
      ...
      // protected:
      pmy_driver_(pfd), cnt_(bsize.nx1 * bsize.nx2 * bsize.nx3),
      gcnt_(pmy_driver_->gcnt_), gid_(igid), fplan_{}, bplan_{},
      norm_factor_(1.0), dim_(pmy_driver_->dim_), loc_(iloc), msize_(msize),
      bsize_(bsize), orig_idx_{dim_, loc_, msize_, bsize_} {
...
#ifdef FFT
#ifdef MPI_PARALLEL
  if (plan->plan3d != nullptr)
    fft_3d_destroy_plan(plan->plan3d);
  if (plan->plan2d != nullptr)
  ...
  for (int k = 0; k < Nx[2]; ++k) {
    for (int j = 0; j < Nx[1]; ++j) {
      for (int i = 0; i < Nx[0]; ++i) {
        ...
        if (in == 1)
          std::cout << std::real(in_[idx]) << " ";
        if (in == -1)
          std::cout << std::real(out_[idx]) << " ";
      }
      std::cout << std::endl;
    }
    std::cout << std::endl;
...
//!                                   int ngh, LogicalLocation loc, RegionSize
//!                                   bsize)
...
//----------------------------------------------------------------------------------------
//! \fn void FFTBlock::LoadSource(const AthenaArray<Real> &src, bool nu, int
//! ngh,
//!                               LogicalLocation loc, RegionSize bsize)
...
AthenaFFTPlan *FFTBlock::QuickCreatePlan(std::complex<Real> *data,
  ...
  if (dim_ == 3)
    return CreatePlan(nfast, nmid, nslow, data, dir);
  else if (dim_ == 2)
    return CreatePlan(nfast, nmid, data, dir);
  else
    return CreatePlan(nfast, data, dir);
}
...
AthenaFFTPlan *FFTBlock::CreatePlan(int nfast, std::complex<Real> *data,
  ...
  plan->dim = dim_;
  if (dir == AthenaFFTDirection::forward)
    plan->plan = fftw_plan_dft_1d(nfast, reinterpret_cast<fftw_complex *>(data),
...
AthenaFFTPlan *FFTBlock::CreatePlan(int nfast, int nslow,
  ...
  if (dir == AthenaFFTDirection::forward) {
    ...
    plan->plan2d = fft_2d_create_plan(
        MPI_COMM_WORLD, nfast, nslow, f_in_->is[0], f_in_->ie[0], f_in_->is[1],
        f_in_->ie[1], f_out_->is[f_in_->iloc[0]], f_out_->ie[f_in_->iloc[0]],
        f_out_->is[f_in_->iloc[1]], f_out_->ie[f_in_->iloc[1]], 0, permute1_,
        &nbuf);
  } else {
    plan->dir = FFTW_BACKWARD;
    plan->plan2d = fft_2d_create_plan(
        MPI_COMM_WORLD, nfast, nslow, b_in_->is[0], b_in_->ie[0], b_in_->is[1],
        b_in_->ie[1], b_out_->is[b_in_->iloc[0]], b_out_->ie[b_in_->iloc[0]],
        b_out_->is[b_in_->iloc[1]], b_out_->ie[b_in_->iloc[1]], 0, permute2_,
        &nbuf);
  }
  plan->plan3d = nullptr;
  plan->plan = nullptr;
#else // MPI_PARALLEL
  if (dir == AthenaFFTDirection::forward)
    plan->plan = fftw_plan_dft_2d(
        nslow, nfast, reinterpret_cast<fftw_complex *>(data),
        reinterpret_cast<fftw_complex *>(data), FFTW_FORWARD, FFTW_MEASURE);
  else
    plan->plan = fftw_plan_dft_2d(
        nslow, nfast, reinterpret_cast<fftw_complex *>(data),
        reinterpret_cast<fftw_complex *>(data), FFTW_BACKWARD, FFTW_MEASURE);
#endif
#endif // FFT
...
AthenaFFTPlan *FFTBlock::CreatePlan(int nfast, int nmid, int nslow,
  ...
  if (dir == AthenaFFTDirection::forward) {
    ...
    plan->dir = FFTW_FORWARD;
    plan->plan3d = fft_3d_create_plan(
        MPI_COMM_WORLD, nfast, nmid, nslow, f_in_->is[0], f_in_->ie[0],
        f_in_->is[1], f_in_->ie[1], f_in_->is[2], f_in_->ie[2], ois[0], oie[0],
        ois[1], oie[1], ois[2], oie[2], 0, permute1_, &nbuf);
  } else {
    for (int l = 0; l < dim_; l++) {
    ...
    plan->dir = FFTW_BACKWARD;
    plan->plan3d = fft_3d_create_plan(
        MPI_COMM_WORLD, nfast, nmid, nslow, b_in_->is[0], b_in_->ie[0],
        b_in_->is[1], b_in_->ie[1], b_in_->is[2], b_in_->ie[2], ois[0], oie[0],
        ois[1], oie[1], ois[2], oie[2], 0, permute2_, &nbuf);
  }
  plan->plan2d = nullptr;
  plan->plan = nullptr;
#else // MPI_PARALLEL
  if (dir == AthenaFFTDirection::forward) {
    plan->plan = fftw_plan_dft_3d(
        nslow, nmid, nfast, reinterpret_cast<fftw_complex *>(data),
        reinterpret_cast<fftw_complex *>(data), FFTW_FORWARD, FFTW_MEASURE);
  } else {
    plan->plan = fftw_plan_dft_3d(
        nslow, nmid, nfast, reinterpret_cast<fftw_complex *>(data),
...
void FFTBlock::Execute(AthenaFFTPlan *plan) {
  ...
  if (plan->dim == 3)
    fft_3d(reinterpret_cast<fftw_complex *>(in_),
           reinterpret_cast<fftw_complex *>(out_), plan->dir, plan->plan3d);
  if (plan->dim == 2)
    fft_2d(reinterpret_cast<fftw_complex *>(in_),
           reinterpret_cast<fftw_complex *>(out_), plan->dir, plan->plan2d);
#else
...
#ifdef MPI_PARALLEL
  if (plan->dim == 3)
    fft_3d(reinterpret_cast<fftw_complex *>(data),
           reinterpret_cast<fftw_complex *>(data), plan->dir, plan->plan3d);
  if (plan->dim == 2)
    fft_2d(reinterpret_cast<fftw_complex *>(data),
           reinterpret_cast<fftw_complex *>(data), plan->dir, plan->plan2d);
#else
...
//----------------------------------------------------------------------------------------
//! \fn void FFTBlock::Execute(AthenaFFTPlan *plan,
//!                             std::complex<Real> *in_data,std::complex<Real>
//!                             *out_data)
...
#ifdef MPI_PARALLEL
  if (plan->dim == 3)
    fft_3d(reinterpret_cast<fftw_complex *>(in_data),
           reinterpret_cast<fftw_complex *>(out_data), plan->dir, plan->plan3d);
  if (plan->dim == 2)
    fft_2d(reinterpret_cast<fftw_complex *>(in_data),
           reinterpret_cast<fftw_complex *>(out_data), plan->dir, plan->plan2d);
#else
...
//! (3) swap axes from the output of forward FFT to prepare backward FFT
//! (swap2==treu).
  ...
  if ((pdim_ == 2 || pdim_ == 1) && dim_ == 3) {
    ...
    if (decomp_ == DecompositionNames::x_decomp) {
      ...
      msg << "Something wrong with " << pdim_ << "D decomposition!" << std::endl
          << "Current MPI Configuration is " << orig_idx_.np[0] << " x "
          << orig_idx_.np[1] << " x " << orig_idx_.np[2] << std::endl;
      ATHENA_ERROR(msg);
    }
  } else {
    // For 3D block decompsition, simply set indices as in original Athena
    // Array. two additional remapping will be performed to prepare and recover
    // indices.
    swap1_ = false;
    swap2_ = false;
    permute0_ = 0;
...
AthenaFFTIndex::AthenaFFTIndex(int dim, LogicalLocation loc, RegionSize msize,
                               RegionSize bsize) {
  dim_ = dim;
  // KGF: loc.lxi are std::int64_t in general, but w/o AMR, they are unilikely
  // to overflow std::int32_t type limits
...
// For safety when linking to other TUs which might use these function templates
// in the future, provide explicit instantiations (not using template argument
// deduction)
...
void AthenaFFTIndex::PrintIndex() {
  std::cout << "Lx:" << Lx[0] << " " << Lx[1] << " " << Lx[2] << std::endl
            ...
            << "iloc:" << iloc[0] << " " << iloc[1] << " " << iloc[2]
            << std::endl
            << "ploc:" << ploc[0] << " " << ploc[1] << " " << ploc[2]
            << std::endl;
}
