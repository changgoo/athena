...
FFTDriver::FFTDriver(Mesh *pm, ParameterInput *pin)
    : nranks_(Globals::nranks), pmy_mesh_(pm), dim_(pm->ndim) {
  ...
  for (int n = ns; n < ne; n++) {
    ...
    if (pm->loclist[n].level > current_level)
      current_level = pm->loclist[n].level;
  }
  ...
  if (pm->nbtotal / nmb != nranks_) {
    // this restriction (to a single cuboid FFTBlock that covers the union of
    // all MeshBlocks owned by an MPI rank) should be relaxed in the future
    std::stringstream msg;
    msg << "### FATAL ERROR in FFTDriver::FFTDriver" << std::endl
        << nmb << " MeshBlocks will be loaded to the FFT block." << std::endl
        << "Number of FFT blocks " << pm->nbtotal / nmb
  ...
  // MeshBlocks in the same processor will be gathered into FFTBlock of the
  // processor.
...
void FFTDriver::InitializeFFTBlock(bool set_norm) {
  ...
  pmy_fb = new FFTBlock(this, fft_loclist_[igid], igid, fft_mesh_size_,
                        fft_block_size_);
  if (set_norm)
