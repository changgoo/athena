...
//! variables in general-relativistic hydrodynamics, as well as for computing
//! wavespeeds.
...
bool ConservedToPrimitiveNormal(const AthenaArray<Real> &dd_vals,
                                const AthenaArray<Real> &ee_vals,
                                const AthenaArray<Real> &mm_vals,
                                Real gamma_adi, Real pgas_old, int k, int j,
                                int i, AthenaArray<Real> &prim,
                                Real *p_gamma_lor);
...
//!   AthenaArray<Real> &cons, const AthenaArray<Real> &prim_old, const
//!   FaceField &bb, AthenaArray<Real> &prim, AthenaArray<Real> &bb_cc,
...
void EquationOfState::ConservedToPrimitive(
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      CalculateNormalConserved(cons, g_, g_inv_, k, j, il, iu, normal_dd_,
                               normal_ee_, normal_mm_);
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        Real ee_min =
            density_floor_local + pressure_floor_local / (gamma_adi - 1.0);
        ...
        bool success = ConservedToPrimitiveNormal(
            normal_dd_, normal_ee_, normal_mm_, gamma_adi,
        ...
        if (fixed) {
          PrimitiveToConservedSingle(prim, gamma_adi, g_, g_inv_, k, j, i, cons,
                                     pco);
        }
      }
    }
...
//!    const AthenaArray<Real> &bb_cc, AthenaArray<Real> &cons, Coordinates
...
//! Notes:
//!  - Single-cell function exists for other purposes; call made to that
//!  function rather
...
void EquationOfState::PrimitiveToConserved(const AthenaArray<Real> &prim,
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      for (int i = il; i <= iu; ++i) {
        PrimitiveToConservedSingle(prim, gamma_, g_, g_inv_, k, j, i, cons,
                                   pco);
      }
    }
  }
...
void EquationOfState::SoundSpeedsSR(Real rho_h, Real pgas, Real vx,
                                    Real gamma_lorentz_sq, Real *plambda_plus,
                                    Real *plambda_minus) {
  const Real gamma_adi = gamma_;
...
//----------------------------------------------------------------------------------------
//! \fn void EquationOfState::SoundSpeedsGR(Real rho_h, Real pgas, Real u0, Real
//! u1,
//!    Real g00, Real g01, Real g11, Real *plambda_plus, Real *plambda_minus)
//! \brief Function for calculating relativistic sound speeds in arbitrary
//! coordinates
//!
//! Inputs:
//!  - rho_h: enthalpy per unit volume
...
void EquationOfState::SoundSpeedsGR(Real rho_h, Real pgas, Real u0, Real u1,
                                    Real g00, Real g01, Real g11,
                                    Real *plambda_plus, Real *plambda_minus) {
  // Parameters and constants
  const Real discriminant_tol =
      -1.0e-10; // values between this and 0 are considered 0
  const Real gamma_adi = gamma_;
...
// Notes:
//   Generalizes and specializes Newman & Hamlin 2014, SIAM J. Sci. Comput.
//   36(4) B661
//       (NH).
