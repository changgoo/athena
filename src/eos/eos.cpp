...
void EquationOfState::ConservedToPrimitiveTest(const AthenaArray<Real> &cons,
...
void EquationOfState::SingleConservativeToPrimitiveHydro(
    Real &u_d, Real &u_m1, Real &u_m2, Real &u_m3, Real &u_e, Real &w_d,
    Real &w_vx, Real &w_vy, Real &w_vz, Real &w_p, Real &dp, bool &dfloor_used,
    bool &pfloor_used) {
...
void EquationOfState::NeighborAveragingConserved(
    ...
    AthenaArray<Real> &cons_avg, AthenaArray<Real> &prim_avg, int k, int j,
    int i, int il, int iu, int jl, int ju, int kl, int ku) {
  cons_avg.ZeroClear();
  ...
  for (int idx = 0; idx < 6; ++idx) {
    ...
    if ((i0 < il) || (i0 > iu) || (j0 < jl) || (j0 > ju) || (k0 < kl) ||
        (k0 > ku))
      continue;
  ...
  // assign means
  for (int n = 0; n < nvars; ++n)
    cons_avg(n) = cons_avg(n) / vol_neighbors;
  ...
  if (NON_BAROTROPIC_EOS) {
    Real gm1 = gamma_ - 1.0;
    Real e_k = 0.5 * di *
               (SQR(cons_avg(IM1)) + SQR(cons_avg(IM2)) + SQR(cons_avg(IM3)));
    Real eint_avg;
...
void EquationOfState::NeighborAveragingEint(const AthenaArray<Real> &cons,
                                            const AthenaArray<Real> &bcc,
                                            Real &eint_avg, int k, int j, int i,
                                            int il, int iu, int jl, int ju,
                                            int kl, int ku) {
  Real gm1 = gamma_ - 1.0;
  Real vol_neighbors = 0.0;
  ...
  // apply floor if there is no good neighbors
  eint_avg =
      vol_neighbors > 0 ? q_neighbors / vol_neighbors : pressure_floor_ * gm1;
