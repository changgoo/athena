...
public:
  ...
  void ConservedToPrimitive(AthenaArray<Real> &cons,
                            ...
                            AthenaArray<Real> &bcc, Coordinates *pco, int il,
                            int iu, int jl, int ju, int kl, int ku);
  void PrimitiveToConserved(const AthenaArray<Real> &prim,
  ...
  void ConservedToPrimitiveTest(const AthenaArray<Real> &cons,
                                const AthenaArray<Real> &bcc, int il, int iu,
                                int jl, int ju, int kl, int ku);
  void SingleConservativeToPrimitiveHydro(Real &u_d, Real &u_m1, Real &u_m2,
                                          Real &u_m3, Real &u_e, Real &w_d,
                                          Real &w_vx, Real &w_vy, Real &w_vz,
                                          Real &w_p, Real &dp,
                                          bool &dfloor_used, bool &pfloor_used);
  void SingleConservativeToPrimitiveMHD(Real &u_d, Real &u_m1, Real &u_m2,
                                        Real &u_m3, Real &u_e, Real &w_d,
                                        Real &w_vx, Real &w_vy, Real &w_vz,
                                        Real &w_p, Real &dp, bool &dfloor_used,
                                        bool &pfloor_used, const Real e_mag);
  ...
  void PassiveScalarConservedToPrimitiveCellAverage(
      ...
      AthenaArray<Real> &r, Coordinates *pco, int il, int iu, int jl, int ju,
      int kl, int ku);

  void NeighborAveragingConserved(const AthenaArray<Real> &cons,
                                  const AthenaArray<Real> &bcc,
                                  AthenaArray<Real> &cons_avg,
                                  AthenaArray<Real> &prim_avg, int k, int j,
                                  int i, int il, int iu, int jl, int ju, int kl,
                                  int ku);

  void NeighborAveragingEint(const AthenaArray<Real> &cons,
                             const AthenaArray<Real> &bcc, Real &eint_avg,
                             int k, int j, int i, int il, int iu, int jl,
                             int ju, int kl, int ku);

  // pass k, j, i to following 2x functions even though x1-sliced input array is
  // expected in order to accomodate position-dependent floors
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this, s, n, k, j) linear(i)
  void ApplyPassiveScalarFloors(AthenaArray<Real> &s, int n, int k, int j,
                                int i);

#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this, s, w, r, n, k, j)   \
    linear(i)
  void ApplyPassiveScalarPrimitiveConservedFloors(AthenaArray<Real> &s,
                                                  const AthenaArray<Real> &w,
                                                  AthenaArray<Real> &r, int n,
                                                  int k, int j, int i);
...
#pragma omp declare simd simdlen(SIMD_WIDTH)                                   \
    uniform(this, prim, cons, bcc, k, j) linear(i)
  void ApplyPrimitiveConservedFloors(AthenaArray<Real> &prim,
                                     AthenaArray<Real> &cons,
                                     AthenaArray<Real> &bcc, int k, int j,
                                     int i);
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  ...
  Real FastMagnetosonicSpeed(const Real[], const Real) { return 0.0; }
  void ApplyPrimitiveConservedFloors(AthenaArray<Real> &, AthenaArray<Real> &,
                                     AthenaArray<Real> &, int, int, int) {
    return;
  }
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  ...
  void SoundSpeedsSR(Real, Real, Real, Real, Real *, Real *) { return; }
  void FastMagnetosonicSpeedsGR(Real wgas, Real pgas, Real u0, Real u1,
...
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  void SoundSpeedsSR(Real rho_h, Real pgas, Real vx, Real gamma_lorentz_sq,
                     Real *plambda_plus, Real *plambda_minus);
#pragma omp declare simd simdlen(SIMD_WIDTH) uniform(this)
  void SoundSpeedsGR(Real rho_h, Real pgas, Real u0, Real u1, Real g00,
                     Real g01, Real g11, Real *plambda_plus,
                     Real *plambda_minus);
  void FastMagnetosonicSpeedsGR(Real, Real, Real, Real, Real, Real, Real, Real,
                                Real *, Real *) {
    return;
