...
//! variables in special-relativistic MHD, as well as for computing wavespeeds.
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        normal_mm_(0, i) = SQR(cons(IM1, k, j, i)) + SQR(cons(IM2, k, j, i)) +
                           SQR(cons(IM3, k, j, i));
        normal_mm_(1, i) = cons(IM1, k, j, i);
        normal_mm_(2, i) = cons(IM2, k, j, i);
        normal_mm_(3, i) = cons(IM3, k, j, i);
        normal_bb_(0, i) = SQR(bb_cc(IB1, k, j, i)) + SQR(bb_cc(IB2, k, j, i)) +
                           SQR(bb_cc(IB3, k, j, i));
        ...
        normal_tt_(i) = cons(IM1, k, j, i) * bb_cc(IB1, k, j, i) +
                        cons(IM2, k, j, i) * bb_cc(IB2, k, j, i) +
                        cons(IM3, k, j, i) * bb_cc(IB3, k, j, i);
      }
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        bool success = ConservedToPrimitiveNormal(
            ...
            gamma_adi, pgas_old, k, j, i, prim, &gamma, &pmag);
        ...
        if (velocity_ceiling) {
          Real b0 = u1 * bb_cc(IB1, k, j, i) + u2 * bb_cc(IB2, k, j, i) +
                    u3 * bb_cc(IB3, k, j, i);
