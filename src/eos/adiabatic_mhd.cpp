...
EquationOfState::EquationOfState(MeshBlock *pmb, ParameterInput *pin)
    : beta(1.0), bookkeeping(false), pmy_block_(pmb),
      neighbor_flooring_{
          pin->GetOrAddBoolean("hydro", "neighbor_flooring", false)},
...
//! \brief For the Hydro, converts conserved into primitive variables in
//! adiabatic MHD.
...
void EquationOfState::ConservedToPrimitive(
  ...
  pmy_block_->pfield->CalculateCellCenteredField(b, bcc, pco, il, iu, jl, ju,
                                                 kl, ku);
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      for (int i = il; i <= iu; ++i) {
        ...
        SingleConservativeToPrimitiveMHD(u_d, u_m1, u_m2, u_m3, u_e, w_d, w_vx,
                                         w_vy, w_vz, w_p, dp, dfloor_used,
                                         pfloor_used, e_mag);
      ...
      for (int j = jl; j <= ju; ++j) {
        for (int i = il; i <= iu; ++i) {
          if (nbavg_d_(k, j, i)) {
            ...
            NeighborAveragingConserved(cons, bcc, cons_avg, prim_avg, k, j, i,
                                       il, iu, jl, ju, kl, ku);
            ...
            if (bookkeeping)
              efloor(k, j, i) += (prim_avg(IPR) * gm1 - eint_prev) * beta;
            ...
            Real eint_avg;
            NeighborAveragingEint(cons, bcc, eint_avg, k, j, i, il, iu, jl, ju,
                                  kl, ku);
            if (bookkeeping)
              efloor(k, j, i) += (eint_avg - eint_prev) * beta;
...
//!        Note that this function assumes cell-centered fields are already
//!        calculated
...
//----------------------------------------------------------------------------------------
//! \fn Real EquationOfState::FastMagnetosonicSpeed(const Real prim[], const
//! Real bx) \brief returns fast magnetosonic speed given vector of primitive
//! variables Note the formula for (C_f)^2 is positive definite, so this func
//! never returns a NaN
Real EquationOfState::FastMagnetosonicSpeed(const Real prim[(NWAVE)],
                                            const Real bx) {
...
//! \brief Apply pressure (prim) floor and correct energy (cons) (typically
//! after W(U))
void EquationOfState::ApplyPrimitiveConservedFloors(AthenaArray<Real> &prim,
  ...
  u_e = (w_p > pressure_floor_) ? u_e : ((pressure_floor_ / gm1) + pb + e_k);
  w_p = (w_p > pressure_floor_) ? w_p : pressure_floor_;
