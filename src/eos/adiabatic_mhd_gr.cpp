...
//! variables in general-relativistic MHD, as well as for computing wavespeeds.
...
namespace {
// Declarations
void CalculateNormalConserved(const AthenaArray<Real> &cons,
                              const AthenaArray<Real> &bb,
                              const AthenaArray<Real> &g,
                              const AthenaArray<Real> &gi, int k, int j, int il,
                              int iu, AthenaArray<Real> &dd,
...
bool ConservedToPrimitiveNormal(
    ...
    const AthenaArray<Real> &tt_vals, Real gamma_adi, Real pgas_old, int k,
    int j, int i, AthenaArray<Real> &prim, Real *p_gamma_lor, Real *p_pmag);
void PrimitiveToConservedSingle(const AthenaArray<Real> &prim, Real gamma_adi,
                                const AthenaArray<Real> &bb_cc,
                                const AthenaArray<Real> &g,
                                const AthenaArray<Real> &gi, int k, int j,
                                int i, AthenaArray<Real> &cons,
                                Coordinates *pco);
} // namespace
...
EquationOfState::EquationOfState(MeshBlock *pmb, ParameterInput *pin)
    : pmy_block_(pmb), gamma_{pin->GetReal("hydro", "gamma")},
      density_floor_{
...
void EquationOfState::ConservedToPrimitive(
    ...
    const FaceField &bb, AthenaArray<Real> &prim, AthenaArray<Real> &bb_cc,
    Coordinates *pco, int il, int iu, int jl, int ju, int kl, int ku) {
  // Parameters
  const Real mm_sq_ee_sq_max =
      1.0 - 1.0e-12; // max. of squared momentum over energy

  // Extract ratio of specific heats
  const Real &gamma_adi = gamma_;

  // Interpolate magnetic field from faces to cell centers
  pmy_block_->pfield->CalculateCellCenteredField(bb, bb_cc, pco, il, iu, jl, ju,
                                                 kl, ku);
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      CalculateNormalConserved(cons, bb_cc, g_, g_inv_, k, j, il, iu,
                               normal_dd_, normal_ee_, normal_mm_, normal_bb_,
                               normal_tt_);
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        if (rho_pow_ != 0.0) {
          density_floor_local = std::max(
              density_floor_local, rho_min_ * std::pow(pco->x1v(i), rho_pow_));
        }
        Real pressure_floor_local = pressure_floor_;
        if (pgas_pow_ != 0.0) {
          pressure_floor_local =
              std::max(pressure_floor_local,
        ...
        Real ee_min = density_floor_local +
                      pressure_floor_local / (gamma_adi - 1.0) +
                      0.5 * normal_bb_(0, i);
        ...
        // Set primitives
        Real gamma, pmag;
        bool success = ConservedToPrimitiveNormal(
            normal_dd_, normal_ee_, normal_mm_, normal_bb_, normal_tt_,
            gamma_adi, prim_old(IPR, k, j, i), k, j, i, prim, &gamma, &pmag);
        ...
        if (sigma_max_ > 0.0) {
          density_floor_local =
              std::max(density_floor_local, 2.0 * pmag / sigma_max_);
        }
        if (beta_min_ > 0.0) {
          pressure_floor_local =
              std::max(pressure_floor_local, beta_min_ * pmag);
        }
        ...
        if (success && (rho_add > 0.0 || pgas_add > 0.0)) {
          ...
          success = ConservedToPrimitiveNormal(
              normal_dd_, normal_ee_, normal_mm_, normal_bb_, normal_tt_,
              gamma_adi, prim_old(IPR, k, j, i), k, j, i, prim, &gamma, &pmag);
        ...
        if (!success) {
          Real tmp = g_(I11, i) * SQR(uu1) + 2.0 * g_(I12, i) * uu1 * uu2 +
        ...
        if (velocity_ceiling) {
          ...
          Real b0 = g_(I01, i) * u0 * bb1 + g_(I02, i) * u0 * bb2 +
                    g_(I03, i) * u0 * bb3 + g_(I11, i) * u1 * bb1 +
                    g_(I12, i) * u1 * bb2 + g_(I13, i) * u1 * bb3 +
                    g_(I12, i) * u2 * bb1 + g_(I22, i) * u2 * bb2 +
                    g_(I23, i) * u2 * bb3 + g_(I13, i) * u3 * bb1 +
                    g_(I23, i) * u3 * bb2 + g_(I33, i) * u3 * bb3;
        ...
        if (fixed) {
          PrimitiveToConservedSingle(prim, gamma_adi, bb_cc, g_, g_inv_, k, j,
                                     i, cons, pco);
        }
      }
    }
...
void EquationOfState::PrimitiveToConserved(const AthenaArray<Real> &prim,
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      for (int i = il; i <= iu; ++i) {
        PrimitiveToConservedSingle(prim, gamma_, bb_cc, g_, g_inv_, k, j, i,
                                   cons, pco);
      }
    }
  }
...
//   bbb: squared normal field \mathcal{B}^2 = g_{ij} \mathcal{B}^i
//   \mathcal{B}^j and
//       its components \mathcal{B}^i
//   tt: projection of momentum onto field \mathcal{T} = g_{ij} M^i
//   \mathcal{B}^j
// Notes:
...
void CalculateNormalConserved(const AthenaArray<Real> &cons,
  ...
  for (int i = il; i <= iu; ++i) {
    ...
    ee(i) = -qq_n;
    mm(0, i) = g_11 * SQR(mm1) + 2.0 * g_12 * mm1 * mm2 +
               2.0 * g_13 * mm1 * mm3 + g_22 * SQR(mm2) +
               2.0 * g_23 * mm2 * mm3 + g_33 * SQR(mm3);
    mm(1, i) = mm1;
    mm(2, i) = mm2;
    mm(3, i) = mm3;
    bbb(0, i) = g_11 * SQR(bbb1) + 2.0 * g_12 * bbb1 * bbb2 +
                2.0 * g_13 * bbb1 * bbb3 + g_22 * SQR(bbb2) +
                2.0 * g_23 * bbb2 * bbb3 + g_33 * SQR(bbb3);
    bbb(1, i) = bbb1;
    bbb(2, i) = bbb2;
    bbb(3, i) = bbb3;
...
//     Like SR, but all 3-vector operations done with respect to g_{ij} rather
//     than
//         \eta_{ij}.
//   Notation here largely follows (NH), so for example writing B^i for what is
//   really
//       \mathcal{B}^i.
//   Symbols:
//     tt: \mathcal{T}
...
bool ConservedToPrimitiveNormal(
  ...
  for (n = 0; n < max_iterations; ++n) {
    ...
    if (n % 3 != 2) {
      ...
      eee = a / 3.0 -
            2.0 / 3.0 * a * std::cos(2.0 / 3.0 * (phi + PI)); // (NH 5.11)
      ll = eee - bb_sq;                                       // (NH 5.5)
      v_sq = (mm_sq * SQR(ll) + SQR(tt) * (bb_sq + 2.0 * ll)) /
      ...
      pgas[(n + 1) % 3] =
          (gamma_adi - 1.0) / gamma_adi * (wgas - rho); // (NH 4.1)
      pgas[(n + 1) % 3] = std::max(pgas[(n + 1) % 3], pgas_min);
    }

    // Step 3: Check for convergence
    if (n % 3 != 2) {
      if (pgas[(n + 1) % 3] > pgas_min &&
          std::abs(pgas[(n + 1) % 3] - pgas[n % 3]) < tol) {
        break;
      }
    }
  ...
  Real eee =
      a / 3.0 - 2.0 / 3.0 * a * std::cos(2.0 / 3.0 * (phi + PI)); // (NH 5.11)
...
void PrimitiveToConservedSingle(const AthenaArray<Real> &prim, Real gamma_adi,
  ...
  Real tmp = g(I11, i) * uu1 * uu1 + 2.0 * g(I12, i) * uu1 * uu2 +
             2.0 * g(I13, i) * uu1 * uu3 + g(I22, i) * uu2 * uu2 +
             2.0 * g(I23, i) * uu2 * uu3 + g(I33, i) * uu3 * uu3;
...
//----------------------------------------------------------------------------------------
// Function for calculating relativistic fast wavespeeds in arbitrary
// coordinates Inputs:
//   wgas: gas enthalpy
//   pgas: gas pressure
...
void EquationOfState::FastMagnetosonicSpeedsGR(Real wgas, Real pgas, Real u0,
                                               Real u1, Real b_sq, Real g00,
                                               Real g01, Real g11,
                                               Real *p_lambda_plus,
                                               Real *p_lambda_minus) {
...
void EquationOfState::ApplyPrimitiveFloors(AthenaArray<Real> &prim, int k,
  ...
  Real &w_d = prim(IDN, i);
  Real &w_p = prim(IPR, i);
  // Eventually, may want to check that small field errors don't overwhelm gas
  // floor Real density_floor_local = density_floor_; if (sigma_max_ > 0.0) {
