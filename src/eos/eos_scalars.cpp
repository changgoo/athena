...
//----------------------------------------------------------------------------------------
//! \fn void
//! EquationOfState::PassiveScalarConservedToPrimitive(AthenaArray<Real> &s,
...
void EquationOfState::PassiveScalarConservedToPrimitive(
    AthenaArray<Real> &s, const AthenaArray<Real> &u,
...
#pragma omp simd
        for (int i = il; i <= iu; ++i) {
          ...
          Real &s_n = s(n, k, j, i);
          Real &r_n = r(n, k, j, i);
          // apply passive scalars floor to conserved variable first, then
          // transform: (multi-D fluxes may have caused it to drop below floor)
          s_n = (s_n < scalar_floor_ * d) ? scalar_floor_ * d : s_n;
          r_n = s_n * di;
          // TODO(felker): continue to monitor the acceptability of this
          // absolute 0. floor (may create very large global conservation
          // violations, e.g. the first few cycles of the slotted cylinder test)
...
// TODO(felker): a ton of overlap with ConservedToPrimitiveCellAverage in
// eos.hpp. AthenaArray<Real> targets + 2x function calls (pointwise EOS and
// flooring)
void EquationOfState::PassiveScalarConservedToPrimitiveCellAverage(
  ...
  // Fourth-order accurate approx to cell-centered Hydro conserved and primitive
  // variables
  AthenaArray<Real> &u_cc = ph->u_cc;
  // Passive scalrs
  AthenaArray<Real> &s_cc = ps->s_cc, &r_cc = ps->r_cc;
  // Laplacians of cell-averaged conserved and 2nd order accurate primitive
  // variables (reuse Hydro scratch arrays)
  AthenaArray<Real> &laplacian_cc =
      ph->scr1_nkji_; // ps->scr1_nkji_; // private
...
#pragma omp simd
        for (int i = il; i <= iu; ++i) {
          // We do not actually need to store all cell-centered conserved
          // variables, but the ConservedToPrimitive() implementation operates
          // on 4D arrays
  ...
  // Compute Laplacian of 2nd-order approximation to cell-averaged primitive
  // variables
  ...
  PassiveScalarConservedToPrimitive(s_cc, u_cc, r_cc, r_cc, pco, il, iu, jl, ju,
                                    kl, ku);
...
#pragma omp simd
        for (int i = il; i <= iu; ++i) {
          // Compute fourth-order approximation to cell-averaged primitive
          // variables
          r(n, k, j, i) = r_cc(n, k, j, i) + C * laplacian_cc(n, k, j, i);
          // Reapply primitive variable floors to cell-average of (prim)
          // dimensionless concentration WITHOUT correcting cell-average of
          // (cons) scalar mass
...
//----------------------------------------------------------------------------------------
//! \fn void EquationOfState::PassiveScalarPrimitiveToConserved(const
//! AthenaArray<Real> &r
//!           const AthenaArray<Real> &u, AthenaArray<Real> &s, Coordinates
...
void EquationOfState::PassiveScalarPrimitiveToConserved(
    ...
    AthenaArray<Real> &s, Coordinates *pco, int il, int iu, int jl, int ju,
    int kl, int ku) {
  for (int n = 0; n < NSCALARS; ++n) {
    for (int k = kl; k <= ku; ++k) {
      for (int j = jl; j <= ju; ++j) {
...
//----------------------------------------------------------------------------------------
//! \fn void EquationOfState::ApplyPassiveScalarFloors(AthenaArray<Real> &prim,
//! int n,
//!                                                     int k, int j, int i)
...
void EquationOfState::ApplyPassiveScalarFloors(AthenaArray<Real> &r, int n,
                                               int k, int j, int i) {
  // TODO(felker): process user-input "hydro/sfloor" in each EquationOfState
  // ctor 8x .cpp files + more in general/. Is there a better way to avoid code
  // duplication?
  ...
  for (int n = 0; n < NSCALARS; ++n) {
    Real &r_n = r(n, i);
    // apply (prim) dimensionless concentration floor WITHOUT adjusting passive
    // scalar mass (conserved), unlike in floor in standard EOS
...
void EquationOfState::ApplyPassiveScalarPrimitiveConservedFloors(
  ...
  r_n = s_n * di;
  // however, if r_n is riding the variable floor, it probably should be applied
  // so that s_n = rho*r_n is consistent (more concerned with conservation than
  // order of accuracy when quantities are floored)
  return;
}
