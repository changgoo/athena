...
EquationOfState::EquationOfState(MeshBlock *pmb, ParameterInput *pin)
    : ptable{pmb->pmy_mesh->peos_table}, pmy_block_{pmb},
      ...
      inv_egas_unit_{1.0 / egas_unit_}, vsqr_unit_{egas_unit_ / rho_unit_},
  ...
  if (pin->DoesParameterExist("hydro", "efloor")) {
    ...
    energy_floor_ =
        pressure_floor_ / (pin->GetOrAddReal("hydro", "gamma", 2.) - 1.);
  ...
  if (EOS_TABLE_ENABLED) {
    if (!ptable) {
      ...
      msg << "### FATAL ERROR in EquationOfState::EquationOfState" << std::endl
          << "EOS table data uninitialized. Should be initialized by Mesh."
          << std::endl;
      ATHENA_ERROR(msg);
    }
  }
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        ...
        // MSBC: if ke >> energy_floor_ then u_e - ke may still be zero at this
        // point due
