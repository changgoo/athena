...
Real x_(Real rho, Real T) {
  return 2. / (1. + std::sqrt(1. + 4. * std::exp(1. / T - 1.5 * std::log(T) +
                                                 std::log(rho))));
}
...
Real P_of_rho_T(Real rho, Real T) { return rho * T * (1. + x_(rho, T)); }
...
Real invert(Real (*f)(Real, Real), Real rho, Real sol, Real Ta, Real Tb) {
  ...
  if (fa * fb > 0) {
    ...
    msg << "### FATAL ERROR in EquationOfState inversion" << std::endl
        << "Root not bracketed" << std::endl;
    ATHENA_ERROR(msg);
  }
