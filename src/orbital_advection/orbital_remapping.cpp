...
//!                                         const int k, const int j, const int
//!                                         il, const int iu, const int shift_)
...
void OrbitalAdvection::RemapFluxPpm(AthenaArray<Real> &pflux_,
  ...
  AthenaArray<Real> &dd = d_src[0], &dd_m1 = d_src[1], &dd_p1 = d_src[2],
                    &dph = d_src[3], &dph_p1 = d_src[4], &d2qc_m1 = d_src[5],
                    &d2qc = d_src[6], &d2qc_p1 = d_src[7], &d2qf = d_src[8],
...
#pragma omp simd simdlen(SIMD_WIDTH)
  for (int i = il; i <= iu; ++i) {
    ...
    qb = std::max(std::max(std::abs(q(i)), std::abs(q_p1(i))),
                  std::abs(q_p2(i)));
