...
OrbitalAdvection::OrbitalAdvection(MeshBlock *pmb, ParameterInput *pin)
    : pmb_(pmb), pm_(pmb->pmy_mesh), ph_(pmb->phydro), pf_(pmb->pfield),
      pco_(pmb->pcoord), pbval_(pmb->pbval), ps_(pmb->pscalars) {
  ...
  if (pmb->pmy_mesh->OrbitalVelocity_ == nullptr) {
    ...
    if (pmb->pmy_mesh->OrbitalVelocityDerivative_[0] != nullptr)
      OrbitalVelocityDerivative[0] =
          pmb->pmy_mesh->OrbitalVelocityDerivative_[0];
    if (pmb->pmy_mesh->OrbitalVelocityDerivative_[1] != nullptr)
      OrbitalVelocityDerivative[1] =
          pmb->pmy_mesh->OrbitalVelocityDerivative_[1];
  }
  ...
  if (orbital_advection_defined) {
    ...
    if ((std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) ||
      ...
      if (pmb_->block_size.nx3 > 1) { // 3D
        ...
        msg << "### FATAL ERROR in OrbitalAdvection Class" << std::endl
            << "Orbital advection requires 2D or 3D in spherical polar "
               "coordinates."
            << std::endl
    ...
    // check parameters about the orbital motion for using default orbital
    // velocity
    if (pmb->pmy_mesh->OrbitalVelocity_ == nullptr) {
      if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
        if ((Omega0 == 0.0) || (qshear == 0.0)) {
          ...
          msg << "### FATAL ERROR in OrbitalAdvection Class" << std::endl
              ...
              << "Check <orbital_advection> Omega0 and qshear in the input "
                 "file."
              << std::endl;
          ATHENA_ERROR(msg);
        }
        ...
        if (gm == 0.0) {
          std::stringstream msg;
          msg << "### FATAL ERROR in OrbitalAdvection Class" << std::endl
              << "The default orbital velocity profile requires non-zero GM."
              << std::endl
              << "Check <problem> GM and <orbital_advection> qshear in the "
                 "input file."
              << std::endl;
          ATHENA_ERROR(msg);
        }
    ...
    if (orbital_advection_active) {
      if (pm_->adaptive == true) { // AMR
        ...
        if (orbital_direction == 1) {       // cartesian or cylindrical
          int64_t nbx =
              pm_->nrbx2 * (1L << (pmb_->loc.level - pm_->root_level));
          LogicalLocation loc;
          ...
          int64_t nbx =
              pm_->nrbx3 * (1L << (pmb_->loc.level - pm_->root_level));
          LogicalLocation loc;
          loc.level = pmb_->loc.level;
          loc.lx1 = pmb_->loc.lx1;
    ...
    if (orbital_direction == 1) { // cartesian or cylindrical
      if (pin->GetOrAddString("mesh", "ix2_bc", "none") != "periodic" ||
        ...
        std::stringstream msg;
        msg << "### FATAL ERROR in OrbitalAdvection Class" << std::endl
            << "Orbital advection in Cartesian or cylindrical coordinates "
               "requires "
    ...
    if (orbital_splitting_order < 0 || orbital_splitting_order > 2) {
      std::stringstream msg;
      msg << "### FATAL ERROR in OrbitalAdvection Class" << std::endl
          << "The order of the orbital splitting must be from 0 to 2."
          << std::endl
    ...
    if (orbital_direction == 1) { // cartesian or cylindrical
      ...
      if (orbital_advection_active) {
        ...
        if (orbital_refinement) {
          vKc_coarse.NewAthenaArray((nc3 + 2 * NGHOST) / 2,
                                    (nc1 + 2 * NGHOST) / 2);
          ofc_coarse.NewAthenaArray((nc3 + 2 * NGHOST) / 2,
                                    (nc1 + 2 * NGHOST) / 2);
        }
        if (MAGNETIC_FIELDS_ENABLED) {
          ...
          if (orbital_refinement) {
            vKf_coarse[0].NewAthenaArray((nc3 + 2 * NGHOST) / 2,
                                         (nc1 + 2 * NGHOST) / 2 + 1);
            off_coarse[0].NewAthenaArray((nc3 + 2 * NGHOST) / 2,
                                         (nc1 + 2 * NGHOST) / 2 + 1);
            vKf_coarse[1].NewAthenaArray((nc3 + 2 * NGHOST) / 2 + 1,
                                         (nc1 + 2 * NGHOST) / 2);
            off_coarse[1].NewAthenaArray((nc3 + 2 * NGHOST) / 2 + 1,
                                         (nc1 + 2 * NGHOST) / 2);
          }
        }
        if (NSCALARS > 0) {
      ...
      if (orbital_advection_active) {
        ...
        if (orbital_refinement) {
          vKc_coarse.NewAthenaArray((nc2 + 2 * NGHOST) / 2,
                                    (nc1 + 2 * NGHOST) / 2);
          ofc_coarse.NewAthenaArray((nc2 + 2 * NGHOST) / 2,
                                    (nc1 + 2 * NGHOST) / 2);
        }
        if (MAGNETIC_FIELDS_ENABLED) {
          ...
          if (orbital_refinement) {
            vKf_coarse[0].NewAthenaArray((nc2 + 2 * NGHOST) / 2,
                                         (nc1 + 2 * NGHOST) / 2 + 1);
            off_coarse[0].NewAthenaArray((nc2 + 2 * NGHOST) / 2,
                                         (nc1 + 2 * NGHOST) / 2 + 1);
            vKf_coarse[1].NewAthenaArray((nc2 + 2 * NGHOST) / 2 + 1,
                                         (nc1 + 2 * NGHOST) / 2);
            off_coarse[1].NewAthenaArray((nc2 + 2 * NGHOST) / 2 + 1,
                                         (nc1 + 2 * NGHOST) / 2);
          }
        }
        if (NSCALARS > 0) {
  ...
  if ((orbital_advection_active || pm_->shear_periodic)) {
    ...
    if (pm_->shear_periodic && MAGNETIC_FIELDS_ENABLED)
      pnum++;
    if (xorder > 2) {
      for (int n = 0; n < 13; n++) {
    ...
    if (orbital_direction == 1) {
      ...
      for (int j = pmb_->js; j <= pmb_->je; j++) {
        for (int i = pmb_->is; i <= pmb_->ie; i++) {
          ...
          Real vorb_max = vorb_c;
          for (int neighbor = 1; neighbor <= xgh; neighbor++) {
            Real vorb_m = vKc(j, i - neighbor) /
                          (pco_->h2v(i - neighbor) * pco_->h32v(j));
            Real vorb_p = vKc(j, i + neighbor) /
                          (pco_->h2v(i + neighbor) * pco_->h32v(j));
            vorb_min = std::min(vorb_min, std::min(vorb_m, vorb_p));
            vorb_max = std::max(vorb_max, std::max(vorb_m, vorb_p));
          }
          for (int neighbor = 1; neighbor <= xgh; neighbor++) {
            Real vorb_m = vKc(j - neighbor, i) /
                          (pco_->h2v(i) * pco_->h32v(j - neighbor));
            Real vorb_p = vKc(j + neighbor, i) /
                          (pco_->h2v(i) * pco_->h32v(j + neighbor));
            vorb_min = std::min(vorb_min, std::min(vorb_m, vorb_p));
            vorb_max = std::max(vorb_max, std::max(vorb_m, vorb_p));
          }
...
Real OrbitalAdvection::NewOrbitalAdvectionDt() { return min_dt; }
...
#pragma omp simd
    ...
    int il = pmb_->is - (NGHOST);
    int jl = pmb_->js - NGHOST;
    int iu = pmb_->ie + (NGHOST);
    int ju = pmb_->je + NGHOST;
    for (int j = jl; j <= ju; j++) {
      Real y_ = pco_->x2v(j);
#pragma omp simd
      for (int i = il; i <= iu; i++) {
...
void OrbitalAdvection::SetVKcCoarse() {
  ...
  if (orbital_direction == 1) {
    int il = pmb_->cis - (NGHOST);
    int kl = pmb_->cks;
    int iu = pmb_->cie + (NGHOST);
    int ku = pmb_->cke;
    if (nc3 > 1) {
      kl -= NGHOST;
      ku += NGHOST;
...
#pragma omp simd
    ...
    int il = pmb_->cis - (NGHOST);
    int jl = pmb_->cjs - NGHOST;
    int iu = pmb_->cie + (NGHOST);
    int ju = pmb_->cje + NGHOST;
    for (int j = jl; j <= ju; j++) {
      Real y_ = cpco->x2v(j);
#pragma omp simd
      for (int i = il; i <= iu; i++) {
