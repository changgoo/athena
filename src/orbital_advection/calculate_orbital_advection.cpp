      ...
      for (int k = ks; k <= ke; ++k) {
        for (int i = is; i <= ie; ++i) {
          ...
          for (int nph = 0; nph < NHYDRO; ++nph) {
            ...
            if (xorder <= 2) {
              RemapFluxPlm(pflux, hbuf, epsilon, osgn, k, i, js, je + 1,
                           shift0);
            } else {
              RemapFluxPpm(pflux, hbuf, epsilon, osgn, k, i, js, je + 1,
                           shift0);
            }
            for (int j = js; j <= je; j++) {
              u(nph, k, j, i) =
                  hbuf(k, i, j + shift) - (pflux(j + 1) - pflux(j));
            }
          }
      ...
      for (int j = js; j <= je; ++j) {
        for (int i = is; i <= ie; ++i) {
          ...
          for (int nph = 0; nph < NHYDRO; ++nph) {
            hbuf.InitWithShallowSlice(orbital_cons, 4, nph, 1);
            if (xorder <= 2) {
              RemapFluxPlm(pflux, hbuf, epsilon, osgn, j, i, ks, ke + 1,
                           shift0);
            } else {
              RemapFluxPpm(pflux, hbuf, epsilon, osgn, j, i, ks, ke + 1,
                           shift0);
            }
            for (int k = ks; k <= ke; k++) {
              u(nph, k, j, i) =
                  hbuf(j, i, k + shift) - (pflux(k + 1) - pflux(k));
            }
          }
          // calculate passive scalar flux
          for (int nsc = 0; nsc < NSCALARS; ++nsc) {
...
//----------------------------------------------------------------------------------------
//! \fn void OrbitalAdvection::CalculateOrbitalAdvectionFC(const Real dt,
//! EdgeField &e) \brief calculate field flux for orbital advection

void OrbitalAdvection::CalculateOrbitalAdvectionFC(const Real dt,
                                                   EdgeField &e) {
  if (!orbital_advection_active)
    return;
  int is = pmb_->is, ie = pmb_->ie;
