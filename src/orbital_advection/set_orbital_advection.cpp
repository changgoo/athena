...
#pragma omp simd
  ...
  if (orbital_refinement) {
    // restriction for orbital communications with refinement
    pmb_->pmr->RestrictFieldX1(b.x1f, b1_coarse_send, pmb_->cis, pmb_->cie + 1,
                               pmb_->cjs, pmb_->cje, pmb_->cks, pmb_->cke);
    if (orbital_direction == 1) { // cartesian or cylindrical
      pmb_->pmr->RestrictFieldX3(b.x3f, b2_coarse_send, pmb_->cis, pmb_->cie,
                                 pmb_->cjs, pmb_->cje, pmb_->cks,
                                 pmb_->cke + 1);
    } else if (orbital_direction == 2) { // spherical_polar
      pmb_->pmr->RestrictFieldX2(b.x2f, b2_coarse_send, pmb_->cis, pmb_->cie,
                                 pmb_->cjs, pmb_->cje + 1, pmb_->cks,
                                 pmb_->cke);
    }
  }
  return;
...
void OrbitalAdvection::SetOrbitalEdgeCC(const Real dt, int *ssize[2],
                                        int *rsize[2]) {
  int is = pmb_->is, ie = pmb_->ie;
  int js = pmb_->js, je = pmb_->je;
  int ks = pmb_->ks, ke = pmb_->ke;
...
void OrbitalAdvection::SetOrbitalEdgeFC(const Real dt, int *ssize[2],
                                        int *rsize[2]) {
  int is = pmb_->is, ie = pmb_->ie;
  int js = pmb_->js, je = pmb_->je;
  int ks = pmb_->ks, ke = pmb_->ke;
    ...
    if (orbital_direction == 1) { // cartesian or cylindrical
      ...
      for (int j = js; j <= je + 1; j++) {
        for (int i = is; i <= ie; i++) {
          ...
          Real olen =
              (h32f > 0.0) ? dt * vKf[1](j, i) / (pco_->h2v(i) * h32f) : 0.0;
          int offset = static_cast<int>(olen / dx);
          if (olen > 0.0)
            offset++;
    ...
    if (orbital_refinement) { // orbital refinement
      ...
      if (orbital_direction == 1) { // cartesian or cylindrical
        ...
        for (int j = pmb_->cjs; j <= pmb_->cje; j++) {
          for (int i = pmb_->cis; i <= pmb_->cie + 1; i++) {
            Real olen =
                dt * vKf_coarse[0](j, i) / (cpco->h2f(i) * cpco->h32v(j));
            int offset = static_cast<int>(olen / cdx);
            if (olen > 0.0)
              offset++;
        ...
        for (int j = pmb_->cjs; j <= pmb_->cje + 1; j++) {
          for (int i = pmb_->cis; i <= pmb_->cie; i++) {
            Real olen =
                dt * vKf_coarse[1](j, i) / (cpco->h2v(i) * cpco->h32f(j));
            int offset = static_cast<int>(olen / cdx);
            if (olen > 0.0)
              offset++;
        ...
        if (hnx3 != 0) {            // 3D
          for (int n3 = 0; n3 < 2; n3++) {
            for (int n1 = 0; n1 < 2; n1++) {
              ...
              int min_off = hnx2;
              for (int k = ks + n3 * hnx3; k <= ke - (1 - n3) * hnx3; k++) {
                for (int i = is + n1 * hnx1; i <= ie - (1 - n1) * hnx1 + 1;
                     i++) {
