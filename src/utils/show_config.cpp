...
//----------------------------------------------------------------------------------------
//! \fn void ShowConfig()
//! \brief prints diagnostic messages about the configuration of an Athena++
//! executable

void ShowConfig() {
  // To match configure.py output: use 2 space indent for option, value output
  // starts on column 30
  std::cout << "This Athena++ executable is configured with:" << std::endl;
  std::cout << "  Problem generator:          " << PROBLEM_GENERATOR
            << std::endl;
  std::cout << "  Coordinate system:          " << COORDINATE_SYSTEM
            << std::endl;
  std::cout << "  Equation of state:          " << EQUATION_OF_STATE
            << std::endl;
  ...
  if (SELF_GRAVITY_ENABLED == 1) {
    ...
    std::cout << "  Self-Gravity:               FFT (using BlockFFTGravity)"
              << std::endl;
  } else {
  ...
  // configure.py output: Doesnt append "Linker flags" in prev. output
  // (excessive space!)
  return;
}
