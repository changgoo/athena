...
//! successive deviates in a sequence. RNMX should appriximate the largest
//! floating-point value that is less than 1.
...
#pragma omp threadprivate(iy, iv, idum2)
  ...
  *idum = IA1 * (*idum - k * IQ1) -
          k * IR1; // Compute idum=(IA1*idum) % IMR1 without
  if (*idum < 0)
    *idum += IMR1; // overflows by Schrage's method
  k = idum2 / IQ2;
  idum2 = IA2 * (idum2 - k * IQ2) -
          k * IR2; // Compute idum2=(IA2*idum) % IMR2 likewise
  if (idum2 < 0)
    idum2 += IMR2;
