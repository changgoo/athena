...
//----------------------------------------------------------------------------------------
//! \fn  void Field::CT
//! \brief Constrained Transport implementation of dB/dt = -Curl(E), where E=-(v
//! X B)
...
#pragma omp simd
        for (int i = is; i <= ie + 1; ++i) {
          b_out.x1f(k, j, i) -=
              (wght / area(i)) *
              (len_p1(i) * e3(k, j + 1, i) - len(i) * e3(k, j, i));
        }
...
#pragma omp simd
          for (int i = is; i <= ie + 1; ++i) {
            b_out.x1f(k, j, i) +=
                (wght / area(i)) *
                (len_p1(i) * e2(k + 1, j, i) - len(i) * e2(k, j, i));
          }
        }
      }
    }
...
#pragma omp simd
      for (int i = is; i <= ie; ++i) {
        b_out.x2f(k, j, i) += (wght / area(i)) * (len(i + 1) * e3(k, j, i + 1) -
                                                  len(i) * e3(k, j, i));
      }
      if (pmb->block_size.nx3 > 1) {
...
#pragma omp simd
        for (int i = is; i <= ie; ++i) {
          b_out.x2f(k, j, i) -=
              (wght / area(i)) *
              (len_p1(i) * e1(k + 1, j, i) - len(i) * e1(k, j, i));
        }
      }
    }
  }
...
#pragma omp simd
      for (int i = is; i <= ie; ++i) {
        b_out.x3f(k, j, i) -= (wght / area(i)) * (len(i + 1) * e2(k, j, i + 1) -
                                                  len(i) * e2(k, j, i));
      }
      if (pmb->block_size.nx2 > 1) {
        pmb->pcoord->Edge1Length(k, j, is, ie, len);
...
#pragma omp simd
        for (int i = is; i <= ie; ++i) {
          b_out.x3f(k, j, i) +=
              (wght / area(i)) *
              (len_p1(i) * e1(k, j + 1, i) - len(i) * e1(k, j, i));
        }
      }
    }
  }
...
//! \brief Constrained Transport implementation of dB/dt = -Curl(E), where E=-(v
//! X B) for STS. RKL2 registers are set to -Curl(E) update if first stage of
//! RKL2 STS.

void Field::CT_STS(const Real wght, int stage, FaceField &b_out,
                   FaceField &ct_update_out) {
  MeshBlock *pmb = pmy_block;
  int is = pmb->is;
  int js = pmb->js;
  int ks = pmb->ks;
...
#pragma omp simd
        for (int i = is; i <= ie + 1; ++i) {
          ...
          if (stage == 1 && pmb->pmy_mesh->sts_integrator == "rkl2") {
            ct_update_out.x1f(k, j, i) =
                -1. * ((0.5 * pmb->pmy_mesh->dt / area(i)) *
                       (len_p1(i) * e3(k, j + 1, i) - len(i) * e3(k, j, i)));
          }
        }
...
#pragma omp simd
      for (int i = is; i <= ie; ++i) {
        b_out.x2f(k, j, i) += (wght / area(i)) * (len(i + 1) * e3(k, j, i + 1) -
                                                  len(i) * e3(k, j, i));
        if (stage == 1 && pmb->pmy_mesh->sts_integrator == "rkl2") {
          ct_update_out.x2f(k, j, i) =
              ((0.5 * pmb->pmy_mesh->dt / area(i)) *
               (len(i + 1) * e3(k, j, i + 1) - len(i) * e3(k, j, i)));
        }
      }
      if (pmb->block_size.nx3 > 1) {
        pmb->pcoord->Edge1Length(k, j, is, ie, len);
...
#pragma omp simd
      for (int i = is; i <= ie; ++i) {
        b_out.x3f(k, j, i) -= (wght / area(i)) * (len(i + 1) * e2(k, j, i + 1) -
                                                  len(i) * e2(k, j, i));
        if (stage == 1 && pmb->pmy_mesh->sts_integrator == "rkl2") {
          ct_update_out.x3f(k, j, i) =
              -1. * ((0.5 * pmb->pmy_mesh->dt / area(i)) *
                     (len(i + 1) * e2(k, j, i + 1) - len(i) * e2(k, j, i)));
        }
      }
      if (pmb->block_size.nx2 > 1) {
        pmb->pcoord->Edge1Length(k, j, is, ie, len);
