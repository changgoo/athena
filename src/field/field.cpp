...
Field::Field(MeshBlock *pmb, ParameterInput *pin)
    : pmy_block(pmb), b(pmb->ncells3, pmb->ncells2, pmb->ncells1),
      ...
      coarse_b_(pmb->ncc3, pmb->ncc2, pmb->ncc1 + 1,
                (pmb->pmy_mesh->multilevel
                     ? AthenaArray<Real>::DataStatus::allocated
                     : AthenaArray<Real>::DataStatus::empty)),
      fbvar(pmb, &b, coarse_b_, e), fdif(pmb, pin) {
  ...
  // registers Note the extra cell in each longitudinal direction for interface
  // fields
  ...
  if (STS_ENABLED) {
    std::string sts_integrator =
        pin->GetOrAddString("time", "sts_integrator", "rkl2");
  ...
  if (pm->multilevel) {
    // "Enroll" in SMR/AMR by adding to vector of pointers in MeshRefinement
    // class
...
//! \brief cell center B-fields are defined as spatial interpolation at the
//! volume center

void Field::CalculateCellCenteredField(const FaceField &bf,
                                       AthenaArray<Real> &bc, Coordinates *pco,
                                       int il, int iu, int jl, int ju, int kl,
                                       int ku) {
  // Defer to Reconstruction class to check if uniform Cartesian formula can be
  // used (unweighted average)
...
#pragma omp simd
      for (int i = il; i <= iu; ++i) {
        ...
        Real lw, rw; // linear interpolation coefficients from lower and upper
                     // cell faces

        // cell center B-fields are defined as spatial interpolation at the
        // volume center
        if (uniform_ave_x1) {
          lw = 0.5;
          rw = 0.5;
