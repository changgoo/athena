...
void FieldDiffusion::CalcDiffusionEMF(FaceField &bi,
...
void FieldDiffusion::AddPoyntingFluxSingleCell(FaceField &p_src, int i, int j,
                                               int k) {
  MeshBlock *pmb = pmy_block;
  AthenaArray<Real> &x1flux = pmb->phydro->flux[X1DIR];
  AthenaArray<Real> &x2flux = pmb->phydro->flux[X2DIR];
        ...
        for (int i = is; i <= ie; ++i)
          dt_h = std::min(
              ...
              static_cast<Real>(fac_h * SQR(len(i)) /
                                (std::abs(etaB(hall, k, j, i)) + TINY_NUMBER)));
      }
    }
  }
  return;
