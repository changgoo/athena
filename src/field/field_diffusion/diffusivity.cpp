...
void ConstDiffusivity(FieldDiffusion *pfdif, MeshBlock *pmb,
...
void FieldDiffusion::CalcCurrent(FaceField &b) {
  ...
  AthenaArray<Real> &area = face_area_, &len = edge_length_,
                    &len_m1 = edge_length_m1_;
...
#pragma omp simd
      for (int i = is - 1; i <= ie + 2; ++i) {
        J2(k, j, i) = -(len(i) * b3i(k, j, i) - len(i - 1) * b3i(k, j, i - 1)) /
                      area(i - 1);
      }
    }
  }
...
#pragma omp simd
        for (int i = is - 2; i <= ie + 2; ++i) {
          J1(k, j, i) =
              (len(i) * b3i(k, j, i) - len_m1(i) * b3i(k, j - 1, i)) / area(i);
        }
      }
    }
...
#pragma omp simd
        for (int i = is - 1; i <= ie + 2; ++i) {
          J3(k, j, i) -=
              (len(i - 1) * b1i(k, j, i) - len_m1(i - 1) * b1i(k, j - 1, i)) /
              area(i - 1);
        }
      }
    }
...
#pragma omp simd
        for (int i = is - 2; i <= ie + 2; ++i) {
          J1(k, j, i) -=
              (len(i) * b2i(k, j, i) - len_m1(i) * b2i(k - 1, j, i)) / area(i);
        }
      }
    }
...
#pragma omp simd
        for (int i = is - 1; i <= ie + 2; ++i) {
          J2(k, j, i) +=
              (len(i - 1) * b1i(k, j, i) - len_m1(i - 1) * b1i(k - 1, j, i)) /
              area(i - 1);
        }
      }
    }
...
void FieldDiffusion::OhmicEMF(const FaceField &b, const AthenaArray<Real> &bc,
  ...
  AthenaArray<Real> &e1 = e.x1e, &e2 = e.x2e, &e3 = e.x3e, &J1 = jedge_.x1e,
                    &J2 = jedge_.x2e, &J3 = jedge_.x3e;
...
#pragma omp simd
      for (int i = is; i <= ie + 1; ++i) {
        ...
        eta_O =
            0.25 * (etaB(ohmic, ks, j, i) + etaB(ohmic, ks, j, i - 1) +
...
#pragma omp simd
      for (int i = is; i <= ie + 1; ++i) {
        Real eta_O =
            0.25 * (etaB(ohmic, k, j, i) + etaB(ohmic, k, j - 1, i) +
                    etaB(ohmic, k - 1, j, i) + etaB(ohmic, k - 1, j - 1, i));
        e1(k, j, i) += eta_O * J1(k, j, i);

        eta_O =
            0.25 * (etaB(ohmic, k, j, i) + etaB(ohmic, k, j, i - 1) +
...
void FieldDiffusion::AmbipolarEMF(const FaceField &b,
  ...
  if (pmb->block_size.nx2 == 1) {
    for (int i = is; i <= ie + 1; ++i) {
      Real eta_A =
          0.5 * (etaB(ambipolar, ks, js, i - 1) + etaB(ambipolar, ks, js, i));
...
#pragma omp simd
      for (int i = is; i <= ie + 1; ++i) {
        // emf.x
        Real eta_A =
            0.5 * (etaB(ambipolar, ks, j, i) + etaB(ambipolar, ks, j - 1, i));
        ...
        // emf.y
        eta_A =
            0.5 * (etaB(ambipolar, ks, j, i) + etaB(ambipolar, ks, j, i - 1));

        intJx = 0.25 * (J1(ks, j, i) + J1(ks, j + 1, i) + J1(ks, j, i - 1) +
                        J1(ks, j + 1, i - 1));
        intJy = J2(ks, j, i);
        ...
        eta_A =
            0.25 *
            (etaB(ambipolar, ks, j, i) + etaB(ambipolar, ks, j, i - 1) +
...
#pragma omp simd
      for (int i = is; i <= ie + 1; ++i) {
        ...
        Real eta_A =
            0.25 *
            (etaB(ambipolar, k, j, i) + etaB(ambipolar, k, j - 1, i) +
             etaB(ambipolar, k - 1, j, i) + etaB(ambipolar, k - 1, j - 1, i));

        Real intJx = J1(k, j, i);
        Real intJy = 0.25 * (J2(k, j, i) + J2(k, j, i + 1) + J2(k, j - 1, i) +
                             J2(k, j - 1, i + 1));
        Real intJz = 0.25 * (J3(k, j, i) + J3(k, j, i + 1) + J3(k - 1, j, i) +
                             J3(k - 1, j, i + 1));
        ...
        eta_A =
            0.25 *
            (etaB(ambipolar, k, j, i) + etaB(ambipolar, k, j, i - 1) +
             etaB(ambipolar, k - 1, j, i) + etaB(ambipolar, k - 1, j, i - 1));

        intJx = 0.25 * (J1(k, j, i) + J1(k, j, i - 1) + J1(k, j + 1, i) +
                        J1(k, j + 1, i - 1));
        intJy = J2(k, j, i);
        intJz = 0.25 * (J3(k, j, i) + J3(k, j + 1, i) + J3(k - 1, j, i) +
                        J3(k - 1, j + 1, i));
        ...
        intJx = 0.25 * (J1(k, j, i) + J1(k, j, i - 1) + J1(k + 1, j, i) +
                        J1(k + 1, j, i - 1));
        intJy = 0.25 * (J2(k, j, i) + J2(k, j - 1, i) + J2(k + 1, j, i) +
                        J2(k + 1, j - 1, i));
        intJz = J3(k, j, i);
...
void FieldDiffusion::PoyntingFlux(EdgeField &e, const AthenaArray<Real> &bc) {
  ...
  AthenaArray<Real> &e1 = e.x1e, &e2 = e.x2e, &e3 = e.x3e, &f1 = pflux.x1f,
                    &f2 = pflux.x2f, &f3 = pflux.x3f;
...
#pragma omp simd
    for (int i = is; i <= ie + 1; ++i) {
      f1(ks, js, i) =
          -0.5 * (bc(IB2, ks, js, i) + bc(IB2, ks, js, i - 1)) * e3(ks, js, i) +
...
#pragma omp simd
      for (int i = is; i <= ie + 1; ++i) {
        f1(ks, j, i) =
            -0.25 * (bc(IB2, ks, j, i) + bc(IB2, ks, j, i - 1)) *
                (e3(ks, j, i) + e3(ks, j + 1, i)) +
            0.5 * (bc(IB3, ks, j, i) + bc(IB3, ks, j, i - 1)) * e2(ks, j, i);

        f2(ks, j, i) =
            -0.5 * (bc(IB3, ks, j, i) + bc(IB3, ks, j - 1, i)) * e1(ks, j, i) +
