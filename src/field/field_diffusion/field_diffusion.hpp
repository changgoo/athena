...
//! currently must be free function for compatibility with user-defined fn via
//! fn pointers
...
public:
  ...
  AthenaArray<Real> jfx, jfy,
      jfz;               // interface current density (for HLL Riemann solver)
  AthenaArray<Real> jcc; // cell-centered current density (for the integrator)

  // array indices for magnetic diffusion types
  // should not be scoped (C++11) since enumerators are only used as "int" to
  // index arrays
  enum DiffProcess { ohmic = 0, hall = 1, ambipolar = 2 };
  // TODO(felker) Unlike HydroDiffusion::DiffProcess, not using optional
  // "unscoped enum name" qualifier when referencing the enumerators in other
  // files. Be consistent
  ...
  // functions
  void CalcDiffusionEMF(FaceField &bi, const AthenaArray<Real> &bc,
                        EdgeField &e);
  ...
  // void HallEMF(const FaceField &b, const AthenaArray<Real> &bc, EdgeField
  // &e);
  void AmbipolarEMF(const FaceField &b, const AthenaArray<Real> &bc,
                    EdgeField &e);
