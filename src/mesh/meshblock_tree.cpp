...
bool operator==(const LogicalLocation &l1, const LogicalLocation &l2) {
  return ((l1.level == l2.level) && (l1.lx1 == l2.lx1) && (l1.lx2 == l2.lx2) &&
          (l1.lx3 == l2.lx3));
}
...
//----------------------------------------------------------------------------------------
//! \fn MeshBlockTree::MeshBlockTree(MeshBlockTree *parent, int ox1, int ox2,
//! int ox3) \brief constructor for a leaf
...
//! \brief create the root grid; the root grid can be incomplete (less than 8
//! leaves)
...
void MeshBlockTree::CreateRootGrid() {
  ...
  if (loc_.level == pmesh_->root_level)
    return;
...
void MeshBlockTree::Refine(int &nnew) {
  ...
  for (oz = ozmin; oz <= ozmax; oz++) {
    ...
    for (oy = oymin; oy <= oymax; oy++) {
      ...
      if (polar)
        nloc.lx3 = (nloc.lx3 + nzmax / 2) % nzmax;
...
void MeshBlockTree::GetMeshBlockList(LogicalLocation *list, int *pglist,
                                     int &count) {
  if (loc_.level == 0)
    count = 0;
...
MeshBlockTree *MeshBlockTree::FindNeighbor(LogicalLocation myloc, int ox1,
                                           int ox2, int ox3, bool amrflag) {
  ...
  if (lx < 0) {
    if (pmesh_->mesh_bcs[BoundaryFace::inner_x1] == BoundaryFlag::periodic ||
        pmesh_->mesh_bcs[BoundaryFace::inner_x1] ==
  ...
  if (lx >= pmesh_->nrbx1 << (ll - pmesh_->root_level)) {
    if (pmesh_->mesh_bcs[BoundaryFace::outer_x1] == BoundaryFlag::periodic ||
        pmesh_->mesh_bcs[BoundaryFace::outer_x1] ==
            BoundaryFlag::shear_periodic)
      lx = 0;
    else
      return nullptr;
  ...
  if (ll < 1)
    return proot_; // single grid; return root
  if (polar)
    lz = (lz + num_x3 / 2) % num_x3;
...
void MeshBlockTree::GetMGOctetList(
