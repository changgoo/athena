...
void MeshRefinement::RestrictFieldX1(const AthenaArray<Real> &fine,
                                     AthenaArray<Real> &coarse, int csi,
                                     int cei, int csj, int cej, int csk,
                                     int cek) {
  MeshBlock *pmb = pmy_block_;
  Coordinates *pco = pmb->pcoord;
...
void MeshRefinement::ProlongateInternalField(FaceField &fine, int si, int ei,
...
#pragma unroll
            for (int ii = 0; ii < 2; ii++) {
              ...
              Uxx += is *
                     (js * (fine.x2f(fk, fjp, fii) * sarea_x2_[0][2 * jj](fii) +
                            fine.x2f(fk + 1, fjp, fii) *
                                sarea_x2_[1][2 * jj](fii)) +
  ...
  if (aret > 0) {
    ...
    if (pmb->loc.level == pmb->pmy_mesh->root_level) {
      ...
      for (int k = ks; k <= ke; k++) {
        for (int j = js; j <= je; j++) {
          for (int i = -1; i <= 1; i++)
            if (pmb->pbval->nblevel[k + 1][j + 1][i + 1] > pmb->loc.level)
              ec++;
        }
      }
      if (ec > 0) {
...
//! restriction on primitive Hydro standard/coarse arrays (only for GR) without
//! changing the var_cc/coarse_buf pointer members of the HydroBoundaryVariable.
//!
//! - BoundaryValues::ProlongateGhostCells()--- to ensure prolongation occurs on
//! conserved (not primitive) variable standard/coarse arrays for Hydro,
//! PassiveScalars
//!
//! Should probably consolidate this function and std::vector of tuples with
//! BoundaryVariable interface ptr members. Too much independent switching of
//! ptrs!
//! __________
...
//! the physical (non-periodic) boundary functions will still apply only to the
//! PRIMITIVE scalar variable arrays, thus S/AMR demand
//! 1. AthenaArray<Real> PassiveScalars::coarse_r
//! 2. ability to switch (s, coarse_s) and (r, coarse_r) ptrs in
//! MeshRefinement::bvals_cc_
