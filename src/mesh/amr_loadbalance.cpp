...
void Mesh::CalculateLoadBalance(double *clist, int *rlist, int *slist,
                                int *nlist, int nb) {
...
#ifdef MPI_PARALLEL
  if (tnref > 0) {
    MPI_Allgatherv(MPI_IN_PLACE, bnref[Globals::my_rank], MPI_BYTE, lref, bnref,
                   brdisp, MPI_BYTE, MPI_COMM_WORLD);
  }
  if (tnderef >= nleaf) {
    MPI_Allgatherv(MPI_IN_PLACE, bnderef[Globals::my_rank], MPI_BYTE, lderef,
                   bnderef, bddisp, MPI_BYTE, MPI_COMM_WORLD);
  }
#endif
...
bool Mesh::GatherCostListAndCheckBalance() {
  if (lb_manual_ || lb_automatic_) {
#ifdef MPI_PARALLEL
    MPI_Allgatherv(MPI_IN_PLACE, nblist[Globals::my_rank], MPI_DOUBLE, costlist,
    ...
    if (adaptive)
      lb_tolerance_ = 2.0 * static_cast<double>(Globals::nranks) /
...
//! ntot) \brief redistribute MeshBlocks according to the new load balance
...
void Mesh::RedistributeAndRefineMeshBlocks(ParameterInput *pin, int ntot) {
  ...
  // use the first MeshBlock in the linked list of blocks belonging to this MPI
  // rank as a representative of all MeshBlocks for counting the "load-balancing
  // registered" and "SMR/AMR-enrolled" quantities (loop over
  // MeshBlock::vars_cc_, not MeshRefinement)
  ...
  int bsc2f = (bnx1 / 2 + 2) * ((bnx2 + 1) / 2 + 2 * f2) *
              ((bnx3 + 1) / 2 + 2 * f3) * nx4_tot;
  ...
  bsc2f += num_fc * (((bnx1 / 2) + 1 + 2) * ((bnx2 + 1) / 2 + 2 * f2) *
                         ((bnx3 + 1) / 2 + 2 * f3) +
                     (bnx1 / 2 + 2) * (((bnx2 + 1) / 2) + f2 + 2 * f2) *
                         ((bnx3 + 1) / 2 + 2 * f3) +
  ...
  if (nsend != 0) {
    ...
    for (int n = gids_; n <= gide_; n++) {
      ...
      if (nloc.level == oloc.level) { // same level
        ...
        for (int l = 0; l < nleaf; l++) {
          if (newrank[nn + l] == Globals::my_rank)
            continue;
  ...
  for (int n = nbs; n <= nbe; n++) {
    ...
    if ((ranklist[on] == Globals::my_rank) &&
      ...
      newlist(n - nbs) = new MeshBlock(n, n - nbs, newloc[n], block_size,
                                       block_bcs, this, pin, gflag, true);
...
//! MeshBlocks. Therefore, unlike PrepareSendCoarseToFineAMR(), etc., it loops
//! over MeshBlock::vars_cc/fc_ containers, not MeshRefinement::pvars_cc/fc_
//! containers
...
void Mesh::PrepareSendSameLevel(MeshBlock *pb, Real *sendbuf) {
  ...
  // (C++11) range-based for loop: (automatic type deduction fails when
  // iterating over container with std::reference_wrapper; could use auto
  // var_cc_r = var_cc.get())
  for (AthenaArray<Real> &var_cc : pb->vars_cc_) {
    int nu = var_cc.GetDim4() - 1;
    BufferUtility::PackData(var_cc, sendbuf, 0, nu, pb->is, pb->ie, pb->js,
                            pb->je, pb->ks, pb->ke, p);
  }
  for (FaceField &var_fc : pb->vars_fc_) {
    BufferUtility::PackData(var_fc.x1f, sendbuf, pb->is, pb->ie + 1, pb->js,
                            pb->je, pb->ks, pb->ke, p);
    BufferUtility::PackData(var_fc.x2f, sendbuf, pb->is, pb->ie, pb->js,
                            pb->je + f2, pb->ks, pb->ke, p);
    BufferUtility::PackData(var_fc.x3f, sendbuf, pb->is, pb->ie, pb->js, pb->je,
                            pb->ks, pb->ke + f3, p);
  }
  //! \warning (felker):
  //! * casting from "Real *" to "int *" in order to append single integer
  //!   to send buffer is slightly unsafe (especially if sizeof(int) >
  //!   sizeof(Real))
  if (adaptive) {
...
void Mesh::PrepareSendCoarseToFineAMR(MeshBlock *pb, Real *sendbuf,
  ...
  if (ox1 == 0)
    il = pb->is - 1, iu = pb->is + pb->block_size.nx1 / 2;
  else
    il = pb->is + pb->block_size.nx1 / 2 - 1, iu = pb->ie + 1;
  if (ox2 == 0)
    jl = pb->js - f2, ju = pb->js + pb->block_size.nx2 / 2;
  else
    jl = pb->js + pb->block_size.nx2 / 2 - f2, ju = pb->je + f2;
  if (ox3 == 0)
    kl = pb->ks - f3, ku = pb->ks + pb->block_size.nx3 / 2;
  else
    kl = pb->ks + pb->block_size.nx3 / 2 - f3, ku = pb->ke + f3;
  int p = 0;
  ...
  for (auto fc_pair : pb->pmr->pvars_fc_) {
    ...
    BufferUtility::PackData((*var_fc).x1f, sendbuf, il, iu + 1, jl, ju, kl, ku,
                            p);
    BufferUtility::PackData((*var_fc).x2f, sendbuf, il, iu, jl, ju + f2, kl, ku,
                            p);
    BufferUtility::PackData((*var_fc).x3f, sendbuf, il, iu, jl, ju, kl, ku + f3,
                            p);
  }
  return;
}
...
void Mesh::PrepareSendFineToCoarseAMR(MeshBlock *pb, Real *sendbuf) {
  ...
  for (auto cc_pair : pmr->pvars_cc_) {
    ...
    int nu = var_cc->GetDim4() - 1;
    pmr->RestrictCellCenteredValues(*var_cc, *coarse_cc, 0, nu, pb->cis,
                                    pb->cie, pb->cjs, pb->cje, pb->cks,
                                    pb->cke);
    BufferUtility::PackData(*coarse_cc, sendbuf, 0, nu, pb->cis, pb->cie,
                            pb->cjs, pb->cje, pb->cks, pb->cke, p);
  }
  for (auto fc_pair : pb->pmr->pvars_fc_) {
    FaceField *var_fc = std::get<0>(fc_pair);
    FaceField *coarse_fc = std::get<1>(fc_pair);
    pmr->RestrictFieldX1((*var_fc).x1f, (*coarse_fc).x1f, pb->cis, pb->cie + 1,
                         pb->cjs, pb->cje, pb->cks, pb->cke);
    BufferUtility::PackData((*coarse_fc).x1f, sendbuf, pb->cis, pb->cie + 1,
                            pb->cjs, pb->cje, pb->cks, pb->cke, p);
    pmr->RestrictFieldX2((*var_fc).x2f, (*coarse_fc).x2f, pb->cis, pb->cie,
                         pb->cjs, pb->cje + f2, pb->cks, pb->cke);
    BufferUtility::PackData((*coarse_fc).x2f, sendbuf, pb->cis, pb->cie,
                            pb->cjs, pb->cje + f2, pb->cks, pb->cke, p);
    pmr->RestrictFieldX3((*var_fc).x3f, (*coarse_fc).x3f, pb->cis, pb->cie,
                         pb->cjs, pb->cje, pb->cks, pb->cke + f3);
    BufferUtility::PackData((*coarse_fc).x3f, sendbuf, pb->cis, pb->cie,
                            pb->cjs, pb->cje, pb->cks, pb->cke + f3, p);
  }
  return;
}
...
//!                                            LogicalLocation &loc)
//! \brief step 7: f2c, same MPI rank, different level (just restrict+copy, no
//! pack/send)
...
void Mesh::FillSameRankFineToCoarseAMR(MeshBlock *pob, MeshBlock *pmb,
  ...
  for (auto cc_pair : pmr->pvars_cc_) {
    ...
    AthenaArray<Real> *coarse_cc = std::get<1>(cc_pair);
    int nu = var_cc->GetDim4() - 1;
    pmr->RestrictCellCenteredValues(*var_cc, *coarse_cc, 0, nu, pob->cis,
                                    pob->cie, pob->cjs, pob->cje, pob->cks,
                                    pob->cke);
  ...
  for (auto fc_pair : pmr->pvars_fc_) {
    FaceField *var_fc = std::get<0>(fc_pair);
    FaceField *coarse_fc = std::get<1>(fc_pair);
    pmr->RestrictFieldX1((*var_fc).x1f, (*coarse_fc).x1f, pob->cis,
                         pob->cie + 1, pob->cjs, pob->cje, pob->cks, pob->cke);
    pmr->RestrictFieldX2((*var_fc).x2f, (*coarse_fc).x2f, pob->cis, pob->cie,
                         pob->cjs, pob->cje + f2, pob->cks, pob->cke);
    pmr->RestrictFieldX3((*var_fc).x3f, (*coarse_fc).x3f, pob->cis, pob->cie,
                         pob->cjs, pob->cje, pob->cks, pob->cke + f3);
...
void Mesh::FillSameRankCoarseToFineAMR(MeshBlock *pob, MeshBlock *pmb,
  ...
  int cjs =
      ((newloc.lx2 & 1LL) == 1LL) * pob->block_size.nx2 / 2 + pob->js - f2;
  int cks =
      ((newloc.lx3 & 1LL) == 1LL) * pob->block_size.nx3 / 2 + pob->ks - f3;
  ...
  for (auto cc_pair : pmr->pvars_cc_) {
    ...
    pmr->ProlongateCellCenteredValues(dst, *var_cc, 0, nu, pob->cis, pob->cie,
  ...
  for (auto fc_pair : pmr->pvars_fc_) {
    ...
    pmr->ProlongateSharedFieldX1(dst_b.x1f, (*var_fc).x1f, pob->cis,
                                 pob->cie + 1, pob->cjs, pob->cje, pob->cks,
                                 pob->cke);
    pmr->ProlongateSharedFieldX2(dst_b.x2f, (*var_fc).x2f, pob->cis, pob->cie,
                                 pob->cjs, pob->cje + f2, pob->cks, pob->cke);
    pmr->ProlongateSharedFieldX3(dst_b.x3f, (*var_fc).x3f, pob->cis, pob->cie,
                                 pob->cjs, pob->cje, pob->cks, pob->cke + f3);
    pmr->ProlongateInternalField(*var_fc, pob->cis, pob->cie, pob->cjs,
...
void Mesh::FinishRecvSameLevel(MeshBlock *pb, Real *recvbuf) {
  ...
  for (AthenaArray<Real> &var_cc : pb->vars_cc_) {
    int nu = var_cc.GetDim4() - 1;
    BufferUtility::UnpackData(recvbuf, var_cc, 0, nu, pb->is, pb->ie, pb->js,
                              pb->je, pb->ks, pb->ke, p);
  }
  for (FaceField &var_fc : pb->vars_fc_) {
    BufferUtility::UnpackData(recvbuf, var_fc.x1f, pb->is, pb->ie + 1, pb->js,
                              pb->je, pb->ks, pb->ke, p);
    BufferUtility::UnpackData(recvbuf, var_fc.x2f, pb->is, pb->ie, pb->js,
                              pb->je + f2, pb->ks, pb->ke, p);
    BufferUtility::UnpackData(recvbuf, var_fc.x3f, pb->is, pb->ie, pb->js,
                              pb->je, pb->ks, pb->ke + f3, p);
...
void Mesh::FinishRecvFineToCoarseAMR(MeshBlock *pb, Real *recvbuf,
  ...
  for (auto cc_pair : pb->pmr->pvars_cc_) {
    AthenaArray<Real> *var_cc = std::get<0>(cc_pair);
    int nu = var_cc->GetDim4() - 1;
    BufferUtility::UnpackData(recvbuf, *var_cc, 0, nu, il, iu, jl, ju, kl, ku,
                              p);
  }
  for (auto fc_pair : pb->pmr->pvars_fc_) {
    FaceField *var_fc = std::get<0>(fc_pair);
    FaceField &dst_b = *var_fc;
    BufferUtility::UnpackData(recvbuf, dst_b.x1f, il, iu + 1, jl, ju, kl, ku,
                              p);
    BufferUtility::UnpackData(recvbuf, dst_b.x2f, il, iu, jl, ju + f2, kl, ku,
                              p);
    BufferUtility::UnpackData(recvbuf, dst_b.x3f, il, iu, jl, ju, kl, ku + f3,
                              p);
    if (pb->block_size.nx2 == 1) {
      for (int i = il; i <= iu; i++)
...
void Mesh::FinishRecvCoarseToFineAMR(MeshBlock *pb, Real *recvbuf) {
  ...
  for (auto cc_pair : pb->pmr->pvars_cc_) {
    ...
    AthenaArray<Real> *coarse_cc = std::get<1>(cc_pair);
    int nu = var_cc->GetDim4() - 1;
    BufferUtility::UnpackData(recvbuf, *coarse_cc, 0, nu, il, iu, jl, ju, kl,
                              ku, p);
    pmr->ProlongateCellCenteredValues(*coarse_cc, *var_cc, 0, nu, pb->cis,
                                      pb->cie, pb->cjs, pb->cje, pb->cks,
                                      pb->cke);
  }
  for (auto fc_pair : pb->pmr->pvars_fc_) {
    ...
    BufferUtility::UnpackData(recvbuf, (*coarse_fc).x1f, il, iu + 1, jl, ju, kl,
                              ku, p);
    BufferUtility::UnpackData(recvbuf, (*coarse_fc).x2f, il, iu, jl, ju + f2,
                              kl, ku, p);
    BufferUtility::UnpackData(recvbuf, (*coarse_fc).x3f, il, iu, jl, ju, kl,
                              ku + f3, p);
    pmr->ProlongateSharedFieldX1((*coarse_fc).x1f, (*var_fc).x1f, pb->cis,
                                 pb->cie + 1, pb->cjs, pb->cje, pb->cks,
                                 pb->cke);
    pmr->ProlongateSharedFieldX2((*coarse_fc).x2f, (*var_fc).x2f, pb->cis,
                                 pb->cie, pb->cjs, pb->cje + f2, pb->cks,
                                 pb->cke);
    pmr->ProlongateSharedFieldX3((*coarse_fc).x3f, (*var_fc).x3f, pb->cis,
                                 pb->cie, pb->cjs, pb->cje, pb->cks,
                                 pb->cke + f3);
    pmr->ProlongateInternalField(*var_fc, pb->cis, pb->cie, pb->cjs, pb->cje,
                                 pb->cks, pb->cke);
  }
  return;
}
...
//!
//! tag = local id of destination (remaining bits) + ox1(1 bit) + ox2(1 bit) +
//! ox3(1 bit)
//!       + physics(5 bits)
//!
