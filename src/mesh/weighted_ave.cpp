...
//----------------------------------------------------------------------------------------
//! \fn void MeshBlock::WeightedAve(AthenaArray<Real> &u_out, AthenaArray<Real>
//! &u_in1,
//!                                 AthenaArray<Real> &u_in2, AthenaArray<Real>
//!                                 &u_in3, AthenaArray<Real> &u_in4, const Real
//!                                 wght[5])
//! \brief Compute weighted average of AthenaArrays (including cell-averaged U
//! in time
//!        integrator step)
//!
//! * consider every possible simplified form of weighted sum operator:
//!   U = a*U + b*U1 + c*U2 + d*U3 + e*U4
//! * assuming all 3x arrays are of the same size (or at least u_out is equal or
//! larger
...
void MeshBlock::WeightedAve(FaceField &b_out, FaceField &b_in1,
  ...
  // Only need to separately account for the final longitudinal face in each
  // loop limit
  if (wght[0] == 1.0) {
    if (wght[4] != 0.0) {
      //---- B1
...
void MeshBlock::WeightedAve(AthenaArray<Real> &u_out, AthenaArray<Real> &u_in1,
  ...
  // assuming all 3x arrays are of the same size (or at least u_out is equal or
  // larger than each input array) in each array dimension, and full range is
  // desired: nx4*(3D real MeshBlock cells)
  const int nu = u_out.GetDim4() - 1;
