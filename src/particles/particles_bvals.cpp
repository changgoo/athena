...
void Particles::AMRCoarseToFine(Particles *pparc, Particles *pparf,
                                MeshBlock *pmbf) {
  // Initialization
  ...
  const bool active1 = pparc->active1_, active2 = pparc->active2_,
  ...
  for (int k = 0; k < pparc->npar_; ++k) {
    ...
    if ((!active1 || (active1 && x1min <= x1 && x1 < x1max)) &&
      ...
      if (npar >= pparf->nparmax_)
        pparf->UpdateCapacity(2 * pparf->nparmax_);
...
void Particles::AMRFineToCoarse(Particles *pparc, Particles *pparf) {
  ...
  if (npar_new > pparc->nparmax_)
    pparc->UpdateCapacity(npar_new);
...
void Particles::LinkNeighbors(MeshBlockTree &tree, int64_t nrbx1, int64_t nrbx2,
                              int64_t nrbx3, int root_level) {
  // Set myself as one of the neighbors.
  Neighbor *pn = &neighbor_[1][1][1];
  pn->pmb = pmy_block;
  pn->pnb = new NeighborBlock;
  pn->pnb->SetNeighbor(Globals::my_rank, pmy_block->loc.level, pmy_block->gid,
                       pmy_block->lid, 0, 0, 0, NeighborConnect::none, -1, -1,
                       false, false, 0, 0);
  ...
  for (int i = 0; i < pbval_->nneighbor; ++i) {
    ...
    if (snb.rank == Globals::my_rank) {
      ...
      recv_[nb.bufid].tag =
          (pmy_block->lid << 11) | (nb.bufid << 5) | (ipar << 2);
#endif
    }
  }
        ...
        for (int n = 0; n < 3; n++) {
          ...
          if (pn->pnb == NULL) {
            ...
            if (0 <= nblevel && nblevel < my_level) {
              int ngid =
                  tree.FindNeighbor(pbval_->loc, l - 1, m - 1, n - 1)->GetGid();
              for (int i = 0; i < pbval_->nneighbor; ++i) {
                NeighborBlock &nb = pbval_->neighbor[i];
                if (nb.snb.gid == ngid) {
                  pn->pnb = &nb;
...
//! \brief sends particles outside boundary to the buffers of neighboring
//! meshblocks.
...
void Particles::SendToNeighbors() {
  ...
  for (int k = 0; k < npar_;) {
    ...
    int x1i = static_cast<int>(xi1_(k)), x2i = static_cast<int>(xi2_(k)),
        x3i = static_cast<int>(xi3_(k));
    int ox1 = CheckSide(x1i, IS, IE), ox2 = CheckSide(x2i, JS, JE),
    ...
    if (pnb->snb.rank == Globals::my_rank) {
      ...
      if (pnb->snb.gid == pmy_block->gid) {
        ...
        pmy_block->pcoord->MeshCoordsToIndices(x1, x2, x3, xi1_(k), xi2_(k),
                                               xi3_(k));
        ++k;
        continue;
      }
  ...
  for (int i = 0; i < pbval_->nneighbor; ++i) {
    ...
    if (dst == Globals::my_rank) {
      ...
      ppar->bstatus_[nb.targetid] = (ppar->recv_[nb.targetid].npar_ > 0)
...
void Particles::SendParticleBuffer(ParticleBuffer &send, int dst) {
  ...
  if (npsend > 0) {
    ...
    MPI_Isend(send.ibuf, npsend * nint_buf, MPI_INT, dst, sendtag + 1, my_comm,
              &req);
    MPI_Request_free(&req);
    MPI_Isend(send.rbuf, npsend * nreal_buf, MPI_ATHENA_REAL, dst, sendtag + 2,
              my_comm, &req);
    MPI_Request_free(&req);
  }
}
  ...
  if (nb_rank != Globals::my_rank && bstatus == BoundaryStatus::waiting) {
    if (!recv.flagn) {
      ...
      if (recv.reqn == MPI_REQUEST_NULL)
        MPI_Irecv(&recv.npar_, 1, MPI_INT, nb_rank, recv.tag, my_comm,
                  &recv.reqn);
      else
    ...
    if (recv.flagn && recv.npar_ > 0) {
      ...
      if (!recv.flagi) {
        if (recv.reqi == MPI_REQUEST_NULL)
          MPI_Irecv(recv.ibuf, recv.npar_ * nint_buf, MPI_INT, nb_rank,
                    recv.tag + 1, my_comm, &recv.reqi);
        else
          MPI_Test(&recv.reqi, &recv.flagi, MPI_STATUS_IGNORE);
      }
      if (!recv.flagr) {
        if (recv.reqr == MPI_REQUEST_NULL)
          MPI_Irecv(recv.rbuf, recv.npar_ * nreal_buf, MPI_ATHENA_REAL, nb_rank,
                    recv.tag + 2, my_comm, &recv.reqr);
        else
...
//! \brief receives particles from neighboring meshblocks and returns a flag
//! indicating
...
//--------------------------------------------------------------------------------------
//! \fn void Particles::ApplyBoundaryConditions(int k, Real &x1, Real &x2, Real
//! &x3) \brief applies boundary conditions to particle k and returns its
//! updated mesh
...
void Particles::ApplyBoundaryConditions(int k, Real &x1, Real &x2, Real &x3) {
  ...
  pcoord->CartesianToMeshCoordsVector(xp(k), yp(k), zp(k), vpx(k), vpy(k),
                                      vpz(k), vp1, vp2, vp3);
  pcoord->CartesianToMeshCoordsVector(xp0(k), yp0(k), zp0(k), vpx0(k), vpy0(k),
                                      vpz0(k), vp10, vp20, vp30);
...
struct Neighbor *Particles::FindTargetNeighbor(int ox1, int ox2, int ox3,
                                               int x1i, int x2i, int x3i) {
  ...
  if (pmy_mesh_->multilevel && pn->pnb != NULL &&
    ...
    int fi[2] = {0, 0}, i = 0;
    if (active1_ && ox1 == 0)
      fi[i++] = 2 * (x1i - pmy_block->is) / bs.nx1;
    if (active2_ && ox2 == 0)
      fi[i++] = 2 * (x2i - pmy_block->js) / bs.nx2;
    if (active3_ && ox3 == 0)
      fi[i++] = 2 * (x3i - pmy_block->ks) / bs.nx3;
    while (pn != NULL) {
      NeighborIndexes &ni = pn->pnb->ni;
      if (ni.fi1 == fi[0] && ni.fi2 == fi[1])
        break;
      pn = pn->next;
    }
  }
...
void Particles::LoadParticleBuffer(ParticleBuffer *ppb, int k) {
  ...
  if (ppb->npar_ >= ppb->nparmax_)
    ppb->Reallocate((ppb->nparmax_ > 0) ? 2 * ppb->nparmax_ : 1, nint_buf,
                    nreal_buf);
    ...
    if (!pbval_->is_shear[upper]) {
      ...
      if (upper == 0) {
        msg << "### FATAL ERROR in function [Particles::SendParticleShear]"
            ...
            << " but the current meshbock's inner boundary is not "
               "shearing-periodic."
            << " This doesn't make sens." << std::endl;
      } else {
        msg << "### FATAL ERROR in function [Particles::SendParticleShear]"
            << " Particle " << k << " has crossed inner shear-periodic boundary"
            << " but the current meshbock's outer boundary is not "
               "shearing-periodic."
            << " This doesn't make sens." << std::endl;
      }
      ATHENA_ERROR(msg);
      ...
      for (int n = 0; n < 4; n++) {
        ...
        if (snb.rank == Globals::my_rank) {
          ...
          ppar->bstatus_recv_sh_[n + upper * 4] =
              (ppar->recv_sh_[n + upper * 4].npar_ > 0)
                  ? BoundaryStatus::arrived
                  : BoundaryStatus::completed;
        } else {
#ifdef MPI_PARALLEL
...
//! \brief receives particles shifted out due to shear and returns a flag
//! indicating
//!        if all receives are completed.
...
//--------------------------------------------------------------------------------------
//! \fn void Particles::ApplyBoundaryConditionsShear(int k, Real &x1, Real &x2,
//! Real &x3) \brief apply shift due to shear boudnary crossings

void Particles::ApplyBoundaryConditionsShear(int k, Real &x1, Real &x2,
                                             Real &x3) {
  bool flag = false;
  RegionSize &mesh_size = pmy_mesh_->mesh_size;
  Coordinates *pcoord = pmy_block->pcoord;
...
//! communication tags, boundary status should be set after the ComputeShear
//! call, which is done in StartupTaskList. This is necessary function call at
//! every substeps.
  ...
  for (int upper = 0; upper < 2; upper++) {
    if (pbval_->is_shear[upper]) {
      for (int n = 0; n < 4; n++) {
        ...
        recv.tag =
            (pmy_block->lid << 11) | ((n + upper * 4) << 5) | (ipar << 2);
#endif
      }
    }
  }
