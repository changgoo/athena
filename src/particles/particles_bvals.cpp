...
void Particles::SendToNeighbors() {
  ...
  for (int k = 0; k < npar; ) {
    ...
    if (pnb->snb.rank == Globals::my_rank) {
      ...
      ppb = &pn->pmb->ppar[ipar]->recv_[pnb->targetid];
    } else {
#ifdef MPI_PARALLEL
      // Use the send buffer.
      ppb = &send_[pnb->bufid];
  ...
  for (int i = 0; i < pbval_->nneighbor; ++i) {
    ...
    if (dst == Globals::my_rank) {
      Particles *ppar = pmy_mesh->FindMeshBlock(nb.snb.gid)->ppar[ipar];
...
void Particles::SendParticlesShear() {
  for (int k = 0; k < npar; ) {
    ...
    if (snb.rank == Globals::my_rank) {
      ...
      // Use the target receive buffer.
      ppb = &ptarget_block->ppar[ipar]->recv_sh_[shid];
    } else {
#ifdef MPI_PARALLEL
      // Use the send buffer.
      ppb = &send_sh_[shid];
    ...
    if (pbval_->is_shear[upper]) {
      for (int n=0; n<4; n++) {
        ...
        if (snb.rank == Globals::my_rank) {
          ...
          Particles *ppar = pmy_mesh->FindMeshBlock(snb.gid)->ppar[ipar];
