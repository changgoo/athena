...
ParticleBuffer::ParticleBuffer(int nparmax0, int nint, int nreal) {
  ...
  if (nparmax0 <= 0) {
    ...
    msg << "### FATAL ERROR in function [ParticleBuffer::ParticleBuffer]"
        << std::endl
...
ParticleBuffer::~ParticleBuffer() {
  ...
  if (rbuf != NULL)
    delete[] rbuf;
#ifdef MPI_PARALLEL
  if (reqn != MPI_REQUEST_NULL)
    MPI_Request_free(&reqn);
  if (reqi != MPI_REQUEST_NULL)
    MPI_Request_free(&reqi);
  if (reqr != MPI_REQUEST_NULL)
...
void ParticleBuffer::Reallocate(int new_nparmax, int nint, int nreal) {
  ...
  if (new_nparmax <= 0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in function [ParticleBuffer::Reallocate]"
        << std::endl
  ...
  if (new_nparmax < npar_) {
    ...
    msg << "### FATAL ERROR in function [ParticleBuffer::Reallocate]"
        << std::endl
        << "new_nparmax = " << new_nparmax << " < npar = " << npar_
        << std::endl;
    ATHENA_ERROR(msg);
    return;
  }
#ifdef MPI_PARALLEL
  if (reqn != MPI_REQUEST_NULL || reqi != MPI_REQUEST_NULL ||
