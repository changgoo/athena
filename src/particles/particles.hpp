...
//! \brief container for parameters read from `<particle?>` block in the input
//! file
...
struct ParticleParameters {
  ...
  ParticleParameters()
      : block_number(0), ipar(-1), table_output(false), gravity(false) {}
};
...
public:
  ...
  // because different particle species may have different number of history
  // output.
  ...
  // Particle interface
  int AddOneParticle(Real mp, Real x1, Real x2, Real x3, Real v1, Real v2,
                     Real v3);
  ...
  void ClearBoundary();
  void ClearNeighbors();
  void LinkNeighbors(MeshBlockTree &tree, int64_t nrbx1, int64_t nrbx2,
  ...
  // Static functions
  static void AMRCoarseToFine(Particles *pparc, Particles *pparf,
                              MeshBlock *pmbf);
  ...
  // Shallow slices of the actual data container (intprop, realprop, auxprop,
  // work)
  ...
  AthenaArray<Real> xp0, yp0,
      zp0; //!> beginning position (SMOON: What is this?)
  AthenaArray<Real> vpx0, vpy0,
      vpz0; //!> beginning velocity (SMOON: What is this?)
...
protected:
  ...
  // an antipattern.
  void
  AssignShorthands(); //!> Needs to be called in the derived class constructor
  void
  AllocateMemory(); //!> Needs to be called in the derived class constructor
  int AddIntProperty();
  int AddRealProperty();
  int AddAuxProperty();
  int AddWorkingArray();
  void
  UpdateCapacity(int new_nparmax); //!> Change the capacity of particle arrays
  bool CheckInMeshBlock(Real x1, Real x2, Real x3);
  // TODO(SMOON) these two functions  can be moved to private once Integrate
...
private:
  ...
  void UpdatePositionIndices(int npar, const AthenaArray<Real> &xp,
                             const AthenaArray<Real> &yp,
                             const AthenaArray<Real> &zp,
                             AthenaArray<Real> &xi1, AthenaArray<Real> &xi2,
  ...
  struct Neighbor *FindTargetNeighbor(int ox1, int ox2, int ox3, int xi1,
                                      int xi2, int xi3);
