...
StarParticles::StarParticles(MeshBlock *pmb, ParameterInput *pin,
...
//!   for output (use ApplyUserWorkBeforeOutput or write an explicit function
//!   for it)
...
void StarParticles::ConstantAcceleration(Real t, Real dt, Real g1, Real g2,
                                         Real g3) {
  for (int k = 0; k < npar_; ++k) {
    if (age(k) >
...
void StarParticles::SourceTerms(Real t, Real dt,
...
StarParticles::UserSourceTerms(Real t, Real dt,
                               const AthenaArray<Real> &meshsrc) {}
...
void StarParticles::ReactToMeshAux(Real t, Real dt,
                                   const AthenaArray<Real> &meshsrc) {
