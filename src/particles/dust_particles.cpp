...
DustParticles::DustParticles(MeshBlock *pmb, ParameterInput *pin,
    ...
    : Particles(pmb, pin, pp), backreaction{pin->GetOrAddBoolean(
                                   input_block_name, "backreaction", false)},
      variable_taus{
          pin->GetOrAddBoolean(input_block_name, "variable_taus", false)},
      itaus(-1), iwx(-1), iwy(-1), iwz(-1),
      taus0{pin->GetOrAddReal(input_block_name, "taus0", 0.0)} {
  // Define stopping time.
  if (variable_taus)
    itaus = AddAuxProperty();
  ...
  if ((taus0 == 0.0) && backreaction) {
    ...
    msg << "### FATAL ERROR in function [DustParticles::DustParticles]"
        << std::endl
        << "backreaction must be turned off when stopping time is zero"
        << std::endl;
    ATHENA_ERROR(msg);
  }

  // TODO(SMOON): It is user's responsibility to set isgravity_ through input
  // file. Temporarily commenting out the below line; this may be replaced by
  // exception throwing when (!backreaction && isgravity_)
...
Real DustParticles::NewDtForDerived() {
  // No further constraints for infinitely tight coupling (zero stopping time),
  // which is equivalent to a tracer particle
  if (taus0 <= 0.0)
...
void DustParticles::AssignShorthandsForDerived() {
  if (variable_taus)
    taus.InitWithShallowSlice(auxprop, 2, itaus, 1);
...
void DustParticles::SourceTerms(Real t, Real dt,
...
DustParticles::UserSourceTerms(Real t, Real dt,
                               const AthenaArray<Real> &meshsrc) {}

//--------------------------------------------------------------------------------------
//! \fn void DustParticles::UserStoppingTime(Real t, Real dt,
//!                                          const AthenaArray<Real>& meshsrc)
//! \brief assigns time-dependent stopping time to each particle, overloaded by
//! the user.

void __attribute__((weak))
DustParticles::UserStoppingTime(Real t, Real dt,
                                const AthenaArray<Real> &meshsrc) {}
...
void DustParticles::ReactToMeshAux(Real t, Real dt,
  ...
  for (int k = 0; k < npar_; ++k)
    //! \todo (ccyang):
    //! - using (xp0, yp0, zp0) is a temporary hack.
    pc->CartesianToMeshCoordsVector(xp0(k), yp0(k), zp0(k), mass(k) * wx(k),
                                    mass(k) * wy(k), mass(k) * wz(k), wx(k),
                                    wy(k), wz(k));
...
void DustParticles::DepositToMesh(Real t, Real dt,
                                  const AthenaArray<Real> &meshsrc,
