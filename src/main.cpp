...
int main(int argc, char *argv[]) {
  ...
  if (res_flag == 1)
    restartfile.Close(); // close the restart file here
  ...
  //--- Step 6.
  //--------------------------------------------------------------------------
  // Set initial conditions by calling problem generator, or reading restart
  // file
  ...
  //=======================================
  // For performance, there is no error handler protecting this step (except
  // outputs)
  ...
  while ((pmesh->time < pmesh->tlim) &&
    ...
    if (STS_ENABLED) {
      ...
      if (pmesh->sts_integrator == "rkl2") { // default
        ...
        pststlist->nstages =
            static_cast<int>(
                0.5 *
                (-1. + std::sqrt(1. + 8. * pmesh->dt / pmesh->dt_parabolic))) +
            1;
      }
    ...
    for (int stage = 1; stage <= ptlist->nstages; ++stage) {
      ...
      if (ptlist->CheckNextMainStage(stage)) {
        if (SELF_GRAVITY_ENABLED ==
            1) // fft (0: discrete kernel, 1: continuous kernel)
...
#endif
      if (pmesh->time <
          pmesh->tlim) // skip the final output as it happens later
        pouts->MakeOutputs(pmesh, pinput);
#ifdef ENABLE_EXCEPTIONS
    } catch (std::bad_alloc &ba) {
      std::cout << "### FATAL ERROR in main" << std::endl
    ...
    if (pinput->GetOrAddBoolean("job", "output_timing", false)) {
      ...
      ptlist->OutputAllTaskTime(pmesh->ncycle,
                                pinput->GetString("job", "problem_id"));
  ...
  if (Globals::my_rank == 0) {
    ...
    clock_t tstop = clock();
    double cpu_time =
        (tstop > tstart ? static_cast<double>(tstop - tstart) : 1.0) /
